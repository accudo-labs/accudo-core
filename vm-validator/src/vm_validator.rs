...
pub trait TransactionValidation: Send + Sync + Clone {
    type ValidationInstance: accudo_vm::VMValidator;
...
pub struct ValidationState<S> {
    ...
    module_cache: UnsyncModuleCache<ModuleId, CompiledModule, Module, AccudoModuleExtension, usize>,
}
...
impl<S: StateView> ValidationState<S> {
    ...
    pub fn new(state_view: S) -> Self {
        ...
        let environment = AccudoEnvironment::new(&state_view);
        Self {
            state_view,
            environment,
    ...
    pub fn reset_all(&mut self, state_view: S) {
        self.state_view = state_view;
        self.environment = AccudoEnvironment::new(&self.state_view);
    ...
    fn try_override_bytes_and_deserialized_into_compiled_module_with_ext(
        ...
        name: &IdentStr,
    ) -> VMResult<(CompiledModule, Arc<AccudoModuleExtension>)> {
        ...
        let extension = Arc::new(AccudoModuleExtension::new(state_value));
...
impl<S: StateView> ModuleCache for ValidationState<S> {
    ...
    type Extension = AccudoModuleExtension;
    type Key = ModuleId;
    ...
    > {
        ...
        // to detect changes on accudo environment too.
...
impl TransactionValidation for PooledVMValidator {
    type ValidationInstance = AccudoVM;
