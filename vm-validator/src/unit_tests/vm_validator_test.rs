...
struct TestValidator {
    ...
    _db_path: accudo_temppath::TempPath,
}

impl TestValidator {
    fn new() -> Self {
        let _db_path = accudo_temppath::TempPath::new();
        _db_path.create_as_dir().unwrap();
        let (db, db_rw) = DbReaderWriter::wrap(AccudoDB::new_for_test(_db_path.path()));
        accudo_executor_test_helpers::bootstrap_genesis::<AccudoVMBlockExecutor>(
            &db_rw,
            &accudo_vm_genesis::test_genesis_transaction(),
        )
...
//   language e2e tests in: accudo-core/language/e2e-testsuite/src/tests/verify_txn.rs ->
...
// * SENDING_ACCOUNT_FROZEN: Tested in functional-tests/tests/accudo_account/freezing.move.
...
fn test_validate_transaction() {
    ...
    let address = account_config::accudo_test_root_address();
    let program = accudo_stdlib::accudo_coin_mint(address, 100);
    let transaction = transaction_test_helpers::get_test_signed_txn(
        address,
        1,
        &accudo_vm_genesis::GENESIS_KEYPAIR.0,
        accudo_vm_genesis::GENESIS_KEYPAIR.1.clone(),
        Some(program),
    );
...
fn test_validate_invalid_signature() {
    ...
    let address = account_config::accudo_test_root_address();
    let program = accudo_stdlib::accudo_coin_transfer(address, 100);
...
fn test_get_account_sequence_number() {
    let vm_validator = TestValidator::new();
    let root_address = account_config::accudo_test_root_address();
