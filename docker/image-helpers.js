...
export function assertTagMatchesSourceVersion(imageTag) {
  const config = toml.parse(fs.readFileSync("accudo-node/Cargo.toml"));
...
const ACCUDO_RELEASE_REGEX = /accudo-node-v(\d+\.\d+\.\d+)/;

function doesTagMatchConfig(imageTag, configVersion) {
  if (!ACCUDO_RELEASE_REGEX.test(imageTag)) {
    reportError(`image tag does not match cargo version: ${imageTag} !== ${configVersion}`, { throwOnFailure: true });
  }
  const version = imageTag.match(ACCUDO_RELEASE_REGEX)[1];
...
export function isReleaseImage(imageTag) {
  return ACCUDO_RELEASE_REGEX.test(imageTag);
}
