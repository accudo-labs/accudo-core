...
ARG GIT_REPO=https://github.com/accudo-labs/accudo-core.git
ARG GIT_REF

RUN git clone $GIT_REPO ./ && git reset $GIT_REF --hard
RUN --mount=type=cache,target=/accudo/target --mount=type=cache,target=$CARGO_HOME/registry \
  cargo build --release \
  -p accudo-rosetta \
  && mkdir dist \
  && cp target/release/accudo-rosetta dist/accudo-rosetta

### Create image with accudo-node and accudo-rosetta ###
...
COPY --from=builder /accudo/dist/accudo-rosetta /usr/local/bin/accudo-rosetta
...
WORKDIR /opt/accudo/data

ENTRYPOINT ["/usr/local/bin/accudo-rosetta"]
CMD ["online", "--config /opt/accudo/fullnode.yaml"]
