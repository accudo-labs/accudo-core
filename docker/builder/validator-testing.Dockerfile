...
RUN addgroup --system --gid 6180 accudo && adduser --system --ingroup accudo --no-create-home --uid 6180 accudo

RUN mkdir -p /opt/accudo/etc
COPY --link --from=node-builder /accudo/dist/accudo-node /usr/local/bin/
COPY --link --from=tools-builder /accudo/dist/accudo-debugger /usr/local/bin/
