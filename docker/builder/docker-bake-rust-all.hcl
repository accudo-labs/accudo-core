...
variable "CI" {
  # whether this build runs in accudo-labs' CI environment which makes certain assumptions about certain registries being available to push to cache layers.
...
variable "ecr_base" {
  default = "${AWS_ECR_ACCOUNT_NUM}.dkr.ecr.us-west-2.amazonaws.com/accudo"
}
...
target "_common" {
  contexts = {
    ...
    node-builder    = "target:accudo-node-builder"
...
function "generate_tags" {
  ...
  result = TARGET_REGISTRY == "remote-all" ? [
    ...
    TARGET_REGISTRY == "gcp" || TARGET_REGISTRY == "remote" ? [
      ...
      "accudo-core/${target}:${IMAGE_TAG_PREFIX}${GIT_SHA}-from-local",
      "accudo-core/${target}:${IMAGE_TAG_PREFIX}from-local",
    ]
  )
}
