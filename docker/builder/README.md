...
This directory contains [Docker](https://www.docker.com/) configuration for building Accudo docker images. This builder requires the use of Buildkit which is available by default in most recent Docker installations.
...
1. `validator-testing` : Image containing the `accudo-node`, `accudo-debugger` binaries and other linux tools useful for debugging and testing. This image is used in Forge tests.
2. `validator` : Image containing the `accudo-node` and `accudo-debugger` binaries. This image is usually used for distribution.
3. `tools`: Image containing all the accudo tools binaries including `accudo-debugger`, `accudo`, `accudo-transaction-emitter`, `accudo-openapi-spec-generator` and `accudo-fn-check-client`. Also, includes the Accudo Move framework for use with genesis generation.
...
1. One of `accudo-node-builder`, `indexer-builder`, or `tools-builder` targets are invoked depending on what image is being built.
2. The target image is built by copying the output of either the `accudo-node-builder` or `tools-builder` target into the target image.

The `accudo-node-builder` is separate from the other builder targets because it allows to build different `accudo-node` binary variants with different features and profiles.
...
3. Add the following `FROM` statements to the new Dockerfile depending on whether you need to copy from the `accudo-node-builder`, `indexer-builder`, `tools-builder`. This ensures that your image references the required builder images to copy the binaries from. These image references are injected as build contexts at build time. This is defined in the `contexts` field in `_common` target in [docker-bake-rust-all.hcl](docker-bake-rust-all.hcl).
...
4. In your new Dockerfile, use the COPY command to copy the output of the `accudo-node-builder`, `indexer-builder`, `tools-builder` target into the image. For example, to copy the `accudo-node` binary into the `validator` image, use the following command:
   ```
   COPY --link --from=node-builder /accudo/dist/accudo-node /usr/local/bin/
   ```
...
The `accudo-node-builder`, `indexer-builder`, `tools-builder` targets build the `accudo-node` binary and the remaining rust binaries, respectively, and is the most expensive. Its output is used by all the other targets that follow.
