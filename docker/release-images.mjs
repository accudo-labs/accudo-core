...
// This script releases the main accudo docker images to docker hub.
// It does so by copying the images from accudo GCP artifact registry to docker hub.
...
// 2. docker login - with authorization to push to the `accudolabs` org
...
// When we release accudo-node, we also want to release related images for tooling, testing, etc. Similarly, other images have other related images
// that we can release together, ie in a release group.
const IMAGES_TO_RELEASE_BY_RELEASE_GROUP = {
  "accudo-node": ["validator", "validator-testing", "faucet", "tools"],
  "accudo-indexer-grpc": ["indexer-grpc"],
};
...
async function main() {
  ...
  const AWS_ECR = `${parsedArgs.AWS_ACCOUNT_ID}.dkr.ecr.us-west-2.amazonaws.com/accudo`;
  const GCP_ARTIFACT_REPO = parsedArgs.GCP_DOCKER_ARTIFACT_REPO;
  const DOCKERHUB = "docker.io/accudolabs";
...
// * tag a release as "accudo-node-vX.Y.Z"
// * tag a release as "accudo-indexer-grpc-vX.Y.Z"
export function getImageReleaseGroupByImageTagPrefix(prefix) {
  ...
  // if there's no match, then release accudo-node by default
  return "accudo-node";
}
