...
impl Indexer {
    ...
    pub fn index_with_annotator<R: StateView>(
        &self,
        annotator: &AccudoValueAnnotator<R>,
        first_version: Version,
        write_sets: &[&WriteSet],
    ) -> Result<()> {
        ...
        match table_info_parser.finish(&mut batch) {
            Ok(_) => {},
            Err(err) => {
                accudo_logger::error!(first_version = first_version, end_version = end_version, error = ?&err);
                write_sets
                    .iter()
                    .enumerate()
                    .for_each(|(i, write_set)| {
                        accudo_logger::error!(version = first_version as usize + i, write_set = ?write_set);
                    });
