...
use accudo_storage_interface::{
    ...
    AccudoDbError, DbReader, DbWriter, LedgerSummary, MAX_REQUEST_LIMIT,
};
use accudo_types::{
...
/// Alternate implementation of [AccudoDB] for use with consensus-only-perf-test feature.
/// It stores and serves data from in-memory data structures as opposed to [AccudoDB],
/// which uses RocksDB. Note that FakeAccudoDB re-implements only a subset of the
/// features of [AccudoDB] while passing through remaining features to the wrapped inner
/// [AccudoDB].
pub struct FakeAccudoDB {
    inner: AccudoDB,
...
impl FakeAccudoDB {
    ...
    fn save_and_compute_root_hash(
        ...
        let (root_hash, writes) =
            MerkleAccumulator::<FakeAccudoDB, TransactionAccumulatorHasher>::append(
                self,
    ...
    fn get_frozen_subtree_hashes(&self, num_transactions: u64) -> Result<Vec<HashValue>> {
        MerkleAccumulator::<FakeAccudoDB, TransactionAccumulatorHasher>::get_frozen_subtree_hashes(
            self,
            num_transactions,
        )
        .map_err(Into::into)
...
impl DbWriter for FakeAccudoDB {
...
impl DbReader for FakeAccudoDB {
    fn get_epoch_ending_ledger_infos(
        &self,
        start_epoch: u64,
        end_epoch: u64,
    ) -> Result<accudo_types::epoch_change::EpochChangeProof> {
    ...
    fn get_events(
        &self,
        event_key: &accudo_types::event::EventKey,
        start: u64,
        order: accudo_storage_interface::Order,
        limit: u64,
        ledger_version: Version,
    ...
    fn get_block_timestamp(&self, version: Version) -> Result<u64> {
        gauged_api("get_block_timestamp", || {
            ...
            if timestamp > 0 {
                ...
                Err(AccudoDbError::NotFound("NewBlockEvent".to_string()).into())
            }
        })
    }
    ...
    fn get_account_ordered_transactions(
        ...
        include_events: bool,
        ledger_version: Version,
    ) -> Result<accudo_types::transaction::AccountOrderedTransactionsWithProof> {
    ...
    fn get_state_value_by_version(
        ...
        // Since the genesis write set is persisted with AccudoDB, we call
        // it to serve state values targeting the framework account
        // (to access AccudoCoin, for example).
    ...
    fn get_pre_committed_ledger_summary(&self) -> Result<LedgerSummary> {
        // If the genesis is not executed yet, we need to get the executed trees from the inner AccudoDB
        // This is because when we call save_transactions for the genesis block, we call [AccudoDB::save_transactions]
        // where there is an expectation that the root of the SMTs are the same pointers. Here,
        // we get from the inner AccudoDB which ensures that the pointers match when save_transactions is called.
    ...
    fn get_latest_version(&self) -> Result<Version> {
        gauged_api("get_latest_version", || {
            self.latest_version
                .lock()
                .ok_or_else(|| AccudoDbError::NotFound("No latest version found.".to_string()))
        })
    }
...
/// requests.
impl HashReader for FakeAccudoDB {
...
fn error_if_too_many_requested(num_requested: u64, max_allowed: u64) -> Result<()> {
    if num_requested > max_allowed {
        Err(AccudoDbError::TooManyRequested(num_requested, max_allowed).into())
    } else {
        Ok(())
    }
}
...
mod tests {
    ...
    proptest! {
        ...
        fn test_save_transactions(input in arb_blocks_to_commit()) {
            ...
            let db = FakeAccudoDB::new(AccudoDB::new_for_test(&tmp_dir));
