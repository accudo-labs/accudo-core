...
use accudo_db_indexer::{db_indexer::InternalIndexerDB, Indexer};
use accudo_logger::prelude::*;
use accudo_metrics_core::{IntGaugeVecHelper, TimerHelper};
use accudo_resource_viewer::AccudoValueAnnotator;
use accudo_schemadb::{Cache, Env};
use accudo_storage_interface::{
    block_info::BlockInfo, db_ensure as ensure, db_other_bail as bail, AccudoDbError, DbReader,
    Order, Result,
};
use accudo_types::{
...
impl AccudoDB {
    ...
    pub(super) fn open_internal(
        ...
        let mut env =
            Env::new().map_err(|err| AccudoDbError::OtherRocksDbError(err.into_string()))?;
    ...
    fn open_indexer(
        ...
        if indexer.next_version() < ledger_next_version {
            ...
            let annotator = AccudoValueAnnotator::new(&state_view);
        ...
        info!("AccudoDB Indexer caught up.");
    ...
    pub(super) fn new_without_pruner<P: AsRef<Path> + Clone>(
        ...
        .expect("Unable to open AccudoDB")
    }
    ...
    pub(super) fn get_raw_block_info_by_height(&self, block_height: u64) -> Result<BlockInfo> {
        if !self.skip_index_and_usage {
            ...
            Ok(self
                ...
                .ok_or_else(|| {
                    AccudoDbError::NotFound(format!("BlockInfo not found at height {block_height}"))
                })?)
        }
    }
    ...
    pub(super) fn get_raw_block_info_by_version(
        ...
        if !self.skip_index_and_usage {
            let (first_version, event_index, block_height) = self
                ...
                .ok_or_else(|| AccudoDbError::NotFound("NewBlockEvent".to_string()))?;
    ...
    pub(super) fn to_api_block_info(
        ...
        let last_version = match self.get_raw_block_info_by_height(block_height + 1) {
            ...
            Err(AccudoDbError::NotFound(..)) => committed_version,
...
impl Debug for AccudoDB {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.write_str("{AccudoDB}")
    }
}

pub(super) fn error_if_too_many_requested(num_requested: u64, max_allowed: u64) -> Result<()> {
    if num_requested > max_allowed {
        Err(AccudoDbError::TooManyRequested(num_requested, max_allowed))
    } else {
        Ok(())
    }
}
