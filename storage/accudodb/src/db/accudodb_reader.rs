...
use crate::{
    ...
    db::{
        accudodb_internal::{error_if_too_many_requested, gauged_api, get_first_seq_num_and_limit},
        AccudoDB,
    },
...
use accudo_storage_interface::{
    ...
    AccudoDbError, BlockHeight, DbReader, LedgerSummary, Order, Result, MAX_REQUEST_LIMIT,
};
use accudo_types::{
...
impl DbReader for AccudoDB {
    ...
    /// Returns the transaction by version, delegates to `AccudoDB::get_transaction_with_proof`.
    ...
    fn get_table_info(&self, handle: TableHandle) -> Result<TableInfo> {
        gauged_api("get_table_info", || {
            self.get_table_info_option(handle)?
                .ok_or_else(|| AccudoDbError::NotFound(format!("TableInfo for {:?}", handle)))
        })
    }
