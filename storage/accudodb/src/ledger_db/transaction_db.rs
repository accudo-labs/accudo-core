...
use accudo_db_indexer_schemas::schema::ordered_transaction_by_account::OrderedTransactionByAccountSchema;
...
use accudo_types::transaction::{IndexedTransactionSummary, ReplayProtector, Transaction, Version};
use rayon::prelude::*;
...
impl TransactionDb {
    ...
    pub(crate) fn get_transaction(&self, version: Version) -> Result<Transaction> {
        self.db
            ...
            .ok_or_else(|| AccudoDbError::NotFound(format!("Txn {version}")))
    }
