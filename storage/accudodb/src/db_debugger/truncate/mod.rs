...
use accudo_config::config::{RocksdbConfigs, StorageDirPaths};
...
impl Cmd {
    pub fn run(self) -> Result<()> {
        ...
        let (ledger_db, state_merkle_db, state_kv_db) = AccudoDB::open_dbs(
...
mod test {
    use super::*;
    use crate::{
        db::{test_helper::arb_blocks_to_commit_with_block_nums, AccudoDB},
        schema::{
    ...
    proptest! {
        ...
        fn test_truncation(input in arb_blocks_to_commit_with_block_nums(80, 120)) {
            ...
            let db = if input.1 { AccudoDB::new_for_test_with_sharding(&tmp_dir, DEFAULT_MAX_NUM_NODES_PER_LRU_CACHE_SHARD) } else { AccudoDB::new_for_test(&tmp_dir) };
            let mut version = 0;
