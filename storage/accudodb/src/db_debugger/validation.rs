...
use crate::{
    schema::state_value_by_key_hash::StateValueByKeyHashSchema, state_kv_db::StateKvDb, AccudoDB,
};
use accudo_config::config::{RocksdbConfig, StorageDirPaths};
use accudo_crypto::{hash::CryptoHash, HashValue};
use accudo_db_indexer::db_ops::open_internal_indexer_db;
...
use accudo_schemadb::{ReadOptions, DB};
...
pub fn validate_db_data(
    ...
    let accudo_db = AccudoDB::new_for_test_with_sharding(db_root_path, 1000000);
    let batch_size = 20_000;
    let start_version = accudo_db.get_first_txn_version()?.unwrap();
    target_ledger_version = std::cmp::min(
        accudo_db.get_synced_version()?.unwrap(),
