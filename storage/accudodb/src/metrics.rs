...
pub static LEDGER_COUNTER: Lazy<IntGaugeVec> = Lazy::new(|| {
    register_int_gauge_vec!(
        // metric name
        "accudo_storage_ledger",
        // metric description
        "Accudo storage ledger counters",
...
pub static COMMITTED_TXNS: Lazy<IntCounter> = Lazy::new(|| {
    register_int_counter!(
        "accudo_storage_committed_txns",
        "Accudo storage committed transactions"
    )
    .unwrap()
});
...
pub static LATEST_TXN_VERSION: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "accudo_storage_latest_transaction_version",
        "Accudo storage latest transaction version"
    )
    .unwrap()
});
...
pub static NEXT_BLOCK_EPOCH: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "accudo_storage_next_block_epoch",
...
pub static STATE_ITEMS: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!("accudo_storage_state_items", "Total number of state items.").unwrap()
});

pub static TOTAL_STATE_BYTES: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "accudo_storage_total_state_bytes",
...
pub static PRUNER_WINDOW: Lazy<IntGaugeVec> = Lazy::new(|| {
    register_int_gauge_vec!(
        // metric name
        "accudo_storage_prune_window",
        // metric description
        "Accudo storage prune window",
        // metric labels (dimensions)
        &["pruner_name",]
    )
    .unwrap()
...
pub static PRUNER_VERSIONS: Lazy<IntGaugeVec> = Lazy::new(|| {
    register_int_gauge_vec!(
        // metric name
        "accudo_pruner_versions",
        // metric description
        "Accudo pruner versions",
        // metric labels (dimensions)
        &["pruner_name", "tag"]
    )
    .unwrap()
...
pub static API_LATENCY_SECONDS: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        // metric name
        "accudo_storage_api_latency_seconds",
        // metric description
        "Accudo storage api latency in seconds",
        // metric labels (dimensions)
        &["api_name", "result"],
...
pub static OTHER_TIMERS_SECONDS: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        // metric name
        "accudo_storage_other_timers_seconds",
        // metric description
...
make_thread_local_histogram_vec!(
    ...
    NODE_CACHE_SECONDS,
    // metric name
    "accudo_storage_node_cache_seconds",
    // metric description
...
pub static ROCKSDB_SHARD_PROPERTIES: Lazy<IntGaugeVec> = Lazy::new(|| {
    register_int_gauge_vec!(
        // metric name
        "accudo_rocksdb_shard_properties",
        // metric description
...
pub(crate) static LATEST_SNAPSHOT_VERSION: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "accudo_storage_latest_state_snapshot_version",
...
pub(crate) static LATEST_CHECKPOINT_VERSION: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "accudo_storage_latest_state_checkpoint_version",
...
pub(crate) static BACKUP_EPOCH_ENDING_EPOCH: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "accudo_backup_handler_epoch_ending_epoch",
...
pub(crate) static BACKUP_TXN_VERSION: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "accudo_backup_handler_transaction_version",
...
pub(crate) static BACKUP_STATE_SNAPSHOT_VERSION: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "accudo_backup_handler_state_snapshot_version",
...
pub(crate) static BACKUP_STATE_SNAPSHOT_LEAF_IDX: Lazy<IntGauge> = Lazy::new(|| {
    register_int_gauge!(
        "accudo_backup_handler_state_snapshot_leaf_index",
...
pub static BACKUP_TIMER: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        "accudo_backup_handler_timers_seconds",
...
pub static CONCURRENCY_GAUGE: Lazy<IntGaugeVec> = Lazy::new(|| {
    register_int_gauge_vec!(
        "accudo_storage_api_concurrency",
...
pub static GAUGE: Lazy<IntGaugeVec> = Lazy::new(|| {
    register_int_gauge_vec!("accudo_storage_gauge", "Various gauges", &["name"]).unwrap()
});
...
make_thread_local_int_counter_vec!(
    ...
    COUNTER,
    // metric name
    "accudo_storage_counter",
    // metric description
    "Various counters for Accudo DB / storage.",
    // metric labels (dimensions)
    &["name"],
);
