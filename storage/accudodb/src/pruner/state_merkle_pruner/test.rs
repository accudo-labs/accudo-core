...
use accudo_config::config::{LedgerPrunerConfig, StateMerklePrunerConfig};
...
fn test_state_store_pruner() {
    ...
    let accudo_db = AccudoDB::new_for_test_no_cache(&tmp_dir);
    let state_store = &accudo_db.state_store;
    ...
    {
        let pruner =
            create_state_merkle_pruner_manager(&accudo_db.state_merkle_db(), prune_batch_size);
...
fn test_state_store_pruner_partial_version() {
    ...
    let tmp_dir = TempPath::new();
    let accudo_db = AccudoDB::new_for_test_with_sharding(&tmp_dir, 0);
    let state_store = &accudo_db.state_store;
    ...
    if accudo_db.state_merkle_db().sharding_enabled() {
