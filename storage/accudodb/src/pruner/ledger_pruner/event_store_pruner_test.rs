...
use crate::{AccudoDB, EventStore, LedgerPrunerManager, PrunerManager};
use accudo_config::config::LedgerPrunerConfig;
use accudo_proptest_helpers::Index;
...
fn verify_event_store_pruner(events: Vec<Vec<ContractEvent>>) {
    ...
    let event_store = &accudo_db.event_store;
    ...
    accudo_db.ledger_db.event_db().write_schemas(batch).unwrap();
