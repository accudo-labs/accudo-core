...
use accudo_storage_interface::{DbReader, Result};
use accudo_types::{
...
/// Provides functionalities for AccudoDB data restore.
#[derive(Clone)]
pub struct RestoreHandler {
    pub accudodb: Arc<AccudoDB>,
    state_store: Arc<StateStore>,
    ledger_db: Arc<LedgerDb>,
}

impl RestoreHandler {
    pub(crate) fn new(accudodb: Arc<AccudoDB>, state_store: Arc<StateStore>) -> Self {
        Self {
            ledger_db: Arc::clone(&accudodb.ledger_db),
            accudodb,
            state_store,
        }
    }
    ...
    pub fn save_ledger_infos(&self, ledger_infos: &[LedgerInfoWithSignatures]) -> Result<()> {
        restore_utils::save_ledger_infos(self.accudodb.ledger_db.metadata_db(), ledger_infos, None)
    }
    ...
    pub fn confirm_or_save_frozen_subtrees(
        ...
        restore_utils::confirm_or_save_frozen_subtrees(
            self.accudodb.ledger_db.transaction_accumulator_db_raw(),
            num_leaves,
            frozen_subtrees,
            None,
        )
    ...
    pub fn get_next_expected_transaction_version(&self) -> Result<Version> {
        Ok(self.accudodb.get_synced_version()?.map_or(0, |ver| ver + 1))
    }
    ...
    pub fn get_state_snapshot_before(
        ...
        self.accudodb.get_state_snapshot_before(version)
    }

    pub fn get_in_progress_state_kv_snapshot_version(&self) -> Result<Option<Version>> {
        let db = self.accudodb.state_kv_db.metadata_db_arc();
