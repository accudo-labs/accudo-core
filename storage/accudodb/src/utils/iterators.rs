    ...
    fn expect_continuous_versions(
        ...
        Ok(ContinuousVersionIter {
            ...
            end_version: first_version
                ...
                .ok_or(AccudoDbError::TooManyRequested(first_version, limit as u64))?,
...
impl<'a> PrefixedStateValueIterator<'a> {
    pub fn new(
        ...
        // accudo/abc|0
        // accudo/abd|1
        // if we seek('accudo/'), and call next, we may not reach `accudo/abd/1` because the prefix extractor we adopted
        // here will stick with prefix `accudo/abc` and return `None` or any arbitrary result after visited all the
        // keys starting with `accudo/abc`.
    ...
    fn next_impl(&mut self) -> Result<Option<Vec<ContractEvent>>> {
        ...
        while let Some(res) = self.inner.peek() {
            let ((version, _index), _event) = res
                .as_ref()
                .map_err(|e| AccudoDbError::Other(format!("Hit error iterating events: {}", e)))?;
        ...
        self.expected_next_version = self
            .expected_next_version
            .checked_add(1)
            .ok_or_else(|| AccudoDbError::Other("expected version overflowed.".to_string()))?;
        Ok(Some(ret))
    }
}
