...
/// This is a wrapper around [AccudoDB] that is used to bootstrap the node for fast sync mode
pub struct FastSyncStorageWrapper {
    // Used for storing genesis data during fast sync
    temporary_db_with_genesis: Arc<AccudoDB>,
    // Used for restoring fast sync snapshot and all the read/writes afterwards
    db_for_fast_sync: Arc<AccudoDB>,
...
impl FastSyncStorageWrapper {
    ...
    /// Otherwise, we return AccudoDB directly and the FastSyncStorageWrapper is None
    pub fn initialize_dbs(
        config: &NodeConfig,
        internal_indexer_db: Option<InternalIndexerDB>,
        update_sender: Option<Sender<(Instant, Version)>>,
    ) -> Result<Either<AccudoDB, Self>> {
        let mut db_main = AccudoDB::open(
                ...
                == 0)
        {
            ...
            let secondary_db = AccudoDB::open(
    ...
    pub fn get_fast_sync_db(&self) -> Arc<AccudoDB> {
        self.db_for_fast_sync.clone()
    }

    pub fn get_temporary_db_with_genesis(&self) -> Arc<AccudoDB> {
    ...
    pub(crate) fn get_accudo_db_read_ref(&self) -> &AccudoDB {
    ...
    pub(crate) fn get_accudo_db_write_ref(&self) -> &AccudoDB {
...
impl DbWriter for FastSyncStorageWrapper {
    ...
    fn finalize_state_snapshot(
        ...
        self.get_accudo_db_write_ref().finalize_state_snapshot(
            version,
            output_with_proof,
            ledger_infos,
        )?;
