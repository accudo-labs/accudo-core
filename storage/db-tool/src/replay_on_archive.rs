...
use accudo_db::{backup::backup_handler::BackupHandler, AccudoDB};
use accudo_logger::prelude::*;
use accudo_storage_interface::{
    state_store::state_view::db_state_view::DbStateViewAtVersion, AccudoDbError, DbReader,
};
use accudo_types::{
    block_executor::{
...
impl Verifier {
    pub fn new(config: &Opt) -> Result<Self> {
        // Open in write mode to create any new DBs necessary.
        {
            if let Err(e) = panic::catch_unwind(|| {
                ...
                warn!("Unable to open AccudoDB in write mode: {:?}", e);
            };
        }
        ...
        let backup_handler = accudo_db.get_backup_handler();
        let arc_db = Arc::new(accudo_db) as Arc<dyn DbReader>;
    ...
    pub fn run(self) -> Result<Vec<Error>> {
        ...
        AccudoVM::set_concurrency_level_once(self.replay_concurrency_level);
    ...
    fn execute_and_verify(
        &self,
        executor: &AccudoVMBlockExecutor,
