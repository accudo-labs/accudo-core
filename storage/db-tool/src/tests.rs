...
fn test_various_cmd_parsing() {
    ...
    run_cmd(&["accudo-db-tool", "backup", "verify", "--local-fs-dir", "."]);
    run_cmd(&[
        "accudo-db-tool",
        "replay-verify",
        "--target-db-dir",
...
mod dbtool_tests {
    ...
    use accudo_executor_test_helpers::integration_test_impl::{
    ...
    fn db_restore_test_setup(
        ...
        use accudo_db_indexer::utils::PrefixedStateValueIterator as IndexerPrefixedStateValueIterator;
        ...
        let accudo_db: Arc<dyn DbReader> = Arc::new(
            AccudoDB::open(
        ...
        let ledger_version = accudo_db.get_latest_ledger_info_version().unwrap();
        for ver in start..=ledger_version {
            let old_block_res = db.get_block_info_by_version(ver);
            let new_block_res = accudo_db.get_block_info_by_version(ver);
