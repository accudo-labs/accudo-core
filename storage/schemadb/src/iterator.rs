...
use crate::{
    IntoDbResult, KeyCodec, Schema, SeekKeyCodec, ValueCodec, ACCUDO_SCHEMADB_ITER_BYTES,
    ...
    S: Schema,
{
    ...
    pub fn seek_to_first(&mut self) {
        let _timer = ACCUDO_SCHEMADB_SEEK_LATENCY_SECONDS
    ...
    /// `seek_key`.
    pub fn seek<SK>(&mut self, seek_key: &SK) -> accudo_storage_interface::Result<()>
    where
        SK: SeekKeyCodec<S>,
    {
        let _timer =
            ACCUDO_SCHEMADB_SEEK_LATENCY_SECONDS.timer_with(&[S::COLUMN_FAMILY_NAME, "seek"]);
    ...
    pub fn seek_for_prev<SK>(&mut self, seek_key: &SK) -> accudo_storage_interface::Result<()>
    where
        SK: SeekKeyCodec<S>,
    {
        let _timer = ACCUDO_SCHEMADB_SEEK_LATENCY_SECONDS
    ...
    fn next_impl(&mut self) -> accudo_storage_interface::Result<Option<(S::Key, S::Value)>> {
        let _timer = ACCUDO_SCHEMADB_ITER_LATENCY_SECONDS.timer_with(&[S::COLUMN_FAMILY_NAME]);
        ...
        ACCUDO_SCHEMADB_ITER_BYTES.observe_with(
...
where
    S: Schema,
{
    type Item = accudo_storage_interface::Result<(S::Key, S::Value)>;
