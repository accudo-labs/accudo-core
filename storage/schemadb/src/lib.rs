...
use crate::{
    metrics::{
        ACCUDO_SCHEMADB_BATCH_COMMIT_BYTES, ACCUDO_SCHEMADB_BATCH_COMMIT_LATENCY_SECONDS,
        ACCUDO_SCHEMADB_GET_BYTES, ACCUDO_SCHEMADB_GET_LATENCY_SECONDS, ACCUDO_SCHEMADB_ITER_BYTES,
        ACCUDO_SCHEMADB_ITER_LATENCY_SECONDS, ACCUDO_SCHEMADB_SEEK_LATENCY_SECONDS,
    },
...
impl DB {
    ...
    pub fn get<S: Schema>(&self, schema_key: &S::Key) -> DbResult<Option<S::Value>> {
        let _timer = ACCUDO_SCHEMADB_GET_LATENCY_SECONDS.timer_with(&[S::COLUMN_FAMILY_NAME]);

        let k = <S::Key as KeyCodec<S>>::encode_key(schema_key)?;
        let cf_handle = self.get_cf_handle(S::COLUMN_FAMILY_NAME)?;

        let result = self.inner.get_cf(cf_handle, k).into_db_res()?;
        ACCUDO_SCHEMADB_GET_BYTES.observe_with(
    ...
    fn write_schemas_inner(&self, batch: impl IntoRawBatch, option: &WriteOptions) -> DbResult<()> {
        ...
        let _timer = ACCUDO_SCHEMADB_BATCH_COMMIT_LATENCY_SECONDS.timer_with(&labels);
        ...
        ACCUDO_SCHEMADB_BATCH_COMMIT_BYTES.observe_with(&[&self.name], serialized_size as f64);
    ...
    pub fn get_property(&self, cf_name: &str, property_name: &str) -> DbResult<u64> {
        self.inner
            ...
            .into_db_res()?
            .ok_or_else(|| {
                accudo_storage_interface::AccudoDbError::Other(
                    format!(
...
fn to_db_err(rocksdb_err: rocksdb::Error) -> AccudoDbError {
    match rocksdb_err.kind() {
        ErrorKind::Incomplete => AccudoDbError::RocksDbIncompleteResult(rocksdb_err.to_string()),
        ErrorKind::NotFound
        ...
        | ErrorKind::Unknown => AccudoDbError::OtherRocksDbError(rocksdb_err.to_string()),
    }
}
