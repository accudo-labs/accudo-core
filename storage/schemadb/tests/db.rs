...
fn open_db(dir: &accudo_temppath::TempPath) -> DB {
...
fn open_db_read_only(dir: &accudo_temppath::TempPath) -> DB {
...
struct TestDB {
    _tmpdir: accudo_temppath::TempPath,
    db: DB,
}

impl TestDB {
    fn new() -> Self {
        let tmpdir = accudo_temppath::TempPath::new();
...
fn collect_values<S: Schema>(db: &TestDB) -> Vec<(S::Key, S::Value)> {
    ...
    iter.seek_to_first();
    iter.collect::<Result<Vec<_>, AccudoDbError>>().unwrap()
}
...
fn test_open_as_secondary() {
    let tmpdir = accudo_temppath::TempPath::new();
    let tmpdir_sec = accudo_temppath::TempPath::new();
...
fn test_checkpoint() {
    let tmpdir = accudo_temppath::TempPath::new();
    let checkpoint = accudo_temppath::TempPath::new();
    {
        let db = open_db(&tmpdir);
        db.put::<TestSchema1>(&TestField(0), &TestField(0)).unwrap();
