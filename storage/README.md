...
id: storage
title: Storage
custom_edit_url: https://github.com/accudo-labs/accudo-core/edit/main/storage/README.md
---

## Overview

The storage modules implement:
* the AccudoDB which holds the authenticated blockchain data structure within a
Accudo Node. It serves the current "state" readable by Move contracts being
executed, as well as a configurable length of the blockchain history to fellow
Accudo Nodes and the Rest API. It takes in new data from either the consensus or
...
can be critical when emergency happens were an AccudoDB needs to be recreated
a. without the help of widely available healthy running Accudo Nodes b. to
...
https://github.com/accudo-labs/accudo-core/tree/main/documentation/specifications/common
* To know more about the state authentication data structure, see
https://github.com/accudo-labs/accudo-core/developer-docs-site/static/papers/jellyfish-merkle-tree/2021-01-14.pdf
* To learn more about the backup system including the backup data format, see
https://github.com/accudo-labs/accudo-core/blob/main/documentation/specifications/db_backup/spec.md
...
As part of the Accudo Node config, these are specific for the storage components.
...
to be changed. Another reason to not overriding them is the developers of Accudo
...
https://github.com/accudo-labs/accudo-core/blob/main/config/src/config/storage_config.rs
...
storage: 
  ...
  # ``` yaml
  # base:
  #   data_dir: /opt/accudo/data
  # ```
  # and this config has the default value (`db`), the DBs will be at
  # `/opt/accudo/data/db/ledger_db` and  `/opt/accudo/data/db/state_merkle_db`
  dir: db
  # AccudoDB persists the state authentication structure off the critical path
  ...
  # AccudoDB keeps recent history of the blockchain ledger and recent versions
...
to boot up a AccudoDB instance on an empty disk. Use State Sync (it's Fast Sync
mode). Read more about state sync here:
https://github.com/accudo-labs/accudo-core/blob/main/state-sync/README.md


### Continuously backing up to a cloud storage

The backup coordinator runs continuously, talks to the backup service embedded
inside a Accudo Node and writes backup data automatically to a configured cloud
storage.

One can make a config file for a specific cloud storage position by updating
one of the examples here
https://github.com/accudo-labs/accudo-core/tree/main/storage/backup/backup-cli/src/storage/command_adapter/sample_configs/


```bash
$ cargo run -p accudo-debugger accudo-db backup continuously --help
    Finished dev [unoptimized + debuginfo] target(s) in 1.06s
     Running `target/debug/accudo-debugger accudo-db backup continuously --help`
accudo-db-tool-backup-continuously 0.1.0
Run the backup coordinator which backs up blockchain data continuously off a Accudo Node.

USAGE:
    accudo-debugger accudo-db backup continuously [OPTIONS] <--local-fs-dir <LOCAL_FS_DIR>|--command-adapter-config <COMMAND_ADAPTER_CONFIG>>

OPTIONS:
        --backup-service-address <ADDRESS>
            Backup service address. By default a Accudo Node runs the backup service serving on tcp
        ...
        --command-adapter-config <COMMAND_ADAPTER_CONFIG>
            ...
            https://github.com/accudo-labs/accudo-networks/tree/main/testnet/backups
...
Example command:
```
$ cargo run -p accudo-debugger accudo-db backup continuously \
...
There are other subcommands of the accudo-debugger accudo-db, all of which are experimental
and can mess up with the backup storage, use only at your own risk.

### Creating an AccudoDB with minimal data at the latest epoch ending in a backup

It's part of the Accudo API functionality to bootstrap a AccudoDB with a backup.
When emergency happens and the need to do the somewhat manual bootstrapping is
high, Accudo will provide a backup source in the form of a yaml config file. Otherwise
...
```bash
accudo-node-bootstrap-db-from-backup 0.3.5
Tool to bootstrap DB from backup

USAGE:
    accudo node bootstrap-db-from-backup [OPTIONS] --config-path <CONFIG_PATH> --target-db-dir <DB_DIR>
...
OPTIONS:
    ...
    -h, --help
        ...
        --target-db-dir <DB_DIR>
            Target dir where the tool recreates a AccudoDB with snapshots and transactions provided
            in the backup. The data folder can later be used to start an Accudo node. e.g. /opt/
            accudo/data/db
...
the "auto" mode of `cargo run -p accudo-debugger accudo-db restore`, but with more
