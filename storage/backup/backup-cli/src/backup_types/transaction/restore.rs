...
impl TransactionRestoreBatchController {
    ...
    async fn run_impl(self) -> Result<()> {
        ...
        if let RestoreRunMode::Restore { restore_handler } = self.global_opt.run_mode.as_ref() {
            ...
            AccudoVM::set_concurrency_level_once(self.global_opt.replay_concurrency_level);
    ...
    async fn save_before_replay_version(
        ...
        >,
    > {
        // get the next expected transaction version of the current accudo db from txn_info CF
    ...
    async fn replay_transactions(
        ...
        let replay_start = Instant::now();
        let db = DbReaderWriter::from_arc(Arc::clone(&restore_handler.accudodb));
        let chunk_replayer = Arc::new(ChunkExecutor::<AccudoVMBlockExecutor>::new(db));
