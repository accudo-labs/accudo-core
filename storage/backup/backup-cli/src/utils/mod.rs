...
use accudo_jellyfish_merkle::{NodeBatch, TreeWriter};
use accudo_logger::info;
use accudo_storage_interface::{AccudoDbError, Result};
use accudo_types::{
    state_store::{
...
impl TryFrom<GlobalRestoreOpt> for GlobalRestoreOptions {
    ...
    fn try_from(opt: GlobalRestoreOpt) -> anyhow::Result<Self> {
        ...
        let run_mode = if let Some(db_dir) = &opt.db_dir {
            ...
            let restore_handler = Arc::new(AccudoDB::open_kv_only(
    ...
    pub fn verify(self) -> Result<HashMap<Version, Waypoint>> {
        ...
        for w in self.trust_waypoint {
            trusted_waypoints
                .insert(w.version(), w)
                .map_or(Ok(()), |w| {
                    Err(AccudoDbError::Other(format!(
...
pub struct ReplayConcurrencyLevelOpt {
    /// AccudoVM::set_concurrency_level_once() is called with this
    #[clap(
        long,
...
impl<T: AsRef<Path>> PathToString for T {
    fn path_to_string(&self) -> Result<String> {
        self.as_ref()
            ...
            .into_os_string()
            .into_string()
            .map_err(|s| AccudoDbError::Other(format!("into_string failed for OsString '{:?}'", s)))
    }
}
