...
# This builds the move docs for the Accudo-framework
...
root_path="$(dirname $0)/.."
move_path="$root_path/accudo-move/framework"
...
RELEASE_PATH="$move_path/accudo-framework/releases/artifacts/current/build"
...
for folder in $(ls $RELEASE_PATH); do
  ...
  for file in $(ls $RELEASE_PATH/$folder/docs); do
    ...
    sed -i \
      's#../../../../../../../accudo-framework/releases/artifacts/current/build/\(.*\)/docs/\(.*.md\)#../\1/\2#g' \
      $outfile
  done
done
