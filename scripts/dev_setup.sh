...
function usage {
  echo "Usage:"
  echo "Installs or updates necessary dev tools for accudolabs/accudo-core."
  ...
  echo "This command must be called from the root folder of the Accudo-core project."
}
...
function install_rustup_components_and_nightly {
  ...
  rustup update
  rustup toolchain install stable # Install the latest toolchain to ensure that dependencies can always be built (even if accudo-core is behind)
...
# this is needed for hdpi crate from accudo-ledger
...
build, test and inspect Accudo Core.
...
if [ ! -f rust-toolchain.toml ]; then
  echo "Unknown location. Please run this from the accudo-core repository. Abort."
  exit 1
fi
