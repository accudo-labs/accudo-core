...
# Note: This must be run from the root of the accudo-core repository
...
if [[ "$SKIP_CHECKS" != "true" ]]; then
  ...
  if curl -s --stderr /dev/null --output /dev/null --head -f "https://github.com/accudo-labs/accudo-core/releases/download/accudo-cli-v$EXPECTED_VERSION/accudo-cli-$EXPECTED_VERSION-Ubuntu-22.04-x86_64.zip"; then
