...
#   ./pgo.sh run a.profdata -p accudo-vm-profiling --bin run-accudo-p2p
#                           ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
#                           You can replace this with arbitrary cargo run args
# 
# Build without running:
#   ./pgo.sh build a.profdata -p accudo-vm-profiling --bin run-accudo-p2p
...
case "$CMD" in
    profile)
        ...
        env RUST_BACKTRACE=1 RUST_MIN_STACK=104857600 \
            ...
            cargo build --profile release -p accudo-vm-profiling --bin run-accudo-p2p
        ...
        "$REPO_ROOT/target/release/run-accudo-p2p"
