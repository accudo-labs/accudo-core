...
# This is currently setup for the accudo-core environment. If move is at a different
...
Flags:
    -h   Print this help
    -t   Run tests
    -i   In addition to -t, run integration tests (Accudo framework and e2e tests)
...
MOVE_CRATES_INTEGRATION_TEST="\
  -p accudo-transactional-test-harness \
...
if [ ! -z "$GEN_ARTIFACTS" ]; then
    ...
    (
        cd $BASE
        cargo build $CARGO_OP_PARAMS -p accudo-cached-packages
    )
fi
