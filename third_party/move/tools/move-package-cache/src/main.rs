...
async fn main() -> Result<()> {
    ...
    let accudo_framework_url =
        Url::from_str("https://github.com/accudo-labs/accudo-framework").unwrap();

    let oid = cache
        .resolve_git_revision(&accudo_framework_url, "main")
        .await?;
    cache.checkout_git_repo(&accudo_framework_url, oid).await?;

    cache
        .fetch_on_chain_package(
            &Url::from_str("https://fullnode.mainnet.accudo.org").unwrap(),
            3022354983,
            AccountAddress::ONE,
            "MoveStdlib",
        )
