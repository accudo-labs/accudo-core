...
pub struct Dependency {
    ...
    /// Location of the dependency: local, git, or accudo (on-chain).
...
pub enum PackageLocation {
    ...
    //       - Leaky abstraction -- can we still want to maintain clear Move/Accudo separation?
    ...
    Accudo {
        /// URL to the Accudo full-node connected to the network where the package is published.
        node_url: String,
...
impl<'de> Deserialize<'de> for Dependency {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        error_on_unneeded_fields!(address, accudo);

        let location = match (raw.local, raw.git, raw.accudo) {
            ...
            (None, None, Some(node_url)) => match raw.address {
                Some(package_addr) => PackageLocation::Accudo {
                    node_url,
                    package_addr,
                },
                None => {
                    return Err(serde::de::Error::custom(
                        "missing field \"address\" for accudo dependency",
                    ))
                },
            },
            (None, None, None) => {
...
impl Dependency {
    fn into_raw(self) -> RawDependency {
        ...
        match self.location {
            ...
            PackageLocation::Accudo {
                node_url,
                package_addr,
            } => {
                raw.accudo = Some(node_url);
