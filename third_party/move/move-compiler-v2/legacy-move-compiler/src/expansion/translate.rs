...
// This is a hack to recognize ACCUDO StdLib, Framework, and Token libs to avoid warnings on some old errors.
// This will be removed after library attributes are cleaned up.
// (See https://github.com/accudo-labs/accudo-core/issues/9410)
fn module_is_in_accudo_libs(module_address: Option<Spanned<Address>>) -> bool {
    const ACCUDO_STDLIB_NAME: &str = "accudo_std";
    static ACCUDO_STDLIB_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
        Lazy::new(|| NumericalAddress::parse_str("0x1").unwrap());
    const ACCUDO_FRAMEWORK_NAME: &str = "accudo_framework";
    static ACCUDO_FRAMEWORK_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
        Lazy::new(|| NumericalAddress::parse_str("0x1").unwrap());
    const ACCUDO_TOKEN_NAME: &str = "accudo_token";
    static ACCUDO_TOKEN_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
        Lazy::new(|| NumericalAddress::parse_str("0x3").unwrap());
    const ACCUDO_TOKEN_OBJECTS_NAME: &str = "accudo_token_objects";
    static ACCUDO_TOKEN_OBJECTS_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
                ...
                Address::Numerical(optional_name, spanned_numerical_address) => match optional_name
                {
                    Some(spanned_symbol) => {
                        ((&spanned_symbol.value as &str) == ACCUDO_STDLIB_NAME
                            ...
                            || ((&spanned_symbol.value as &str) == ACCUDO_TOKEN_NAME
                                && (spanned_numerical_address.value
                                    == *ACCUDO_TOKEN_NUMERICAL_ADDRESS))
                            || ((&spanned_symbol.value as &str) == ACCUDO_TOKEN_OBJECTS_NAME
                                && (spanned_numerical_address.value
                                    == *ACCUDO_TOKEN_OBJECTS_NUMERICAL_ADDRESS))
                    },
                    None => false,
                },
...
fn module_(
    ...
    if context.env.flags().warn_of_deprecation_use_in_accudo_libs() {
        context.in_accudo_libs = false;
    } else {
        context.in_accudo_libs = module_is_in_accudo_libs(module_address);
    }
...
fn check_for_deprecated_module_use(context: &mut Context, mident: &ModuleIdent) -> bool {
    ...
    if !warn_deprecation || context.in_deprecated_code || context.in_accudo_libs {
        return false;
    }
