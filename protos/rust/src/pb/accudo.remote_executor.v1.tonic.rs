        ...
        <T::ResponseBody as Body>::Error: Into<StdError> + Send,
    {
        ...
        pub async fn simple_msg_exchange(
            ...
            let path = http::uri::PathAndQuery::from_static(
                "/accudo.remote_executor.v1.NetworkMessageService/SimpleMsgExchange",
            );
            let mut req = request.into_request();
            req.extensions_mut()
                .insert(
                    GrpcMethod::new(
                        "accudo.remote_executor.v1.NetworkMessageService",
                        "SimpleMsgExchange",
                    ),
                );
...
pub mod network_message_service_server {
        ...
        fn call(&mut self, req: http::Request<B>) -> Self::Future {
            match req.uri().path() {
                "/accudo.remote_executor.v1.NetworkMessageService/SimpleMsgExchange" => {
                    ...
                    let fut = async move {
                        let method = SimpleMsgExchangeSvc(inner);
                        let codec = tonic::codec::ProstCodec::default();
    ...
    for NetworkMessageServiceServer<T> {
        const NAME: &'static str = "accudo.remote_executor.v1.NetworkMessageService";
    }
}
