...
use accudo_crypto::{x25519::PrivateKey, Uniform};
use accudo_event_notifications::{ReconfigNotification, ReconfigNotificationListener};
use accudo_infallible::{Mutex, MutexGuard, RwLock};
...
pub struct NodeNetworkInterface {
    ...
    pub(crate) network_reqs_rx: accudo_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
    /// Peer notification sender for sending outgoing messages to other peers
    pub(crate) network_notifs_tx: accudo_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>,
}
...
fn setup_node_network_interface(
    ...
    let (connection_reqs_tx, _) = accudo_channel::new(QueueStyle::FIFO, MAX_QUEUE_SIZE, None);
...
fn start_node_mempool(
    ...
    let runtime = accudo_runtimes::spawn_named_runtime("shared-mem".into(), None);
