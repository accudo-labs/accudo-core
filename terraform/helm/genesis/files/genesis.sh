...
#
# Runs an automated genesis ceremony for validators spun up by the accudo-node helm chart
#
# Expect the following environment variables to be set before execution:
# NUM_VALIDATORS
# ERA
# WORKSPACE: default /tmp
# USERNAME_PREFIX: default accudo-node
...
USERNAME_PREFIX=${USERNAME_PREFIX:-accudo-node}
...
MOVE_FRAMEWORK_DIR=${MOVE_FRAMEWORK_DIR:-"/accudo-framework/move"}
...
export ACCUDO_DISABLE_TELEMETRY=true
...
for i in $(seq 0 $(($NUM_VALIDATORS - 1))); do
  ...
  if [[ -z "${RANDOM_SEED}" ]]; then
    accudo genesis generate-keys --output-dir $user_dir
  else
    seed=$(printf "%064x" "$((${RANDOM_SEED_IN_DECIMAL} + i))")
    echo "seed=$seed for ${i}th validator"
    accudo genesis generate-keys --random-seed $seed --output-dir $user_dir
  fi

  accudo genesis set-validator-configuration --owner-public-identity-file $user_dir/public-keys.yaml --local-repository-dir $WORKSPACE \
    --username $username \
...
# get the framework
# this is the directory the accudo-framework is located in the accudolabs/tools docker image
