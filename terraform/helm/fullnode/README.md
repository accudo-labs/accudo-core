...
|-----|------|---------|-------------|
| affinity | object | `{}` |  |
| accudo_chains | object | `{"devnet":{"genesis_blob_url":"https://devnet.accudo.org/genesis.blob","waypoint_txt_url":"https://devnet.accudo.org/waypoint.txt"},"mainnet":{"genesis_blob_url":"https://raw.githubusercontent.com/accudo-labs/accudo-networks/main/mainnet/genesis.blob","waypoint_txt_url":"https://raw.githubusercontent.com/accudo-labs/accudo-networks/main/mainnet/waypoint.txt"},"testnet":{"genesis_blob_url":"https://raw.githubusercontent.com/accudo-labs/accudo-networks/main/testnet/genesis.blob","waypoint_txt_url":"https://raw.githubusercontent.com/accudo-labs/accudo-networks/main/testnet/genesis_waypoint.txt"}}` | For each supported chain, specify the URLs from which to download the genesis.blob and waypoint.txt |
...
| backup.image.repo | string | `"accudolabs/tools"` | Image repo to use for backup images |
...
| chain.name | string | `"devnet"` | Name of the testnet to connect to. There must be a corresponding entry in .Values.accudo_chains |
| fullnode.config | object | `{"full_node_networks":[{"identity":{},"inbound_rate_limit_config":null,"network_id":"public","outbound_rate_limit_config":null}]}` | Fullnode configuration. See NodeConfig https://github.com/accudo-labs/accudo-core/blob/main/config/src/config/mod.rs |
| image.pullPolicy | string | `"IfNotPresent"` | Image pull policy to use for fullnode images |
| image.repo | string | `"accudolabs/validator"` | Image repo to use for fullnode images. Fullnodes and validators use the same image |
...
| restore.image.repo | string | `"accudolabs/tools"` | Image repo to use for restore images |
...
| service.type | string | `"ClusterIP"` | The Kubernetes ServiceType to use for the fullnode. Change this to LoadBalancer expose the REST API, accudonet endpoint externally |
...
This Helm chart deploys a public fullnode for the Accudo blockchain network. The
fullnode connects to Accudo validators and synchronises the blockchain state to
...
To connect to the Accudo devnet, you must have the correct genesis blob and waypoint. The source of truth for these are hosted here: https://github.com/accudo-labs/accudo-genesis-waypoint

The waypoint and genesis blobs are download at runtime, and their URLs are specified in `.Values.accudo_chains`.
...
[REST API]: https://github.com/accudo-labs/accudo-core/blob/main/api/doc/spec.yaml
[values.yaml]: values.yaml
[Accudo dockerhub]: https://hub.docker.com/r/accudolabs/validator/tags?page=1&ordering=last_updated
