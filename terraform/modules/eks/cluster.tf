resource "aws_cloudwatch_log_group" "eks" {
  name              = "/aws/eks/accudo-${local.workspace_name}/cluster"
  retention_in_days = 7
  tags              = local.default_tags
}

resource "aws_eks_cluster" "accudo" {
...
data "aws_eks_cluster_auth" "accudo" {
  name = aws_eks_cluster.accudo.name
}
...
resource "aws_launch_template" "nodes" {
  ...
  name          = "accudo-${local.workspace_name}/${each.key}"
...
resource "aws_eks_node_group" "nodes" {
  for_each        = local.pools
  cluster_name    = aws_eks_cluster.accudo.name
  node_group_name = each.key
  version         = aws_eks_cluster.accudo.version
...
resource "aws_iam_openid_connect_provider" "cluster" {
  ...
  url             = aws_eks_cluster.accudo.identity[0].oidc[0].issuer
}
...
resource "aws_iam_role" "aws-ebs-csi-driver" {
  name                 = "accudo-${local.workspace_name}-ebs-csi-controller"
  path                 = var.iam_path
...
data "aws_iam_policy_document" "cluster-autoscaler" {
  statement {
    ...
    condition {
      test     = "StringEquals"
      variable = "aws:ResourceTag/k8s.io/cluster-autoscaler/${aws_eks_cluster.accudo.name}"
      values   = ["owned"]
    }
  }
...
resource "aws_iam_role" "cluster-autoscaler" {
  name                 = "accudo-fullnode-${local.workspace_name}-cluster-autoscaler"
  path                 = var.iam_path
  permissions_boundary = var.permissions_boundary_policy
...
resource "helm_release" "autoscaling" {
  ...
  values = [
    jsonencode({
      autoscaler = {
        enabled     = true
        clusterName = aws_eks_cluster.accudo.name
        image = {
          # EKS does not report patch version
          tag = "v${aws_eks_cluster.accudo.version}.0"
        }
        serviceAccount = {
          annotations = {
