...
resource "local_file" "kubernetes" {
  ...
  content = jsonencode({
    kubernetes_host        = aws_eks_cluster.accudo.endpoint
    kubernetes_ca_cert     = base64decode(aws_eks_cluster.accudo.certificate_authority[0].data)
    issuer                 = aws_eks_cluster.accudo.identity[0].oidc[0].issuer
    service_account_prefix = "accudo-pfn"
