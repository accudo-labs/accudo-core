resource "google_storage_bucket" "genesis" {
  ...
  name                        = "accudo-${terraform.workspace}-genesis"
  project                     = var.project
  location                    = var.region
...
resource "google_storage_bucket" "signed-url" {
  count                       = var.enable_forge ? 1 : 0
  name                        = "accudo-${terraform.workspace}-signed-url-gcf"
  project                     = var.project
  location                    = var.region
  uniform_bucket_level_access = true
}
...
resource "google_service_account" "signed-url" {
  count      = var.enable_forge ? 1 : 0
  project    = var.project
  account_id = "accudo-${local.workspace_name}-signed-url"
}
...
resource "google_cloudfunctions2_function" "signed-url" {
  count    = var.enable_forge ? 1 : 0
  project  = var.project
  name     = "accudo-${local.workspace_name}-signed-url"
  location = var.region
