...
resource "helm_release" "chaos-mesh" {
  ...
  values = [
    jsonencode({
      chaos-mesh = {
        images = {
          registry = "us-docker.pkg.dev/accudo-registry/docker/ghcr.io"
          tag      = "accudo-patch" // Same as the patched chart in helm/chaos
        },
        chaosDaemon = {
          runtime    = "containerd"
  ...
  values = [
    jsonencode({
      ...
      extraArgs = [
        ...
        "--txt-owner-id=accudo-${local.workspace_name}",
        # "--txt-prefix=accudo-",
      ]
    })
  ]
...
resource "google_compute_global_address" "testnet-addons-ingress" {
  count   = var.zone_name != "" ? 1 : 0
  project = var.project
  name    = "accudo-${local.workspace_name}-testnet-addons-ingress"
}
...
resource "helm_release" "testnet-addons" {
  ...
  values = [
    jsonencode({
      ...
      genesis = {
        era             = var.era
        username_prefix = local.accudo_node_helm_prefix
      ...
      ingress = {
        gce_static_ip           = "accudo-${local.workspace_name}-testnet-addons-ingress"
        gce_managed_certificate = var.create_google_managed_ssl_certificate ? "accudo-${local.workspace_name}-${var.zone_name}-testnet-addons" : null
      }
    }),
