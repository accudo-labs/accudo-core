...
variable "chain_id" {
  description = "Accudo chain ID. If var.enable_forge set, defaults to 4"
  type        = number
  default     = 4
}
...
variable "chain_name" {
  description = "Accudo chain name. If unset, defaults to using the workspace name"
  type        = string
  default     = ""
}

variable "image_tag" {
  description = "Docker image tag for all Accudo workloads, including validators, fullnodes, backup, restore, genesis, and other tooling"
  type        = string
  default     = "devnet"
}
...
variable "accudo_node_helm_values" {
  description = "Map of values to pass to accudo-node helm chart"
  type        = any
  default     = {}
}
...
variable "manage_via_tf" {
  description = "Whether to manage the accudo-node k8s workload via Terraform. If set to false, the helm_release resource will still be created and updated when values change, but it may not be updated on every apply"
  type        = bool
  default     = true
}
