...
module "validator" {
  ...
  helm_release_name_override = var.enable_forge ? "accudo-node" : ""
  ...
  helm_values         = var.accudo_node_helm_values
...
locals {
  accudo_node_helm_prefix = var.enable_forge ? "accudo-node" : "${module.validator.helm_release_name}-accudo-node"
}
