provider "kubernetes" {
  host                   = digitalocean_kubernetes_cluster.accudo.endpoint
  cluster_ca_certificate = base64decode(digitalocean_kubernetes_cluster.accudo.kube_config[0].cluster_ca_certificate)
  token                  = digitalocean_kubernetes_cluster.accudo.kube_config[0].token
}

resource "kubernetes_namespace" "accudo" {
  metadata {
...
resource "helm_release" "fullnode" {
  ...
  values = [
    jsonencode({
      ...
      nodeSelector = {
        "doks.digitalocean.com/node-pool" = digitalocean_kubernetes_cluster.accudo.node_pool[0].name
      }
      storageClass = {
