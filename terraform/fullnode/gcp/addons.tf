...
resource "google_service_account_iam_binding" "k8s-gcp-integrations" {
  ...
  members            = ["serviceAccount:${google_container_cluster.accudo.workload_identity_config[0].workload_pool}[kube-system/k8s-gcp-integrations]"]
}
  ...
  values = [
    jsonencode({
      ...
      ingress = {
        ...
        gce_managed_certificate         = var.create_google_managed_ssl_certificate ? "accudo-${local.workspace_name}-ingress" : null
