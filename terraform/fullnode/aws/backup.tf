...
resource "aws_s3_bucket" "backup" {
  bucket = "accudo-${local.workspace_name}-backup-${random_id.backup-bucket.hex}"
}
...
data "aws_iam_policy_document" "backup-assume-role" {
  statement {
    ...
    condition {
      ...
      values = [for i in range(var.num_fullnodes) : "system:serviceaccount:default:pfn${i}-accudo-fullnode"]
    }
...
resource "aws_iam_role" "backup" {
  name               = "accudo-${local.workspace_name}-backup"
  path               = var.iam_path
