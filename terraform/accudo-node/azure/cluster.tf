...
resource "azurerm_kubernetes_cluster" "accudo" {
  name                            = "accudo-${terraform.workspace}"
  resource_group_name             = azurerm_resource_group.accudo.name
  location                        = azurerm_resource_group.accudo.location
  dns_prefix                      = "accudo-${terraform.workspace}"
  ...
  service_principal {
    client_id     = azuread_service_principal.accudo.application_id
    client_secret = azuread_application_password.accudo.value
  }
}

resource "azurerm_kubernetes_cluster_node_pool" "validators" {
  kubernetes_cluster_id = azurerm_kubernetes_cluster.accudo.id
  orchestrator_version  = azurerm_kubernetes_cluster.accudo.kubernetes_version
  ...
  os_disk_size_gb = 30
  node_taints     = var.validator_instance_enable_taint ? ["accudo.org/nodepool=validators:NoExecute"] : []
}

resource "azurerm_log_analytics_workspace" "accudo" {
  name                = "accudo-${terraform.workspace}"
  resource_group_name = azurerm_resource_group.accudo.name
  location            = azurerm_resource_group.accudo.location
...
resource "azurerm_monitor_diagnostic_setting" "cluster" {
  name                       = "cluster"
  target_resource_id         = azurerm_kubernetes_cluster.accudo.id
  log_analytics_workspace_id = azurerm_log_analytics_workspace.accudo.id
