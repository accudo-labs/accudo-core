...
resource "azurerm_virtual_network" "accudo" {
...
resource "azurerm_subnet" "nodes" {
  name                 = "nodes"
  resource_group_name  = azurerm_resource_group.accudo.name
  virtual_network_name = azurerm_virtual_network.accudo.name
  address_prefixes     = [cidrsubnet(local.vnet_address, 4, 0)]
  service_endpoints    = ["Microsoft.Storage"]
}

resource "azurerm_public_ip" "nat" {
  name                = "accudo-${terraform.workspace}-nat"
  resource_group_name = azurerm_resource_group.accudo.name
  location            = azurerm_resource_group.accudo.location
...
locals {
  cluster_ips = concat(
    ...
    [azurerm_kubernetes_cluster.accudo.network_profile[0].service_cidr,
    azurerm_kubernetes_cluster.accudo.network_profile[0].pod_cidr]
  )
}

resource "azurerm_network_security_group" "nodes" {
  name                = "accudo-${terraform.workspace}-nodes"
  resource_group_name = azurerm_resource_group.accudo.name
  location            = azurerm_resource_group.accudo.location
