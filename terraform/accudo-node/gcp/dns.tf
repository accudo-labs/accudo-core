...
locals {
  ...
  domain      = var.zone_name != "" ? "${local.dns_prefix}${data.google_dns_managed_zone.accudo[0].dns_name}" : null
}

data "kubernetes_service" "validator-lb" {
  count = var.zone_name != "" && var.create_dns_records ? 1 : 0
  metadata {
    name = "${local.workspace_name}-accudo-node-0-validator-lb"
  }
...
data "kubernetes_service" "fullnode-lb" {
  count = var.zone_name != "" && var.create_dns_records ? 1 : 0
  metadata {
    name = "${local.workspace_name}-accudo-node-0-fullnode-lb"
  }
  depends_on = [time_sleep.lb_creation]
}

data "google_dns_managed_zone" "accudo" {
...
resource "google_dns_record_set" "validator" {
  count        = var.zone_name != "" && var.create_dns_records ? 1 : 0
  managed_zone = data.google_dns_managed_zone.accudo[0].name
  project      = data.google_dns_managed_zone.accudo[0].project
  name         = "${random_string.validator-dns.result}.${local.record_name}.${data.google_dns_managed_zone.accudo[0].dns_name}"
  type         = "A"
  ttl          = var.dns_ttl
...
resource "google_dns_record_set" "fullnode" {
  ...
  managed_zone = data.google_dns_managed_zone.accudo[0].name
  project      = data.google_dns_managed_zone.accudo[0].project
  name         = "${local.record_name}.${data.google_dns_managed_zone.accudo[0].dns_name}"
  type         = "A"
  ttl          = var.dns_ttl
