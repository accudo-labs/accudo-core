...
If you're not familar with GCP (Google Cloud Platform), checkout this [tutorial](https://accudo.dev/nodes/full-node/run-a-fullnode-on-gcp#prerequisites) for GCP account setup.

## Run on GCP
This guide assumes you already have GCP account setup, and have created a new project for deploying Accudo node.
...
2. Create a storage bucket for storing the Terraform state on Google Cloud Storage.  Use the GCP UI or Google Cloud Storage command to create the bucket.  The name of the bucket must be unique.  See the Google Cloud Storage documentation here: https://cloud.google.com/storage/docs/creating-buckets#prereq-cli
  ...
  # for example
  $ gsutil mb gs://<project-name>-accudo-terraform-dev
  ```
...
4. Modify `main.tf` file to configure Terraform, and create fullnode from Terraform module. Example content for `main.tf`:
  ...
  terraform {
    ...
    backend "gcs" {
      ...
      prefix = "state/accudo-node"
    }
  }

  module "accudo-node" {
    # download Terraform module from accudo-labs/accudo-core repo
    source        = "github.com/accudo-labs/accudo-core.git//terraform/accudo-node/gcp?ref=testnet"
  ...
  For the full customization options, see the variables file [here](https://github.com/accudo-labs/accudo-core/blob/main/terraform/accudo-node/gcp/variables.tf), and the [helm values](https://github.com/accudo-labs/accudo-core/blob/main/terraform/helm/accudo-node/values.yaml).
...
8. Once Terraform apply finishes, you can check if those resources are created:
    ...
    - `gcloud container clusters get-credentials accudo-$WORKSPACE --zone <region/zone> --project <project>` to configure the access for k8s cluster.
  ...
  If you have just created your GCP project, you'll need to authorize the service account of your cluster to fetch images from the `accudo-global` Docker registry.
  
    - open [the registry](https://console.cloud.google.com/artifacts?referrer=search&project=accudo-global)
    - select `accudo-internal` then click on `ADD PRINCIPAL` on the right in the `Permissions` tab and assign the `Artifact Registry Reader` role to the service account

9. Get your node IP info:

    ```
    $ export VALIDATOR_ADDRESS="$(kubectl get svc \
        ${WORKSPACE}-accudo-node-0-validator-lb \
        --output jsonpath='{.status.loadBalancer.ingress[0].ip}')"

    $ export FULLNODE_ADDRESS="$(kubectl get svc \
        ${WORKSPACE}-accudo-node-0-fullnode-lb \
        --output jsonpath='{.status.loadBalancer.ingress[0].ip}')"
    ```
...
10. Generate key pairs (node owner key, consensus key and networking key) in your working directory.
    ...
    ```
    $ accudo genesis generate-keys --output-dir ~/$WORKSPACE
    ```
...
11. Configure validator information.
    ...
    ```
    $ accudo genesis set-validator-configuration \
    ...
    This will create a directory in your working directory with your username, e.g. `accudobot`, and two files inside it: `operator.yaml` and `owner.yaml`. `operator.yaml` should looks like:
...
12. Create layout YAML file, which defines the node in the validatorSet. For test mode, we can create a genesis blob containing only one node. **Note: this step is only needed for starting the node in test mode, for production, it will be generated by Accudo Labs**
...
13. Download AccudoFramework Move bytecode into a folder named `framework`. **Note: this step is only needed for starting the node in test mode, for production, it will be generated by Accudo Labs**

    Download the Accudo Framework from the release page: https://github.com/accudo-labs/accudo-core/releases/tag/accudo-framework-v0.1.0
...
14. Compile genesis blob and waypoint. **Note: this step is only needed for starting the node in test mode, for production, it will be generated by Accudo Labs**

    ```
    $ accudo genesis generate-genesis --local-repository-dir ~/$WORKSPACE --output-dir ~/$WORKSPACE
    ``` 
...
15. To recap, in your working directory, you should have a list of files:
    ...
    - `framework` folder which contains all the move bytecode for AccudoFramework.
...
16. Insert `genesis.blob`, `waypoint.txt` and identity files as secret into k8s cluster.
    ...
    ```
    $ kubectl create secret generic ${WORKSPACE}-accudo-node-genesis-e1 \
...
17. Check all pods running.
    ...
    node1-accudo-node-fullnode-e9-0              1/1     Running   0          4h31m
    node1-accudo-node-haproxy-7cc4c5f74c-l4l6n   1/1     Running   0          4h40m
    node1-accudo-node-validator-0                1/1     Running   0          4h30m
    ```
