resource "google_compute_network" "accudo" {
...
resource "time_sleep" "create-subnetworks" {
  create_duration = "30s"
  depends_on      = [google_compute_network.accudo]
}

data "google_compute_subnetwork" "region" {
  name       = google_compute_network.accudo.name
  depends_on = [time_sleep.create-subnetworks]
}

resource "google_compute_router" "nat" {
  name    = "accudo-${local.workspace_name}-nat"
  network = google_compute_network.accudo.id
}
