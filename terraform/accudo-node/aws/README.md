...
Install pre-requisites if needed:
   ...
   * Accudo CLI: https://github.com/accudo-labs/accudo-core/releases/tag/accudo-cli-v0.1.0-alpha
...
4. Modify `main.tf` file to configure Terraform, and create fullnode from Terraform module. Example content for `main.tf`:
  ...
  terraform {
    ...
    backend "s3" {
      bucket = "terraform.accudo-node"
      key    = "state/accudo-node"
      region = var.region
    }
  }
  ...
  module "accudo-node" {
    ...
    source        = "github.com/accudo-labs/accudo-core.git//terraform/accudo-node/aws?ref=main"
...
For the full customization options, see the variables file [here](https://github.com/accudo-labs/accudo-core/blob/main/terraform/accudo-node/aws/variables.tf), and the [helm values](https://github.com/accudo-labs/accudo-core/blob/main/terraform/helm/accudo-node/values.yaml).
...
8. Once terraform apply finishes, you can check if those resources are created:
    ...
    - `aws eks update-kubeconfig --name accudo-$WORKSPACE` to configure access for your k8s cluster.
...
9. Get your node IP info:
    ...
    ```
    $ export VALIDATOR_ADDRESS="$(kubectl get svc ${WORKSPACE}-accudo-node-validator-lb --output jsonpath='{.status.loadBalancer.ingress[0].hostname}')"

    $ export FULLNODE_ADDRESS="$(kubectl get svc ${WORKSPACE}-accudo-node-fullnode-lb --output jsonpath='{.status.loadBalancer.ingress[0].hostname}')"
    ```
...
11. Configure validator information.
    ...
    ```
    $ accudo genesis set-validator-configuration --keys-dir ~/$WORKSPACE --local-repository-dir ~/$WORKSPACE --username <pick a username for your node> --validator-host $VALIDATOR_ADDRESS:6180 --full-node-host $FULLNODE_ADDRESS:6182

    ```

    This will create a YAML file in your working directory with your username, e.g. `accudobot.yaml`, it should looks like below:
...
13. Download AccudoFramework Move bytecodes into a folder named `framework`.
