...
locals {
  ...
  domain = var.zone_id != "" ? "${local.dns_prefix}${data.aws_route53_zone.accudo[0].name}" : null
}

data "kubernetes_service" "validator-lb" {
  count = var.zone_id == "" || !var.create_records ? 0 : 1
  metadata {
    # This is the main validator LB service that is created by the accudo-node helm chart
...
data "kubernetes_service" "fullnode-lb" {
  count = var.zone_id == "" || !var.create_records ? 0 : 1
  metadata {
    # This is the main fullnode LB service that is created by the accudo-node helm chart
