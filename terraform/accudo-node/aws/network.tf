...
resource "aws_vpc" "vpc" {
  ...
  tags = merge(local.default_tags, {
    Name                                                  = "accudo-${local.workspace_name}"
    "kubernetes.io/cluster/accudo-${local.workspace_name}" = "shared"
  })
}
...
resource "aws_subnet" "public" {
  ...
  tags = merge(local.default_tags, {
    Name                                                  = "accudo-${local.workspace_name}/public-${local.aws_availability_zones[count.index]}"
    "kubernetes.io/cluster/accudo-${local.workspace_name}" = "shared"
...
resource "aws_route_table" "public" {
  ...
  tags = merge(local.default_tags, {
    Name = "accudo-${local.workspace_name}/public"
  })
}
...
resource "aws_subnet" "private" {
  ...
  tags = merge(local.default_tags, {
    Name                                                  = "accudo-${local.workspace_name}/private-${local.aws_availability_zones[count.index]}"
    "kubernetes.io/cluster/accudo-${local.workspace_name}" = "shared"
...
resource "aws_eip" "nat" {
  ...
  tags = merge(local.default_tags, {
    Name = "accudo-${local.workspace_name}-nat"
  })
}
...
resource "aws_route_table" "private" {
  ...
  tags = merge(local.default_tags, {
    Name = "accudo-${local.workspace_name}/private"
  })
}
...
resource "aws_security_group" "cluster" {
  name        = "accudo-${local.workspace_name}/cluster"
  description = "k8s masters"
  vpc_id      = aws_vpc.vpc.id

  tags = merge(local.default_tags, {
    "kubernetes.io/cluster/accudo-${local.workspace_name}" = "owned"
  })
}
...
resource "aws_security_group" "nodes" {
  name        = "accudo-${local.workspace_name}/nodes"
