provider "kubernetes" {
  host                   = aws_eks_cluster.accudo.endpoint
  cluster_ca_certificate = base64decode(aws_eks_cluster.accudo.certificate_authority[0].data)
  token                  = data.aws_eks_cluster_auth.accudo.token
}
...
resource "null_resource" "delete-gp2" {
  provisioner "local-exec" {
    command = <<-EOT
      aws --region ${var.region} eks update-kubeconfig --name ${aws_eks_cluster.accudo.name} --kubeconfig ${local.kubeconfig} &&
