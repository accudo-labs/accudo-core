...
const DEVTOOL_TARGET_DIRECTORY: &str = "target/accudo-x-tool";

// File types in `accudo-core` that are not relevant to the rust build and test process.
...
// Paths in `accudo-core` that are not relevant to the rust build and test process.
// Note: this is a best effort list and will need to be updated as time goes on.
...
fn read_metadata_from_gcs(
    ...
    let url = format!(
        "https://storage.googleapis.com/accudo-core-cargo-metadata-public/{}",
        remote_file_name
    );
...
fn recompute_merge_base_metadata(
    ...
    // Clone the accudo-core repository to the local directory
    debug!(
        "Cloning accudo-core repository to local directory: {:?}",
        local_dir_path
    );
    let clone_directory = format!("{}/accudo-core", local_dir_path);
    Command::new("git")
        .arg("clone")
        .arg("--depth")
        .arg("500") // Clone the last 500 commits to ensure the merge base is included
        .arg("https://github.com/accudo-labs/accudo-core.git")
        .arg(clone_directory.clone())
        .output()
        .expect("failed to execute git clone");

    // Check out the merge base commit for the accudo-core clone
    debug!(
        "Checking out merge base commit for accudo-core clone: {:?}",
        merge_base
    );
    Command::new("git")
