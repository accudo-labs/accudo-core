...
use accudo_indexer_grpc_fullnode::stream_coordinator::{
...
impl TableInfoService {
        ...
        let backup_is_enabled = match self.backup_restore_operator.clone() {
            Some(backup_restore_operator) => {
                ...
                let _task = tokio::spawn(async move {
                    loop {
                        accudo_logger::info!("[Table Info] Checking for snapshots to backup.");
    ...
    async fn backup_snapshot_if_present(
        ...
        if epochs_to_backup.is_empty() {
            ...
            accudo_logger::info!("[Table Info] No snapshot to backup. Skipping the backup.");
            return;
        }
        accudo_logger::info!(
