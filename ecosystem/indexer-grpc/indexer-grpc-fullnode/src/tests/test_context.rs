...
use accudo_crypto::{hash::HashValue, SigningKey};
use accudo_mempool::mocks::MockSharedMempool;
use accudo_protos::extractor::v1::Transaction as TransactionPB;
use accudo_sdk::{
    transaction_builder::TransactionFactory,
    types::{
        account_config::accudo_test_root_address, transaction::SignedTransaction, LocalAccount,
    },
};
...
pub fn new_test_context(test_name: &str, fake_start_time_usecs: u64) -> TestContext {
    ...
    let (db, db_rw) = DbReaderWriter::wrap(AccudoDB::new_for_test_with_indexer(&tmp_dir));
    let ret =
        db_bootstrapper::maybe_bootstrap::<AccudoVM>(&db_rw, &genesis, genesis_waypoint).unwrap();
    assert!(ret);
    ...
    TestContext::new(
        ...
        root_key,
        validator_owner,
        Box::new(BlockExecutor::<AccudoVM>::new(db_rw)),
        mempool,
        db,
        test_name.to_string(),
...
impl TestContext {
    ...
    pub fn root_account(&self) -> LocalAccount {
        LocalAccount::new(accudo_test_root_address(), self.root_key.private_key(), 0)
    }
