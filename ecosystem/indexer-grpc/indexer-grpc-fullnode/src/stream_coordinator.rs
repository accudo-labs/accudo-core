...
use accudo_api_types::{AsConverter, Transaction as APITransaction, TransactionOnChainData};
use accudo_indexer_grpc_utils::{
    chunk_transactions,
    constants::MESSAGE_SIZE_LIMIT,
    counters::{log_grpc_step_fullnode, IndexerGrpcStep},
};
use accudo_logger::{error, info, sample, sample::SampleRate};
use accudo_protos::{
...
impl IndexerStreamCoordinator {
    ...
    fn convert_to_api_txns(
        ...
        let mut epoch_bcs = accudo_api_types::U64::from(epoch);
        let mut block_height = block_event.height();
        let mut block_height_bcs = accudo_api_types::U64::from(block_height);
        ...
        for (ind, raw_txn) in raw_txns.into_iter().enumerate() {
            ...
            if ind > 0 {
                ...
                if let Some(txn) = raw_txn.transaction.try_as_block_metadata_ext() {
                    ...
                    epoch = txn.epoch();
                    epoch_bcs = accudo_api_types::U64::from(epoch);
                    block_height += 1;
                    block_height_bcs = accudo_api_types::U64::from(block_height);
