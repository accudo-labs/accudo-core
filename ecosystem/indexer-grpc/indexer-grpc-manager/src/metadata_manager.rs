...
impl MetadataManager {
    ...
    async fn ping_live_data_service(
        ...
        if let Some(info) = response.into_inner().info {
            match info {
                accudo_protos::indexer::v1::ping_data_service_response::Info::LiveDataServiceInfo(info) => {
    ...
    async fn ping_historical_data_service(
        ...
        if let Some(info) = response.into_inner().info {
            match info {
                accudo_protos::indexer::v1::ping_data_service_response::Info::HistoricalDataServiceInfo(info) => {
                    self.handle_historical_data_service_info(address, info)
                },
                _ => bail!("Bad response."),
            }
