...
impl MoveStructType for TokenCancelOfferEvent {
    const MODULE_NAME: &'static IdentStr = ident_str!("token_transfers");
    const STRUCT_NAME: &'static IdentStr = ident_str!("TokenCancelOfferEvent");
}

impl MoveEventV1Type for TokenCancelOfferEvent {}

pub static TOKEN_CANCEL_OFFER_EVENT_TYPE: Lazy<TypeTag> = Lazy::new(|| {
    TypeTag::Struct(Box::new(StructTag {
        address: TOKEN_ADDRESS,
        module: ident_str!("token_transfers").to_owned(),
        name: ident_str!("TokenCancelOfferEvent").to_owned(),
        type_args: vec![],
    }))
});
