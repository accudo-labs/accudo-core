...
impl MoveStructType for CancelOffer {
    const MODULE_NAME: &'static IdentStr = ident_str!("token_transfers");
    const STRUCT_NAME: &'static IdentStr = ident_str!("CancelOffer");
}

impl MoveEventV2Type for CancelOffer {}

pub static CANCEL_OFFER_TYPE: Lazy<TypeTag> = Lazy::new(|| {
    TypeTag::Struct(Box::new(StructTag {
        address: TOKEN_ADDRESS,
        module: ident_str!("token_transfers").to_owned(),
        name: ident_str!("CancelOffer").to_owned(),
        type_args: vec![],
    }))
});
