...
impl MoveStructType for Claim {
    const MODULE_NAME: &'static IdentStr = ident_str!("token_transfers");
    const STRUCT_NAME: &'static IdentStr = ident_str!("Claim");
}

impl MoveEventV2Type for Claim {}

pub static CLAIM_TYPE: Lazy<TypeTag> = Lazy::new(|| {
    TypeTag::Struct(Box::new(StructTag {
        address: TOKEN_ADDRESS,
        module: ident_str!("token_transfers").to_owned(),
        name: ident_str!("Claim").to_owned(),
        type_args: vec![],
    }))
});
