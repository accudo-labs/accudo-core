...
/// Accudo specific metadata (`accudo::` here).
pub static ACCUDO_METADATA_KEY: &[u8] = "accudo::metadata_v0".as_bytes();
pub static ACCUDO_METADATA_KEY_V1: &[u8] = "accudo::metadata_v1".as_bytes();

/// Accudo specific metadata attached to the metadata section of file_format.
...
/// V1 of Accudo specific metadata attached to the metadata section of file_format.
...
pub fn get_metadata(md: &[Metadata]) -> Option<Arc<RuntimeModuleMetadataV1>> {
    if let Some(data) = find_metadata(md, ACCUDO_METADATA_KEY_V1) {
    ...
    } else if let Some(data) = find_metadata(md, ACCUDO_METADATA_KEY) {
...
fn check_metadata_format(module: &CompiledModule) -> Result<(), MalformedError> {
    ...
    for data in module.metadata.iter() {
        if data.key == *ACCUDO_METADATA_KEY || data.key == *ACCUDO_METADATA_KEY_V1 {
            if exist {
                return Err(MalformedError::DuplicateKey);
            }
            exist = true;

            if data.key == *ACCUDO_METADATA_KEY {
                bcs::from_bytes::<RuntimeModuleMetadata>(&data.value)
                    .map_err(|e| MalformedError::DeserializedError(data.key.clone(), e))?;
            } else if data.key == *ACCUDO_METADATA_KEY_V1 {
...
pub fn get_metadata_from_compiled_code(
    ...
    if let Some(data) = find_metadata(code.metadata(), ACCUDO_METADATA_KEY_V1) {
        ...
        metadata
    } else if let Some(data) = find_metadata(code.metadata(), ACCUDO_METADATA_KEY) {
