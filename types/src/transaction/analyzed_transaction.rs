...
pub fn coin_store_location(address: AccountAddress) -> StorageLocation {
    StorageLocation::Specific(
        StateKey::resource_typed::<CoinStoreResource<AccudoCoinType>>(&address).unwrap(),
    )
}
...
pub fn accudo_coin_info_location() -> StorageLocation {
    StorageLocation::Specific(
        StateKey::resource_typed::<CoinInfoResource<AccudoCoinType>>(
         ...
         -> (Vec<StorageLocation>, Vec<StorageLocation>) {
            match (
                *func.module().address(),
                ...
                (AccountAddress::ONE, "accudo_account", "transfer") => {
                    let receiver_address = bcs::from_bytes(&func.args()[0]).unwrap();
