...
/// ## Overview
///
/// Accudo `NetworkAddress` is a compact, efficient, self-describing and
...
/// 4. Perform a AccudoNet version negotiation handshake (version 1).
///
...
/// ## Transport
///
/// In addition, `NetworkAddress` is integrated with the AccudoNet concept of a
...
///
/// use accudo_types::network_address::NetworkAddress;
/// use bcs;
...
impl NetworkAddress {
    ...
    ///
    /// ```rust
    /// use accudo_crypto::{traits::ValidCryptoMaterialStringExt, x25519};
    /// use accudo_types::network_address::NetworkAddress;
    /// use std::str::FromStr;
    ///
    ...
    ///
    /// "typical" AccudoNet addresses begin with a transport protocol:
    ///
    ...
    /// assert!(addr.is_accudonet_addr());
    /// ```
    pub fn is_accudonet_addr(&self) -> bool {
        parse_accudonet_protos(self.as_slice()).is_some()
    }
...
#[cfg(any(test, feature = "fuzzing"))]
pub fn arb_accudonet_addr() -> impl Strategy<Value = NetworkAddress> {
    ...
    (arb_transport_protos, arb_accudonet_protos).prop_map(
        |(mut transport_protos, mut accudonet_protos)| {
            transport_protos.append(&mut accudonet_protos);
...
/// parse canonical accudonet protocols
///
/// See: [`NetworkAddress::is_accudonet_addr`]
fn parse_accudonet_protos(protos: &[Protocol]) -> Option<&[Protocol]> {
...
mod test {
    ...
    proptest! {
        #[test]
        fn test_network_address_canonical_serialization(addr in arb_accudonet_addr()) {
        ...
        #[test]
        fn test_network_address_display_roundtrip(addr in arb_accudonet_addr()) {
        ...
        #[test]
        fn test_is_accudonet_addr(addr in arb_accudonet_addr()) {
            assert!(addr.is_accudonet_addr(), "addr.is_accudonet_addr() = false; addr: '{}'", addr);
        }
    }
}
