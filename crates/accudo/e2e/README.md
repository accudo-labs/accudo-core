...
```
poetry run python main.py -d --base-network mainnet --test-cli-path ~/accudo-core/target/debug/accudo
```
...
```
DOCKER_DEFAULT_PLATFORM=linux/amd64 poetry run python main.py --base-network testnet --test-cli-path ~/accudo-core/target/debug/accudo
```
...
Traceback (most recent call last):
  File "/Users/dport/a/core/crates/accudo/e2e/main.py", line 194, in <module>
    if main():
  File "/Users/dport/a/core/crates/accudo/e2e/main.py", line 156, in main
    run_helper.prepare()
  File "/Users/dport/a/core/crates/accudo/e2e/test_helpers.py", line 155, in prepare
    self.prepare_cli()
  File "/Users/dport/a/core/crates/accudo/e2e/test_helpers.py", line 189, in prepare_cli
    raise RuntimeError(
RuntimeError: When using --test-cli-path you must use workspace configuration, try running `accudo config set-global-config --config-type workspace`
```
...
```
accudo config set-global-config --config-type workspace
```
...
The best place to look is the test framework working directory, by default `/tmp/accudo-cli-tests`.
...
```
$ cat /tmp/accudo-cli-tests/out/007_test_move_compile_script.stdout
{
  "Error": "Move compilation failed: Unable to resolve packages for package 'TwoByTwoTransfer': While resolving dependency 'AccudoFramework' in package 'TwoByTwoTransfer': While processing dependency 'AccudoFramework': Unable to find package manifest for 'AccudoFramework' at \"Move.toml/move/scripts/two_by_two_transfer/../../../framework/accudo-framework\""
}
```
