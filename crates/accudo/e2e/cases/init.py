...
def test_init(run_helper: RunHelper, test_name=None):
    ...
    run_helper.run_command(
        test_name,
        ["accudo", "init", "--assume-yes", "--network", "local"],
        input="\n",
    )

    # Assert that the CLI config is there.
    config_path = os.path.join(
        run_helper.host_working_directory, ".accudo", "config.yaml"
    )
    if not os.path.exists(config_path):
        raise TestError(
            f"{config_path} not found (in host working dir) after running accudo init"
        )
...
@test_case
def test_accudo_header_included(run_helper: RunHelper, test_name=None):
    # Make sure the accudo-cli header is included on the original request
    response = requests.get(run_helper.get_metrics_url())

    if 'request_source_client="accudo-cli' not in response.text:
        raise TestError("Request should contain the correct accudo header: accudo-cli")
