...
def run_node(network: Network, image_repo_with_project: str, pull=True):
    ...
    container_name = f"accudo-tools-{network}"
    LOG.info(f"Trying to run accudo CLI localnet from image: {image_name}")
    ...
    LOG.info(f"Running accudo CLI localnet from image: {image_name}. Container name: {container_name}")
