...
from accudo_sdk.async_client import RestClient
from common import METRICS_PORT, NODE_PORT, AccountInfo, Network, build_image_name
from accudo_sdk.account_address import AccountAddress
...
class RunHelper:
    ...
    def prepare_move(self):
        shutil.copytree(
            "../../../accudo-move/move-examples/cli-e2e-tests",
    ...
    def prepare_cli(self):
        if self.image_tag:
            ...
            response = self.run_command(
                ...
                ["accudo", "config", "show-global-config"],
            )
            ...
            if response["Result"]["config_type"].lower() != "workspace":
                raise RuntimeError(
                    ...
                    "try running `accudo config set-global-config --config-type workspace`"
                )

    # Get the account info of the account created by test_init.
    def get_account_info(self):
        path = os.path.join(self.host_working_directory, ".accudo", "config.yaml")
        with open(path) as f:
