...
Example (testing locally built CLI binary):
  python3 main.py --base-network devnet --test-cli-path ~/accudo-core/target/release/accudo

This means, run the CLI test suite using a CLI built from mainnet_0431e2251d0b42920d89a52c63439f7b9eda6ac3 against a localnet built from the testnet branch of accudo-core.
...
from cases.init import test_accudo_header_included, test_init, test_metrics_accessible
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="What branch the Accudo CLI used for the localnet should be built from",
    )
    parser.add_argument(
    ...
    test_cli_args.add_argument(
        "--test-cli-path",
        help="Path to CLI binary we want to test, e.g. /home/dport/accudo-core/target/release/accudo",
    )
    parser.add_argument(
        "--working-directory",
        default="/tmp/accudo-cli-tests",
...
async def run_tests(run_helper):
    ...
    # Make sure the accudo-cli header is included on the original request
    test_accudo_header_included(run_helper)
