# Accudo CLI Changelog

All notable changes to the Accudo CLI will be captured in this file. This project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html) and the format set out by [Keep a Changelog](https://keepachangelog.com/en/1.0.0/).
...
## [7.9.1]
- Add mem as pre-compiled module to avoid compatibility issue when using accudo-framework
...
- Add flag `--file-path <FILE_PATH>...` to `accudo move fmt`, which allows to specify individual files to format.
- Fixed bug where `accudo move fmt` would format current directory if invalid `--package-path` was provided.

## [7.3.0]

- Update boogie from 3.2.4 to 3.5.1.
- Change behavior of `accudo init` to first look and see if the account has an APT balance rather than checking if the account exists

## [7.2.0]

- Add ability to retrieve fungible asset balances
- Add `accudo key extract-public-key` which generates a public key or a proof of possession for the given key.

## [7.1.0]
- Add CLI outputs and on-disk storage to be stored in AIP-80 format.  Will allow for legacy formats to be taken in as well

## [7.0.0]
- Compiler v1 is now deprecated. It is now removed from the Accudo CLI.
- Added a new option `accudo move compile --fail-on-warning` which fails the compilation if any warnings are found.
...
- The new subcommand `accudo update move-mutation-test` will install/update the external binary `move-mutation-test`, which performs mutation testing on a Move project to find blind spots in Move unit tests.
...
## [6.1.1]
- Added a new feature to `accudo workspace run`: The workspace server now listens for a "stop" command from
stdin, which triggers a graceful shutdown when received.

## [6.1.0]
- Remove FFI support from Accudo CLI.
- Various compiler bug fixes.
...
## [6.0.2] - 2025/01/24
- Fix `accudo workspace run` so it does not panic when writing to closed stdout/stderr, allowing it to finish its graceful shutdown sequence when used as a child process.
...
- Add flag `--benchmark` to `accudo move prove`, which allows to benchmark verification times of individual functions in a package.
- Add flag `--only <name>` to `accudo move prove`, which allows to scope verification to a function.
- Fix `accudo init` to show the explorer link for accounts when account is already created on chain instead of prompting to fund the account.
- Set Compiler v2 as the default compiler and Move 2 as the default language version.
- Add new `--move-1` flag to use Compiler v1 and Move 1.
...
## [5.0.0] - 2024/12/11
- [**Breaking Change**] `accudo init` and `accudo account fund-with-faucet` no longer work directly with testnet, you must now use the minting page at the [Accudo dev docs](https://accudo.dev/network/faucet).
## [4.7.0] - 2024/12/10
- [`Fix`] CLI config should not always require a private key field to be present.

## [4.6.0] - 2024/11/29
- Add `--node-api-key` flag to `accudo move replay` to allow for querying the fullnode with an API key.
...
- Add option `--print-metadata-only` to `accudo move decompile` and `accudo move disassemble` to print out the metadata attached to the bytecode.
- Add `--existing-hasura-url` flag to localnet to tell it to use an existing Hasura instance instead of run Hasura itself. See https://github.com/accudo-labs/accudo-core/pull/15313.
...
## [4.4.0] - 2024/11/06
- Fix typos in `accudo move compile` help text.
- Update the default version of `movefmt` to be installed from 1.0.5 to 1.0.6
- Add `--host-postgres-host` flag: https://github.com/accudo-labs/accudo-core/pull/15216.
...
## [4.2.4] - 2024/10/21
- Releasing Move 2.1, which adds compound assignments (`x += 1`) and loop labels to the language. See [Move 2 Release Notes](https://accudo.dev/en/build/smart-contracts/book/move-2).
- multiple bug fixes in the Move 2 compilation chain.
- `accudo move fmt` formats move files inside the `tests` and `examples` directory of a package.
- Added `accudo update prover-dependencies`, which installs the dependency of Move prover, boogie, z3 and cvc5.
...
- Set compiler V2 as default when using `accudo move prove`.
...
- Adds `accudo move lint` to produce lint warnings for the current package. Only a few lint rules are implemented for now,
  but more are coming.
- Adds `accudo move fmt`, which runs the Move formatter, `movefmt`, on the current package. Also adds
  `accudo update movefmt`. This installs / updates the `movefmt` binary.
...
- [Early beta release of the Move compiler v2](https://accudo.dev/move/compiler_v2/) is now accessible through the CLI. We now allow specifying the Move compiler version and the Move language version via the CLI.
...
## [3.0.1] - 2024/03/05
- Fix bug in `accudo update revela` if default install directory doesn't exist.

## [3.0.0] - 2024/03/05
- **Breaking Change**: `accudo update` is now `accudo update accudo`.
- Added `accudo update revela`. This installs / updates the `revela` binary, which is needed for the new `accudo move decompile` subcommand.
- Extended `accudo move download` with an option `--bytecode` to also download the bytecode of a module
- Integrated the Revela decompiler which is now available via `accudo move decompile`
- Extended `accudo move disassemble` and the new `accudo move decompile` to also work on entire packages instead of only single files
...
- Added `for` loop.
  ...
  - Documentation: https://accudo.dev/move/book/loops
...
## [2.3.1] - 2023/11/07
### Updated
- Updated processor code from https://github.com/accudo-labs/accudo-indexer-processors for the localnet to 2d5cb211a89a8705674e9e1e741c841dd899c558.
...
- Updated processor code from https://github.com/accudo-labs/accudo-indexer-processors for the localnet to bcba94c26c8a6372056d2b69ce411c5719f98965.
...
## [2.2.2] - 2023/10/16
### Updated
- Updated processor code from https://github.com/accudo-labs/accudo-indexer-processors for the localnet to d6f55d4baba32960ea7be60878552e73ffbe8b7e.

## [2.2.1] - 2023/10/13
### Fixed
- Fixed postgres data persistence between restarts when using `accudo node run-local-testnet --with-indexer-api`.

## [2.2.0] - 2023/10/11
### Added
- Added `--with-indexer-api` to `accudo node run-local-testnet`. With this flag you can run a full processor + indexer API stack as part of your localnet. You must have Docker installed to use this feature. For more information, see https://accudo.dev/nodes/local-testnet/local-testnet-index.
### Updated
- Updated CLI source compilation to use rust toolchain version 1.72.1 (from 1.71.1).

## [2.1.1] - 2023/09/27
### Added
- Added an option `--print-metadata` to the command `accudo move download` to print out the metadata of the package to be downloaded.
  - Example: `accudo move download  --account 0x1 --package AccudoFramework --url https://mainnet.accudo.org/v1 --print-metadata`
### Updated
- The `--with-faucet` flag has been removed from `accudo node run-local-testnet`, we now run a faucet by default. To disable the faucet use the `--no-faucet` flag.
- **Breaking change**: When using `accudo node run-local-testnet` we now expose a transaction stream. Learn more about the transaction stream service here: https://accudo.dev/indexer/txn-stream/. Opt out of this with `--no-txn-stream`. This is marked as a breaking change since the CLI now uses a port (50051 by default) that it didn't used to. If you need this port, you can tell the CLI to use a different port with `--txn-stream-port`.
...
- Added basic ledger support for CLI
  - Example: `accudo init --ledger` to create a new profile from ledger. After this, you can use it the same way as other profiles.
  - Note: `Ledger Nano s Plus` or `Ledger Nano X` is highly recommended.

## [2.0.3] - 2023/08/04
### Fixed
- Fixed the following input arguments issue when running `accudo move view`
...
- init command from config subcommand is removed. Please use init from the root command.
  - Example: `accudo config init` -> `accudo init`
### Fixed
- Panic issue when running `accudo move test` is fixed - GitHub issue #8516
...
- Added custom header *x-accudo-client* for analytic purpose
...
## [1.0.13] - 2023/04/27
### Fixed
* Previously `--skip-fetch-latest-git-deps` would not actually do anything when used with `accudo move test`. This has been fixed.
* Fixed the issue of the hello_blockchain example where feature enable was missing

## [1.0.12] - 2023/04/25
### Added
* Support for creating and interacting with multisig accounts v2. More details can be found at [AIP 12](https://github.com/accudo-foundation/AIPs/blob/main/aips/aip-12.md).
* Added `disassemble` option to the CLI - This can be invoked using `accudo move disassemble` to disassemble the bytecode and save it to a file
* Fixed handling of `vector<string>` as an entry function argument in `accudo move run`

## [1.0.11] - 2023/04/14
### Fixed
* Fixed creating a new test account with `accudo init` would fail if the account didn't already exist

## [1.0.10] - 2023/04/13
### Fixed
* If `accudo init` is run with a faucet URL specified (which happens by default when using the local, devnet, or testnet network options) and funding the account fails, the account creation is considered a failure and nothing is persisted. Previously it would report success despite the account not being created on chain.
...
* Update `accudo init` to fix an incorrect account address issue, when trying to init with a rotated private key. Right now it does an actual account lookup instead of deriving from public key
...
## [1.0.9] - 2023/03/29
### Added
* `accudo move show abi` allows for viewing the ABI of a compiled move package
...
## [1.0.8] - 2023/03/16
### Added
* Added an `accudo account derive-resource-account-address` command to add the ability to derive an address easily
* Added the ability for different input resource account seeds, to allow matching directly with onchain code
* Added beta support for coverage via `accudo move coverage` and `accudo move test --coverage`
...
* `vector<string>` inputs to now work for both `accudo move view` and `accudo move run`
...
## [1.0.7]
* For logs earlier than 1.0.7, please check out the [releases on GitHub](https://github.com/accudo-labs/accudo-core/releases?q="Accudo+CLI+Release")
