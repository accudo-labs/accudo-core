...
The [Accudo command line interface (CLI)](https://accudo.dev/tools/accudo-cli/install-cli/) may be installed
via [Homebrew](https://brew.sh/) for simplicity. This is an in-depth overview of Homebrew and the Accudo formula. In this
guide, we go over each section of the Homebrew formula and steps to implement changes in the future.

## Quick guide

- [Formula in Homebrew GitHub](https://github.com/Homebrew/homebrew-core/blob/master/Formula/accudo.rb)
- [Accudo 1.0.3 New Formula PR for GitHub](https://github.com/Homebrew/homebrew-core/pull/119832)
- [Accudo Formula Fix PR to use build_cli_release.sh](https://github.com/Homebrew/homebrew-core/pull/120051)
...
# accudo 1.0.5
# Accudo Labs <opensource@accudo.org>
# Command Line Interface (CLI) for developing and interacting with the Accudo blockchain
# ...
```

## Change guide

Note: This guide is for developers who are trying to update the Accudo homebrew formula.

You can get the latest formula here: https://github.com/Homebrew/homebrew-core/blob/master/Formula/a/accudo.rb

Copy the `accudo.rb` file to your `homebrew` `formula` directory. For example, on macOS with an M1, this will likely be:
...
After you've copied `accudo.rb` to your local `homebrew` `formula` directory, you can modify it and use the commands
below for testing.
...
brew audit --new-formula accudo      # For new formula
brew audit accudo --strict --online
brew install accudo
brew test accudo

# For debugging issues during the installation process you can do
brew install accudo --interactive    # Interactive, gives you access to the shell
brew install accudo -d               # Debug mode

# Livecheck
brew livecheck --debug accudo
```
...
1. Commit your changes to `accudo-core` in `crates/accudo/homebrew`.
...
## Accudo.rb structure overview

### Header

```ruby
class Accudo < Formula
  desc "Layer 1 blockchain built to support fair access to decentralized assets for all"
  homepage "https://accudo.org/"
  url "https://github.com/accudo-labs/accudo-core/archive/refs/tags/accudo-cli-v1.0.3.tar.gz"
...
software by checking upstream. The strategy used below checks for all `accudo-cli-v<SEMVER>` tags for `accudo-core`. The
...
...
  ...
  # with matching regex of href="<URL>/tag/accudo-cli-v<SEMVER>". This
  # is done to automatically check for new release versions of the CLI.
  livecheck do
    url :stable
    regex(/^accudo-cli[._-]v?(\d+(?:\.\d+)+)$/i)
  end
...
```
...
```ruby
  # Installs listed homebrew dependencies before Accudo installation
  # Dependencies needed: https://accudo.dev/cli-tools/build-accudo-cli
  # See scripts/dev_setup.sh in accudo-core for more info
  ...
  # root Cargo.toml file of accudo-core (currently it is pegged to Rust 
...
The current test generates a new key via the Accudo CLI and ensures the shell output matches the filename(s) for that
key.

```ruby
  ...
  test do
    assert_match(/output.pub/i, shell_output("#{bin}/accudo key generate --output-file output"))
  end
  ...
```
