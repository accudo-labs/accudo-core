...
use accudo_rest_client::{
    accudo_api_types::{HashValue, WriteResource, WriteSetChange},
    Transaction,
};
...
impl CliCommand<TransferSummary> for TransferCoins {
    ...
    async fn execute(self) -> CliTypedResult<TransferSummary> {
        self.txn_options
            .submit_transaction(accudo_stdlib::accudo_account_transfer(
                self.account,
                self.amount,
            ))
            .await
...
const SUPPORTED_COINS: [&str; 1] = ["0x1::coin::CoinStore<0x1::accudo_coin::AccudoCoin>"];
