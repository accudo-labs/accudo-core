...
use accudo_rest_client::{
    accudo_api_types::{HexEncodedBytes, WriteResource, WriteSetChange},
    Transaction,
};
use accudo_types::{
...
impl CliCommand<CreateSummary> for Create {
    ...
    async fn execute(self) -> CliTypedResult<CreateSummary> {
        self.txn_options
            .submit_transaction(accudo_stdlib::multisig_account_create_with_owners(
    ...
    async fn execute(self) -> CliTypedResult<serde_json::Value> {
        ...
        if expected_payload_hash.eq(&actual_payload_hash) {
            ...
            Err(CliError::UnexpectedError(format!(
                ...
                https://accudo.dev/move/move-on-accudo/cli#multisig-governance"
            )))
        }
    }
}
...
impl CliCommand<TransactionSummary> for Approve {
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        self.txn_options
            .submit_transaction(accudo_stdlib::multisig_account_approve_transaction(
...
impl CliCommand<TransactionSummary> for Reject {
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        self.txn_options
            .submit_transaction(accudo_stdlib::multisig_account_reject_transaction(
                self.multisig_account_with_sequence_number
                    .multisig_account
                    .multisig_address,
                self.multisig_account_with_sequence_number.sequence_number,
