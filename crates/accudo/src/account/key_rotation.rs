...
use accudo_ledger;
use accudo_rest_client::{error::RestError, Client};
use accudo_types::{
    ...
    async fn execute(self) -> CliTypedResult<RotateSummary> {
        ...
        let rotation_proof_signed_by_current_private_key =
            if let Some(current_derivation_path) = current_derivation_path.clone() {
                ...
                let challenge_signature = accudo_ledger::sign_message(
        ...
        let txn_summary = self
            .txn_options
            .submit_transaction(accudo_stdlib::account_rotate_authentication_key(
                0,
