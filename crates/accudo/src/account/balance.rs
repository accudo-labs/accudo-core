...
use accudo_types::{account_address::AccountAddress, AccudoCoinType, CoinType};
...
pub struct Balance {
    ...
    /// Coin type to lookup.  Defaults to 0x1::accudo_coin::AccudoCoin
    #[clap(long)]
    ...
    async fn execute(self) -> CliTypedResult<Vec<AccountBalance>> {
        ...
        .map(|p| p.account)
        {
            ...
            return Err(CliError::CommandArgumentError(
                "Please provide an account using --account or run accudo init".to_string(),
            ));
        };
