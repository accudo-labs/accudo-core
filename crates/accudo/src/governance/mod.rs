...
use accudo_rest_client::{
    accudo_api_types::{Address, HexEncodedBytes, U128, U64},
    Client, Transaction,
};
use accudo_sdk::move_types::language_storage::CORE_CODE_ADDRESS;
use accudo_types::{
    account_address::AccountAddress,
    account_config::is_accudo_governance_create_proposal_event,
    event::EventHandle,
    ...
    async fn execute(mut self) -> CliTypedResult<Vec<ProposalSummary>> {
        ...
        let events = client
            .get_account_events_bcs(
                AccountAddress::ONE,
                "0x1::accudo_governance::GovernanceEvents",
...
async fn get_proposal(
    client: &accudo_rest_client::Client,
...
impl CliCommand<ProposalSubmissionSummary> for SubmitProposal {
    ...
    async fn execute(mut self) -> CliTypedResult<ProposalSubmissionSummary> {
        ...
        let txn: Transaction = if self.args.is_multi_step {
            self.args
                .txn_options
                .submit_transaction(accudo_stdlib::accudo_governance_create_proposal_v2(
            ...
            self.args
                .txn_options
                .submit_transaction(accudo_stdlib::accudo_governance_create_proposal(
                    self.pool_address_args.pool_address,
                    script_hash.to_vec(),
                    self.args.metadata_url.to_string().as_bytes().to_vec(),
                    metadata_hash.to_hex().as_bytes().to_vec(),
    ...
    async fn vote_before_partial_governance_voting(
        ...
        let voting_records = client
            .get_account_resource_bcs::<VotingRecords>(
                CORE_CODE_ADDRESS,
                "0x1::accudo_governance::VotingRecords",
            )
            .await
            .unwrap()
            .into_inner()
        ...
        for pool_address in &self.pool_addresses {
            let voting_record = client
                .get_table_item(
                    voting_records,
                    "0x1::accudo_governance::RecordKey",
                    "bool",
                    VotingRecord {
            ...
            summaries.push(
                self.args
                    .txn_options
                    .submit_transaction(accudo_stdlib::accudo_governance_vote(
                        *pool_address,
                        proposal_id,
                        vote,
                    ))
        ...
        for pool_address in &self.pool_addresses {
            let remaining_voting_power = self
                ...
                .view(ViewFunction {
                    module: ModuleId::new(
                        AccountAddress::ONE,
                        ident_str!("accudo_governance").to_owned(),
                    ),
            ...
            summaries.push(
                self.args
                    .txn_options
                    .submit_transaction(accudo_stdlib::accudo_governance_partial_vote(
                        *pool_address,
                        proposal_id,
                        voting_power,
                        vote,
...
/// This is needed for upgrading large packages such as accudo-framework.
#[derive(Parser)]
...
impl CliCommand<TransactionSummary> for ApproveExecutionHash {
    ...
    async fn execute(mut self) -> CliTypedResult<TransactionSummary> {
        Ok(self
            .txn_options
            .submit_transaction(
                accudo_stdlib::accudo_governance_add_approved_script_hash_script(self.proposal_id),
            )
            .await
            .map(TransactionSummary::from)?)
    }
    ...
    pub fn generate_hash(&self) -> CliTypedResult<(Vec<u8>, HashValue)> {
        let framework_local_dir = if self.framework_local_dir.is_some() {
            ...
            Option::from({
                ...
                path.pop();
                path.pop();
                path.join("accudo-move")
                    .join("framework")
                    .join("accudo-framework")
                    .canonicalize()
                    .map_err(|err| {
                        CliError::IO(
                            format!("Failed to canonicalize accudo framework path: {:?}", path),
                            err,
                        )
                    })?
            })
...
struct JsonProposal {
    ...
    execution_hash: accudo_rest_client::accudo_api_types::HashValue,
    expiration_secs: U64,
    is_resolved: bool,
    min_vote_threshold: U128,
    no_votes: U128,
