...
/// 1. Download the package with `accudo move download --account 0x42 --package PackName --bytecode`
/// 2. Disassemble the package bytecode with `accudo move disassemble --package-path PackName/bytecode_modules`
#[derive(Debug, Parser)]
...
/// 1. Download the package with `accudo move download --account 0x42 --package PackName --bytecode`
/// 2. Decompile the package bytecode with `accudo decompile --package-path PackName/bytecode_modules`
#[derive(Debug, Parser)]
...
struct BytecodeMetadata {
    accudo_metadata: Option<RuntimeModuleMetadataV1>,
...
impl BytecodeCommand {
    ...
    fn print_metadata(&self, bytecode_path: &Path) -> Result<String, CliError> {
        ...
        let metadata = if self.is_script {
            ...
            BytecodeMetadata {
                accudo_metadata: get_metadata_from_compiled_code(&script),
            ...
            BytecodeMetadata {
                accudo_metadata: get_metadata_from_compiled_code(&module),
