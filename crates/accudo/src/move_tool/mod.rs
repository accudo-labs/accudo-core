...
use accudo_gas_schedule::{MiscGasParameters, NativeGasParameters};
use accudo_move_debugger::accudo_debugger::AccudoDebugger;
use accudo_rest_client::{
    accudo_api_types::{EntryFunctionId, HexEncodedBytes, IdentifierWrapper, MoveModuleId},
    error::RestError,
    AccudoBaseUrl, Client,
};
use accudo_types::{
...
const HELLO_BLOCKCHAIN_EXAMPLE: &str = include_str!(
    "../../../../accudo-move/move-examples/hello_blockchain/sources/hello_blockchain.move"
);
...
pub struct FrameworkPackageArgs {
    /// Git revision or branch for the Accudo framework
    ///
    ...
    /// Local framework directory for the Accudo framework
    ///
...
impl FrameworkPackageArgs {
    pub fn init_move_dir(
        ...
        const ACCUDO_FRAMEWORK: &str = "AccudoFramework";
        const ACCUDO_GIT_PATH: &str = "https://github.com/accudo-labs/accudo-framework.git";
        const SUBDIR_PATH: &str = "accudo-framework";
...
/// This can then be run with `accudo move run-script`
#[derive(Parser)]
...
pub struct TestPackage {
    ...
    /// Collect coverage information for later use with the various `accudo move coverage` subcommands
    #[clap(long = "coverage")]
...
impl CliCommand<&'static str> for TestPackage {
    ...
    async fn execute(self) -> CliTypedResult<&'static str> {
        ...
        let result = move_cli::base::test::run_move_unit_tests(
            ...
            accudo_debug_natives::accudo_debug_natives(
                NativeGasParameters::zeros(),
                MiscGasParameters::zeros(),
            ),
            accudo_test_feature_flags_genesis(),
            None,
            None,
        ...
        if self.compute_coverage {
            ...
            println!("Please use `accudo move coverage -h` for more detailed source or bytecode test coverage of this package");
        }
...
/// Publishes the modules in a Move package to the Accudo blockchain
#[derive(Parser)]
...
fn create_package_publication_data(
    ...
    let payload = match publish_type {
        PublishType::AccountDeploy => {
            accudo_cached_packages::accudo_stdlib::code_publish_package_txn(
                metadata_serialized.clone(),
                compiled_units.clone(),
            )
        },
        PublishType::ObjectDeploy => {
            accudo_cached_packages::accudo_stdlib::object_code_deployment_publish(
                metadata_serialized.clone(),
                compiled_units.clone(),
            )
        },
        PublishType::ObjectUpgrade => {
            accudo_cached_packages::accudo_stdlib::object_code_deployment_upgrade(
                metadata_serialized.clone(),
                compiled_units.clone(),
...
/// Publishes the modules in a Move package to the Accudo blockchain, under an object (legacy version of `deploy-object`)
#[derive(Parser)]
...
/// Publishes the modules in a Move package to the Accudo blockchain, under an object.
#[derive(Parser)]
...
async fn submit_chunked_publish_transactions(
    ...
    if !is_staging_area_empty(txn_options, large_packages_module_address).await? {
        ...
        \nUse the `accudo move clear-staging-area` command to clean up the `StagingArea` resource under the account.",
    ...
    for (idx, payload) in payloads.into_iter().enumerate() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                \nTo avoid this, use the `accudo move clear-staging-area` command to clean up the `StagingArea` resource under your account before retrying.".bold());
                return Err(e);
            },
        }
    }
...
async fn is_staging_area_empty(
    ...
    match staging_area_response {
        ...
        Err(RestError::Api(accudo_error_response))
            if accudo_error_response.error.error_code == AccudoErrorCode::ResourceNotFound =>
        {
...
/// Publishes the modules in a Move package to the Accudo blockchain under a resource account
#[derive(Parser)]
...
impl CliCommand<TransactionSummary> for CreateResourceAccountAndPublishPackage {
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        ...
        .map(|p| p.account)
        {
            ...
            return Err(CliError::CommandArgumentError(
                "Please provide an account using --profile or run accudo init".to_string(),
            ));
        };
        ...
        let payload = accudo_cached_packages::accudo_stdlib::resource_account_create_resource_account_and_publish_package(
            seed,
...
impl ReplayNetworkSelection {
    pub fn to_base_url(&self) -> CliTypedResult<AccudoBaseUrl> {
        match self {
            ReplayNetworkSelection::Mainnet => Ok(AccudoBaseUrl::Mainnet),
            ReplayNetworkSelection::Testnet => Ok(AccudoBaseUrl::Testnet),
            ReplayNetworkSelection::Devnet => Ok(AccudoBaseUrl::Devnet),
            ReplayNetworkSelection::RestEndpoint(url) => {
                Ok(AccudoBaseUrl::Custom(Url::parse(url).map_err(|e| {
...
impl CliCommand<TransactionSummary> for Replay {
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        ...
        let debugger = AccudoDebugger::rest_client(client)?;
...
/// Duplicated from accudo_types, as we also need to load_account_arg from the CLI.
#[derive(Debug, Clone)]
pub struct MemberId {
