...
use accudo_framework::chunked_publish::{
    default_large_packages_module_address, CHUNK_SIZE_IN_BYTES,
};
use accudo_global_constants::adjust_gas_headroom;
...
use accudo_rest_client::{
    accudo_api_types::{EntryFunctionId, HashValue, MoveType, ViewRequest},
    error::RestError,
    AccudoBaseUrl, Client, Transaction,
};
use accudo_sdk::{
    transaction_builder::TransactionFactory,
    types::{HardwareWalletAccount, HardwareWalletType, LocalAccount, TransactionSigner},
};
use accudo_transaction_simulation::SimulationStateStore;
use accudo_transaction_simulation_session::Session;
use accudo_types::{
...
pub const USER_AGENT: &str = concat!("accudo-cli/", env!("CARGO_PKG_VERSION"));
...
pub const ACCUDO_FOLDER_GIT_IGNORE: &str = indoc! {"
    *
    testnet/
    config.yaml
"};
...
const X_ACCUDO_CLIENT_VALUE: &str = concat!("accudo-cli/", env!("CARGO_PKG_VERSION"));
...
pub enum CliError {
    ...
    #[error("Unable to find config {0}, have you run `accudo init`?")]
...
impl From<accudo_config::config::Error> for CliError {
    fn from(e: accudo_config::config::Error) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}

impl From<accudo_github_client::Error> for CliError {
    fn from(e: accudo_github_client::Error) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
impl From<accudo_crypto::CryptoMaterialError> for CliError {
    fn from(e: accudo_crypto::CryptoMaterialError) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
impl From<accudo_ledger::AccudoLedgerError> for CliError {
    fn from(e: accudo_ledger::AccudoLedgerError) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
/// Config saved to `.accudo/config.yaml`
...
pub const CONFIG_FOLDER: &str = ".accudo";

/// An individual profile
#[derive(Debug, Default, Serialize, Deserialize)]
pub struct ProfileConfig {
    /// Name of network being used, if setup from accudo init
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// URL for the Accudo rest endpoint
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl CliConfig {
    ...
    pub fn config_exists(mode: ConfigSearchMode) -> bool {
        if let Ok(folder) = Self::accudo_folder(mode) {
    ...
    pub fn load(mode: ConfigSearchMode) -> CliTypedResult<Self> {
        let folder = Self::accudo_folder(mode)?;
    ...
    /// Saves the config to ./.accudo/config.yaml
    pub fn save(&self) -> CliTypedResult<()> {
        let accudo_folder = Self::accudo_folder(ConfigSearchMode::CurrentDir)?;

        // Create if it doesn't exist
        let no_dir = !accudo_folder.exists();
        create_dir_if_not_exist(accudo_folder.as_path())?;

        // If the `.accudo/` doesn't exist, we'll add a .gitignore in it to ignore the config file
        // so people don't save their credentials...
        if no_dir {
            write_to_user_only_file(
                accudo_folder.join(GIT_IGNORE).as_path(),
                GIT_IGNORE,
                ACCUDO_FOLDER_GIT_IGNORE.as_bytes(),
            )?;
        }

        // Save over previous config file
        let config_file = accudo_folder.join(CONFIG_FILE);
        ...
        let legacy_config_file = accudo_folder.join(LEGACY_CONFIG_FILE);
    ...
    /// Finds the current directory's .accudo folder
    fn accudo_folder(mode: ConfigSearchMode) -> CliTypedResult<PathBuf> {
...
impl RestOptions {
    ...
    pub fn url(&self, profile: &ProfileOptions) -> CliTypedResult<reqwest::Url> {
        ...
        .map(|p| p.rest_url)
        {
            ...
            Err(CliError::CommandArgumentError("No rest url given.  Please add --url or add a rest_url to the .accudo/config.yaml for the current profile".to_string()))
        }
    }

    pub fn client(&self, profile: &ProfileOptions) -> CliTypedResult<Client> {
        let mut client = Client::builder(AccudoBaseUrl::Custom(self.url(profile)?))
            .timeout(Duration::from_secs(self.connection_timeout_secs))
            .header(accudo_api_types::X_ACCUDO_CLIENT, X_ACCUDO_CLIENT_VALUE)?;
...
pub fn load_account_arg(str: &str) -> Result<AccountAddress, CliError> {
            ...
            .map(|p| p.private_key)
    {
        ...
        Err(CliError::CommandArgumentError(
            "'--account' or '--profile' after using accudo init must be provided".to_string(),
        ))
    }
}
...
pub struct FaucetOptions {
    /// URL for the faucet endpoint e.g. `https://faucet.devnet.accudo.org`
    #[clap(long)]
        ...
        match profile.faucet_url {
            ...
            None => match profile.network {
                Some(Network::Mainnet) => {
                    Err(CliError::CommandArgumentError("There is no faucet for mainnet. Please create and fund the account by transferring funds from another account. If you are confident you want to use a faucet, set --faucet-url or add a faucet URL to .accudo/config.yaml for the current profile".to_string()))
                },
                Some(Network::Testnet) => {
                    Err(CliError::CommandArgumentError(format!("To get testnet APT you must visit {}. If you are confident you want to use a faucet programmatically, set --faucet-url or add a faucet URL to .accudo/config.yaml for the current profile", get_mint_site_url(None))))
                },
                _ => {
                    Err(CliError::CommandArgumentError("No faucet given. Please set --faucet-url or add a faucet URL to .accudo/config.yaml for the current profile".to_string()))
                },
            },
        }
    }
...
impl TransactionOptions {
        ...
        ) -> CliTypedResult<(VMStatus, VMOutput)>,
    {
        ...
        let debugger = AccudoDebugger::rest_client(client).unwrap();
...
pub fn get_mint_site_url(address: Option<AccountAddress>) -> String {
    ...
    format!("https://accudo.dev/network/faucet{}", params)
}
