...
use accudo_logger::{debug, Level};
use accudo_rest_client::{accudo_api_types::HashValue, Account, Client, FaucetClient, State};
use accudo_telemetry::service::telemetry_is_disabled;
use accudo_types::{
...
async fn send_telemetry_event(command: &str, latency: Duration, error: Option<&str>) {
    ...
    // Send the event
    accudo_telemetry::cli_metrics::send_cli_telemetry_event(
        build_information,
        command.into(),
        latency,
        error.is_none(),
...
pub fn start_logger(level: Level) {
    let mut logger = accudo_logger::Logger::new();
...
pub fn explorer_account_link(hash: AccountAddress, network: Option<Network>) -> String {
    ...
    if let Some(network) = network {
        format!(
            "https://explorer.accudo.org/account/{}?network={}",
            hash, network
        )
    } else {
        format!("https://explorer.accudo.org/account/{}", hash)
    }
}
...
pub fn explorer_transaction_link(
    ...
    if let Some(network) = network {
        format!(
            "https://explorer.accudo.org/txn/{}?network={}",
            hash.to_hex_literal(),
            network
        )
    } else {
...
///
/// [Read about AIP-80](https://github.com/accudo-foundation/AIPs/blob/main/aips/aip-80.md)
