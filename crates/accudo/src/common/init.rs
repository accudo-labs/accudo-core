...
use accudo_crypto::{ed25519::Ed25519PrivateKey, PrivateKey, ValidCryptoMaterialStringExt};
use accudo_ledger;
...
/// Tool to initialize current directory for the accudo tool
///
/// Configuration will be pushed into .accudo/config.yaml
#[derive(Debug, Parser)]
pub struct InitTool {
    ...
    ///
    /// Make sure that you have your Ledger device connected and unlocked, with the Accudo app installed and opened.
...
impl CliCommand<()> for InitTool {
    ...
    async fn execute(self) -> CliTypedResult<()> {
        ...
        let mut profile_config = if let Some(profile_config) = config.remove_profile(profile_name) {
            prompt_yes_with_override(&format!("Accudo already initialized for profile {}, do you want to overwrite the existing config?", profile_name), self.prompt_options)?;
            profile_config
        } else {
        ...
        match network {
            ...
            Network::Devnet => {
                profile_config.rest_url = Some("https://fullnode.devnet.accudo.org".to_string());
                profile_config.faucet_url = Some("https://faucet.devnet.accudo.org".to_string());
            },
            Network::Local => {
            ...
            self.hardware_wallet_options.extract_derivation_path()?
        {
            ...
            let account_map = accudo_ledger::fetch_batch_accounts(Some(0..5))?;
            eprintln!(
            ...
            let path = accudo_ledger::DERIVATION_PATH.replace("{index}", input_index);

            // Validate the path
            if !accudo_ledger::validate_derivation_path(&path) {
        ...
        let public_key = if self.is_hardware_wallet() {
            match accudo_ledger::get_public_key(
                derivation_path
                    .ok_or_else(|| {
        ...
        let client = accudo_rest_client::Client::new(rest_url);
        ...
        let funded = matches!(client
            .get_account_balance(address, "0x1::AccudoCoin::AccudoCoin")
        ...
        eprintln!(
            "\n---\nAccudo CLI is now set up for account {} as profile {}!\n---\n",
