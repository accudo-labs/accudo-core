...
#[derive(Debug, Parser)]
pub struct AccudoUpdateTool {
    /// The owner of the repo to download the binary from.
    #[clap(long, default_value = "accudo-labs")]
    repo_owner: String,

    /// The name of the repo to download the binary from.
    #[clap(long, default_value = "accudo-core")]
    repo_name: String,
...
impl BinaryUpdater for AccudoUpdateTool {
    fn check(&self) -> bool {
        self.check
    }

    fn pretty_name(&self) -> String {
        "Accudo CLI".to_string()
    }
    ...
    fn get_update_info(&self) -> Result<UpdateRequiredInfo> {
        ...
        // this will fail. See https://github.com/accudo-labs/accudo-core/issues/6411.
        ...
        let latest_release = loop {
            ...
            if release.version.starts_with("accudo-cli-") {
                break release;
            }
        };
    ...
    fn build_updater(&self, info: &UpdateRequiredInfo) -> Result<Box<dyn ReleaseUpdate>> {
        ...
        match installation_method {
            ...
            InstallationMethod::Homebrew => {
                return Err(anyhow!(
                    "Detected this CLI comes from homebrew, use `brew upgrade accudo` instead"
                ));
            },
        ...
        Update::configure()
            ...
            .target_version_tag(&format!("accudo-cli-v{}", info.target_version))
            .target(target)
...
#[async_trait]
impl CliCommand<String> for AccudoUpdateTool {
