...
use crate::{
    common::{
        types::{
            ...
            ProfileSummary, ACCUDO_FOLDER_GIT_IGNORE, CONFIG_FOLDER, GIT_IGNORE,
        },
...
use accudo_cli_common::generate_cli_completions;
...
/// Tool for interacting with configuration of the Accudo CLI tool
///
...
impl CliCommand<()> for GenerateShellCompletions {
    ...
    async fn execute(self) -> CliTypedResult<()> {
        generate_cli_completions::<Tool>("accudo", self.shell, self.output_file.as_path())
...
pub struct SetGlobalConfig {
    ...
    ///
    /// `Workspace` will put the `.accudo/` folder in the current directory, where
    /// `Global` will put the `.accudo/` folder in your home directory
    #[clap(long)]
...
pub enum ConfigType {
    /// Per system user configuration put in `<HOME>/.accudo`
    Global,
    /// Per directory configuration put in `<CURRENT_DIR>/.accudo`
    Workspace,
}
