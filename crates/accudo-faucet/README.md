...
The Accudo Faucet is a service that runs alongside a test network and mints coins for users to test and develop on Accudo.
...
In all cases, if a directory holds a crate, the name of that crate is `accudo-faucet-<directory>`. For example the name of the crate in `metrics-server/` is `accudo-faucet-metrics-server`.
...
```
cargo run -p accudo-faucet-service -- run-simple --key <private_key> --node-url <api_url> --chain-id TESTING
```

Another example, running alongside a localnet (without `--use-faucet`):
```
cargo run -p accudo -- node run-local-testnet --force-restart --assume-yes
cargo run -p accudo-faucet-service -- run-simple --key ~/.accudo/testnet/mint.key --node-url http://127.0.0.1:8080 --chain-id TESTING
```
...
```
cargo run -p accudo-faucet-service -- run -c <path_to_config_file>
```
...
```
cd accudo-move/move-examples/scripts/minter
accudo move compile
```

If you have issues with this, try deleting `~/.move`, updating your Accudo CLI, and changing the AccudoFramework version.
...
```
cd crates/accudo-faucet/configs
ls . | xargs -I@ cargo run -p accudo-faucet-service -- validate-config -c @
```
