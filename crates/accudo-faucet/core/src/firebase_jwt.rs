...
impl FirebaseJwtVerifier {
    ...
    pub async fn validate_jwt(&self, headers: Arc<HeaderMap>) -> Result<String, AccudoTapError> {
        ...
        let token_data = match verify.await {
            ...
            None => {
                return Err(AccudoTapError::new(
                    ...
                    AccudoTapErrorCode::AuthTokenInvalid,
                ));
            },
        };
...
pub fn jwt_sub(headers: Arc<HeaderMap>) -> Result<String, AccudoTapError> {
    let is_jwt = headers
        .get(X_IS_JWT_HEADER)
