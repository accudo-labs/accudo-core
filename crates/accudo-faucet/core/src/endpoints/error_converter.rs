...
use super::errors::AccudoTapErrorResponse;
...
/// be a JSON representation of AccudoTapError. For our own errors, this is exactly
...
pub async fn convert_error(error: poem::Error) -> impl poem::IntoResponse {
    ...
    if is_framework_error {
        // Build an AccudoTapErrorResponse and then reset its status code
...
fn build_error_response(error_string: String) -> Response {
    AccudoTapErrorResponse::from(AccudoTapError::new_with_error_code(
        error_string,
        AccudoTapErrorCode::WebFrameworkError,
    ))
    .into_response()
}
