...
pub struct AccudoTapError {
    /// A message describing the error
    pub message: String,
    /// A code describing the error for programmatic use.
    pub error_code: AccudoTapErrorCode,
...
impl AccudoTapError {
    pub fn new(message: String, error_code: AccudoTapErrorCode) -> Self {
        if *USE_HELPFUL_ERRORS.get().unwrap_or(&true) {
            ...
            Self {
                ...
                error_code: AccudoTapErrorCode::YeahNahYeahYeahYeahNahYeahNah,
                rejection_reasons: vec![],
                txn_hashes: vec![],
            }
        }
...
impl std::fmt::Display for AccudoTapError {
...
impl From<AccudoTapError> for AccudoTapErrorResponse {
    fn from(error: AccudoTapError) -> Self {
...
pub enum AccudoTapErrorCode {
    ...
    /// Failed when making requests to the Accudo API.
    AccudoApiError = 50,
...
impl AccudoTapErrorCode {
    pub fn status(&self) -> StatusCode {
        match self {
            AccudoTapErrorCode::InvalidRequest
            | AccudoTapErrorCode::AccountDoesNotExist
            | AccudoTapErrorCode::EndpointNotEnabled => StatusCode::BAD_REQUEST,
            AccudoTapErrorCode::Rejected
            | AccudoTapErrorCode::SourceIpMissing
            | AccudoTapErrorCode::TransactionFailed
            | AccudoTapErrorCode::AuthTokenInvalid => StatusCode::FORBIDDEN,
            AccudoTapErrorCode::AccudoApiError
            | AccudoTapErrorCode::TransactionTimedOut
            | AccudoTapErrorCode::SerializationError
            | AccudoTapErrorCode::BypasserError
            | AccudoTapErrorCode::CheckerError
            | AccudoTapErrorCode::StorageError
            | AccudoTapErrorCode::InternalError => StatusCode::INTERNAL_SERVER_ERROR,
            AccudoTapErrorCode::ServerOverloaded | AccudoTapErrorCode::FunderAccountProblem => {
                StatusCode::SERVICE_UNAVAILABLE
            },
            AccudoTapErrorCode::YeahNahYeahYeahYeahNahYeahNah => StatusCode::IM_A_TEAPOT,
            // We shouldn't get here, this code is only used in error_converter.rs.
            AccudoTapErrorCode::WebFrameworkError => StatusCode::INTERNAL_SERVER_ERROR,
        }
    }
}
