...
impl FundApi {
    ...
    ) -> poem::Result<Json<FundResponse>, AccudoTapErrorResponse> {
        let txns = self
            .components
    ...
    async fn is_eligible(
        ...
        // Same thing, this uses FromRequest.
        header_map: &HeaderMap,
    ) -> poem::Result<(), AccudoTapErrorResponse> {
...
impl FundApiComponents {
    ...
    async fn preprocess_request(
        ...
        header_map: &HeaderMap,
        dry_run: bool,
    ) -> poem::Result<(CheckerData, bool, Option<SemaphorePermit<'_>>), AccudoTapError> {
        let permit = match &self.concurrent_requests_semaphore {
            Some(semaphore) => match semaphore.try_acquire() {
                Ok(permit) => Some(permit),
                Err(_) => {
                    return Err(AccudoTapError::new(
                        "Server overloaded, please try again later".to_string(),
                        AccudoTapErrorCode::ServerOverloaded,
                    ))
                },
            },
            None => None,
        ...
        for bypasser in &self.bypassers {
            if bypasser
                ...
                .await
                .map_err(|e| {
                    AccudoTapError::new_with_error_code(e, AccudoTapErrorCode::BypasserError)
                })?
            {
                info!(
        ...
        for checker in &self.checkers {
            rejection_reasons.extend(checker.check(checker_data.clone(), dry_run).await.map_err(
                |e| AccudoTapError::new_with_error_code(e, AccudoTapErrorCode::CheckerError),
            )?);
    ...
    async fn fund_inner(
        ...
        header_map: &HeaderMap,
        dry_run: bool,
    ) -> poem::Result<Vec<SignedTransaction>, AccudoTapError> {
        let (checker_data, bypass, _semaphore_permit) = self
...
pub async fn mint(
    ...
    // We take the AccudoTapError and convert it into an anyhow error with just the
    ...
    if return_txns.unwrap_or(false) {
        let txn_bcs =
            accudo_sdk::bcs::to_bytes(&txns).map_err(|e| poem::Error::from(anyhow::anyhow!(e)))?;
