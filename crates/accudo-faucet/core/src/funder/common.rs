...
const DEFAULT_KEY_FILE_PATH: &str = "/opt/accudo/etc/mint.key";
...
pub struct ApiConnectionConfig {
    /// Accudo node (any node type with an open API) server URL.
    /// Include the port in this if not using the default for the scheme.
    #[clap(long, default_value = "https://fullnode.testnet.accudo.org/")]
    pub node_url: Url,
    ...
    /// To keep Testnet simple, we used one private key for accudo root account
...
impl ApiConnectionConfig {
    ...
    pub fn get_key(&self) -> Result<Ed25519PrivateKey> {
        ...
        let result = accudo_sdk::bcs::from_bytes(&key_bytes); //.with_context(|| "bad bcs");
        if let Ok(x) = result {
            return Ok(x);
        }
...
pub async fn update_sequence_numbers(
    ...
    amount: u64,
    wait_for_outstanding_txns_secs: u64,
) -> Result<(u64, Option<u64>), AccudoTapError> {
...
pub async fn submit_transaction(
    ...
    receiver_address: &AccountAddress,
    wait_for_transactions: bool,
) -> Result<SignedTransaction, AccudoTapError> {
    let (result, event_on_success) = if wait_for_transactions {
        ...
        // anyhow error. https://github.com/accudo-labs/accudo-tap/issues/60.
        (
            client
                ...
                .map(|_| ())
                .map_err(|e| {
                    AccudoTapError::new_with_error_code(e, AccudoTapErrorCode::TransactionFailed)
                }),
            "transaction_success",
        )
    } else {
...
pub struct GasUnitPriceManager {
    api_client: accudo_sdk::rest_client::Client,
...
impl GasUnitPriceManager {
    pub fn new(node_url: Url, cache_ttl: Duration) -> Self {
        Self {
            api_client: accudo_sdk::rest_client::Client::new(node_url),
