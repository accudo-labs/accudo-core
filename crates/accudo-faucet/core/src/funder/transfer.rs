...
use crate::{
    endpoints::{AccudoTapError, AccudoTapErrorCode, RejectionReason, RejectionReasonCode},
...
use accudo_sdk::{
    ...
    rest_client::{AccudoBaseUrl, Client},
...
impl TransferFunder {
    ...
    pub fn get_api_client(&self) -> Client {
        let mut builder = Client::builder(AccudoBaseUrl::Custom(self.node_url.clone()));
    ...
    async fn get_gas_unit_price(&self) -> Result<u64, AccudoTapError> {
        match self.gas_unit_price_override {
            ...
            None => self
                ...
                .await
                .map_err(|e| {
                    AccudoTapError::new_with_error_code(e, AccudoTapErrorCode::AccudoApiError)
                }),
        }
    }
    ...
    async fn is_healthy_as_result(&self) -> Result<(), AccudoTapError> {
...
impl FunderTrait for TransferFunder {
    ...
    async fn fund(
    ...
    ) -> Result<Vec<SignedTransaction>, AccudoTapError> {
        ...
        let transactions = if check_only {
            ...
            let txn = self
                .execute_transaction(
                    &client,
                    accudo_stdlib::accudo_account_transfer(receiver_address, amount),
                    &receiver_address,
                )
                .await?;
            info!(
