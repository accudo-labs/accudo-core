...
static MINTER_SCRIPT: &[u8] = include_bytes!(
    "../../../../../accudo-move/move-examples/scripts/minter/build/Minter/bytecode_scripts/main.mv"
);
...
impl MintFunder {
    ...
    async fn get_transaction_factory(&self) -> Result<TransactionFactory, AccudoTapError> {
        Ok(self
            .transaction_factory
            .clone()
    ...
    pub async fn use_delegated_account(&mut self) -> Result<()> {
        ...
        client
            .submit_and_wait(&delegated_account.sign_with_transaction_builder(
                transaction_factory.payload(accudo_stdlib::accudo_coin_claim_mint_capability()),
            ))
            .await
