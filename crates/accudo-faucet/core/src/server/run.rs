    ...
    pub fn build_for_cli(
        ...
        Self {
            ...
            funder_config: FunderConfig::MintFunder(MintFunderConfig {
                ...
                mint_account_address: Some(accudo_test_root_address()),
                do_not_delegate,
            }),
            handler_config: HandlerConfig {
...
mod test {
    ...
    use crate::{
        endpoints::{
            AccudoTapError, AccudoTapErrorCode, FundRequest, FundResponse, RejectionReasonCode,
        },
    ...
    async fn start_server(config_content: &'static str) -> Result<(u16, JoinHandle<Result<()>>)> {
        ...
        let port = accudo_config::utils::get_available_port();
    ...
    async fn test_checkers() -> Result<()> {
        ...
        let response = reqwest::Client::new()
            ...
            .send()
            .await?;
        let accudo_error = AccudoTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as AccudoError");
        assert!(!accudo_error.rejection_reasons.is_empty());
        ...
        let accudo_error = AccudoTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as AccudoError");
        let rejection_reason_codes: HashSet<RejectionReasonCode> = accudo_error
            .rejection_reasons
            .into_iter()
            .map(|r| r.get_code())
            .collect();
    ...
    async fn test_redis_ratelimiter() -> Result<()> {
        ...
        let accudo_node_api_client = accudo_sdk::rest_client::Client::new(
            reqwest::Url::from_str("http://127.0.0.1:8080").unwrap(),
        );
        accudo_node_api_client
            .get_index_bcs()
            .await
    ...
    async fn test_transfer_health() -> Result<()> {
        ...
        let serialized_keys = accudo_sdk::bcs::to_bytes(&private_key)?;
        ...
        unwrap_reqwest_result(
            reqwest::Client::new()
                .post("https://faucet.devnet.accudo.org/fund")
                .body(
                    FundRequest {
        ...
        let accudo_error = AccudoTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as AccudoError");
        assert_eq!(
            accudo_error.error_code,
            AccudoTapErrorCode::FunderAccountProblem
        );
    ...
    async fn test_mint_funder() -> Result<()> {
        ...
        let response = accudo_node_api_client
