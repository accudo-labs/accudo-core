...
use crate::endpoints::{
    AccudoTapError, AccudoTapErrorCode, RejectionReason, RejectionReasonCode, CAPTCHA_KEY,
    CAPTCHA_VALUE,
};
    ...
    async fn check(
        ...
        let captcha_key = match data.headers.get(CAPTCHA_KEY) {
            Some(header_value) => match header_value
                .to_str()
                .map_err(|e| {
                    AccudoTapError::new_with_error_code(e, AccudoTapErrorCode::InvalidRequest)
                })?
                .parse::<u32>()
            {
                Ok(value) => value,
