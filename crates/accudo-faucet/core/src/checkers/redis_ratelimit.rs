...
impl RatelimitKeyProvider {
    ...
    /// JWT's `sub` field).
    pub async fn ratelimit_key_value(&self, data: &CheckerData) -> Result<String, AccudoTapError> {
        match self {
...
/// worry about it as a followup: https://github.com/accudo-labs/accudo-tap/issues/15.
...
impl RedisRatelimitChecker {
    ...
    pub async fn get_redis_connection(&self) -> Result<Connection, AccudoTapError> {
        self.db_pool.get().await.map_err(|e| {
            AccudoTapError::new_with_error_code(
...
impl CheckerTrait for RedisRatelimitChecker {
    ...
    async fn complete(&self, data: CompleteData) -> Result<(), AccudoTapError> {
