...
/// ```
///
/// But we can't due to `accudo-crypto`'s dependency on an older version of `rand` and `rand_core`
...
pub const DST_RAND_CORE_HELL: &[u8; 24] = b"ACCUDO_RAND_CORE_HELL_DST";

pub fn random_nonzero_scalar<R>(rng: &mut R) -> Scalar
where
    R: rand_core::RngCore + rand::Rng + rand_core::CryptoRng + rand::CryptoRng,
{
    accudo_crypto::blstrs::random_scalar_internal(rng, true)
}
...
where
    R: rand_core::RngCore + rand::Rng + rand_core::CryptoRng + rand::CryptoRng,
{
    let s = accudo_crypto::blstrs::random_scalar(rng);
    ...
    R: rand_core::RngCore + rand::Rng + rand_core::CryptoRng + rand::CryptoRng,
{
    ...
    for _ in 0..n {
        v.push(accudo_crypto::blstrs::random_scalar(rng));
    }
