...
/// The global [AccudoNodeIdentity]
static ACCUDO_NODE_IDENTITY: OnceCell<Arc<AccudoNodeIdentity>> = OnceCell::new();

/// Structure that holds information related to a node's identity
pub struct AccudoNodeIdentity {
...
/// Initializes the [AccudoNodeIdentity] using the provided [PeerId] and
/// sets it globally exactly once.
pub fn init(peer_id: Option<PeerId>) -> Result<()> {
    let identity = AccudoNodeIdentity {
        chain_id: OnceCell::new(),
        peer_id,
        peer_id_str: peer_id.map(|id| id.to_string()),
    };

    ACCUDO_NODE_IDENTITY
        .set(Arc::new(identity))
        .map_err(|_| format_err!("ACCUDO_NODE_IDENTITY was already set"))
}

/// Sets the [ChainId] in the global [AccudoNodeIdentity], returning an error
/// if [init] was not called already.
pub fn set_chain_id(chain_id: ChainId) -> Result<()> {
    match ACCUDO_NODE_IDENTITY.get() {
        Some(identity) => identity
            .chain_id
            .set(chain_id)
            .map_err(|_| format_err!("chain_id was already set.")),
        None => Err(format_err!("ACCUDO_NODE_IDENTITY has not been set yet")),
    }
}

/// Returns the [PeerId] from the global `ACCUDO_NODE_IDENTITY`
...
/// Returns the [PeerId] as [str] from the global `ACCUDO_NODE_IDENTITY`
...
/// Returns the [ChainId] from the global `ACCUDO_NODE_IDENTITY`
