...
use accudo_sdk::{bcs, types::LocalAccount};
use accudo_types::{
...
async fn build_module(address: AccountAddress) -> Result<BuiltPackage, TestFailure> {
    // get file to compile
    let move_dir = PathBuf::from("./accudo-move/move-examples/hello_blockchain");
...
async fn publish_module(
    ...
    // create payload
    let payload: accudo_types::transaction::TransactionPayload =
