...
async fn warn_detailed_error(
    ...
    txn: &SignedTransaction,
    err: Result<&accudo_types::transaction::TransactionInfo, &RestError>,
) {
...
fn is_account_not_found(error: &RestError) -> bool {
    match error {
        RestError::Api(error) => matches!(error.error.error_code, AccudoErrorCode::AccountNotFound),
        _ => false,
    }
}
            ...
            .retry_if(
                ...
                |error: &RestError| match error {
                    RestError::Api(error) => !matches!(
                        ...
                        AccudoErrorCode::AccountNotFound | AccudoErrorCode::InvalidInput
                    ),
