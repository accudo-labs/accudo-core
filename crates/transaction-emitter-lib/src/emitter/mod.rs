...
impl Default for EmitJobRequest {
    fn default() -> Self {
        Self {
            ...
            max_gas_per_txn: accudo_global_constants::MAX_GAS_AMOUNT,
            gas_price: accudo_global_constants::GAS_UNIT_PRICE,
...
async fn wait_for_orderless_txns(
    ...
    loop {
        ...
        if accudo_infallible::duration_since_epoch().as_secs() >= txn_expiration_ts_secs + 240 {
            sample!(
    ...
    match &result {
        ...
        Err(e) => {
            ...
            if let RestError::Api(api_error) = e {
                if let AccudoErrorCode::AccountNotFound = api_error.error.error_code {
                    return Ok((
                        0,
...
pub fn gen_transfer_txn_request(
    ...
    sender.sign_with_transaction_builder(
        txn_factory.payload(accudo_stdlib::accudo_coin_transfer(*receiver, num_coins)),
    )
}
