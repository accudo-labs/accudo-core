...
### Installing fullnode
Please follow standard fullnode installation guide on accudo.dev (https://accudo.dev/nodes/full-node/fullnode-source-code-or-docker)

### Running indexer
```bash
cargo run -p accudo-node --features "indexer" --release -- -f <some_path>/fullnode.yaml
```
...
> - Accudo tests use the `INDEXER_DATABASE_URL` env var. It needs to be set for the relevant tests to run.
...
```bash
alias testnet="cd ~/Desktop/accudo-core; CARGO_NET_GIT_FETCH_WITH_CLI=true cargo run -p accudo-node -- --test"
```
