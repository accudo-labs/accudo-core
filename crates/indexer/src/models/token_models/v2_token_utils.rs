...
pub struct TokenV2AggregatedData {
    pub accudo_collection: Option<AccudoCollection>,
...
#[derive(Serialize, Deserialize, Debug, Clone)]
pub struct AccudoCollection {
    pub mutable_description: bool,
    pub mutable_uri: bool,
}

impl AccudoCollection {
    pub fn from_write_resource(
        write_resource: &WriteResource,
        txn_version: i64,
    ) -> anyhow::Result<Option<Self>> {
        ...
        if let V2TokenResource::AccudoCollection(inner) =
...
pub enum V2TokenResource {
    AccudoCollection(AccudoCollection),
