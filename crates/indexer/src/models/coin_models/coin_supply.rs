...
impl CoinSupply {
    /// Currently only supports accudo_coin. Aggregator table detail is in CoinInfo which for accudo coin appears during genesis.
    ...
    pub fn from_write_table_item(
        ...
        maybe_accudo_coin_info: &Option<CoinInfoQuery>,
        txn_version: i64,
        txn_timestamp: chrono::NaiveDateTime,
        txn_epoch: i64,
    ) -> anyhow::Result<Option<Self>> {
        if let Some(accudo_coin_info) = maybe_accudo_coin_info {
            // Return early if we don't have the accudo aggregator table info
            if accudo_coin_info.supply_aggregator_table_key.is_none()
                || accudo_coin_info.supply_aggregator_table_handle.is_none()
            {
                return Ok(None);
            }
            if let Some(data) = &write_table_item.data {
                ...
                // Return early if not accudo coin aggregator key
                let table_key = data
                    .key
                    .as_str()
                ...
                return Ok(Some(Self {
                    ...
                    coin_type_hash: accudo_coin_info.coin_type_hash.clone(),
                    coin_type: accudo_coin_info.coin_type.clone(),
                    supply,
