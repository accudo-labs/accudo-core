...
use crate::{
    ...
    models::{
        ...
        token_models::{
            ...
            v2_token_utils::{
                AccudoCollection, BurnEvent, FixedSupply, ObjectWithMetadata, PropertyMap, TokenV2,
...
use accudo_api_types::{Transaction, TransactionPayload, WriteSetChange};
            ...
            for wsc in user_txn.info.changes.iter() {
                if let WriteSetChange::WriteResource(wr) = wsc {
                    ...
                    if let Some(aggregated_data) = token_v2_metadata_helper.get_mut(&address) {
                        ...
                        if let Some(accudo_collection) =
                            AccudoCollection::from_write_resource(wr, txn_version).unwrap()
                        {
                            aggregated_data.accudo_collection = Some(accudo_collection);
                        }
