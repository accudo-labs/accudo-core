...
use accudo_api_types::Transaction as APITransaction;
use accudo_types::{AccudoCoinType, CoinType};
...
impl TransactionProcessor for CoinTransactionProcessor {
    ...
    async fn process_transactions(
        ...
        // get accudo_coin info for supply tracking
        // TODO: This only needs to be fetched once. Need to persist somehow
        let maybe_accudo_coin_info = &CoinInfoQuery::get_by_coin_type(
            AccudoCoinType::type_tag().to_canonical_string(),
            &mut conn,
        )
        .unwrap();
        ...
        for txn in &transactions {
            let (
                ...
                current_coin_balances,
                mut coin_supply,
            ) = CoinActivity::from_transaction(txn, maybe_accudo_coin_info);
