...
impl AccudoBaseUrl {
    pub fn to_url(&self) -> Url {
        match self {
            AccudoBaseUrl::Mainnet => Url::from_str("https://api.mainnet.accudo.org").unwrap(),
            AccudoBaseUrl::Devnet => Url::from_str("https://api.devnet.accudo.org").unwrap(),
            AccudoBaseUrl::Testnet => Url::from_str("https://api.testnet.accudo.org").unwrap(),
            AccudoBaseUrl::Custom(url) => url.to_owned(),
        }
    }
}
...
impl ClientBuilder {
    pub fn new(accudo_base_url: AccudoBaseUrl) -> Self {
        let mut headers = HeaderMap::new();
        headers.insert(
            X_ACCUDO_CLIENT,
            HeaderValue::from_static(X_ACCUDO_SDK_HEADER_VALUE),
        );

        let mut client_builder = Self {
            reqwest_builder: ReqwestClient::builder(),
            base_url: accudo_base_url.to_url(),
