...
use accudo_api_types::{
    ...
    AccudoError, AccudoErrorCode, BcsBlock, Block, GasEstimation, HexEncodedBytes, IndexResponse,
...
use accudo_logger::{debug, info, sample, sample::SampleRate};
use accudo_types::{
...
const X_ACCUDO_SDK_HEADER_VALUE: &str = concat!("accudo-rust-sdk/", env!("CARGO_PKG_VERSION"));

type AccudoResult<T> = Result<T, RestError>;
...
impl Client {
    pub fn builder(accudo_base_url: AccudoBaseUrl) -> ClientBuilder {
        ClientBuilder::new(accudo_base_url)
    }

    pub fn new(base_url: Url) -> Self {
        Self::builder(AccudoBaseUrl::Custom(base_url)).build()
    }
    ...
    pub fn version_path_base(mut self, version_path_base: String) -> AccudoResult<Self> {
    ...
    pub fn build_path(&self, path: &str) -> AccudoResult<Url> {
    ...
    pub async fn get_accudo_version(&self) -> AccudoResult<Response<AccudoVersion>> {
        self.get_resource::<AccudoVersion>(CORE_CODE_ADDRESS, "0x1::version::Version")
            .await
    }
    ...
    pub async fn get_block_by_height_bcs(
        ...
        height: u64,
        with_transactions: bool,
    ) -> AccudoResult<Response<BcsBlock>> {
    ...
    pub async fn lookup_address(
        ...
        must_exist: bool,
    ) -> AccudoResult<Response<AccountAddress>> {
        ...
        {
            Ok(inner) => Ok(inner),
            Err(RestError::Api(AccudoErrorResponse {
                error:
                    AccudoError {
                        error_code: AccudoErrorCode::TableItemNotFound,
                        ..
                    },
                ..
            })) => {
    ...
    /// * A coin type (e.g. "0x1::accudo_coin::AccudoCoin")
    /// * A fungible asset metadata address (e.g. "0xa")
    /// For more details, see: https://accudo.dev/en/build/apis/fullnode-rest-api-reference#tag/accounts/GET/accounts/{address}/balance/{asset_type}
    pub async fn get_account_balance(
        &self,
        address: AccountAddress,
        asset_type: &str,
    ) -> AccudoResult<Response<u64>> {
        let url = self.build_path(&format!(
            "accounts/{}/balance/{}",
            address.to_hex(),
            asset_type
    ...
    pub async fn view_apt_account_balance_at_version(
        ...
        self.view_account_balance_bcs_impl(address, "0x1::accudo_coin::AccudoCoin", Some(version))
            .await
    }
    ...
    pub async fn view_apt_account_balance(
        ...
        self.view_account_balance_bcs_impl(address, "0x1::accudo_coin::AccudoCoin", None)
            .await
    }

    pub async fn get_index(&self) -> AccudoResult<Response<IndexResponse>> {
        self.get(self.build_path("")?).await
    }

    pub async fn get_index_bcs(&self) -> AccudoResult<Response<IndexResponseBcs>> {
        let url = self.build_path("")?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }

    // TODO: Remove this, just use `get_index`: https://github.com/accudo-labs/accudo-core/issues/5597.
    pub async fn get_ledger_information(&self) -> AccudoResult<Response<State>> {
    ...
    pub async fn view(
        ...
        request: &ViewRequest,
        version: Option<u64>,
    ) -> AccudoResult<Response<Vec<serde_json::Value>>> {
    ...
    ) -> AccudoResult<Response<Vec<UserTransaction>>> {
    ...
    pub async fn simulate_bcs(
        &self,
        txn: &SignedTransaction,
    ) -> AccudoResult<Response<TransactionOnChainData>> {
        let txn_payload = bcs::to_bytes(txn)?;
        let url = self.build_path("transactions/simulate")?;
    ...
    pub async fn submit(
        &self,
        txn: &SignedTransaction,
    ) -> AccudoResult<Response<PendingTransaction>> {
        let txn_payload = bcs::to_bytes(txn)?;
    ...
    pub async fn submit_bcs(&self, txn: &SignedTransaction) -> AccudoResult<Response<()>> {
        let txn_payload = bcs::to_bytes(txn)?;
        let url = self.build_path("transactions")?;
    ...
    ) -> AccudoResult<Response<TransactionsBatchSubmissionResult>> {
    ...
    pub async fn submit_and_wait(
        &self,
        txn: &SignedTransaction,
    ) -> AccudoResult<Response<Transaction>> {
    ...
    where
        F: Fn(HashValue) -> Fut,
        Fut: Future<Output = AccudoResult<WaitForTransactionResult<T>>>,
    {
        ...
        loop {
            ...
            match fetch(hash).await {
                ...
                Ok(WaitForTransactionResult::NotFound(error)) => {
                    if let RestError::Api(accudo_error_response) = error {
                        if let Some(state) = accudo_error_response.state {
                            if expiration_timestamp_secs <= state.timestamp_usecs / 1_000_000 {
                                if reached_mempool {
            ...
            if let Some(max_server_lag_wait_duration) = max_server_lag_wait {
                if accudo_infallible::duration_since_epoch().as_secs()
                    ...
                    debug!(
                        ...
                        if let Some(timestamp_usecs) = chain_timestamp_usecs {
                            format!(
                                ...
                                accudo_infallible::duration_since_epoch()
    ...
    pub async fn get_transactions(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> AccudoResult<Response<Vec<Transaction>>> {
        let url = self.build_path("transactions")?;
    ...
    pub async fn get_transactions_bcs(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> AccudoResult<Response<Vec<TransactionOnChainData>>> {
        let url = self.build_path("transactions")?;
    ...
    pub async fn get_persisted_auxiliary_infos(
        ...
        start: Version,
        limit: u64,
    ) -> AccudoResult<Vec<PersistedAuxiliaryInfo>> {
    ...
    pub async fn get_transaction_by_hash_bcs(
        &self,
        hash: HashValue,
    ) -> AccudoResult<Response<TransactionData>> {
    ...
    pub async fn get_account_resources(
        &self,
        address: AccountAddress,
    ) -> AccudoResult<Response<Vec<Resource>>> {
    ...
    pub async fn get_account_resources_bcs(
        &self,
        address: AccountAddress,
    ) -> AccudoResult<Response<BTreeMap<StructTag, Vec<u8>>>> {
    ...
    pub async fn get_account_resource(
        ...
        address: AccountAddress,
        resource_type: &str,
    ) -> AccudoResult<Response<Option<Resource>>> {
        let url = self.build_path(&format!(
            "accounts/{}/resource/{}",
            address.to_hex(),
            resource_type
    ...
    pub async fn get_account_modules(
        &self,
        address: AccountAddress,
    ) -> AccudoResult<Response<Vec<MoveModuleBytecode>>> {
        self.paginate_with_cursor(
    ...
    pub async fn get_account_modules_bcs(
        &self,
        address: AccountAddress,
    ) -> AccudoResult<Response<BTreeMap<MoveModuleId, Vec<u8>>>> {
        self.paginate_with_cursor_bcs(
            &format!("accounts/{}/modules", address.to_hex()),
            MODULES_PER_CALL_PAGINATION,
            None,
    ...
    pub async fn get_account_module(
        ...
        address: AccountAddress,
        module_name: &str,
    ) -> AccudoResult<Response<MoveModuleBytecode>> {
        let url = self.build_path(&format!(
            "accounts/{}/module/{}",
            address.to_hex(),
            module_name
    ...
    pub async fn get_account_module_bcs(
        ...
        address: AccountAddress,
        module_name: &str,
    ) -> AccudoResult<Response<bytes::Bytes>> {
        let url = self.build_path(&format!(
            "accounts/{}/module/{}",
            address.to_hex(),
            module_name
    ...
    pub async fn get_account_events(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> AccudoResult<Response<Vec<VersionedEvent>>> {
        let url = self.build_path(&format!(
            "accounts/{}/events/{}/{}",
    ...
    pub async fn get_account_sequence_number(
        ...
        match res {
            ...
            Err(error) => match error {
                RestError::Api(error) => {
                    if matches!(error.error.error_code, AccudoErrorCode::AccountNotFound) {
    ...
    pub async fn get_account_events_bcs(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> AccudoResult<Response<Vec<EventWithVersion>>> {
        let url = self.build_path(&format!(
            "accounts/{}/events/{}/{}",
            address.to_hex_literal(),
            struct_tag,
    ...
    pub async fn get_account(&self, address: AccountAddress) -> AccudoResult<Response<Account>> {
    ...
    pub async fn get_account_bcs(
        &self,
        address: AccountAddress,
    ) -> AccudoResult<Response<AccountResource>> {
        let url = self.build_path(&format!("accounts/{}", address.to_hex()))?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }
    ...
    pub async fn get_account_transaction_summaries(
        ...
        limit: Option<u16>,
    ) -> AccudoResult<Response<Vec<IndexedTransactionSummary>>> {
        let url = self.build_path(&format!(
    ...
    pub async fn estimate_gas_price(&self) -> AccudoResult<Response<GasEstimation>> {
        let url = self.build_path("estimate_gas_price")?;
        let response = self.inner.get(url).send().await?;
        self.json(response).await
    }

    pub async fn set_failpoint(&self, name: String, actions: String) -> AccudoResult<String> {
    ...
    ) -> AccudoResult<(reqwest::Response, State)> {
        if !response.status().is_success() {
            Err(parse_error(response).await)
        } else {
    ...
    pub async fn health_check(&self, seconds: u64) -> AccudoResult<()> {
    ...
    async fn get<T: DeserializeOwned>(&self, url: Url) -> AccudoResult<Response<T>> {
        self.json(self.inner.get(url).send().await?).await
    }

    async fn get_bcs(&self, url: Url) -> AccudoResult<Response<bytes::Bytes>> {
        let response = self.inner.get(url).header(ACCEPT, BCS).send().await?;
        self.check_and_parse_bcs_response(response).await
    }

    pub async fn make_prover_request(&self, req: ProverRequest) -> AccudoResult<ZeroKnowledgeSig> {
    ...
    pub async fn make_pepper_request(&self, req: PepperRequest) -> AccudoResult<Pepper> {
    ...
    where
        F: Fn() -> Fut,
        RetryFun: Fn(StatusCode, Option<AccudoError>) -> bool,
        Fut: Future<Output = AccudoResult<T>>,
    {
    ...
    pub async fn paginate_with_cursor_bcs<T: for<'a> Deserialize<'a> + Ord>(
        ...
        limit_per_request: u64,
        ledger_version: Option<u64>,
    ) -> AccudoResult<Response<BTreeMap<T, Vec<u8>>>> {
...
pub fn retriable_with_404(status_code: StatusCode, accudo_error: Option<AccudoError>) -> bool {
    retriable(status_code, accudo_error) | matches!(status_code, StatusCode::NOT_FOUND)
}

pub fn retriable(status_code: StatusCode, _accudo_error: Option<AccudoError>) -> bool {
    matches!(
        status_code,
...
fn parse_state(response: &reqwest::Response) -> AccudoResult<State> {
