...
/// A layer that translates tracing events into accudo-logger events.
pub struct TracingToAccudoDataLayer;
...
impl<S> Layer<S> for TracingToAccudoDataLayer
where
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    fn on_event(&self, event: &Event, ctx: Context<S>) {
        let metadata = match translate_metadata(event.metadata()) {
            ...
            None => {
                dl::warn!(
                    "[tracing-to-accudo-logger] failed to translate event due to unknown level {:?}",
