...
impl LogEntry {
    fn new(event: &Event, thread_name: Option<&str>, enable_backtrace: bool) -> Self {
        ...
        #[cfg(node_identity)]
        {
            peer_id = accudo_node_identity::peer_id_as_str();
            chain_id = accudo_node_identity::chain_id().map(|chain_id| chain_id.id());
        }
...
/// A builder for a `AccudoData`, configures what, where, and how to write logs.
pub struct AccudoDataBuilder {
    channel_size: usize,
...
impl AccudoDataBuilder {
    ...
    fn build_logger(&mut self) -> Arc<AccudoData> {
    ...
    pub fn build(&mut self) -> Arc<AccudoData> {
...
impl AccudoData {
    pub fn builder() -> AccudoDataBuilder {
        AccudoDataBuilder::new()
    }

    #[allow(clippy::new_ret_no_self)]
    pub fn new() -> AccudoDataBuilder {
        Self::builder()
    }

    pub fn init_for_testing() {
        // Create the Accudo Data Builder
...
/// or to a `AccudoData` for external logging.
struct LoggerService {
    receiver: sync::mpsc::Receiver<LoggerServiceEvent>,
    printer: Option<Box<dyn Writer>>,
    facade: Arc<AccudoData>,
...
/// Example:
/// 2020-03-07 05:03:03 INFO [thread_name] common/accudo-logger/src/lib.rs:261 Hello { "world": true }
...
pub struct LoggerFilterUpdater {
    logger: Arc<AccudoData>,
    logger_builder: AccudoDataBuilder,
}

impl LoggerFilterUpdater {
    pub fn new(logger: Arc<AccudoData>, logger_builder: AccudoDataBuilder) -> Self {
        Self {
            logger,
            logger_builder,
        }
...
mod tests {
    ...
    use super::{text_format, AccudoData, LogEntry};
    use crate::{
        accudo_logger::{json_format, TruncatedLogString, RUST_LOG_TELEMETRY},
        debug, error, info,
        logger::Logger,
        telemetry_log_writer::TelemetryLog,
        trace, warn, AccudoDataBuilder, Event, Key, KeyValue, Level, LoggerFilterUpdater, Metadata,
    ...
    fn basic() {
        ...
        let expected = format!("{{\"level\":\"INFO\",\"source\":{{\"package\":\"accudo_logger\",\"file\":\"crates/accudo-logger/src/accudo_logger.rs:{line_num}\"}},\"thread_name\":\"{thread_name}\",\"hostname\":\"test-host\",\"timestamp\":\"2022-07-24T23:42:29.540278Z\",\"message\":\"This is a log\",\"data\":{{\"bar\":\"foo_bar\",\"category\":\"name\",\"display\":\"12345\",\"foo\":5,\"test\":true}}}}");
    ...
    fn new_async_logger() -> (AccudoDataBuilder, Arc<AccudoData>) {
        let mut logger_builder = AccudoDataBuilder::new();
    ...
    fn new_text_logger() -> (
        ...
        let logger = AccudoDataBuilder::new()
            .level(Level::Debug)
