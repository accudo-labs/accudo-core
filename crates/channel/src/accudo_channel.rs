...
//! accudo_channel provides an mpsc channel which has two ends `accudo_channel::Receiver`
//! and `accudo_channel::Sender` similar to existing mpsc data structures.
...
/// The sending end of the accudo_channel.
#[derive(Debug)]
pub struct Sender<K: Eq + Hash + Clone, M> {
    shared_state: Arc<Mutex<SharedState<K, M>>>,
}

/// The status of an element inserted into a accudo_channel. If the element is successfully
...
/// The receiving end of the accudo_channel.
#[derive(Debug)]
...
/// Configuration for a new accudo_channel queue.
#[derive(Clone, Copy)]
pub struct Config {
...
impl Config {
    /// The accudo_channel has a "sub-queue" per key. The `max_capacity` controls
...
pub fn new<K: Eq + Hash + Clone, M>(
    ...
    let max_queue_size_per_key =
        NonZeroUsize!(max_queue_size_per_key, "accudo_channel cannot be of size 0");
