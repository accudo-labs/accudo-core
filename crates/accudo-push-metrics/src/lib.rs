...
use accudo_metrics_core::{Encoder, TextEncoder};
use std::{
    env,
    ops::Sub,
    sync::mpsc,
...
const DEFAULT_DASHBOARD_BASE_URL: &str = "https://accudolabs.grafana.net/d/execution/execution";
...
impl MetricsPusher {
    fn push(
        ...
        if let Err(e) = TextEncoder::new().encode(&accudo_metrics_core::gather(), &mut buffer) {
