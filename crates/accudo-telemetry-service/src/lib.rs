...
#[clap(name = "Accudo Telemetry Service", author, version)]
pub struct AccudoTelemetryServiceArgs {
    #[clap(short = 'f', long, value_parser)]
    config_path: PathBuf,
}

impl AccudoTelemetryServiceArgs {
    pub async fn run(self) {
        ...
        let validators = Arc::new(accudo_infallible::RwLock::new(HashMap::new()));
        let validator_fullnodes = Arc::new(accudo_infallible::RwLock::new(HashMap::new()));
        let peer_locations = Arc::new(accudo_infallible::RwLock::new(HashMap::new()));
...
fn verify_tool() {
    ...
    AccudoTelemetryServiceArgs::command().debug_assert()
}
