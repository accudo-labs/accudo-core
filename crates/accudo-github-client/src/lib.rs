...
impl Client {
    ...
    pub fn delete_file(&self, path: &str) -> Result<(), Error> {
        ...
        let resp = self
            ...
            .send_json(
                json!({ "branch": self.branch.to_string(), "message": "accudo-secure", "sha": hash }),
            );
    ...
    pub fn put(&self, path: &str, content: &str) -> Result<(), Error> {
        let json = match self.get_sha(path) {
            Ok(hash) => {
                json!({ "branch": self.branch.to_string(), "content": content, "message": format!("[accudo-management] {}", path), "sha": hash })
            },
            Err(Error::NotFound(_)) => {
                json!({ "branch": self.branch.to_string(), "content": content, "message": format!("[accudo-management] {}", path) })
            },
            Err(e) => return Err(e),
        };
