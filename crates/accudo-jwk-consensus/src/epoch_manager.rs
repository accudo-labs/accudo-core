...
use accudo_safety_rules::{safety_rules_manager::storage, PersistentSafetyStorage};
use accudo_types::{
...
pub struct EpochManager<P: OnChainConfigProvider> {
    ...
    jwk_updated_event_txs: Option<accudo_channel::Sender<(), ObservedJWKsUpdated>>,
    jwk_rpc_msg_tx:
        Option<accudo_channel::Sender<AccountAddress, (AccountAddress, IncomingRpcRequest)>>,
    jwk_manager_close_tx: Option<oneshot::Sender<oneshot::Sender<()>>>,

    // network utils
    self_sender: accudo_channels::Sender<Event<JWKConsensusMsg>>,
    ...
    async fn start_new_epoch(&mut self, payload: OnChainConfigPayload<P>) -> Result<()> {
        ...
        if jwk_manager_should_run && my_index.is_some() {
            ...
            let rb = ReliableBroadcast::new(
                ...
                accudo_time_service::TimeService::real(),
            ...
            let (jwk_event_tx, jwk_event_rx) = accudo_channel::new(QueueStyle::KLAST, 1, None);
            self.jwk_updated_event_txs = Some(jwk_event_tx);
            let (jwk_rpc_msg_tx, jwk_rpc_msg_rx) = accudo_channel::new(QueueStyle::FIFO, 100, None);
