...
pub struct NetworkReceivers {
    pub rpc_rx: accudo_channel::Receiver<AccountAddress, (AccountAddress, IncomingRpcRequest)>,
}

pub struct NetworkTask {
    all_events: Box<dyn Stream<Item = Event<JWKConsensusMsg>> + Send + Unpin>,
    rpc_tx: accudo_channel::Sender<AccountAddress, (AccountAddress, IncomingRpcRequest)>,
}
...
impl NetworkTask {
    ...
    pub fn new(
        ...
        self_receiver: accudo_channels::Receiver<Event<JWKConsensusMsg>>,
    ) -> (NetworkTask, NetworkReceivers) {
        let (rpc_tx, rpc_rx) = accudo_channel::new(QueueStyle::FIFO, 10, None);
