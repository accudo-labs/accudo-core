...
/// Identifier for a "block".  On Accudo, we use a transaction model, so the index
...
pub struct NetworkIdentifier {
    /// Blockchain name, should always be `accudo` and be hardcoded
...
impl From<accudo_crypto::HashValue> for TransactionIdentifier {
    fn from(hash: accudo_crypto::HashValue) -> Self {
        TransactionIdentifier {
