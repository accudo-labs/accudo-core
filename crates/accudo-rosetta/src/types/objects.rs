...
use accudo_rest_client::accudo_api_types::{ResourceGroup, TransactionOnChainData, U64};
use accudo_types::{
    access_path::Path,
...
pub struct Block {
    ...
    pub timestamp: u64,
    /// Transactions associated with the version.  In accudo there should only be one transaction
    pub transactions: Vec<Transaction>,
}

/// A combination of a transaction and the block associated.  In Accudo, this is just the same
...
pub struct CurrencyMetadata {
    /// Move coin type e.g. 0x1::accudo_coin::AccudoCoin
    #[serde(skip_serializing_if = "Option::is_none")]
...
///
/// This is known as a write set change within Accudo
...
/// Cryptographic signature type used for signing transactions.  Accudo only uses
...
impl Transaction {
    pub async fn from_transaction(
        ...
        use accudo_types::transaction::Transaction::*;
    ...
    if let TransactionPayload::EntryFunction(inner) = payload {
        match (
            *inner.module().address(),
            ...
            | (AccountAddress::ONE, ACCUDO_ACCOUNT_MODULE, TRANSFER_COINS_FUNCTION) => {
            ...
            (AccountAddress::ONE, ACCUDO_ACCOUNT_MODULE, TRANSFER_FUNCTION) => {
            ...
            | (AccountAddress::ONE, ACCUDO_ACCOUNT_MODULE, TRANSFER_FUNGIBLE_ASSETS_FUNCTION) => {
    ...
    /*    if let Some(owner_address) = server_context.pool_address_to_owner.get(&pool_address) {
            if let Ok(stakepool) = bcs::from_bytes::<StakePool>(data) {
                ...
                let add_stake_events = filter_events(
                    ...
                    |event_key, event| {
                        if let Ok(event) = bcs::from_bytes::<accudo_types::stake_pool::AddStakeEvent>(
                            event.event_data(),
                        ) {
                            Some(event)
                        } else {
                ...
                let set_operator_events = filter_events(
                    ...
                    |event_key, event| {
                        if let Ok(event) = bcs::from_bytes::<accudo_types::stake_pool::SetOperatorEvent>(
                            event.event_data(),
                        ) {
                            Some(event)
                        } else {
    ...
    pub fn payload(
        ...
        Ok(match self {
                ...
                if let Some(ref metadata) = currency.metadata {
                    match (&metadata.move_type, &metadata.fa_address) {
                        ...
                        (Some(coin_type), Some(_)) | (Some(coin_type), None) => {
                            ...
                            (
                                accudo_stdlib::accudo_account_transfer_coins(
                                    coin_type_tag,
                                    transfer.receiver,
                                    transfer.amount.0,
                                ),
            ...
            InternalOperation::SetOperator(set_operator) => {
                ...
                (
                    accudo_stdlib::staking_contract_switch_operator_with_same_commission(
            ...
            InternalOperation::SetVoter(set_voter) => {
                ...
                (
                    accudo_stdlib::staking_contract_update_voter(
            ...
            InternalOperation::InitializeStakePool(init_stake_pool) => (
                accudo_stdlib::staking_contract_create_staking_contract(
            ...
            InternalOperation::ResetLockup(reset_lockup) => (
                accudo_stdlib::staking_contract_reset_lockup(reset_lockup.operator),
                reset_lockup.owner,
            ),
            InternalOperation::UnlockStake(unlock_stake) => (
                accudo_stdlib::staking_contract_unlock_stake(
            ...
            InternalOperation::UpdateCommission(update_commision) => (
                accudo_stdlib::staking_contract_update_commision(
            ...
            InternalOperation::DistributeStakingRewards(distribute_staking_rewards) => (
                accudo_stdlib::staking_contract_distribute(
            ...
            InternalOperation::AddDelegatedStake(add_delegated_stake) => (
                accudo_stdlib::delegation_pool_add_stake(
            ...
            InternalOperation::UnlockDelegatedStake(unlock_delegated_stake) => (
                accudo_stdlib::delegation_pool_unlock(
            ...
            InternalOperation::WithdrawUndelegated(withdraw_undelegated) => (
                accudo_stdlib::delegation_pool_withdraw(
