...
use accudo_config::config::{ApiConfig, DEFAULT_MAX_PAGE_SIZE};
use accudo_logger::prelude::*;
use accudo_node::AccudoNodeArgs;
use accudo_rosetta::{bootstrap, common::native_coin, types::Currency};
use accudo_sdk::move_types::language_storage::StructTag;
...
async fn main() {
    ...
    match args {
        CommandArgs::OnlineRemote(_) => {
            println!("accudo-rosetta: Starting Rosetta in Online remote (no local full node) mode")
        },
        CommandArgs::Online(_) => {
            println!("accudo-rosetta: Starting Rosetta in Online (with local full node) mode")
        },
        CommandArgs::Offline(_) => println!("accudo-rosetta: Starting Rosetta in Offline mode"),
    }
    ...
    {
        println!("accudo-rosetta: Starting local full node");
        let node_args = node_args.clone();
        let runtime = thread::spawn(move || node_args.run());

        // Wait and ensure the node is running on the URL
        let client = accudo_rest_client::Client::new(online_args.rest_api_url.clone());
            ...
            match client.get_index_bcs().await {
                ...
                Err(err) => {
                    sample!(
                        ...
                        println!(
                            "accudo-rosetta: Full node REST API isn't responding yet.  You should check the node logs.  It's been waiting {} seconds.  Error: {:?}",
        ...
        println!("accudo-rosetta: Local full node started successfully");
        Some(runtime)
    } else {
    ...
    println!("accudo-rosetta: Starting rosetta");
    ...
    .expect("accudo-rosetta: Should bootstrap rosetta server");

    println!("accudo-rosetta: Rosetta started");
...
trait ServerArgs {
    ...
    fn rest_client(&self) -> Option<accudo_rest_client::Client>;
...
/// Accudo Rosetta API Server
///
/// Provides an implementation of [Rosetta](https://www.rosetta-api.org/docs/Reference.html) on Accudo.
#[derive(Debug, Parser)]
#[clap(name = "accudo-rosetta", author, version, propagate_version = true)]
...
pub struct OnlineRemoteArgs {
    ...
    /// URL for the Accudo REST API. e.g. https://fullnode.devnet.accudo.org
...
impl ServerArgs for OnlineRemoteArgs {
    ...
    fn rest_client(&self) -> Option<accudo_rest_client::Client> {
        Some(accudo_rest_client::Client::new(self.rest_api_url.clone()))
    }
