...
[Rosetta](https://en.wikipedia.org/wiki/Rosetta_(software)) works as a sidecar to an Accudo fullnode.  Rosetta then proxies the Rosetta standard
API calls to underlying Accudo REST API calls and builds the appropriate data.  


## Running Rosetta

The `accudo-rosetta` binary can run in three modes:
1. `online` -> This runs a local fullnode and blocks the Accudo REST API from outside access, using it only as a local proxy for Rosetta APIs.
...
All transactions are parsed from the events provided by the AccudoFramework.  There are a few exceptions to this that use the transaction payload, but only for errors.
...
file to run the automated checks.  Additionally, the [accudo.ros](./accudo.ros)
file uses the Rosetta CLI DSL to describe the possible operations that
can be run.

Additionally, we have our `accudo-rosetta-cli` crate for local testing.
