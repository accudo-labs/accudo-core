...
use accudo_config::config::{AdminServiceConfig, AuthenticationConfig, NodeConfig};
...
use accudo_system_utils::utils::reply_with_status;
#[cfg(target_os = "linux")]
use accudo_system_utils::{
...
pub struct Context {
    ...
    accudo_db: RwLock<Option<Arc<DbReaderWriter>>>,
...
impl Context {
    fn set_accudo_db(&self, accudo_db: Arc<DbReaderWriter>) {
        *self.accudo_db.write() = Some(accudo_db);
    }
...
impl AdminService {
    ...
    pub fn new(node_config: &NodeConfig) -> Self {
        ...
        let runtime = accudo_runtimes::spawn_named_runtime("admin".into(), None);
    ...
    pub fn set_accudo_db(&self, accudo_db: Arc<DbReaderWriter>) {
        self.context.set_accudo_db(accudo_db)
    }
