...
use accudo_metrics_core::const_metric::ConstMetric;
use prometheus::{
...
impl BasicNodeInfoCollector {
    pub fn new(maybe_build_info: Option<&BTreeMap<String, String>>) -> Self {
        ...
        let build_info = if let Some(build_info) = maybe_build_info {
            ...
            let git_hash = accudo_build_info::get_git_hash();
            fallback_build_info.insert(accudo_build_info::BUILD_COMMIT_HASH.into(), git_hash);
            &fallback_build_info
        };
        ...
        let git_hash = build_info
            .get(accudo_build_info::BUILD_COMMIT_HASH)
            .cloned()
        ...
        let node_version = build_info
            .get(accudo_build_info::BUILD_PKG_VERSION)
            .cloned()
            .unwrap_or_else(|| String::from(UNKNOW_LABEL));
        let version_metric =
