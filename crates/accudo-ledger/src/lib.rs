...
//! # accudo-ledger
//!
//! `accudo-ledger` provides convenience methods to communicate with the Accudo app on ledger

#![deny(missing_docs)]

use accudo_crypto::ed25519::Ed25519Signature;
pub use accudo_crypto::{ed25519::Ed25519PublicKey, ValidCryptoMaterialStringExt};
...
/// 637 is the key for Accudo
pub const DERIVATION_PATH: &str = "m/44'/637'/{index}'/0'/0'";

const CLA_ACCUDO: u8 = 0x5B; // Accudo CLA Instruction class
...
#[derive(Debug, Error)]
/// Accudo Ledger Error
pub enum AccudoLedgerError {
    /// Error when trying to open a connection to the Ledger device
    #[error("Device not found")]
    DeviceNotFound,

    /// Error when communicating with Accudo app on Ledger
    #[error("Error - {0}")]
    AccudoError(AccudoLedgerStatusCode),
...
impl From<LedgerHIDError> for AccudoLedgerError {
    fn from(e: LedgerHIDError) -> Self {
        AccudoLedgerError::UnexpectedError(e.to_string(), None)
    }
}
...
/// Most Accudo ones defined here <https://github.com/accudo-labs/ledger-app-accudo/blob/main/doc/COMMANDS.md#status-words>
/// Some of the ledger status code defined here - <https://www.eftlab.com/knowledge-base/complete-list-of-apdu-responses>
pub enum AccudoLedgerStatusCode {
    /// Accudo ledger app related status code
    ...
    /// Accudo ledger app is not opened
    AppNotOpen = 0x6E01,
...
impl AccudoLedgerStatusCode {
    fn description(&self) -> &str {
        match self {
            AccudoLedgerStatusCode::Deny => "Request rejected by user",
            AccudoLedgerStatusCode::WrongPip2 => "Wrong P1 or P2",
            AccudoLedgerStatusCode::WrongDataLength => "Wrong data length",
            ...
            AccudoLedgerStatusCode::TxnParsingFail => "Failed to parse raw transaction",
            AccudoLedgerStatusCode::TxnHashFail => {
                "Failed to compute hash digest of raw transaction"
            },
            AccudoLedgerStatusCode::BadState => "Security issue with bad state",
            AccudoLedgerStatusCode::SignatureFail => "Signature of raw transaction failed",
            AccudoLedgerStatusCode::Success => "Success",
            AccudoLedgerStatusCode::LedgerLocked => "Ledger device is locked",
            AccudoLedgerStatusCode::AppNotOpen => "Accudo ledger app is not opened",
            AccudoLedgerStatusCode::Unknown => "Unknown status code",
        }
    }

    fn map_status_code(status_code: u16) -> AccudoLedgerStatusCode {
        match status_code {
            0x6985 => AccudoLedgerStatusCode::Deny,
            0x6A86 => AccudoLedgerStatusCode::WrongPip2,
            0x6A87 => AccudoLedgerStatusCode::WrongDataLength,
            0x6D00 => AccudoLedgerStatusCode::InsNotSupported,
            0x6E00 => AccudoLedgerStatusCode::ClaNotSupported,
            0xB000 => AccudoLedgerStatusCode::WrongResponseLength,
            0xB001 => AccudoLedgerStatusCode::DisplayBip32PathFail,
            0xB002 => AccudoLedgerStatusCode::DisplayAddressFail,
            0xB003 => AccudoLedgerStatusCode::DisplayAmountFail,
            0xB004 => AccudoLedgerStatusCode::WrongTxnLength,
            0xB005 => AccudoLedgerStatusCode::TxnParsingFail,
            0xB006 => AccudoLedgerStatusCode::TxnHashFail,
            0xB007 => AccudoLedgerStatusCode::BadState,
            0xB008 => AccudoLedgerStatusCode::SignatureFail,
            0x9000 => AccudoLedgerStatusCode::Success,
            0x5515 => AccudoLedgerStatusCode::LedgerLocked,
            0x6E01 => AccudoLedgerStatusCode::AppNotOpen,
            _ => AccudoLedgerStatusCode::Unknown,
        }
    }
}

impl Display for AccudoLedgerStatusCode {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{} (0x{:x})", self.description(), *self as u16)
    }
}

/// Accudo version in format major.minor.patch
#[derive(Debug)]
pub struct Version {
    major: u8,
    minor: u8,
...
/// Returns the current version of the Accudo app on Ledger
pub fn get_app_version() -> Result<Version, AccudoLedgerError> {
    ...
    match transport.exchange(&APDUCommand {
        ...
        Ok(response) => {
            ...
            if response.retcode() == APDU_CODE_SUCCESS {
                ...
                let error_code = AccudoLedgerStatusCode::map_status_code(response.retcode());
                Err(AccudoLedgerError::AccudoError(error_code))
            }
        },
        Err(err) => Err(AccudoLedgerError::from(err)),
    }
}

/// Returns the official app name register in Ledger
pub fn get_app_name() -> Result<String, AccudoLedgerError> {
    // Open connection to ledger
    let transport = open_ledger_transport()?;
        ...
        Ok(response) => {
            if response.retcode() == APDU_CODE_SUCCESS {
                let app_name = match str::from_utf8(response.data()) {
                    Ok(v) => v,
                    Err(e) => return Err(AccudoLedgerError::UnexpectedError(e.to_string(), None)),
                };
...
pub fn fetch_batch_accounts(
...
) -> Result<BTreeMap<String, AccountAddress>, AccudoLedgerError> {
    ...
    if range.end - range.start > 10 {
        return Err(AccudoLedgerError::UnexpectedError(
        ...
        match transport.exchange(&APDUCommand {
            ...
            Ok(response) => {
                ...
                if response.retcode() == APDU_CODE_SUCCESS {
                    ...
                    // Skipping weird 0x04 - because of how the Accudo Ledger parse works when return pub key
                    offset += 1;
                    ...
                    let public_key = match Ed25519PublicKey::from_encoded_string(&hex_string) {
                        Ok(pk) => Ok(pk),
                        Err(err) => Err(AccudoLedgerError::UnexpectedError(
                            err.to_string(),
                    ...
                    let error_code = AccudoLedgerStatusCode::map_status_code(response.retcode());
                    return Err(AccudoLedgerError::AccudoError(error_code));
                }
            },
            Err(err) => return Err(AccudoLedgerError::from(err)),
        }
    }
...
/// Returns the public key of your Accudo account in Ledger device at index 0
///
/// # Arguments
///
/// * `display` - If true, the public key will be displayed on the Ledger device, and confirmation is needed
pub fn get_public_key(path: &str, display: bool) -> Result<Ed25519PublicKey, AccudoLedgerError> {
    // Open connection to ledger
    let transport = open_ledger_transport()?;
        ...
        Ok(response) => {
            ...
            if response.retcode() == APDU_CODE_SUCCESS {
                ...
                match Ed25519PublicKey::from_encoded_string(&hex_string) {
                    Ok(pk) => Ok(pk),
                    Err(err) => Err(AccudoLedgerError::UnexpectedError(err.to_string(), None)),
                }
            } else {
                let error_code = AccudoLedgerStatusCode::map_status_code(response.retcode());
                Err(AccudoLedgerError::AccudoError(error_code))
...
pub fn sign_message(path: &str, raw_message: &[u8]) -> Result<Ed25519Signature, AccudoLedgerError> {
    // open connection to ledger
    let transport = open_ledger_transport()?;
    ...
    if let Err(err) = sign_start {
        return Err(AccudoLedgerError::UnexpectedError(err.to_string(), None));
    }
                ...
                if response.retcode() == APDU_CODE_SUCCESS {
                    if is_last_chunk {
                        ...
                        return Ed25519Signature::try_from(signature_buffer).map_err(|err| {
                            AccudoLedgerError::UnexpectedError(err.to_string(), None)
                        });
                    }
                } else {
                    let error_code = AccudoLedgerStatusCode::map_status_code(response.retcode());
    ...
    Err(AccudoLedgerError::UnexpectedError(
...
fn open_ledger_transport() -> Result<TransportNativeHID, AccudoLedgerError> {
    // open connection to ledger
    // NOTE: ledger has to be unlocked
    let hidapi = match HidApi::new() {
        Ok(hidapi) => hidapi,
        Err(_err) => return Err(AccudoLedgerError::DeviceNotFound),
    };
