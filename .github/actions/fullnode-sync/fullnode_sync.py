...
# This script runs the fullnode-sync from the root of accudo-core.

# Useful file constants
FAST_SYNC_BOOTSTRAPPING_MODE = "DownloadLatestStates" # The bootstrapping string for fast sync
FULLNODE_CONFIG_NAME = "public_full_node.yaml" # Relative to the accudo-core repo
FULLNODE_CONFIG_TEMPLATE_PATH = "config/src/config/test_data/public_full_node.yaml" # Relative to the accudo-core repo
GENESIS_BLOB_PATH = "https://raw.githubusercontent.com/accudo-labs/accudo-networks/main/{network}/genesis.blob" # Location inside the accudo-networks repo
...
LEDGER_VERSION_METRICS_STRING = "accudo_state_sync_version.synced" # The string to fetch the ledger version from the metrics API
MAX_TIME_BETWEEN_SYNC_INCREASES_SECS = 1800 # The number of seconds after which to fail if the node isn't syncing
REMOTE_REST_ENDPOINTS = "https://fullnode.{network}.accudo.org/v1" # The remote rest endpoint
SYNCING_DELTA_VERSIONS = 20000 # The number of versions to sync beyond the highest known at the job start
SYNCED_STATES_METRICS_STRING = "accudo_state_sync_version.synced_states" # The string to fetch the synced states from the metrics API
WAYPOINT_FILE_PATH = "https://raw.githubusercontent.com/accudo-labs/accudo-networks/main/{network}/waypoint.txt" # Location inside the accudo-networks repo
...
def spawn_fullnode(git_ref, network, bootstrapping_mode, continuous_syncing_mode, node_log_file_path):
  ...
  process_handle = subprocess.Popen(["cargo", "run", "-p", "accudo-node", "--release", "--", "-f", FULLNODE_CONFIG_NAME], stdout=node_log_file, stderr=node_log_file)
...
def checkout_git_ref(git_ref):
  """Checkout the specified git ref. This assumes the working directory is accudo-core"""
