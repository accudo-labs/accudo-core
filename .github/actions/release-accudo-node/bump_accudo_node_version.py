...
def get_release_number_from_release_tag(release_tag: str) -> str:
    """Get the release number from the release tag. The release tag looks like accudo-node-vX.Y.Z"""
...
def main():
    required_envs = ["RELEASE_TAG", "ACCUDO_NODE_CARGO_TOML"]
    ...
    ACCUDO_NODE_TOML = os.environ.get("ACCUDO_NODE_CARGO_TOML")

    new_lines = []  # construct the file again
    with open(ACCUDO_NODE_TOML, "r") as f:
        lines = f.readlines()
        for line in lines:
            new_line = line
    ...
    with open(ACCUDO_NODE_TOML, "w") as f:
...
class CheckAccudoNodeVersionTests(unittest.TestCase):
    def test_envs_not_specified(self):
        with self.assertRaises(SystemExit):
            os.environ["RELEASE_TAG"] = ""
            os.environ["ACCUDO_NODE_CARGO_TOML"] = ""
            main()
    ...
    def test_version_match(self):
        ...
        os.environ["ACCUDO_NODE_CARGO_TOML"] = tmp.name
