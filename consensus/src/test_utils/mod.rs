...
use accudo_crypto::{HashValue, PrivateKey, Uniform};
use accudo_logger::Level;
use accudo_types::{ledger_info::LedgerInfo, validator_signer::ValidatorSigner};
...
pub fn consensus_runtime() -> runtime::Runtime {
    if nocapture() {
        ::accudo_logger::Logger::new().level(Level::Debug).init();
    }

    accudo_runtimes::spawn_named_runtime("consensus".into(), None)
}
