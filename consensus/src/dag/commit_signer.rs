...
impl CommitSignerProvider for DagCommitSigner {
    fn sign_commit_vote(
        &self,
        _ledger_info: accudo_types::ledger_info::LedgerInfoWithSignatures,
        new_ledger_info: accudo_types::ledger_info::LedgerInfo,
    ) -> Result<bls12381::Signature, accudo_safety_rules::Error> {
        let signature = self
            .signer
            .sign(&new_ledger_info)
            .map_err(|err| accudo_safety_rules::Error::SerializationError(err.to_string()))?;
