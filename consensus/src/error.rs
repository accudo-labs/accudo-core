...
impl From<accudo_storage_interface::AccudoDbError> for DbError {
    fn from(e: accudo_storage_interface::AccudoDbError) -> Self {
...
impl From<accudo_executor_types::ExecutorError> for StateSyncError {
    fn from(e: accudo_executor_types::ExecutorError) -> Self {
        StateSyncError { inner: e.into() }
    }
}
...
pub fn error_kind(e: &anyhow::Error) -> &'static str {
    if e.downcast_ref::<accudo_executor_types::ExecutorError>()
        .is_some()
    {
        return "Execution";
    }
    if let Some(e) = e.downcast_ref::<StateSyncError>() {
        if e.inner
            .downcast_ref::<accudo_executor_types::ExecutorError>()
            .is_some()
        {
            return "Execution";
        }
    ...
    if e.downcast_ref::<accudo_safety_rules::Error>().is_some() {
        return "SafetyRules";
    }
...
mod tests {
    ...
    fn conversion_and_downcast() {
        let error = accudo_executor_types::ExecutorError::InternalError {
