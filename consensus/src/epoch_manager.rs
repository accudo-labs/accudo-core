...
use crate::{
    ...
    liveness::{
        ...
        leader_reputation::{
            extract_epoch_to_proposers, AccudoDBBackend, LeaderReputation,
...
use accudo_crypto::bls12381::PrivateKey;
use accudo_dkg::{
...
use accudo_mempool::QuorumStoreRequest;
use accudo_network::{application::interface::NetworkClient, protocols::network::Event};
use accudo_safety_rules::{
...
pub struct EpochManager<P: OnChainConfigProvider> {
    ...
    self_sender: accudo_channels::UnboundedSender<Event<ConsensusMsg>>,
    network_sender: ConsensusNetworkClient<NetworkClient<ConsensusMsg>>,
    timeout_sender: accudo_channels::Sender<Round>,
    ...
    rand_manager_msg_tx: Option<accudo_channel::Sender<AccountAddress, IncomingRandGenRequest>>,
    // channels to round manager
    round_manager_tx: Option<
        accudo_channel::Sender<(Author, Discriminant<VerifiedEvent>), (Author, VerifiedEvent)>,
    >,
    buffered_proposal_tx: Option<accudo_channel::Sender<Author, VerifiedEvent>>,
    round_manager_close_tx: Option<oneshot::Sender<oneshot::Sender<()>>>,
    epoch_state: Option<Arc<EpochState>>,
    block_retrieval_tx:
        Option<accudo_channel::Sender<AccountAddress, IncomingBlockRetrievalRequest>>,
    ...
    batch_retrieval_tx:
        Option<accudo_channel::Sender<AccountAddress, IncomingBatchRetrievalRequest>>,
    bounded_executor: BoundedExecutor,
    // recovery_mode is set to true when the recovery manager is spawned
    recovery_mode: bool,

    accudo_time_service: accudo_time_service::TimeService,
    dag_rpc_tx: Option<accudo_channel::Sender<AccountAddress, IncomingDAGRequest>>,
...
impl<P: OnChainConfigProvider> EpochManager<P> {
    ...
    fn create_proposer_election(
        ...
        match &onchain_config.proposer_election_type() {
            ...
            ProposerElectionType::LeaderReputation(leader_reputation_type) => {
                ...
                let backend = Arc::new(AccudoDBBackend::new(
                    window_size,
                    seek_len,
                    self.storage.accudo_db(),
                ));
    ...
    fn spawn_block_retrieval_task(
        ...
        let (request_tx, mut request_rx) = accudo_channel::new::<_, IncomingBlockRetrievalRequest>(
            QueueStyle::KLAST,
    ...
    async fn start_recovery_manager(
        ...
        let (recovery_manager_tx, recovery_manager_rx) = accudo_channel::new(
            QueueStyle::KLAST,
            self.config.internal_per_key_channel_size,
    ...
    async fn start_round_manager(
        ...
        rand_msg_rx: accudo_channel::Receiver<AccountAddress, IncomingRandGenRequest>,
    ) {
        ...
        let (round_manager_tx, round_manager_rx) = accudo_channel::new(
            QueueStyle::KLAST,
            self.config.internal_per_key_channel_size,
            Some(&counters::ROUND_MANAGER_CHANNEL_MSGS),
        );

        let (buffered_proposal_tx, buffered_proposal_rx) = accudo_channel::new(
            QueueStyle::KLAST,
            self.config.internal_per_key_channel_size,
            Some(&counters::ROUND_MANAGER_CHANNEL_MSGS),
        );
        ...
        let (opt_proposal_loopback_tx, opt_proposal_loopback_rx) =
            accudo_channels::new_unbounded(&counters::OP_COUNTERS.gauge("opt_proposal_queue"));
    ...
    async fn start_new_epoch_with_dag(
        ...
        let bootstrapper = DagBootstrapper::new(
            ...
            network_sender_arc.clone(),
            network_sender_arc,
            self.accudo_time_service.clone(),
            payload_manager,
            payload_client,
            self.execution_client
        ...
        let (dag_rpc_tx, dag_rpc_rx) = accudo_channel::new(QueueStyle::FIFO, 10, None);
    ...
    fn forward_event_to<K: Eq + Hash + Clone, V>(
        mut maybe_tx: Option<accudo_channel::Sender<K, V>>,
        key: K,
        value: V,
    ) -> anyhow::Result<()> {
    ...
    pub async fn start(
        mut self,
        mut round_timeout_sender_rx: accudo_channels::Receiver<Round>,
