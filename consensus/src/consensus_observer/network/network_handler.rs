...
use accudo_channels::{
    accudo_channel,
    accudo_channel::{Receiver, Sender},
    message_queues::QueueStyle,
};
use accudo_config::{config::ConsensusObserverConfig, network_id::PeerNetworkId};
...
impl ConsensusObserverNetworkHandler {
    pub fn new(
        ...
        let (observer_message_sender, observer_message_receiver) = accudo_channel::new(
            QueueStyle::FIFO,
        ...
        let (publisher_message_sender, publisher_message_receiver) = accudo_channel::new(
            QueueStyle::FIFO,
            consensus_observer_config.max_network_channel_size as usize,
            None,
        );
...
mod test {
    ...
    use accudo_channels::{accudo_channel, accudo_channel::Receiver, message_queues::QueueStyle};
    use accudo_config::{
    ...
    /// Creates and returns a single Accudo channel
    fn create_accudo_channel<K: Eq + Hash + Clone, T>(
    ) -> (accudo_channel::Sender<K, T>, accudo_channel::Receiver<K, T>) {
        accudo_channel::new(QueueStyle::FIFO, 10, None)
    }
    ...
    fn create_network_sender_and_events(
        ...
        HashMap<NetworkId, accudo_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>>,
        ...
        for network_id in network_ids {
            ...
            let (inbound_request_sender, inbound_request_receiver) = create_accudo_channel();
            let (outbound_request_sender, outbound_request_receiver) = create_accudo_channel();
            let (connection_outbound_sender, _connection_outbound_receiver) =
                create_accudo_channel();
    ...
    async fn wait_for_handler_processing(
        ...
        observer_message_receiver: &mut accudo_channel::Receiver<
            (),
            ConsensusObserverNetworkMessage,
        >,
        publisher_message_receiver: &mut accudo_channel::Receiver<
            (),
            ConsensusPublisherNetworkMessage,
        >,
    ...
    async fn wait_for_outbound_processing(
        ...
        outbound_request_receivers: &mut HashMap<
            NetworkId,
            accudo_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
        >,
