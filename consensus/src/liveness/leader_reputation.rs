...
use accudo_infallible::{Mutex, MutexGuard};
...
impl AccudoDBBackend {
    pub fn new(window_size: usize, seek_len: usize, accudo_db: Arc<dyn DbReader>) -> Self {
        Self {
            window_size,
            seek_len,
            accudo_db,
    ...
    fn refresh_db_result(
        ...
        let events = self.accudo_db.get_latest_block_events(limit)?;
...
impl MetadataBackend for AccudoDBBackend {
    ...
    fn get_block_metadata(
        ...
        let latest_db_version = self.accudo_db.get_latest_ledger_info_version().unwrap_or(0);
        // lazy init db_result
        if locked.is_none() {
