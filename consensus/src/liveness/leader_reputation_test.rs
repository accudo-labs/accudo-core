...
use super::leader_reputation::{
    extract_epoch_to_proposers_impl, AccudoDBBackend, ProposerAndVoterHeuristic,
};
use crate::liveness::{
    leader_reputation::{
...
struct Example1 {
    ...
    accudo_db: Arc<MockDbReader>,
    backend: AccudoDBBackend,
}
...
impl Example1 {
    fn new(window_size: usize) -> Self {
        ...
        let accudo_db = Arc::new(MockDbReader::new());
        let backend = AccudoDBBackend::new(window_size, 0, accudo_db.clone());
...
fn test_api(use_root_hash: bool) {
    ...
    for epoch in 1..1000 {
        ...
        assert_eq!(
            (epoch, 1),
            accudo_db.add_event_with_data(proposers[0], vec![1, 2], vec![])
        );
        assert_eq!(
            (epoch, 2),
            accudo_db.add_event_with_data(proposers[0], vec![3], vec![])
        );
        let backend = Arc::new(AccudoDBBackend::new(1, 4, accudo_db.clone()));
        ...
        let state = if use_root_hash {
            [
                accudo_db.get_accumulator_root_hash(0).unwrap().to_vec(),
...
impl DbReader for MockDbReader {
    ...
    fn get_accumulator_root_hash(
        &self,
        _version: Version,
    ) -> accudo_storage_interface::Result<HashValue> {
        Ok(HashValue::zero())
    }
}
...
fn backend_wrapper_test() {
    let accudo_db = Arc::new(MockDbReader::new());
    let backend = AccudoDBBackend::new(3, 3, accudo_db.clone());

    accudo_db.add_event(0, 1);
    accudo_db.new_epoch();
    accudo_db.skip_rounds(1);
    for i in 2..6 {
        accudo_db.add_event(1, i);
    }
    ...
    let mut assert_history = |round, expected_history: Vec<Round>, to_fetch| {
        ...
        assert_eq!(fetch_count, accudo_db.fetched(), "At round {}", round);
    };
    ...
    accudo_db.add_another_transaction();
    assert_history(12, vec![12, 11, 10], false);
    ...
    accudo_db.add_another_transaction();
    accudo_db.add_event_after_call(1, 13);
...
fn backend_test_cross_epoch() {
    ...
    accudo_db.add_event(0, 1);
    accudo_db.new_epoch();
    accudo_db.add_event(1, 1);
    accudo_db.add_event(1, 2);
    accudo_db.add_event(1, 3);
    accudo_db.new_epoch();
    accudo_db.add_event(2, 1);
    accudo_db.add_event(2, 2);
