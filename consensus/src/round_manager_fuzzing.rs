...
use accudo_consensus_types::{proposal_msg::ProposalMsg, utils::PayloadTxnsSize};
...
use accudo_safety_rules::{test_utils, SafetyRules, TSafetyRules};
use accudo_types::{
...
fn create_node_for_fuzzing() -> RoundManager {
    ...
    let (self_sender, _self_receiver) = accudo_channels::new_unbounded_test();
    ...
    let (opt_proposal_loopback_tx, _) =
        accudo_channels::new_unbounded(&counters::OP_COUNTERS.gauge("opt_proposal_loopback_queue"));
