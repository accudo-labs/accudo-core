...
pub trait PersistentLivenessStorage: Send + Sync {
    ...
    /// Returns a handle of the accudodb.
    fn accudo_db(&self) -> Arc<dyn DbReader>;
...
impl RootMetadata {
    ...
    pub fn new_empty() -> Self {
        Self {
            accu_hash: *accudo_crypto::hash::ACCUMULATOR_PLACEHOLDER_HASH,
...
impl StorageWriteProxy {
    pub fn new(config: &NodeConfig, accudo_db: Arc<dyn DbReader>) -> Self {
        let db = Arc::new(ConsensusDB::new(config.storage.dir()));
        StorageWriteProxy { db, accudo_db }
    }
}
