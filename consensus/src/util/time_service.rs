...
where
    T: Send + 'static,
{
    sender: Option<accudo_channels::Sender<T>>,
    message: Option<T>,
}
    ...
    T: Send + 'static,
{
    ...
    pub fn make(sender: accudo_channels::Sender<T>, message: T) -> Box<dyn ScheduledTask> {
        Box::new(SendTask {
            sender: Some(sender),
...
async fn test_time_service_abort() {
    ...
    let (tx, mut rx) = accudo_channels::new_test(10);
