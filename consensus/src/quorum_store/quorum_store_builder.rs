...
use accudo_config::config::{BatchTransactionFilterConfig, QuorumStoreConfig};
...
impl QuorumStoreBuilder {
    pub fn init_payload_manager(
        ...
        Option<accudo_channel::Sender<AccountAddress, (Author, VerifiedEvent)>>,
    ) {
        match self {
    ...
    pub fn start(
        ...
        accudo_channel::Sender<AccountAddress, IncomingBatchRetrievalRequest>,
    )> {
        match self {
...
pub struct InnerBuilder {
    ...
    quorum_store_msg_tx: accudo_channel::Sender<AccountAddress, (Author, VerifiedEvent)>,
    quorum_store_msg_rx: Option<accudo_channel::Receiver<AccountAddress, (Author, VerifiedEvent)>>,
...
impl InnerBuilder {
    ...
    pub(crate) fn new(
        ...
        let (quorum_store_msg_tx, quorum_store_msg_rx) =
            accudo_channel::new::<AccountAddress, (Author, VerifiedEvent)>(
                QueueStyle::FIFO,
                config.channel_size,
                None,
            );
    ...
    fn spawn_quorum_store(
        ...
        let (batch_retrieval_tx, mut batch_retrieval_rx) =
            accudo_channel::new::<AccountAddress, IncomingBatchRetrievalRequest>(
                QueueStyle::LIFO,
                10,
                Some(&counters::BATCH_RETRIEVAL_TASK_MSGS),
            );
        let accudo_db_clone = self.accudo_db.clone();
        spawn_named!("batch_serve", async move {
            ...
            while let Some(rpc_request) = batch_retrieval_rx.next().await {
                    ...
                    batch_store.get_batch_from_local(&rpc_request.req.digest())
                {
                    ...
                    match accudo_db_clone.get_latest_ledger_info() {
