...
use accudo_config::config::ConsensusObserverConfig;
...
use accudo_network::protocols::{rpc::error::RpcError, wire::handshake::v1::ProtocolId};
use accudo_reliable_broadcast::{DropGuard, ReliableBroadcast};
...
pub struct BufferManager {
    ...
    commit_msg_rx: Option<
        accudo_channels::accudo_channel::Receiver<
            AccountAddress,
...
impl BufferManager {
    ...
    pub fn new(
        ...
        signing_phase_rx: Receiver<SigningResponse>,
        commit_msg_tx: Arc<NetworkSender>,
        commit_msg_rx: accudo_channels::accudo_channel::Receiver<
            AccountAddress,
            (AccountAddress, IncomingCommitRequest),
        >,
        persisting_phase_tx: Sender<CountedRequest<PersistingRequest>>,
    ...
    fn generate_commit_message(commit_vote: CommitVote) -> CommitMessage {
        fail_point!("consensus::create_invalid_commit_vote", |_| {
            CommitMessage::Vote(CommitVote::new_with_signature(
                ...
                accudo_crypto::bls12381::Signature::dummy_signature(),
            ))
        });
