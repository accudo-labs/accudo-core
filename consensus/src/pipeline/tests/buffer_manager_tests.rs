...
use accudo_config::{config::ConsensusObserverConfig, network_id::NetworkId};
use accudo_consensus_types::{
    block::block_test_utils::certificate_for_genesis, pipelined_block::PipelinedBlock,
    vote_proposal::VoteProposal,
};
use accudo_crypto::{hash::ACCUMULATOR_PLACEHOLDER_HASH, HashValue};
...
pub fn prepare_buffer_manager(
    ...
    Sender<ResetRequest>,
    accudo_channel::Sender<AccountAddress, (AccountAddress, IncomingCommitRequest)>,
    ...
    let (self_loop_tx, self_loop_rx) = accudo_channels::new_unbounded_test();
    ...
    let (msg_tx, msg_rx) = accudo_channel::new::<
        AccountAddress,
...
async fn loopback_commit_vote(
    ...
    msg_tx: &accudo_channel::Sender<AccountAddress, (AccountAddress, IncomingCommitRequest)>,
