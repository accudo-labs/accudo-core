...
pub const ACCUDO_COIN_TYPE_STR: &str = "0x1::accudo_coin::AccudoCoin";
lazy_static! {
...
/// LocalAccount represents an account on the Accudo blockchain. Internally it
...
pub fn get_paired_fa_metadata_address(coin_type_name: &MoveStructTag) -> AccountAddress {
    ...
    if coin_type_name == ACCUDO_COIN_TYPE_STR {
...
impl LocalAccount {
    ...
    /// does not actually create an account on the Accudo blockchain, just a
    ...
    /// create an account on the Accudo blockchain, it just generates a new
    /// account locally.
...
impl HardwareWalletAccount {
    ...
    /// This requires the Ledger device to be connected, unlocked and the Accudo app to be opened
    pub fn from_ledger(
        derivation_path: String,
        sequence_number: u64,
    ) -> Result<Self, AccudoLedgerError> {
        let public_key = accudo_ledger::get_public_key(&derivation_path, false)?;
    ...
    pub fn sign_arbitrary_message(
        &self,
        message: &[u8],
    ) -> Result<Ed25519Signature, AccudoLedgerError> {
        accudo_ledger::sign_message(&self.derivation_path, message)
    }
}
...
mod tests {
    ...
    use accudo_rest_client::{AccudoBaseUrl, FaucetClient};
    use reqwest::Url;

    #[test]
    fn test_recover_account_from_derive_path() {
        // Same constants in test cases of TypeScript
        // https://github.com/accudo-labs/accudo-core/blob/main/ecosystem/typescript/sdk/src/accudo_account.test.ts
    ...
    async fn test_derive_keyless_account() {
        let accudo_rest_client = Client::builder(AccudoBaseUrl::Devnet).build();
        // This JWT is taken from https://github.com/accudo-labs/accudo-ts-sdk/blob/f644e61beb70e69dfd489e75287c67b527385135/tests/e2e/api/keyless.test.ts#L11
        ...
        if balance < 10000000 {
            ...
            let faucet_client = FaucetClient::new_from_rest_client(
                Url::from_str("https://faucet.devnet.accudo.org").unwrap(),
                accudo_rest_client.clone(),
            );
            faucet_client
        ...
        let coin_client = CoinClient::new(&accudo_rest_client);
