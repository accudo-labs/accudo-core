...
use accudo_vault_client::Client;
#[cfg(any(test, feature = "testing"))]
use accudo_vault_client::ReadResponse;
use chrono::DateTime;
...
impl VaultStorage {
        ...
        if self.renew_ttl_secs.is_some() {
            ...
            if now >= next_renewal {
                ...
                if let Ok(ttl) = result {
                    ...
                    accudo_logger::error!("Unable to renew lease: {}", e.to_string());
                }
            }
        }
        &self.client
    ...
    fn reset_crypto(&self) -> Result<(), Error> {
        let keys = match self.client().list_keys() {
            ...
            Err(accudo_vault_client::Error::NotFound(_, _)) => return Ok(()),
...
impl CryptoStorage for VaultStorage {
    ...
    fn sign<T: CryptoHash + Serialize>(
        ...
        let name = self.crypto_name(name);
        let mut bytes = <T::Hasher as accudo_crypto::hash::CryptoHasher>::seed().to_vec();
...
pub mod policy {
    ...
    use accudo_vault_client as vault;

    const ACCUDO_DEFAULT: &str = "accudo_default";
        ...
        pub fn set_policies(
            ...
            for perm in &policy.permissions {
                match &perm.id {
                    ...
                    Identity::Anyone => {
                        self.set_policy(ACCUDO_DEFAULT, engine, name, &perm.capabilities)?
                    },
                    Identity::NoOne => (),
                };
            }
