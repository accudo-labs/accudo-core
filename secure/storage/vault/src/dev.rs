...
static ACCUDO_VAULT: Lazy<Option<VaultRunner>> = Lazy::new(|| match VaultRunner::run() {
    Err(err) => {
        assert!(
            std::env::var("ACCUDO_REQUIRE_VAULT_TESTS").is_err(),
...
pub fn test_host_safe() -> Option<String> {
    ACCUDO_VAULT.as_ref().map(|v| v.host().to_string())
}
