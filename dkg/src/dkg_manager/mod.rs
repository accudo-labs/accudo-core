...
pub struct DKGManager<DKG: DKGTrait> {
    ...
    agg_trx_tx: Option<accudo_channel::Sender<(), DKG::Transcript>>,

    // When we put vtxn in the pool, we also put a copy of this so later pool can notify us.
    pull_notification_tx: accudo_channel::Sender<(), Arc<ValidatorTransaction>>,
    pull_notification_rx: accudo_channel::Receiver<(), Arc<ValidatorTransaction>>,
...
impl<DKG: DKGTrait> DKGManager<DKG> {
    ...
    pub async fn run(
        ...
        mut dkg_start_event_rx: accudo_channel::Receiver<(), DKGStartEvent>,
        mut rpc_msg_rx: accudo_channel::Receiver<
            AccountAddress,
        ...
        let (agg_trx_tx, mut agg_trx_rx) = accudo_channel::new(QueueStyle::KLAST, 1, None);
