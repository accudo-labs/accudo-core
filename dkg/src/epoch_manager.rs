...
pub struct EpochManager<P: OnChainConfigProvider> {
    ...
    dkg_start_event_tx: Option<accudo_channel::Sender<(), DKGStartEvent>>,
    vtxn_pool: VTxnPoolState,

    // Network utils
    self_sender: accudo_channels::Sender<Event<DKGMessage>>,
    ...
    async fn start_new_epoch(&mut self, payload: OnChainConfigPayload<P>) -> Result<()> {
        ...
        if let (true, Some(my_index)) = (randomness_enabled, my_index) {
            ...
            let (dkg_start_event_tx, dkg_start_event_rx) =
                accudo_channel::new(QueueStyle::KLAST, 1, None);
            self.dkg_start_event_tx = Some(dkg_start_event_tx);

            let (dkg_rpc_msg_tx, dkg_rpc_msg_rx) = accudo_channel::new::<
                AccountAddress,
