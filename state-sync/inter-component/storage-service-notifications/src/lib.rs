...
pub fn new_storage_service_notifier_listener_pair(
    ...
    let (notification_sender, notification_receiver) = accudo_channel::new(
        QueueStyle::LIFO,
...
pub struct StorageServiceNotifier {
    notification_sender: accudo_channel::Sender<(), StorageServiceCommitNotification>,
}
...
pub struct StorageServiceNotificationListener {
    notification_receiver: accudo_channel::Receiver<(), StorageServiceCommitNotification>,
}
