...
impl EventSubscriptionService {
    ...
    pub fn subscribe_to_events(
        ...
        let (notification_sender, notification_receiver) =
            accudo_channel::new(QueueStyle::KLAST, EVENT_NOTIFICATION_CHANNEL_SIZE, None);
    ...
    pub fn subscribe_to_reconfigurations(
        ...
        let (notification_sender, notification_receiver) =
            accudo_channel::new(QueueStyle::KLAST, RECONFIG_NOTIFICATION_CHANNEL_SIZE, None);
...
struct EventSubscription {
    ...
    pub notification_sender: accudo_channels::accudo_channel::Sender<(), EventNotification>,
}
...
struct ReconfigSubscription {
    pub notification_sender:
        accudo_channels::accudo_channel::Sender<(), ReconfigNotification<DbBackedOnChainConfig>>,
}
...
impl OnChainConfigProvider for DbBackedOnChainConfig {
    fn get<T: OnChainConfig>(&self) -> Result<T> {
        let bytes = self
            ...
            .ok_or_else(|| {
                anyhow!(
                    "no config {} found in accudo root account state",
                    T::CONFIG_ID
                )
            })?
            .bytes()
...
pub struct NotificationListener<T> {
    pub notification_receiver: accudo_channels::accudo_channel::Receiver<(), T>,
}
