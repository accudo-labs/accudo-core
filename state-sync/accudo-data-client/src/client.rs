...
use accudo_config::{
    config::{AccudoDataClientConfig, BaseConfig},
    network_id::PeerNetworkId,
};
use accudo_id_generator::{IdGenerator, U64IdGenerator};
use accudo_infallible::Mutex;
use accudo_logger::{info, sample, sample::SampleRate, trace, warn};
use accudo_network::{
...
/// An [`AccudoDataClientInterface`] that fulfills requests from remote peers' Storage Service
/// over AccudoNet.
///
/// The `AccudoDataClient`:
///
/// 1. Sends requests to connected Accudo peers.
...
/// and/or threads.
#[derive(Clone, Debug)]
pub struct AccudoDataClient {
    /// The base config of the node.
    base_config: Arc<BaseConfig>,
    /// The config for the AccudoNet data client.
    data_client_config: Arc<AccudoDataClientConfig>,
    /// The underlying AccudoNet storage service client.
...
impl AccudoDataClient {
        ...
        E: Into<Error>,
    {
        ...
        for peer in peers {
            ...
            let accudo_data_client = self.clone();
    ...
    async fn send_request_to_peer(
        ...
        match result {
            Ok(response) => {
                ...
                // with this RPC.
                let response_callback = AccudoNetResponseCallback {
            ...
            Err(error) => {
                ...
                let client_error = match error {
                    accudo_storage_service_client::Error::RpcError(rpc_error) => match rpc_error {
                    ...
                    accudo_storage_service_client::Error::StorageServiceError(err) => {
...
#[async_trait]
impl AccudoDataClientInterface for AccudoDataClient {
...
/// The AccudoNet-specific request context needed to update a peer's scoring.
struct AccudoNetResponseCallback {
    data_client: AccudoDataClient,
    id: ResponseId,
    peer: PeerNetworkId,
    request: StorageServiceRequest,
}

impl ResponseCallback for AccudoNetResponseCallback {
...
impl fmt::Debug for AccudoNetResponseCallback {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("AccudoNetResponseCallback")
            .field("data_client", &"..")
            .field("id", &self.id)
