...
use crate::{
    client::AccudoDataClient,
    error::Result,
    global_summary::GlobalDataSummary,
    interface::{AccudoDataClientInterface, Response, SubscriptionRequestMetadata},
    poller::DataSummaryPoller,
    priority::PeerPriority,
};
use accudo_channels::{accudo_channel, message_queues::QueueStyle};
use accudo_config::{
    config::{AccudoDataClientConfig, BaseConfig, RoleType},
...
use accudo_peer_monitoring_service_types::{
...
use accudo_storage_service_server::network::{NetworkRequest, ResponseSender};
...
pub struct MockNetwork {
    ...
    peer_mgr_reqs_rxs:
        HashMap<NetworkId, accudo_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>>, // The peer manager request receivers
}

impl MockNetwork {
    pub fn new(
        base_config: Option<BaseConfig>,
        data_client_config: Option<AccudoDataClientConfig>,
        networks: Option<Vec<NetworkId>>,
    ) -> (Self, MockTimeService, AccudoDataClient, DataSummaryPoller) {
        // Initialize the logger for testing
        ::accudo_logger::Logger::init_for_testing();
        ...
        for network in &networks {
            ...
            let queue_cfg = accudo_channel::Config::new(10).queue_style(QueueStyle::FIFO);
        ...
        let (client, poller) = AccudoDataClient::new(
            data_client_config,
            base_config.clone(),
            mock_time.clone(),
...
pub fn create_mock_data_client() -> Arc<dyn AccudoDataClientInterface + Send + Sync> {
    Arc::new(MockAccudoDataClient::new())
}

// This automatically creates a MockAccudoDataClient
mock! {
    pub AccudoDataClient {}
...
impl DbReader for MockDatabaseReader {
    fn get_block_timestamp(&self, version: Version) -> accudo_storage_interface::Result<u64> {
        Ok(version * 100_000)
    }
}
