...
use accudo_executor_types::ChunkCommitNotification;
use accudo_infallible::{Mutex, RwLock};
use accudo_mempool_notifications::MempoolNotificationListener;
use accudo_storage_interface::{AccudoDbError, DbReaderWriter};
...
async fn test_save_states_invalid_chunk() {
    ...
    snapshot_receiver
        .expect_add_chunk()
        .with(always(), always())
        .returning(|_, _| Err(AccudoDbError::Other("Invalid chunk!".to_string())));
