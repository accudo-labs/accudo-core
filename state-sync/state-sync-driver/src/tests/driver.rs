...
use accudo_config::config::{NodeConfig, RoleType, StateSyncDriverConfig};
use accudo_consensus_notifications::{ConsensusNotificationSender, ConsensusNotifier};
...
async fn create_driver_for_tests(
    ...
    // Create test accudo database
    let db_path = accudo_temppath::TempPath::new();
    db_path.create_as_dir().unwrap();
    let (_, db_rw) = DbReaderWriter::wrap(AccudoDB::new_for_test(db_path.path()));

    // Bootstrap the genesis transaction
    let (genesis, _) = accudo_vm_genesis::test_genesis_change_set_and_validators(Some(1));
    let genesis_txn = Transaction::GenesisTransaction(WriteSetPayload::Direct(genesis));
    bootstrap_genesis::<AccudoVMBlockExecutor>(&db_rw, &genesis_txn).unwrap();
    ...
    let (consensus_notifier, consensus_listener) =
        accudo_consensus_notifications::new_consensus_notifier_listener_pair(5000);
