...
use accudo_network::protocols::network::RpcError;
use accudo_types::{
        ...
        for use_request_v2 in [false, true] {
            ...
            for chunk_size in [100, max_output_chunk_size] {
                ...
                for fallback_to_transactions in [false, true] {
                    ...
                    let mut response_receiver_1 =
                        utils::subscribe_to_transactions_or_outputs_for_peer(
                            &mut mock_client,
                            peer_version_1,
                            highest_epoch,
                            false,
        ...
        for use_request_v2 in [false, true] {
            ...
            for fallback_to_transactions in [false, true] {
                ...
                for batch_id in 0..num_batches_to_send {
                    ...
                    let mut response_receivers =
                        send_transaction_or_output_subscription_request_batch(
                            &mut mock_client,
                            peer_network_id,
                            first_request_index,
                            last_request_index,
    ...
    for use_size_and_time_aware_chunking in [false, true] {
        ...
        for use_request_v2 in [false, true] {
            ...
            for fallback_to_transactions in [false, true] {
                ...
                for (i, (start_version, end_version)) in
                    chunk_start_and_end_versions.iter().enumerate()
                {
                    // Set expectations for transaction output reads
            ...
            for fallback_to_transactions in [false, true] {
                ...
                for stream_request_index in 0..num_stream_requests {
                    let response_receiver =
                        response_receivers.remove(&stream_request_index).unwrap();
                    if fallback_to_transactions {
                        utils::verify_new_transactions_or_outputs_with_proof(
                            &mut mock_client,
                            response_receiver,
                            use_request_v2,
                            Some(
                                transaction_lists_with_proofs[stream_request_index as usize]
                                    .clone(),
                            ),
                            None,
                            highest_ledger_info.clone(),
