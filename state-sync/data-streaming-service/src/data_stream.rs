...
use accudo_config::config::{AccudoDataClientConfig, DataStreamingServiceConfig};
use accudo_data_client::{
    global_summary::{AdvertisedData, GlobalDataSummary},
    interface::{
        AccudoDataClientInterface, Response, ResponseContext, ResponseError, ResponsePayload,
...
/// has already been sent along the stream to the client and the in-flight Accudo
...
pub struct DataStream<T> {
    ...
    data_client_config: AccudoDataClientConfig,
    ...
    // The data client through which to fetch data from the Accudo network
    accudo_data_client: T,
    ...
    stream_update_notifier: accudo_channel::Sender<(), StreamUpdateNotification>,
...
impl<T: AccudoDataClientInterface + Send + Clone + 'static> DataStream<T> {
    pub fn new(
        data_client_config: AccudoDataClientConfig,
    ...
    /// Notifies the Accudo data client of a bad client response
    ...
    fn check_subscription_stream_lag(
    ...
    ) -> Result<(), accudo_data_client::error::Error> {
        ...
        if let Some(mut subscription_stream_lag) = self.subscription_stream_lag.take() {
            ...
            {
                return Err(
                    accudo_data_client::error::Error::SubscriptionStreamIsLagging(format!(
    ...
    fn notify_new_data_request_error(
        ...
        error: accudo_data_client::error::Error,
    ) -> Result<(), Error> {
    ...
    fn handle_data_client_error(
        &mut self,
        data_client_request: &DataClientRequest,
        data_client_error: &accudo_data_client::error::Error,
    ) -> Result<(), Error> {
        // Log the error
        warn!(LogSchema::new(LogEntry::ReceivedDataResponse)
            .stream_id(self.data_stream_id)
...
fn create_missing_epoch_ending_ledger_infos_request(
    ...
    match response_payload {
        ...
        payload => Err(Error::AccudoDataClientResponseIsInvalid(format!(
...
fn create_missing_transactions_or_outputs_request(
    ...
    let num_received_data_items = match response_payload {
        ...
        payload => {
            return Err(Error::AccudoDataClientResponseIsInvalid(format!(
...
/// can be sent to the Accudo data client.
...
fn spawn_request_task<T: AccudoDataClientInterface + Send + Clone + 'static>(
    data_stream_id: DataStreamId,
    data_client_request: DataClientRequest,
    accudo_data_client: T,
    ...
    tokio::spawn(async move {
        ...
        let client_response = match data_client_request {
            DataClientRequest::EpochEndingLedgerInfos(request) => {
                get_epoch_ending_ledger_infos(accudo_data_client, request, request_timeout_ms).await
            },
            DataClientRequest::NewTransactionsWithProof(request) => {
                get_new_transactions_with_proof(accudo_data_client, request, request_timeout_ms)
                    .await
            },
            ...
            DataClientRequest::NumberOfStates(request) => {
                get_number_of_states(accudo_data_client, request, request_timeout_ms).await
            },
            DataClientRequest::StateValuesWithProof(request) => {
            ...
            DataClientRequest::TransactionOutputsWithProof(request) => {
                get_transaction_outputs_with_proof(accudo_data_client, request, request_timeout_ms)
                    .await
            },
            DataClientRequest::TransactionsWithProof(request) => {
                get_transactions_with_proof(accudo_data_client, request, request_timeout_ms).await
            },
            DataClientRequest::TransactionsOrOutputsWithProof(request) => {
...
async fn get_states_values_with_proof<T: AccudoDataClientInterface + Send + Clone + 'static>(
    accudo_data_client: T,
    request: StateValuesWithProofRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, accudo_data_client::error::Error> {
    let client_response = accudo_data_client.get_state_values_with_proof(
        request.version,
        request.start_index,
        request.end_index,
        request_timeout_ms,
...
async fn get_epoch_ending_ledger_infos<T: AccudoDataClientInterface + Send + Clone + 'static>(
    accudo_data_client: T,
    request: EpochEndingLedgerInfosRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, accudo_data_client::error::Error> {
    let client_response = accudo_data_client.get_epoch_ending_ledger_infos(
        request.start_epoch,
        request.end_epoch,
        request_timeout_ms,
    );
...
async fn get_new_transaction_outputs_with_proof<
    T: AccudoDataClientInterface + Send + Clone + 'static,
>(
    accudo_data_client: T,
    request: NewTransactionOutputsWithProofRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, accudo_data_client::error::Error> {
    let client_response = accudo_data_client.get_new_transaction_outputs_with_proof(
...
async fn get_new_transactions_with_proof<T: AccudoDataClientInterface + Send + Clone + 'static>(
    accudo_data_client: T,
    request: NewTransactionsWithProofRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, accudo_data_client::error::Error> {
    let client_response = accudo_data_client.get_new_transactions_with_proof(
        request.known_version,
        request.known_epoch,
...
>(
    ...
    let client_response = accudo_data_client.get_new_transactions_or_outputs_with_proof(
        request.known_version,
        request.known_epoch,
        request.include_events,
        request_timeout_ms,
...
async fn get_number_of_states<T: AccudoDataClientInterface + Send + Clone + 'static>(
    accudo_data_client: T,
    request: NumberOfStatesRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, accudo_data_client::error::Error> {
    let client_response =
        accudo_data_client.get_number_of_states(request.version, request_timeout_ms);
    client_response
        .await
        .map(|response| response.map(ResponsePayload::from))
}
...
>(
    ...
    let client_response = accudo_data_client.get_transaction_outputs_with_proof(
...
async fn get_transactions_with_proof<T: AccudoDataClientInterface + Send + Clone + 'static>(
    accudo_data_client: T,
    request: TransactionsWithProofRequest,
    request_timeout_ms: u64,
) -> Result<Response<ResponsePayload>, accudo_data_client::error::Error> {
    let client_response = accudo_data_client.get_transactions_with_proof(
        request.proof_version,
        request.start_version,
        request.end_version,
        request.include_events,
...
>(
    ...
    let client_response = accudo_data_client.get_transactions_or_outputs_with_proof(
        request.proof_version,
        request.start_version,
        request.end_version,
        request.include_events,
...
>(
    ...
    let client_response = accudo_data_client.subscribe_to_transactions_with_proof(
...
>(
    ...
    let client_response = accudo_data_client.subscribe_to_transaction_outputs_with_proof(
        subscription_request_metadata,
        request_timeout_ms,
    );
    client_response
...
>(
    ...
    let client_response = accudo_data_client.subscribe_to_transactions_or_outputs_with_proof(
        subscription_request_metadata,
        request.include_events,
        request_timeout_ms,
    );
...
mod test {
    use super::*;
    use crate::tests::utils::MockAccudoDataClient;
    use accudo_channels::message_queues::QueueStyle;
    ...
    async fn completed_request_notifies_streaming_service() {
        ...
        let data_client_config = AccudoDataClientConfig::default();
        let accudo_data_client =
            MockAccudoDataClient::new(data_client_config, true, false, true, true);
        ...
        let (stream_update_notifier, mut stream_update_listener) =
            accudo_channel::new(QueueStyle::LIFO, 1, None);
