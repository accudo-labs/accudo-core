...
use crate::{
    ...
    tests::utils::{
        ...
        initialize_logger, MockAccudoDataClient, NoopResponseCallback, MAX_ADVERTISED_EPOCH_END,
...
use accudo_config::config::{
    AccudoDataClientConfig, DataStreamingServiceConfig, DynamicPrefetchingConfig,
};
...
use accudo_id_generator::U64IdGenerator;
use accudo_infallible::Mutex;
use accudo_storage_service_types::responses::CompleteDataRange;
...
async fn test_stream_blocked() {
    ...
    let (mut data_stream, mut stream_listener) = create_state_value_stream(
        AccudoDataClientConfig::default(),
...
async fn test_stream_data_error() {
    ...
    let pending_response = PendingClientResponse::new_with_response(
        client_request.clone(),
        Err(accudo_data_client::error::Error::DataIsUnavailable(
            "Missing data!".into(),
        )),
    );
    insert_response_into_pending_queue(&mut data_stream, pending_response);
...
async fn test_continuous_stream_optimistic_fetch_timeout() {
    ...
    let data_client_config = AccudoDataClientConfig {
...
async fn advertise_new_data_and_verify_requests(
    data_stream: &mut DataStream<MockAccudoDataClient>,
...
fn create_state_value_stream(
    ...
    version: Version,
) -> (DataStream<MockAccudoDataClient>, DataStreamListener) {
...
fn create_continuous_transaction_output_stream(
    ...
    DataStream<MockAccudoDataClient>,
    DataStreamListener,
    TimeService,
) {
...
fn create_stream_update_notifier() -> accudo_channel::Sender<(), StreamUpdateNotification> {
    let (stream_update_notifier, _) = accudo_channel::new(QueueStyle::LIFO, 1, None);
...
fn set_failure_response_in_queue(data_stream: &mut DataStream<MockAccudoDataClient>, index: usize) {
    set_response_in_queue(
        data_stream,
        index,
        accudo_data_client::error::Error::UnexpectedErrorEncountered("Oops!".into()),
    );
}

/// Sets the client response at the index in the pending
/// queue to contain a pending response.
fn set_pending_response_in_queue(data_stream: &mut DataStream<MockAccudoDataClient>, index: usize) {
...
fn set_timeout_response_in_queue(data_stream: &mut DataStream<MockAccudoDataClient>, index: usize) {
    set_response_in_queue(
        data_stream,
        index,
        accudo_data_client::error::Error::TimeoutWaitingForResponse("Timed out!".into()),
    );
}
...
fn set_response_in_queue(
    data_stream: &mut DataStream<MockAccudoDataClient>,
    index: usize,
    error_response: accudo_data_client::error::Error,
) {
    // Get the pending response at the specified index
    let (sent_requests, _) = data_stream.get_sent_requests_and_notifications();
    let pending_response = sent_requests.as_mut().unwrap().get_mut(index).unwrap();
...
async fn wait_for_data_client_to_respond(
    ...
    loop {
        if let Some(client_response) = &pending_response.lock().client_response {
            if !matches!(
                client_response,
                Err(accudo_data_client::error::Error::TimeoutWaitingForResponse(
                    _
                ))
            ) {
                return;
...
fn set_transaction_response_at_queue_head(data_stream: &mut DataStream<MockAccudoDataClient>) {
    let (sent_requests, _) = data_stream.get_sent_requests_and_notifications();
