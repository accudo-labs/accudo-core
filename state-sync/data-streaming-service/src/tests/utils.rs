...
use accudo_config::config::AccudoDataClientConfig;
use accudo_crypto::{ed25519::Ed25519PrivateKey, HashValue, PrivateKey, SigningKey, Uniform};
use accudo_data_client::{
    global_summary::{AdvertisedData, GlobalDataSummary, OptimalChunkSizes},
    interface::{
        AccudoDataClientInterface, Response, ResponseCallback, ResponseContext, ResponseError,
...
/// A simple mock of the Accudo Data Client
#[derive(Clone, Debug)]
pub struct MockAccudoDataClient {
    pub accudo_data_client_config: AccudoDataClientConfig,
...
impl MockAccudoDataClient {
    pub fn new(
        accudo_data_client_config: AccudoDataClientConfig,
    ...
    fn clone_without_timeout_verification(&self) -> Self {
        ...
        let mut accudo_data_client = self.clone();
        accudo_data_client.skip_timeout_verification = true;
    ...
    async fn emulate_network_request_timeout(&self) -> accudo_data_client::error::Error {
        // Sleep for a while
        tokio::time::sleep(Duration::from_secs(MAX_NOTIFICATION_TIMEOUT_SECS)).await;

        // Return a timeout error
        accudo_data_client::error::Error::TimeoutWaitingForResponse("RPC timed out!".into())
    }
    ...
    fn verify_request_timeout_value(
        ...
        if !self.skip_timeout_verification {
            ...
            let expected_timeout = if is_optimistic_fetch_request {
                self.accudo_data_client_config.optimistic_fetch_timeout_ms
            } else if is_subscription_request {
                self.accudo_data_client_config
                    .subscription_response_timeout_ms
            } else {
                let min_timeout = self.accudo_data_client_config.response_timeout_ms;
                let max_timeout = self.accudo_data_client_config.max_response_timeout_ms;
...
#[async_trait]
impl AccudoDataClientInterface for MockAccudoDataClient {
    ...
    async fn get_state_values_with_proof(
        ...
        end_index: u64,
        request_timeout_ms: u64,
    ) -> Result<Response<StateValueChunkWithProof>, accudo_data_client::error::Error> {
    ...
    async fn get_epoch_ending_ledger_infos(
        ...
        end_epoch: Epoch,
        request_timeout_ms: u64,
    ) -> Result<Response<Vec<LedgerInfoWithSignatures>>, accudo_data_client::error::Error> {
        // Verify the request timeout
    ...
    async fn get_new_transaction_outputs_with_proof(
        ...
        accudo_data_client::error::Error,
    > {
        // Verify the request timeout
        let data_request =
        ...
        if self.data_beyond_highest_advertised && known_version < MAX_REAL_TRANSACTION_OUTPUT {
            ...
            let accudo_data_client = self.clone_without_timeout_verification();

            // Determine the target ledger info
            let target_ledger_info =
                determine_target_ledger_info(known_epoch, request_timeout_ms, &accudo_data_client)
                    .await;

            // Fetch the new transaction outputs
            let outputs_with_proof = accudo_data_client
                .get_transaction_outputs_with_proof(
                    known_version + 1,
                    known_version + 1,
                    known_version + 1,
                    self.accudo_data_client_config.response_timeout_ms,
                )
                .await
                .unwrap()
                .payload;
    ...
    > {
        ...
        if self.data_beyond_highest_advertised && known_version < MAX_REAL_TRANSACTION {
            ...
            let transactions_with_proof = accudo_data_client
    ...
    async fn get_new_transactions_or_outputs_with_proof(
        ...
        include_events: bool,
        request_timeout_ms: u64,
    ) -> accudo_data_client::error::Result<
    ...
    > {
        ...
        let response = if return_transactions_instead_of_outputs() {
            let (transactions, ledger_info) = accudo_data_client
            ...
            let (outputs, ledger_info) = accudo_data_client
    ...
    async fn get_number_of_states(
        ...
        version: Version,
        request_timeout_ms: u64,
    ) -> Result<Response<u64>, accudo_data_client::error::Error> {
        // Verify the request timeout
    ...
    async fn get_transaction_outputs_with_proof(
        ...
        end_version: Version,
        request_timeout_ms: u64,
    ) -> Result<Response<TransactionOutputListWithProofV2>, accudo_data_client::error::Error> {
        // Verify the request timeout
        let data_request =
    ...
    async fn get_transactions_with_proof(
        ...
        include_events: bool,
        request_timeout_ms: u64,
    ) -> Result<Response<TransactionListWithProofV2>, accudo_data_client::error::Error> {
        // Verify the request timeout
    ...
    async fn get_transactions_or_outputs_with_proof(
        ...
        include_events: bool,
        request_timeout_ms: u64,
    ) -> accudo_data_client::error::Result<Response<TransactionOrOutputListWithProofV2>> {
        // Verify the request timeout
        let data_request =
...
async fn determine_target_ledger_info(
    known_epoch: Epoch,
    request_timeout_ms: u64,
    accudo_data_client: &MockAccudoDataClient,
) -> LedgerInfoWithSignatures {
...
/// Initializes the Accudo logger for tests
pub fn initialize_logger() {
    accudo_logger::Logger::builder()
        .is_async(false)
        .level(Level::Info)
        .build();
}
