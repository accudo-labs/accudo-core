...
pub enum Error {
    ...
    #[error("Error returned by the accudo data client: {0}")]
    AccudoDataClientError(String),
    #[error("The response from the accudo data client is invalid! Error: {0}")]
    AccudoDataClientResponseIsInvalid(String),
...
impl Error {
    ...
    pub fn get_label(&self) -> &'static str {
        match self {
            ...
            Self::AccudoDataClientError(_) => "accudo_data_client_error",
            Self::AccudoDataClientResponseIsInvalid(_) => "accudo_data_client_response_is_invalid",
...
impl From<accudo_data_client::error::Error> for Error {
    fn from(error: accudo_data_client::error::Error) -> Self {
        Error::AccudoDataClientError(error.to_string())
    }
}
