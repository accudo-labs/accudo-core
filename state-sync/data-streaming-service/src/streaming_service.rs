...
use accudo_data_client::{
    ...
    interface::AccudoDataClientInterface,
};
...
pub struct DataStreamingService<T> {
    ...
    stream_update_listener: accudo_channel::Receiver<(), StreamUpdateNotification>,
...
impl<T: AccudoDataClientInterface + Send + Clone + 'static> DataStreamingService<T> {
    pub fn new(
        data_client_config: AccudoDataClientConfig,
        streaming_service_config: DataStreamingServiceConfig,
        accudo_data_client: T,
        ...
        let (stream_update_notifier, stream_update_listener) =
            accudo_channel::new(QueueStyle::LIFO, STREAM_PROGRESS_UPDATE_CHANNEL_SIZE, None);
...
fn spawn_global_data_summary_refresher<T: AccudoDataClientInterface + Send + Clone + 'static>(
...
fn refresh_global_data_summary<T: AccudoDataClientInterface + Send + Clone + 'static>(
    accudo_data_client: T,
    cached_global_data_summary: Arc<ArcSwap<GlobalDataSummary>>,
) {
    // Fetch the global data summary and update the cache
    match fetch_global_data_summary(accudo_data_client) {
...
fn fetch_global_data_summary<T: AccudoDataClientInterface + Send + Clone + 'static>(
    accudo_data_client: T,
) -> Result<GlobalDataSummary, Error> {
    // Fetch the global data summary from the data client
    let global_data_summary = accudo_data_client.get_global_data_summary();
...
fn verify_optimal_chunk_sizes(optimal_chunk_sizes: &OptimalChunkSizes) -> Result<(), Error> {
    ...
    {
        Err(Error::AccudoDataClientResponseIsInvalid(format!(
...
mod streaming_service_tests {
    ...
    use accudo_channels::{accudo_channel, message_queues::QueueStyle};
    use accudo_config::config::DataStreamingServiceConfig;
    use futures::{
