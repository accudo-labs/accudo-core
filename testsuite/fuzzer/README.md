...
## Introduction
This directory contains tools and scripts essential for fuzz testing on Accudo Core. Fuzz targets run continuously on daily versions of `main` on Google's OSS-Fuzz infrastructure.
...
To set up a fuzz harness in Accudo-core using `cargo-fuzz`:
...
### OSS-Fuzz Corpus
Create a `.zip` archive containing your fuzzer's corpus and name it according to the following format: `[fuzzer_name]_seed_corpus.zip` (e.g., `move_accudovm_publish_and_run_seed_corpus.zip`). Follow these steps for hosting and integrating the archive:
...
`./fuzz.sh block-builder generate_runnable_state_from_project data/0x1/string/generic fuzz/corpus/move_accudovm_publish_and_run`
...
#### Bulk Build
Use `./fuzz.sh block-builder generate_runnable_states_recursive data/0x1/ fuzz/corpus/move_accudovm_publish_and_run` to compile all the modules under a specific directory.
...
1. `gcloud auth login`
2. `gcloud storage cp gs://accudo-core-corpora/move_accudovm_publish_and_run_seed_corpus.zip move_accudovm_publish_and_run_seed_corpus.zip`
3. `unzip move_accudovm_publish_and_run_seed_corpus.zip -d move_accudovm_publish_and_run_seed_corpus`
4. `./fuzz.sh block-builder generate_runnable_states_recursive data/0x1/ move_accudovm_publish_and_run_seed_corpus`
5. Normally we would run cmin but we assume that manually created inputs are fine and serve a specific purse.
6. `zip -r move_accudovm_publish_and_run_seed_corpus.zip move_accudovm_publish_and_run_seed_corpus`
7. `gsutil storage cp move_accudovm_publish_and_run_seed_corpus.zip gs://accudo-core-corpora/move_accudovm_publish_and_run_seed_corpus.zip`
8. We need to restore ACL (public URL remain the same): `gsutil storage acl ch -u AllUsers:R gs://accudo-core-corpora/move_accudovm_publish_and_run_seed_corpus.zip`
...
- [Native Functions](https://accudo.dev/en/build/smart-contracts/move-reference?branch=mainnet&page=move-stdlib%2Fdoc%2Fmem.md)
