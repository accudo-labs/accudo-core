...
use accudo_language_e2e_tests::{account::Account, executor::FakeExecutor};
use accudo_transaction_simulation::GENESIS_CHANGE_SET_HEAD;
use accudo_types::{
    chain_id::ChainId,
...
use accudo_vm_environment::{prod_configs, prod_configs::LATEST_GAS_FEATURE_VERSION};
    ...
    if e.status_type() == StatusType::InvariantViolation {
        ...
        if !is_known_false_positive && e.status_type() == StatusType::InvariantViolation {
            panic!(
                ...
                e.major_status(),
                "./fuzz.sh run move_accudovm_publish_and_run <ARTIFACT>"
            );
        }
    }
}
...
fn run_case(mut input: RunnableState) -> Result<(), Corpus> {
    ...
    let verifier_config =
        prod_configs::accudo_prod_verifier_config(LATEST_GAS_FEATURE_VERSION, &Features::default());
    let deserializer_config = DeserializerConfig::new(BYTECODE_VERSION, 255);

    for m in input.dep_modules.iter_mut() {
        // m.metadata = vec![]; // we could optimize metadata to only contain accudo metadata
    ...
    AccudoVM::set_concurrency_level_once(FUZZER_CONCURRENCY_LEVEL);
