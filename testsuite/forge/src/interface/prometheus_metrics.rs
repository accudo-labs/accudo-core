...
pub async fn fetch_validator_error_metrics(
    ...
    let error_query = r#"accudo_error_log_count{role=~"validator"}"#;
...
pub async fn fetch_latency_breakdown(
    ...
    let consensus_proposal_to_ordered_query = r#"quantile(0.67, rate(accudo_consensus_block_tracing_sum{role=~"validator", stage="ordered"}[1m]) / rate(accudo_consensus_block_tracing_count{role=~"validator", stage="ordered"}[1m]))"#;
    let consensus_proposal_to_commit_query = r#"quantile(0.67, rate(accudo_consensus_block_tracing_sum{role=~"validator", stage="committed"}[1m]) / rate(accudo_consensus_block_tracing_count{role=~"validator", stage="committed"}[1m]))"#;

    let mempool_to_block_creation_query = r#"sum(
        rate(accudo_core_mempool_txn_commit_latency_sum{
            role=~"validator",
            stage="commit_accepted_block"
        }[1m])
    ) / sum(
        rate(accudo_core_mempool_txn_commit_latency_count{
            role=~"validator",
            stage="commit_accepted_block"
        }[1m])
    )"#;
    ...
    if swarm.has_indexer() {
        ...
        // These are processor latencies for both original core processors and those written with the processor SDK: https://github.com/accudo-labs/accudo-indexer-processor-sdk
        ...
        let indexer_sdk_processor_latency_query =
            "max(accudo_procsdk_step__processed_transaction_latency_secs{})";
