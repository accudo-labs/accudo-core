...
pub trait AccudoTest: Test {
    /// Executes the test against the given context.
    async fn run<'t>(&self, ctx: &mut AccudoContext<'t>) -> Result<()>;
}

pub struct AccudoContext<'t> {
    core: CoreContext,
    public_info: AccudoPublicInfo,
    pub report: &'t mut TestReport,
}

impl<'t> AccudoContext<'t> {
    pub fn new(
        core: CoreContext,
        public_info: AccudoPublicInfo,
    ...
    pub fn transaction_factory(&self) -> TransactionFactory {
        let unit_price = std::cmp::max(accudo_global_constants::GAS_UNIT_PRICE, 1);
        TransactionFactory::new(self.chain_id()).with_gas_unit_price(unit_price)
    }

    pub fn accudo_transaction_factory(&self) -> TransactionFactory {
...
impl AccudoPublicInfo {
        ...
        let create_account_txn =
            self.root_account
                .sign_with_transaction_builder(self.transaction_factory().payload(
                    accudo_stdlib::accudo_account_create_account(auth_key.account_address()),
                ));
        self.rest_client
    ...
    pub async fn mint(&mut self, addr: AccountAddress, amount: u64) -> Result<()> {
        let mint_txn = self.root_account.sign_with_transaction_builder(
            self.transaction_factory()
                .payload(accudo_stdlib::accudo_coin_mint(addr, amount)),
        );
    ...
    pub async fn transfer_non_blocking(
        ...
        let tx = from_account.sign_with_transaction_builder(self.transaction_factory().payload(
            accudo_stdlib::accudo_coin_transfer(to_account.address(), amount),
        ));
    ...
    pub async fn get_approved_execution_hash_at_accudo_governance(
        &self,
        proposal_id: u64,
    ) -> Vec<u8> {
        let approved_execution_hashes = self
            ...
            .get_account_resource_bcs::<SimpleMap<u64, Vec<u8>>>(
                CORE_CODE_ADDRESS,
                "0x1::accudo_governance::ApprovedExecutionHashes",
            )
            .await;
    ...
    pub async fn get_balance(&self, address: AccountAddress) -> u64 {
        self.rest_client
            .get_account_balance(address, "0x1::accudo_coin::AccudoCoin")
            .await
            .unwrap()
            .into_inner()
    }
...
pub async fn reconfig(
    ...
    let txns = {
        vec![root_account.sign_with_transaction_builder(
            transaction_factory
                .clone()
                .payload(accudo_stdlib::accudo_governance_force_end_epoch_test_only()),
        )]
    };
