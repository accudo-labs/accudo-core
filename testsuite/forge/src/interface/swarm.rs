...
pub trait Swarm: Sync + Send {
    ...
    fn accudo_public_info(&self) -> AccudoPublicInfo {
        self.chain_info().into_accudo_public_info()
    }

    fn chain_info_for_node(&mut self, idx: usize) -> ChainInfo;

    fn accudo_public_info_for_node(&mut self, idx: usize) -> AccudoPublicInfo {
        self.chain_info_for_node(idx).into_accudo_public_info()
    }
