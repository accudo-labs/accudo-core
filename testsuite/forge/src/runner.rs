...
#[derive(Debug, Parser)]
#[clap(about = "Forged in Fire", styles = accudo_cli_common::accudo_cli_style())]
pub struct Options {
...
impl Test for AnyTestRef<'_> {
    ...
    fn should_fail(&self) -> ShouldFail {
        match self {
            AnyTestRef::Accudo(t) => t.should_fail(),
...
impl<'cfg, F: Factory> Forge<'cfg, F> {
    ...
    pub fn run(&self) -> Result<TestSummary> {
        ...
        if test_count > 0 {
            ...
            // Run AccudoTests
            for test in self.filter_tests(&self.tests.accudo_tests) {
                let mut accudo_ctx = AccudoContext::new(
                    CoreContext::from_rng(&mut rng),
                    swarm.chain_info().into_accudo_public_info(),
                    &mut report,
                );
                let result = process_test_result(runtime.block_on(test.run(&mut accudo_ctx)));
