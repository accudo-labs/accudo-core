...
pub struct ForgeConfig {
    ...
    pub accudo_tests: Vec<Box<dyn AccudoTest>>,
...
impl ForgeConfig {
    ...
    pub fn add_accudo_test<T: AccudoTest + 'static>(mut self, accudo_test: T) -> Self {
        self.accudo_tests.push(Box::new(accudo_test));
        self
    }
    ...
    pub fn with_accudo_tests(mut self, accudo_tests: Vec<Box<dyn AccudoTest>>) -> Self {
        self.accudo_tests = accudo_tests;
        self
    }
    ...
    pub fn build_node_helm_config_fn(&self, retain_debug_logs: bool) -> Option<NodeConfigFn> {
        ...
        // Override specific helm values. See reference: terraform/helm/accudo-node/values.yaml
        Some(Arc::new(move |helm_values: &mut serde_yaml::Value| {
            ...
            if retain_debug_logs {
                helm_values["validator"]["podAnnotations"]["accudo.dev/min-log-level-to-retain"] =
                    serde_yaml::Value::String("debug".to_owned());
                helm_values["fullnode"]["podAnnotations"]["accudo.dev/min-log-level-to-retain"] =
                    serde_yaml::Value::String("debug".to_owned());
    ...
    pub fn number_of_tests(&self) -> usize {
        self.admin_tests.len() + self.network_tests.len() + self.accudo_tests.len()
    }
    ...
    pub fn all_tests(&self) -> Vec<Box<AnyTestRef<'_>>> {
        self.admin_tests
            ...
            .chain(
                self.accudo_tests
                    .iter()
                    .map(|t| Box::new(AnyTestRef::Accudo(t.as_ref()))),
            )
            .collect()
    }
}
