...
/// at _forge_ compile time, decide what kind of build we will use for `accudo-node`
pub fn use_release() -> bool {
    option_env!("LOCAL_SWARM_NODE_RELEASE").is_some()
}

/// at _forge_ compile time, decide to build `accudo-node` only for consensus perf tests
pub fn build_consensus_only_node() -> bool {
    option_env!("CONSENSUS_ONLY_PERF_TEST").is_some()
}

/// at forge _run_ time, compile `accudo-node` without indexer
pub fn build_accudo_node_without_indexer() -> bool {
...
/// Get the accudo node binary from the current working directory
pub fn get_accudo_node_binary_from_worktree() -> Result<(String, PathBuf)> {
    ...
    let bin_path = cargo_build_accudo_node(&metadata.workspace_root, &metadata.target_directory)?;

    Ok((revision, bin_path))
}

/// This function will attempt to build the accudo-node binary at an arbitrary revision.
/// Using the `target/forge` as a working directory it will do the following:
///     1. Look for a binary named `accudo-node--<revision>`, if it already exists return it
...
///        binary and then move it to `target/forge/accudo-node--<revision>`
pub fn get_accudo_node_binary_at_revision(revision: &str) -> Result<(String, PathBuf)> {
    let metadata = metadata()?;
    ...
    let accudo_node_bin = forge_directory.join(format!(
        "accudo-node--{}{}",
        revision,
        env::consts::EXE_SUFFIX
    ));

    if accudo_node_bin.exists() {
        return Ok((revision, accudo_node_bin));
    }
    ...
    fs::rename(
        cargo_build_accudo_node(&checkout_dir, &forge_target_directory)?,
        &accudo_node_bin,
    )?;

    let _ = fs::remove_dir_all(&checkout_dir);

    Ok((revision, accudo_node_bin))
}
...
/// accudo-labs/accudo-core repository, falling back to "origin" if unable to locate the remote
pub fn git_get_upstream_remote() -> Result<String> {
    let output = Command::new("sh")
        .arg("-c")
        .arg(
            "git remote -v | grep \"https://github.com/accudo-labs/accudo-core.* (fetch)\" | cut -f1",
        )
        .output()
...
pub fn cargo_build_common_args() -> Vec<&'static str> {
    let mut args = if build_accudo_node_without_indexer() {
...
fn cargo_build_accudo_node<D, T>(directory: D, target_directory: T) -> Result<PathBuf>
where
    D: AsRef<Path>,
    T: AsRef<Path>,
{
    ...
    // build the accudo-node package directly to avoid feature unification issues
    args.push("--package=accudo-node");
    ...
    let output = Command::new("cargo")
        ...
        .args(&args)
        .output()
        .context("Failed to build accudo-node")?;
    ...
    if output.status.success() {
        ...
        if !bin_path.exists() {
            bail!(
                "Can't find binary accudo-node at expected path {:?}",
                bin_path
            );
        }
        ...
        bail!(
            "Failed to build accudo-node: 'cd {} && CARGO_TARGET_DIR={} cargo build --bin=accudo-node",
            directory.display(),
