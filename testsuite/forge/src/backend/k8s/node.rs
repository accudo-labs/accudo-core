...
const ACCUDO_DATA_DIR: &str = "/opt/accudo/data";
...
impl Node for K8sNode {
    ...
    async fn clear_storage(&self) -> Result<()> {
        ...
        let ledger_db_path = format!("{}/db/{}", ACCUDO_DATA_DIR, LEDGER_DB_NAME);
        let state_db_path = format!("{}/db/{}", ACCUDO_DATA_DIR, STATE_MERKLE_DB_NAME);
        let state_sync_db_path = format!("{}/db/{}", ACCUDO_DATA_DIR, STATE_SYNC_DB_NAME);
