...
use crate::{
    ...
    NodeConfigFn, ReadWrite, Result, ACCUDO_NODE_HELM_RELEASE_NAME, DEFAULT_ROOT_KEY,
...
async fn wait_node_haproxy(
    ...
    accudo_retrier::retry_async(k8s_wait_nodes_strategy(), || {
        ...
        Box::pin(async move {
            for i in 0..num_haproxy {
                let haproxy_deployment_name =
                    format!("{}-{}-haproxy", ACCUDO_NODE_HELM_RELEASE_NAME, i);
...
/// Deletes a collection of resources in k8s as part of accudo-node
...
pub(crate) async fn delete_k8s_resources(client: K8sClient, kube_namespace: &str) -> Result<()> {
    ...
    let accudo_node_helm_selector = "app.kubernetes.io/part-of=accudo-node";
    let testnet_addons_helm_selector = "app.kubernetes.io/part-of=testnet-addons";
    let genesis_helm_selector = "app.kubernetes.io/part-of=accudo-genesis";
    ...
    // cross this with all the compute resources created by accudo-node helm chart
...
pub async fn uninstall_testnet_resources(kube_namespace: String) -> Result<()> {
    ...
    info!(
        "accudo-node resources for Forge removed in namespace: {}",
        kube_namespace
    );
...
fn get_node_default_helm_path() -> String {
    match ForgeRunnerMode::try_from_env().unwrap_or(ForgeRunnerMode::K8s) {
        ForgeRunnerMode::Local => {
            "testsuite/forge/src/backend/k8s/helm-values/accudo-node-default-values.yaml"
        },
        ForgeRunnerMode::K8s => "/accudo/terraform/accudo-node-default-values.yaml",
    }
    .to_string()
}
...
/// Installs a testnet in a k8s namespace by first running genesis, and the installing the accudo-nodes via helm
...
pub async fn install_testnet_resources(
    ...
    let mut accudo_node_helm_values =
        get_default_helm_release_values_from_cluster(ACCUDO_NODE_HELM_RELEASE_NAME)
    ...
    let accudo_node_helm_values_override = construct_node_helm_values_from_input(
        node_helm_config_fn,
    ...
    info!(
        "accudo_node_helm_values: {}",
        serde_yaml::to_string(&accudo_node_helm_values).unwrap()
    );
    info!(
    ...
    info!(
        "accudo_node_helm_values after override: {}",
        serde_yaml::to_string(&accudo_node_helm_values).unwrap(),
    );

    // disable uploading genesis to blob storage since indexer requires it in the cluster
    if enable_indexer {
        accudo_node_helm_values["genesis_blob_upload_url"] = "".into();
    }
    ...
    let config: serde_json::Value = serde_json::from_value(serde_json::json!({
        ...
        "testnet-values": accudo_node_helm_values,
