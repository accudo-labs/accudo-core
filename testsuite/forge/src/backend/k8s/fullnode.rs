...
// these are constants given by the accudo-node helm chart
// see terraform/helm/accudo-node/templates/validator.yaml
...
const GENESIS_CONFIG_VOLUME_PATH: &str = "/opt/accudo/genesis";
const GENESIS_CONFIG_WRITABLE_VOLUME_NAME: &str = "writable-genesis";

// the path where the config file is mounted in the fullnode
const ACCUDO_CONFIG_VOLUME_NAME: &str = "accudo-config";
const ACCUDO_CONFIG_VOLUME_PATH: &str = "/opt/accudo/etc";

// the path where the data volume is mounted in the fullnode
const ACCUDO_DATA_VOLUME_NAME: &str = "accudo-data";
const ACCUDO_DATA_VOLUME_PATH: &str = "/opt/accudo/data";
...
fn create_fullnode_persistent_volume_claim(
    ...
    Ok(PersistentVolumeClaim {
        metadata: ObjectMeta {
            name: Some(ACCUDO_DATA_VOLUME_NAME.to_string()),
            ..ObjectMeta::default()
        },
...
fn create_fullnode_container(
    ...
    Ok(Container {
        ...
        args: Some(vec![
            "/usr/local/bin/accudo-node".to_string(),
            "-f".to_string(),
            format!("/opt/accudo/etc/{}", FULLNODE_CONFIG_MAP_KEY),
        ]),
        volume_mounts: Some(vec![
            VolumeMount {
                mount_path: ACCUDO_CONFIG_VOLUME_PATH.to_string(),
                name: ACCUDO_CONFIG_VOLUME_NAME.to_string(),
                ..VolumeMount::default()
            },
            VolumeMount {
                mount_path: ACCUDO_DATA_VOLUME_PATH.to_string(),
                name: ACCUDO_DATA_VOLUME_NAME.to_string(),
                ..VolumeMount::default()
            },
            VolumeMount {
...
pub fn get_default_pfn_node_config() -> NodeConfig {
    ...
    NodeConfig {
        base: BaseConfig {
            ...
            data_dir: PathBuf::from(ACCUDO_DATA_VOLUME_PATH),
...
mod tests {
    ...
    use accudo_config::config::Identity;
    use accudo_sdk::crypto::{x25519::PrivateKey, Uniform};
    use k8s_openapi::apimachinery::pkg::api::resource::Quantity;

    /// Get a dummy validator persistent volume claim that looks like one created by terraform/helm/accudo-node/templates/validator.yaml
    fn get_dummy_validator_persistent_volume_claim() -> PersistentVolumeClaim {
        PersistentVolumeClaim {
            metadata: ObjectMeta {
                name: Some("accudo-node-0-validator-e42069".to_string()),
                ..ObjectMeta::default()
            },
            spec: Some(PersistentVolumeClaimSpec {
                access_modes: Some(vec!["ReadWriteOnce".to_string()]),
    ...
    /// Get a dummy validator stateful set that looks like one created by terraform/helm/accudo-node/templates/validator.yaml
    fn get_dummy_validator_stateful_set() -> StatefulSet {
        let labels: BTreeMap<String, String> = [
            ...
            (
                ...
                "accudo-node-0-validator-0".to_string(),
            ),
            (
                "app.kubernetes.io/part-of".to_string(),
                "forge-pfn".to_string(),
        ...
        StatefulSet {
            metadata: ObjectMeta {
                name: Some("accudo-node-0-validator".to_string()),
                ...
                template: PodTemplateSpec {
                    ...
                    spec: Some(PodSpec {
                        containers: vec![Container {
                            ...
                            image: Some(
                                "banana.fruit.accudo/potato/validator:banana_image_tag".to_string(),
                            ),
                            command: Some(vec![
                                "/usr/local/bin/accudo-node".to_string(),
                                "-f".to_string(),
                                "/opt/accudo/etc/validator.yaml".to_string(),
                            ]),
                            volume_mounts: Some(vec![
                                VolumeMount {
                                    mount_path: ACCUDO_CONFIG_VOLUME_PATH.to_string(),
    ...
    async fn test_create_node_config_map() {
        let config_map_name = "accudo-node-0-validator-0-config".to_string();
    ...
    fn test_create_fullnode_stateful_set_and_service_connected() {
        ...
        let fullnode_genesis_secret_name = format!("accudo-node-0-genesis-e{}", era);
