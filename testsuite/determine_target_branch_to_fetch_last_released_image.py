...
def get_all_release_branches():
    """Get all accudo-release-vX.Y branches"""
    pattern = "accudo-release-v*"
    regex = r"refs/heads/(accudo-release-v\d+\.\d+)$"
...
def get_all_release_branches_with_times():
    """Get all accudo-release-vX.Y branches with their creation times"""
...
def get_latest_branch_for_previous_major(major):
    """Get the latest accudo-release-v(previous_major).Y branch"""
    prev_major = int(major) - 1
    pattern = f"accudo-release-v{prev_major}.*"
    regex = rf"refs/heads/(accudo-release-v{prev_major}\.\d+)$"
...
def determine_target_branch(base_branch):
    ...
    match = re.match(r"^accudo-release-v(\d+)\.(\d+)", base_branch)
    if match:
        major, minor = match.groups()
        if int(minor) == 0:
            return get_latest_branch_for_previous_major(major)
        else:
            return f"accudo-release-v{major}.{int(minor) - 1}"
