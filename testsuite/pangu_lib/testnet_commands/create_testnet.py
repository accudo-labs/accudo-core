...
async def create_node_stateful_sets(
    ...
    volume_mount4: client.V1VolumeMount = client.V1VolumeMount(
        name=util.ACCUDO_DATA_NAME, mount_path=util.ACCUDO_DATA_DIR
    )
    ...
    spec_container: client.V1Container = client.V1Container(
        ...
        image=image,
        command=[
            "/usr/local/bin/accudo-node",
            "-f",
            f"/opt/accudo/etc/{type.value}.yaml",
        ],
        volume_mounts=[
            volume_mount1,
            volume_mount2,
