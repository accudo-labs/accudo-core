...
@click.option(
    "--framework-path",
    required=True,
    help="Pass in the path to the compiled move framework (head.mrb, or framework.mrb) file. To compile it, run: $ cargo run --locked --package accudo-framework -- release",
)
@click.option(
    "--num-of-validators",
    default=10,
...
@click.option(
    "--accudo-cli-path",
    default="accudo",
    help="Pass the path to the accudo CLI if it is not in your $PATH var.",
)
@click.option(
    "--dry-run",
...
def create(
    ...
    Args:
        ...
        accudo_cli_path (str): path to accudo cli
