...
class testnet_tests_create_testnet(unittest.TestCase):
    def test_create_testnet(self) -> None:
        ...
        accudo_cli_path: str = "test_accudo_cli"
        dry_run: bool = False
        ...
        expected_commands = [
            ...
            FakeCommand(
                f"{accudo_cli_path} genesis generate-genesis --local-repository-dir {workspace}/{util.PANGU_WORKSPACE_NAME}/{namespace} --output-dir {workspace}/{util.PANGU_WORKSPACE_NAME}/{namespace}",
                RunResult(0, b"output"),
            ),
        ]
        ...
        create_testnet_main(
            CreateArgs(
                ...
                accudo_cli_path=accudo_cli_path,
                dry_run=dry_run,
                name=namespace,
            ),
    ...
    def test_generate_genesis(self) -> None:
        ...
        accudo_cli_path: str = "test_accudo"
        ...
        expected_commands = [
            FakeCommand(
                f"{accudo_cli_path} genesis generate-keys --output-dir {user_dir}",
                RunResult(0, b"output"),
            ),
            FakeCommand(
                f"{accudo_cli_path} genesis set-validator-configuration --owner-public-identity-file {workspace}/blueprint1-node-1/public-keys.yaml --local-repository-dir {workspace} --username {username} --validator-host {validator_host} --full-node-host {fullnode_host} --stake-amount {cur_stake_amount}",
                RunResult(0, b"output"),
            ),
            FakeCommand(
            ...
            FakeCommand(
                f"{accudo_cli_path} genesis generate-genesis --local-repository-dir {workspace} --output-dir {workspace}",
                RunResult(0, b"output"),
            ),
        ]
