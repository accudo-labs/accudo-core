    ...
    def testFindRecentImage(self) -> None:
        shell = SpyShell(
            [
                ...
                FakeCommand(
                    "aws ecr describe-images --repository-name accudo/validator-testing --image-ids imageTag=potato",
                    RunResult(1, b""),
                ),
                FakeCommand("git rev-parse HEAD~1", RunResult(0, b"lychee\n")),
                FakeCommand(
                    "aws ecr describe-images --repository-name accudo/validator-testing --image-ids imageTag=lychee",
                    RunResult(0, b""),
                ),
            ]
        )
    ...
    def testFindRecentFailpointsImage(self) -> None:
        shell = SpyShell(
            [
                ...
                FakeCommand(
                    "aws ecr describe-images --repository-name accudo/validator-testing --image-ids imageTag=failpoints_tomato",
                    RunResult(0, b""),
                ),
            ]
        )
    ...
    def testFindRecentPerformanceImage(self) -> None:
        shell = SpyShell(
            [
                FakeCommand("git rev-parse HEAD~0", RunResult(0, b"potato\n")),
                FakeCommand(
                    "aws ecr describe-images --repository-name accudo/validator-testing --image-ids imageTag=performance_potato",
                    RunResult(0, b""),
                ),
            ]
        )
    ...
    def testDidntFindRecentImage(self) -> None:
        shell = SpyShell(
            [
                ...
                FakeCommand(
                    "aws ecr describe-images --repository-name accudo/validator-testing --image-ids imageTag=crab",
                    RunResult(1, b""),
                ),
            ]
        )
        ...
        with self.assertRaises(Exception):
            list(
                find_recent_images(
                    shell, git, 1, "accudo/validator-testing", commit_threshold=1
                )
            )
    ...
    def testFindRecentFewImages(
        ...
        shell = SpyShell(
            [
                FakeCommand("git rev-parse HEAD~0", RunResult(0, b"crab\n")),
                FakeCommand(
                    "aws ecr describe-images --repository-name accudo/validator --image-ids imageTag=crab",
                    RunResult(0, b""),
                ),
                FakeCommand("git rev-parse HEAD~1", RunResult(0, b"shrimp\n")),
                FakeCommand(
                    "aws ecr describe-images --repository-name accudo/validator --image-ids imageTag=shrimp",
                    RunResult(0, b""),
                ),
            ]
        )
...
class ForgeFormattingTests(unittest.TestCase, AssertFixtureMixin):
    ...
    def testDashboardLinkAutoRefresh(self) -> None:
        self.assertFixture(
            get_dashboard_link(
                "forge-pr-2983",
                # Chain names don't use the "accudo-" prefix.
                "forge-big-1",
                True,
            ),
        ...
        shell = SpyShell(
            [
                ...
                FakeCommand(
                    ...
                    RunResult(0, b'{ "clusters": [ "accudo-forge-big-1" ] }'),
                ),
                FakeCommand(
                    # NOTE: with multi-cloud support, we set the kubeconfig to ensure auth before continuing
                    # See changes in: https://github.com/accudo-labs/accudo-core/pull/6166
                    "aws eks update-kubeconfig --name accudo-forge-big-1 --kubeconfig temp1",
                    RunResult(0, b""),
                ),
                FakeCommand("git rev-parse HEAD~0", RunResult(0, b"banana")),
                FakeCommand(
                    "aws ecr describe-images --repository-name accudo/validator-testing "
                ...
                FakeCommand(
                    "aws ecr describe-images --repository-name accudo/forge --image-ids "
                    "imageTag=banana",
                    RunResult(0, b""),
                ),
                FakeCommand(
            ...
            stack.enter_context(
                patch.object(
                    ...
                    lambda *_: FakeConfigBackend(
                        {
                            "enabled_clusters": ["accudo-forge-big-1"],
                            "all_clusters": ["accudo-forge-big-1", "banana"],
                            "test_suites": {},
                        }
                    ),
                )
    ...
    def testListClusters(self) -> None:
        fake_clusters = json.dumps(
            AwsListClusterResult(
                clusters=[
                    "banana-fake-1",
                    "accudo-forge-banana-1",
                    "accudo-forge-potato-2",
                ]
            ),
        )
        shell = SpyShell(
        ...
        self.assertEqual(clusters, ["accudo-forge-banana-1", "accudo-forge-potato-2"])
        shell.assert_commands(self)
...
class GetForgeJobsTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def testGetAllForgeJobs(self) -> None:
        fake_clusters = ["accudo-forge-banana", "accudo-forge-apple-2"]
        ...
        fake_forge_first_first_cluster_pods = GetPodsResult(
            items=[
                fake_pod_item("accudo-node-0-validator", "Running"),
                fake_pod_item("accudo-node-1-validator", "Running"),
            ]
        )
        fake_forge_first_failed_cluster_pods = GetPodsResult(
            items=[
                fake_pod_item("accudo-node-0-validator", "Running"),
                fake_pod_item("accudo-node-1-validator", "Running"),
                fake_pod_item("accudo-node-0-fullnode", "Running"),
                fake_pod_item("accudo-node-1-fullnode", "Running"),
            ]
        )
        fake_forge_first_ignore_me_cluster_pods = GetPodsResult(
            items=[
                fake_pod_item("accudo-node-0-validator", "Failed"),
                fake_pod_item("accudo-node-1-validator", "Running"),
            ]
        )
        ...
        shell = SpyShell(
            [
                FakeCommand(
                    "aws eks update-kubeconfig --name accudo-forge-banana --kubeconfig temp1",
                    RunResult(0, b""),
                ),
                FakeCommand(
                ...
                FakeCommand(
                    "aws eks update-kubeconfig --name accudo-forge-apple-2 --kubeconfig temp2",
                    RunResult(0, b""),
                ),
                FakeCommand(
...
class ForgeConfigTests(unittest.TestCase):
            ...
            validate_forge_config(
                {
                    ...
                    "all_clusters": ["banana", "apple"],
                    "default_helm_values": {
                        "accudo-node": {"image": {"tag": "banana"}},
                        "accudo-genesis": {"image": {"tag": "banana"}},
                    },
                }
            ),
            [],
    ...
    def testHelmGetConfig(self) -> None:
        ...
        helm_after_missing = {
            ...
            "all_clusters": ["banana", "apple"],
            "default_helm_values": {
                "accudo-node": {"apple": "enabled", "banana": {"enabled": "true"}}
            },
        }
        helm_after_complete = {
            "enabled_clusters": ["banana"],
            ...
            "default_helm_values": {
                "accudo-node": {"apple": "enabled", "banana": {"enabled": "true"}},
                "accudo-genesis": {"apple": "enabled", "banana": {"enabled": "true"}},
            },
        }
        runner = CliRunner()
        shell = SpyShell(
        ...
        with ExitStack() as stack:
            ...
            result_helm_config_not_present: Result = runner.invoke(
                main,
                ["--no-log-metadata", "config", "helm", "get", "accudo-node"],
                catch_exceptions=True,
            )
            result_helm_config_present_missing = runner.invoke(
                main,
                ["--no-log-metadata", "config", "helm", "get", "accudo-genesis"],
                catch_exceptions=True,
            )
            ...
            self.assertEqual(
                ...
                Exception("No helm values found for chart accudo-genesis").args,
            )
            ...
            self.assertIsNotNone(helm_after_complete.get("default_helm_values").get("accudo-node"))  # type: ignore
            # the output config is printed with an extra newline
            self.assertEqual(
                result_helm_config_present_complete.stdout_bytes,
                f'{json.dumps(helm_after_complete.get("default_helm_values").get("accudo-node"), indent=2)}\n'.encode(),  # type: ignore
            )
    ...
    def testHelmSetConfig(self) -> None:
        ...
        config_before = {
            ...
            "all_clusters": ["banana", "apple"],
            "default_helm_values": {
                "accudo-node": {"apple": "enabled", "banana": {"enabled": "false"}}
            },
        }
        config_after = {
            **config_before,
        ...
        filesystem = SpyFilesystem(
            ...
            },
            {
                ...
                "temp2": json.dumps(
                    config_after["default_helm_values"]["accudo-node"]
                ).encode(),
            },
        )
        with ExitStack() as stack:
            ...
            ret = runner.invoke(
                main,
                ["config", "helm", "set", "accudo-node", "--config", "temp2", "-y"],
                catch_exceptions=True,
            )
            shell.assert_commands(self)
            filesystem.assert_reads(self)
