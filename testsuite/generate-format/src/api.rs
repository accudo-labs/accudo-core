...
fn trace_crypto_values(tracer: &mut Tracer, samples: &mut Samples) -> Result<()> {
    ...
    let secp256r1_ecdsa_public_key =
        accudo_crypto::PrivateKey::public_key(&secp256r1_ecdsa_private_key);
...
pub fn get_registry() -> Result<Registry> {
    ...
    // api types
    tracer.trace_type::<accudo_api_types::TransactionData>(&samples)?;
    tracer.trace_type::<accudo_api_types::TransactionOnChainData>(&samples)?;

    // output types
    tracer.trace_type::<CoinStoreResource<AccudoCoinType>>(&samples)?;
