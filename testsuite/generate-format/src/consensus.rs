...
struct TestAccudoCrypto(String);
...
fn trace_crypto_values(tracer: &mut Tracer, samples: &mut Samples) -> Result<()> {
    ...
    let secp256k1_public_key = accudo_crypto::PrivateKey::public_key(&secp256k1_private_key);
...
pub fn get_registry() -> Result<Registry> {
    ...
    tracer.trace_value(
        &mut samples,
        &accudo_consensus_types::block::Block::make_genesis_block(),
    )?;
    ...
    tracer.trace_type::<accudo_types::keyless::EphemeralCertificate>(&samples)?;
    ...
    tracer.trace_type::<accudo_consensus::quorum_store::types::BatchResponse>(&samples)?;
    tracer.trace_type::<accudo_consensus_types::round_timeout::RoundTimeoutReason>(&samples)?;
    tracer.trace_type::<accudo_consensus::network_interface::ConsensusMsg>(&samples)?;
    tracer.trace_type::<accudo_consensus::network_interface::CommitMessage>(&samples)?;
    tracer.trace_type::<accudo_consensus_types::block_data::BlockType>(&samples)?;
