...
custom_edit_url: https://github.com/accudo-labs/accudo-core/edit/main/testsuite/generate-format/README.md
---

`generate-format` hosts the Accudo core type checker to ensure compatibility and uses 
[`serde-reflection`](https://github.com/accudo-labs/serde-reflection) to properly track type changes over time. 
...
Feel free to follow along here: https://github.com/accudo-labs/accudo-core/pull/10755/files
...
> Note: Because `[api|accudo|consensus].rs`, are already tracking the `AnyPublicKey` and `AnySignature` struct, no further tracers are necessary here

Additionally, ensure that `enums` are tracked correctly across `[api|accudo|consensus].rs`
```rust
fn get_registry(){
    ...
