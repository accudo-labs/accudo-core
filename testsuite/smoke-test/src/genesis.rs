...
/// 3. Use the accudo CLI to generate a genesis transaction that removes the last validator from the set.
/// 4. Use the accudo-debugger to manually apply the genesis transaction to all remaining validators.
/// 5. Verify that the network is able to resume consensus and that the last validator is no longer in the set.
/// 6. Use the accudo-debugger to manually apply the genesis transaction to all VFNs.
/// 7. Verify that the VFNs are able to sync with the rest of the network.
async fn test_fullnode_genesis_transaction_flow() {
    println!("0. Building the Accudo CLI and debugger!");
    let accudo_debugger = workspace_builder::get_bin("accudo-debugger");
    ...
    let (genesis_blob_path, genesis_transaction) =
        generate_genesis_transaction(&mut swarm, accudo_cli);
    ...
    let output = Command::new(accudo_debugger.as_path())
...
/// 4. Use the accudo CLI to generate a genesis transaction that removes the last validator from the set.
/// 5. Use the accudo-debugger to manually apply the genesis transaction to all remaining validators.
...
fn generate_genesis_transaction(
    ...
    Command::new(accudo_cli.as_path())
        .current_dir(workspace_root())
        .args(vec![
            "genesis",
...
fn parse_waypoint(bootstrap_command_output: &str) -> Waypoint {
    let waypoint = Regex::new(r"Got waypoint: (\d+:\w+)")
        ...
        .ok_or_else(|| {
            anyhow!("Failed to parse `accudo-debugger accudo-db bootstrap` waypoint output!")
        });
