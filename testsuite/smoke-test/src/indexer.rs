...
use accudo_cached_packages::accudo_stdlib::accudo_token_stdlib;
use accudo_forge::{AccudoPublicInfo, Result, Swarm};
use accudo_indexer::{
...
pub async fn execute_nft_txns(creator: LocalAccount, info: &mut AccudoPublicInfo) -> Result<()> {
    ...
    let token_mutator =
        info.transaction_factory()
            .payload(accudo_token_stdlib::token_mutate_token_properties(
                creator.address(),
                creator.address(),
                collection_name.clone(),
                token_name.clone(),
...
async fn test_old_indexer() {
    if accudo_indexer::should_skip_pg_tests() {
        return;
    }
    ...
    let swarm = crate::smoke_test_environment::SwarmBuilder::new_local(1)
        ...
        .with_init_config(Arc::new(|_, config, _| {
            ...
            config.indexer.processor =
                Some(accudo_indexer::processors::default_processor::NAME.to_string());
        }))
        .build()
        .await;
