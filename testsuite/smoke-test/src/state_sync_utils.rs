...
use accudo_forge::{LocalNode, LocalSwarm, Node, NodeExt, Swarm};
...
pub fn enable_consensus_observer(use_consensus_observer: bool, node_config: &mut NodeConfig) {
    if use_consensus_observer {
        match node_config.base.role {
            accudo_config::config::RoleType::Validator => {
                node_config.consensus_observer.publisher_enabled = true;
            },
            accudo_config::config::RoleType::FullNode => {
...
fn verify_first_ledger_info(node: &mut LocalNode) {
    ...
    // Verify that the ledger info exists at version 0
    let accudo_db = AccudoDB::new_for_test_with_sharding(db_path_buf.as_path(), 1 << 13);
    accudo_db.get_epoch_ending_ledger_info(0).unwrap();
...
async fn verify_pruning_metrics_after_fast_sync(
    ...
    let state_merkle_pruner_version = node_inspection_client
        .get_node_metric_i64(
            "accudo_pruner_versions{pruner_name=state_merkle_pruner,tag=min_readable}",
        )
        .await
        .unwrap()
        .unwrap();
    let epoch_snapshot_pruner_version = node_inspection_client
        .get_node_metric_i64(
            "accudo_pruner_versions{pruner_name=epoch_snapshot_pruner,tag=min_readable}",
        )
        .await
        .unwrap()
        .unwrap();
    let ledger_pruner_version = node_inspection_client
        .get_node_metric_i64("accudo_pruner_versions{pruner_name=ledger_pruner,tag=min_readable}")
        .await
        .unwrap()
        .unwrap();
