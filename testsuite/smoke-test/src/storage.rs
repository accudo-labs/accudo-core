...
async fn test_db_restore() {
    ...
    workspace_builder::get_bin("accudo-debugger");
    info!("---------- 1. pre-building finished.");

    let mut swarm = SwarmBuilder::new_local(4).with_accudo().build().await;
...
fn db_backup_verify(backup_path: &Path, trusted_waypoints: &[Waypoint]) {
    info!("---------- running accudo-debugger accudo-db backup-verify");
    let now = Instant::now();
    let bin_path = workspace_builder::get_bin("accudo-debugger");
    let metadata_cache_path = TempPath::new();

    metadata_cache_path.create_as_dir().unwrap();

    let mut cmd = Command::new(bin_path.as_path());
    cmd.args(["accudo-db", "backup", "verify"]);
...
fn replay_verify(backup_path: &Path, trusted_waypoints: &[Waypoint]) {
    ...
    let mut cmd = Command::new(bin_path.as_path());
    cmd.args(["accudo-db", "replay-verify"]);
    trusted_waypoints.iter().for_each(|w| {
        cmd.arg("--trust-waypoint");
        cmd.arg(w.to_string());
    });
...
pub(crate) fn db_backup(
    ...
    info!("---------- running accudo db tool backup");
    let now = Instant::now();
    let bin_path = workspace_builder::get_bin("accudo-debugger");
...
pub(crate) fn db_restore(
    ...
    let mut cmd = Command::new(bin_path.as_path());
    cmd.args(["accudo-db", "restore", "bootstrap-db"]);
    trusted_waypoints.iter().for_each(|w| {
        cmd.arg("--trust-waypoint");
        cmd.arg(w.to_string());
    });
...
async fn do_transfer_or_reconfig(info: &mut AccudoPublicInfo) -> Result<()> {
...
async fn do_transfers_and_reconfigs(mut info: AccudoPublicInfo, quit_flag: Arc<AtomicBool>) {
    // loop until aborted
...
async fn test_db_restart() {
    ...
    let mut pub_chain_info = AccudoPublicInfo::new(
        chain_info.chain_id(),
