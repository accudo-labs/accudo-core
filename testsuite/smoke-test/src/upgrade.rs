...
use crate::{
    accudo::move_test_helpers, smoke_test_environment::SwarmBuilder,
...
use accudo_gas_algebra::GasQuantity;
use accudo_gas_schedule::{AccudoGasParameters, InitialGasSchedule, ToOnChainGasSchedule};
use accudo_release_builder::{
    components::{
...
use accudo_types::on_chain_config::{FeatureFlag as AccudoFeatureFlag, OnChainConsensusConfig};
...
#[tokio::test]
/// This test verifies the flow of accudo framework upgrade process.
/// i.e: The network will be alive after applying the new accudo framework release.
async fn test_upgrade_flow() {
    // prebuild tools.
    let accudo_cli = workspace_builder::get_bin("accudo");
    ...
    let url = env.accudo_public_info().url().to_string();
    let private_key = env
        .accudo_public_info()
        .root_account()
        .private_key()
    ...
    // TODO: Replace this logic with accudo-gas
    let mut gas_parameters = AccudoGasParameters::initial();
    gas_parameters.vm.txn.max_transaction_size_in_bytes = GasQuantity::new(100_000_000);

    let gas_schedule = accudo_types::on_chain_config::GasScheduleV2 {
    ...
    assert!(Command::new(accudo_cli.as_path())
    ...
    let config = accudo_release_builder::ReleaseConfig {
            ...
            Proposal {
                ...
                update_sequence: vec![
                    ReleaseEntry::FeatureFlag(Features {
                        enabled: AccudoFeatureFlag::default_features()
                            .into_iter()
    ...
    let base_path_v1 = base_dir.join("src/accudo/package_publish_modules_v1/");

    move_test_helpers::publish_package(&mut env.accudo_public_info(), base_path_v1)
        .await
        .unwrap();
...
// to repeatedly compile all the accudo framework pacakges as dependency
//
#[ignore]
...
async fn test_release_validate_tool_multi_step() {
    let (mut env, _, _) = SwarmBuilder::new_local(1)
        ...
        .build_with_cli(2)
        .await;
    let config = accudo_release_builder::ReleaseConfig::default();
    ...
    let network_config = accudo_release_builder::validate::NetworkConfig {
    ...
    accudo_release_builder::validate::validate_config(config, network_config, None)
        .await
        .unwrap();

    let root_account = env.accudo_public_info().root_account().address();
    // Test the module publishing workflow
