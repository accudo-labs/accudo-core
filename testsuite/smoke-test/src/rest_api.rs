...
use accudo_config::config::GasEstimationConfig;
use accudo_crypto::ed25519::Ed25519Signature;
use accudo_forge::{LocalSwarm, NodeExt, Swarm, TransactionType};
use accudo_global_constants::{DEFAULT_BUCKETS, GAS_UNIT_PRICE};
use accudo_rest_client::{
    accudo_api_types::{MoveModuleId, TransactionData, ViewFunction, ViewRequest},
    Client,
};
...
async fn test_basic_client() {
    let swarm = SwarmBuilder::new_local(1)
        ...
        .build()
        .await;
    let mut info = swarm.accudo_public_info();
    ...
    //            testsuite/forge/src/interface/accudo.rs?
    let account1 = info
    ...
    let tx = account1.sign_with_transaction_builder(
        info.transaction_factory()
            .payload(accudo_stdlib::accudo_coin_transfer(account2.address(), 1)),
    );
...
async fn test_bcs() {
    ...
    for (i, expected_transaction) in transactions.iter().enumerate() {
        ...
        let expected_hash =
            accudo_crypto::HashValue::from(expected_transaction.transaction_info().unwrap().hash);
    ...
    for (i, json_txn) in json_txns.iter().enumerate() {
        ...
        assert_eq!(
            accudo_crypto::HashValue::from(json_txn.transaction_info().unwrap().hash),
    ...
    assert_eq!(
        accudo_crypto::HashValue::from(json_txn.info.hash),
        bcs_txn.info.transaction_hash()
    );
    ...
    assert_eq!(
        accudo_crypto::HashValue::from(json_block.block_hash),
    ...
    assert_eq!(
        accudo_crypto::HashValue::from(first_json_txn.transaction_info().unwrap().hash),
    ...
    assert_eq!(
        accudo_crypto::HashValue::from(json_block_by_height.block_hash),
...
async fn test_view_function() {
    ...
    let view_request = ViewRequest {
        ...
        type_arguments: vec!["0x1::accudo_coin::AccudoCoin".parse().unwrap()],
    ...
    let bcs_view_request = ViewFunction {
        ...
        ty_args: vec![TypeTag::Struct(Box::new(
            StructTag::from_str("0x1::accudo_coin::AccudoCoin").unwrap(),
        ))],
