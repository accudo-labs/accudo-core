...
use crate::{
    smoke_test_environment::new_local_swarm_with_accudo,
...
use accudo_forge::{NodeExt, Result, Swarm, SwarmExt};
...
async fn test_indexer() {
    let mut swarm = new_local_swarm_with_accudo(1).await;
    ...
    let account1 = swarm.accudo_public_info().random_account();
    let account2 = swarm.accudo_public_info().random_account();

    let mut chain_info = swarm.chain_info().into_accudo_public_info();
    ...
    let txn = account1.sign_with_transaction_builder(
        factory.payload(accudo_stdlib::accudo_coin_transfer(account2.address(), 10)),
    );
...
where
    T: accudo_schemadb::schema::Schema,
    U: accudo_schemadb::schema::KeyCodec<T> + std::cmp::Ord + std::fmt::Debug,
    std::collections::HashSet<U>:
        std::iter::FromIterator<<T as accudo_schemadb::schema::Schema>::Key>,
{
