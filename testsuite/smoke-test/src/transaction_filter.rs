...
use accudo_crypto::ed25519::{Ed25519PrivateKey, Ed25519PublicKey};
use accudo_forge::{LocalSwarm, NodeExt, Swarm};
...
async fn test_consensus_block_filter() {
    ...
    let accudo_public_info = swarm.accudo_public_info();
    let response = accudo_public_info
        .client()
...
async fn create_account_with_funds(
    ...
    let mut accudo_public_info = swarm.accudo_public_info();
    accudo_public_info
...
async fn create_signed_transaction_from_sender(
    ...
    let accudo_public_info = swarm.accudo_public_info();
    let sequence_number = accudo_public_info
        .client()
    ...
    let unsigned_txn = accudo_public_info
        .transaction_factory()
        .payload(accudo_stdlib::accudo_coin_transfer(receiver.address(), 100))
...
async fn create_transaction_from_sender(
    ...
    let mut accudo_public_info = swarm.accudo_public_info();
    let receiver = accudo_public_info.random_account();
