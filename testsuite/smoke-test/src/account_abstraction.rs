...
use accudo_crypto::SigningKey;
use accudo_forge::Swarm;
use accudo_sdk::types::{AccountKey, LocalAccount};
use accudo_types::function_info::FunctionInfo;
use ethers::{
...
async fn test_solana_derivable_account() {
    let swarm = SwarmBuilder::new_local(1).with_accudo().build().await;
    ...
    let account = LocalAccount::new_domain_aa(
        ...
        account_identity,
        Arc::new(move |x: &[u8]| {
            let function_name = "0x1::accudo_account::create_account";
            let digest = format!("0x{}", hex::encode(x));
            let message = format!(
                "{} wants you to sign in with your Solana account:\n{}\n\nPlease confirm you explicitly initiated this request from {}. You are approving to execute transaction {} on Accudo blockchain (local).\n\nNonce: {}",
                domain,
                base58_public_key,
                domain,
                function_name,
    ...
    let create_txn = account.sign_aa_transaction_with_transaction_builder(
        ...
        info.transaction_factory()
            .payload(accudo_stdlib::accudo_account_create_account(
