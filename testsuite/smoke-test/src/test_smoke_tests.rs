...
use accudo_forge::{NodeExt, Swarm};
use std::{
    sync::Arc,
...
/// Previously get_bin triggered a rebuild of accudo-node, which caused issues that were only seen
/// during parallel execution of tests.
/// This test should make regressions obvious.
#[tokio::test]
async fn test_accudo_node_after_get_bin() {
    ...
    // Before #5308 this re-compiled accudo-node and caused a panic on the vfn.
    let _accudo_cli = crate::workspace_builder::get_bin("accudo");
