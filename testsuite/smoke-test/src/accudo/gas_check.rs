...
async fn test_gas_check() {
    ...
    let transfer_txn = account1.sign_with_transaction_builder(
        info.transaction_factory()
            .payload(accudo_stdlib::accudo_coin_transfer(account2.address(), 100)),
    );
    ...
    let mut gas_params = AccudoGasParameters::initial();
    ...
    let update_txn = info
        ...
        .sign_with_transaction_builder(txn_factory.payload(
            accudo_stdlib::gas_schedule_set_gas_schedule(gas_schedule_blob),
        ));
