...
use accudo::{common::types::GasOptions, test::CliTestFramework};
...
use accudo_forge::{AccudoPublicInfo, LocalSwarm, NodeExt, Swarm, SwarmExt};
use accudo_logger::{debug, info};
use accudo_rest_client::Client;
use accudo_sdk::types::{
...
script {
    ...
    fun main(core_resources: &signer) {
        let framework = accudo_governance::get_signer_testnet_only(core_resources, @0x1);
...
use std::option;
fun main(core_resources: &signer) {{
    let framework_signer = accudo_governance::get_signer_testnet_only(core_resources, @0x1);
