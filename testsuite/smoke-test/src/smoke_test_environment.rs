...
use accudo_config::{config::NodeConfig, keys::ConfigKey, utils::get_available_port};
use accudo_crypto::ed25519::Ed25519PrivateKey;
use accudo_faucet_core::server::{FunderKeyEnum, RunConfig};
use accudo_forge::{ActiveNodesGuard, Factory, LocalFactory, LocalSwarm, Node};
...
impl SwarmBuilder {
    ...
    pub fn with_accudo(mut self) -> Self {
        self.genesis_framework = Some(accudo_cached_packages::head_release_bundle().clone());
        self
    }

    pub fn with_accudo_testnet(mut self) -> Self {
        self.genesis_framework = Some(accudo_framework::testnet_release_bundle().clone());
        self
    }
...
// Gas is not enabled with this setup, it's enabled via forge instance.
pub async fn new_local_swarm_with_accudo(num_validators: usize) -> LocalSwarm {
