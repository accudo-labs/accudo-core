...
use accudo::{common::types::GasOptions, move_tool::MemberId, test::CliTestFramework};
use accudo_forge::{NodeExt, Swarm, SwarmExt};
use accudo_logger::info;
use accudo_types::on_chain_config::OnChainRandomnessConfig;
...
pub async fn publish_on_chain_dice_module(
    ...
    cli.init_package(
        ...
        package_addresses,
        Some(CliTestFramework::accudo_framework_dir()),
    )
    .await
    .unwrap();

    let content =
        include_str!("../../../../accudo-move/move-examples/on_chain_dice/sources/dice.move")
            .to_string();
