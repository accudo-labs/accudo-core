...
GRPC_IS_READY_MESSAGE = f"""
    ...
    - For non-TLS:
        grpcurl -plaintext -d '{{ "starting_version": 0 }}' \\
            -H "x-accudo-data-authorization:dummy_token" \\
            {GRPC_DATA_SERVICE_NON_TLS_URL} accudo.indexer.v1.RawData/GetTransactions
    - For TLS:
        grpcurl -insecure -d '{{ "starting_version": 0 }}' \\
            -H "x-accudo-data-authorization:dummy_token" \\
            {GRPC_DATA_SERVICE_TLS_URL} accudo.indexer.v1.RawData/GetTransactions
    ======================================
"""

SHARED_DOCKER_VOLUME_NAMES = ["accudo-shared", "indexer-grpc-file-store"]
        ...
        try:
            res = context.shell.run(
                [
                    ...
                    '{ "starting_version": 0 }',
                    "-H",
                    "x-accudo-data-authorization:dummy_token",
                    "-import-path",
                    "protos/proto",
                    "-proto",
                    "accudo/indexer/v1/raw_data.proto",
                    "-plaintext",
...
def main() -> None:
    # Change to the root of accudo-core.
    ...
    if platform.system() == "Darwin" and platform.processor().startswith("arm"):
        ...
        # https://github.com/accudo-labs/accudo-core/issues/9878
