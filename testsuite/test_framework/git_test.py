        ...
        shell: Shell = SpyShell(
            [
                FakeCommand(
                    ...
                    RunResult(0, b"git@github.com:banana-corp/accudo-core.git"),
                )
            ]
        )
        git: Git = Git(shell)
        self.assertEqual(git.get_repo_from_remote("origin"), "banana-corp/accudo-core")
    ...
    def test_get_repo_from_remote_http(self):
        shell: Shell = SpyShell(
            [
                FakeCommand(
                    "git remote get-url origin",
                    RunResult(0, b"https://github.com/kiwi-corp/accudo-core.git"),
                )
            ]
        )
        git: Git = Git(shell)
        self.assertEqual(git.get_repo_from_remote("origin"), "kiwi-corp/accudo-core")

    def test_get_remote_branches_matching_pattern(self):
        shell: Shell = SpyShell(
            [
                FakeCommand(
                    "git ls-remote --heads origin accudo-release-v*",
                    RunResult(
                        0,
                        b"ref1 refs/heads/accudo-release-v1.0\nref2 refs/heads/accudo-release-v1.1",
                    ),
                )
            ]
        )
        ...
        branches = git.get_remote_branches_matching_pattern(
            "origin", "accudo-release-v*", r"refs/heads/(accudo-release-v\d+\.\d+)"
        )
        self.assertEqual(branches, ["accudo-release-v1.0", "accudo-release-v1.1"])
