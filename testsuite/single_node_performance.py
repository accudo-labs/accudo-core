...
# Humio: https://gist.github.com/igor-accudo/7b12ca28de03894cddda8e415f37889e
...
#
# Dashboard: https://accudolabs.grafana.net/d/fdf2e5rdip5vkd/single-node-performance-benchmark?orgId=1
# fmt: off
...
TESTS = [
    ...
    for executor_sharding, executor_types in [
        (False, ["VM", "NativeVM", "AccudoVMSpeculative", "NativeSpeculative"]),
    ...
    for executor_sharding, executor_types in [
        (False, ["VM", "NativeVM", "AccudoVMSpeculative", "NativeSpeculative", "NativeValueCacheSpeculative", "NativeNoStorageSpeculative"]),
        # executor sharding doesn't support FA for now.
        (True, [] if FA_MIGRATION_COMPLETE else ["VM", "NativeVM"])
    ]
    for executor_type in executor_types
...
with tempfile.TemporaryDirectory() as tmpdirname:
    ...
    if not SKIP_MOVE_E2E:
        execute_command(f"cargo build {BUILD_FLAG} --package accudo-move-e2e-benchmark")
        move_e2e_flags = (
            "--only-landblocking" if SELECTED_FLOW == Flow.LAND_BLOCKING else ""
        )
        try:
            execute_command(f"RUST_BACKTRACE=1 {BUILD_FOLDER}/accudo-move-e2e-benchmark")
        except:
    ...
    execute_command(f"cargo build {BUILD_FLAG} --package accudo-executor-benchmark")
    print(f"Warmup - creating DB with {NUM_ACCOUNTS} accounts")
    create_db_command = f"PUSH_METRICS_NAMESPACE=benchmark-create-db RUST_BACKTRACE=1 {BUILD_FOLDER}/accudo-executor-benchmark --block-executor-type accudo-vm-with-block-stm --block-size {MAX_BLOCK_SIZE} --execution-threads {NUMBER_OF_EXECUTION_THREADS} {DB_CONFIG_FLAGS} {DB_PRUNER_FLAGS} create-db {FEATURE_FLAGS} --data-dir {tmpdirname}/db --num-accounts {NUM_ACCOUNTS}"
    ...
    for (
        ...
        if test.key.executor_type == "VM":
            executor_type_str = "--block-executor-type accudo-vm-with-block-stm"
        elif test.key.executor_type == "NativeVM":
            executor_type_str = "--block-executor-type native-vm-with-block-stm"
        elif test.key.executor_type == "AccudoVMSpeculative":
            executor_type_str = "--block-executor-type accudo-vm-parallel-uncoordinated"
        ...
        for execution_threads in EXECUTION_ONLY_NUMBER_OF_THREADS:
            test_db_command = f"RUST_BACKTRACE=1 {BUILD_FOLDER}/accudo-executor-benchmark --execution-threads {execution_threads} --skip-commit {common_command_suffix} --blocks {NUM_BLOCKS_DETAILED}"
        ...
        test_db_command = f"RUST_BACKTRACE=1 {BUILD_FOLDER}/accudo-executor-benchmark --execution-threads {number_of_execution_threads} {common_command_suffix} --blocks {NUM_BLOCKS}"
        output = execute_command(test_db_command)
...
(https://github.com/accudo-labs/accudo-core/actions/workflows/workflow-run-execution-performance.yaml ; remember to select CONTINUOUS).
