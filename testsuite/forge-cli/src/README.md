...
id: forge cli
title: Forge CLI
custom_edit_url: https://github.com/accudo-labs/accudo-core/edit/main/testsuite/forge-cli/README.md
---

# Forge CLI

This crate contains the Forge command line interface (CLI) tool. This enables users to
run local and remote Accudo swarms (i.e., networks of validators and validator fullnodes). For
example, to deploy a local validator swarm, run:

```
cargo run -p accudo-forge-cli -- --suite "run_forever" --num-validators 4 test local-swarm
```
...
```
...
2022-09-01T15:41:27.228289Z [main] INFO crates/accudo-genesis/src/builder.rs:462 Building genesis with 4 validators. Directory of output: "/private/var/folders/dx/c0l2rrkn0656gfx6v5_dy_p80000gn/T/.tmpq9uPMJ"
...
...
...
2022-09-01T15:41:28.094800Z [main] INFO testsuite/forge/src/backend/local/node.rs:129 Started node 0 (PID: 78939) with command: ".../accudo-core/target/debug/accudo-node" "-f" "/private/var/folders/dx/c0l2rrkn0656gfx6v5_dy_p80000gn/T/.tmpq9uPMJ/0/node.yaml"
...
```
kill -9 <Node 0 PID>
cargo run -p accudo-node -- -f <Location to the node 0 configuration file displayed above>
```
...
```bash
cargo run -p accudo-faucet-service -- run-simple --key <key> --node-url <node_url>
```
...
```
cargo run -p accudo-faucet-cli -- --amount 10 --accounts <account_address> --key <private_key>
```

:::tip Faucet and Accudo CLI
See more on how the faucet works in the [README](https://github.com/accudo-labs/accudo-core/tree/main/crates/accudo-faucet).

Also see how to use the [Accudo CLI](../cli-tools/accudo-cli/use-cli/use-accudo-cli.md#account-examples) with an existing faucet.
:::
...
```
cargo run -p accudo-forge-cli -- --suite "run_forever" --num-validators 3 --num-validator-fullnodes 1 test local-swarm
```
...
```
cargo run -p accudo-forge-cli --help
```
