...
pub fn run_forever() -> ForgeConfig {
    ForgeConfig::default()
        ...
        .with_genesis_module_bundle(accudo_cached_packages::head_release_bundle().clone())
        .add_accudo_test(RunForever)
}

pub fn local_test_suite() -> ForgeConfig {
    ForgeConfig::default()
        .add_accudo_test(FundAccount)
        .add_accudo_test(TransferCoins)
        .add_admin_test(GetMetadata)
...
fn background_emit_request(high_gas_price: bool) -> EmitJobRequest {
    ...
    if high_gas_price {
        result = result.gas_price(5 * accudo_global_constants::GAS_UNIT_PRICE);
    }
    result
}
...
pub(crate) fn single_cluster_test(
    ...
    ForgeConfig::default()
        ...
        .with_emit_job(
            EmitJobRequest::default()
                .mode(EmitJobMode::ConstTps { tps: 10 })
                .gas_price(5 * accudo_global_constants::GAS_UNIT_PRICE),
        )
...
pub fn optimize_state_sync_for_throughput(node_config: &mut NodeConfig, max_chunk_size: u64) {
    ...
    let data_client_config = &mut node_config.state_sync.accudo_data_client;
...
impl AccudoTest for RunForever {
    async fn run<'t>(&self, _ctx: &mut AccudoContext<'t>) -> Result<()> {
...
impl AdminTest for GetMetadata {
    fn run(&self, ctx: &mut AdminContext<'_>) -> Result<()> {
        ...
        runtime.block_on(client.get_accudo_version()).unwrap();
...
#[async_trait::async_trait]
impl AccudoTest for FundAccount {
...
#[async_trait::async_trait]
impl AccudoTest for TransferCoins {
    async fn run<'t>(&self, ctx: &mut AccudoContext<'t>) -> Result<()> {
        let client = ctx.client();
        ...
        let transfer_txn = payer.sign_with_transaction_builder(
            ctx.accudo_transaction_factory()
                .payload(accudo_stdlib::accudo_coin_transfer(payee.address(), 10)),
        );
