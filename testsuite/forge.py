...
GAR_REPO_NAME = "us-docker.pkg.dev/accudo-registry/docker"
...
# o11y resources
GRAFANA_BASE_URL = "https://accudolabs.grafana.net/d/overview/overview?orgId=1&refresh=10s&var-Datasource=VictoriaMetrics%20Global%20%28Non-mainnet%29&var-BigQuery=Google%20BigQuery"

# helm chart default override values
HELM_CHARTS = ["accudo-node", "accudo-genesis"]
...
class ForgeContext:
    ...
    def forge_chain_name(self) -> str:
        forge_chain_name = self.forge_cluster.name.lstrip("accudo-")
...
def get_cpu_profile_link(
    ...
    base_url = "https://grafana.accudo.org/a/grafana-pyroscope-app/profiles-explorer"
...
def get_humio_link_for_test_runner_logs(
    ...
    columns = [
        ...
        },
        {
            ...
            "openInNewBrowserTab": "***",
            "style": "button",
            "hrefTemplate": 'https://github.com/accudo-labs/accudo-core/pull/{{fields["github_pr"]}}',
...
def get_axiom_link_for_test_runner_logs(
    ...
    logs_url = f"https://app.axiom.co/accudolabs-hghf/explorer?initForm={urlquote(json.dumps({'apl': apl_query, 'queryOptions': apply_axiom_time_filter(time_filter), }))}"
...
def get_current_cluster_name(shell: Shell) -> str:
    ...
    matches = re.findall(r"accudo.*", current_context)
    if len(matches) != 1:
...
def test(
    ...
    # forge_cluster_name = "accudo-forge-0"
