/// Copied from accudo-move/move-examples/swap/sources/liquidity_pool.move
///
...
module publisher_address::liquidity_pool {
    ...
    // use accudo_std::comparator;
    use accudo_std::math128;
    use accudo_std::math64;
    use accudo_std::smart_table::{Self, SmartTable};
    use accudo_std::smart_vector::{Self, SmartVector};
    ...
    friend publisher_address::liquidity_pool_wrapper;
    ...
    public fun liquidity_pool_address(
        ...
        // };
        object::create_object_address(&@publisher_address, get_pool_seeds(token_1, token_2, is_stable))
    }
    ...
    public fun create(
        ...
        let pool_constructor_ref = create_lp_token(publisher, token_1, token_2, is_stable);
    ...
    public(friend) fun claim_fees(
        ...
        let fees_1 = if (claimable_1 > 0) {
            fungible_asset::withdraw(publisher, pool_data.fees_store_1, (claimable_1 as u64))
        } else {
            fungible_asset::zero(fungible_asset::store_metadata(pool_data.fees_store_1))
        };
        let fees_2 = if (claimable_2 > 0) {
            fungible_asset::withdraw(publisher, pool_data.fees_store_2, (claimable_2 as u64))
        } else {
    ...
    inline fun create_lp_token(
        ...
        let lp_token_constructor_ref = &object::create_named_object(publisher, seeds);
    ...
    inline fun unchecked_mut_liquidity_pool_configs(): &mut LiquidityPoolConfigs acquires LiquidityPoolConfigs {
        borrow_global_mut<LiquidityPoolConfigs>(@publisher_address)
    }
