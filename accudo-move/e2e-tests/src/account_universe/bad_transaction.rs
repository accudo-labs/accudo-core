...
use accudo_gas_algebra::{FeePerGasUnit, Gas, GasExpression};
use accudo_gas_schedule::{AccudoGasParameters, InitialGasSchedule, LATEST_GAS_FEATURE_VERSION};
...
impl AUTransactionGen for InsufficientBalanceGen {
    fn apply(
        ...
        let gas_params = AccudoGasParameters::initial();
...
pub struct InvalidAuthkeyGen {
    sender: Index,
    #[proptest(
        strategy = "accudo_crypto::test_utils::uniform_keypair_strategy_with_perturbation(1)"
    )]
