...
use accudo_gas_meter::{AccudoGasMeter, GasAlgebra, StandardGasAlgebra, StandardGasMeter};
...
use accudo_types::{
    ...
    move_utils::MemberId,
    on_chain_config::{
        AccudoVersion, CurrentTimeMicroseconds, FeatureFlag, Features, OnChainConfig, ValidatorSet,
    },
...
use accudo_validator_interface::{DebuggerStateView, RestDebuggerInterface};
use accudo_vm::{
    block_executor::AccudoVMBlockExecutorWrapper,
    data_cache::AsMoveResolver,
    gas::make_prod_gas_meter,
    move_vm_ext::{AccudoMoveResolver, MoveVmExt, SessionExt, SessionId},
    AccudoVM, VMValidator,
};
use accudo_vm_environment::environment::AccudoEnvironment;
use accudo_vm_genesis::{generate_genesis_change_set_for_testing_with_count, GenesisOptions};
...
///
/// This struct is a mock in-memory implementation of the Accudo executor.
...
impl<O: OutputLogger> FakeExecutorImpl<O> {
    ...
    fn from_remote_state_impl(
        ...
        let mut builder = accudo_rest_client::Client::builder(network_url);
    ...
    /// Creates a [`FakeExecutor`] from a remote network state at the version specified by the
    /// transaction id.
    pub fn from_remote_state(network_url: AccudoBaseUrl, txn_id: u64) -> Self {
    ...
    fn set_tracing(&mut self, test_name: &str, file_name: String) {
        ...
        if let Some(env_trace_dir) = env::var_os(ENV_TRACE_DIR) {
            let accudo_version =
                AccudoVersion::fetch_config(&self.state_store).map_or(0, |v| v.major);
            ...
            let mut name_file = OpenOptions::new()
                ...
                .unwrap();
            write!(name_file, "{}::{}", test_name, accudo_version).unwrap();
            for sub_dir in &[
                TRACE_DIR_META,
                TRACE_DIR_DATA,
                TRACE_DIR_INPUT,
    ...
    pub fn stdlib_only_genesis() -> Self {
        ...
        for (bytes, module) in
            accudo_cached_packages::head_release_bundle().code_and_compiled_modules()
        {
    ...
    pub fn custom_genesis(framework: &ReleaseBundle, validator_accounts: Option<usize>) -> Self {
        let genesis = accudo_vm_genesis::generate_test_genesis(framework, validator_accounts);
    ...
    pub fn read_apt_coin_info_resource(&self) -> Option<CoinInfoResource<AccudoCoinType>> {
        self.read_resource(&AccudoCoinType::coin_info_address())
    }
    ...
    fn execute_transaction_block_impl_with_state_view(
        ...
        AccudoVMBlockExecutorWrapper::execute_block_on_thread_pool::<
            _,
    ...
    pub fn execute_transaction_with_gas_profiler(
        ...
        let env = AccudoEnvironment::new(&self.state_store);
        let vm = AccudoVM::new(&env, self.get_state_view());

        let resolver = self.state_store.as_move_resolver();
        let code_storage = self.get_state_view().as_accudo_code_storage(&env);
    ...
    pub fn validate_transaction(&self, txn: SignedTransaction) -> VMValidatorResult {
        ...
        vm.validate_transaction(
            txn,
            &self.state_store,
            &self.state_store.as_accudo_code_storage(&env),
        )
    }
    ...
    pub fn exec_func_record_running_time(
        ...
        let module_storage = self.state_store.as_accudo_code_storage(&env);
    ...
    pub fn exec_abstract_usage(
        ...
        let (write_set, _events) = {
            let env = AccudoEnvironment::new_with_gas_hook(
                &self.state_store,
    ...
    pub fn execute_view_function(
        ...
        AccudoVM::execute_view_function(
            self.get_state_view(),
            fun.module_id,
            fun.member_id,
            type_args,
...
/// responsibility of the adapter, i.e., [AccudoVM]).
