...
use accudo_vm::{data_cache::AsMoveResolver, AccudoVM};
...
impl Session {
    ...
    /// Creates a new session using an empty base state, then applies the Accudo genesis
    ...
    pub fn init_with_remote_state(
        ...
        let mut builder = Client::builder(AccudoBaseUrl::Custom(node_url));
    ...
    pub fn load(session_path: impl AsRef<Path>) -> Result<Self> {
        ...
        let base = match &config.base {
            ...
            BaseState::Remote {
                ...
                let mut builder = Client::builder(AccudoBaseUrl::Custom(node_url.clone()));
                if let Some(api_key) = api_key {
                    builder = builder.api_key(api_key)?;
                }
                let client = builder.build();
    ...
    pub fn execute_view_function(
        ...
        let (summary, res) = match output.values {
            Ok(values) => {
                let annotator = AccudoValueAnnotator::new(&self.state_store);
                ...
                for ret_val in returns {
                    vals.push(accudo_api_types::MoveValue::try_from(ret_val)?.json()?);
                }
    ...
    pub fn view_resource(
        ...
        let json_val = match self.state_store.get_state_value_bytes(&state_key)? {
            Some(bytes) => {
                ...
                Some(accudo_api_types::MoveValue::try_from(annotated)?.json()?)
            },
            None => None,
        };
            ...
            Some(bytes) => {
                ...
                for (resource_tag, bytes) in group {
                    ...
                    group_deserialized.insert(
                        ...
                        accudo_api_types::MoveValue::try_from(annotated)?.json()?,
                    );
                }
...
async fn test_init_then_load_session_remote() -> Result<()> {
    ...
    let session = Session::init_with_remote_state(
        session_path,
        Url::parse("https://mainnet.accudo.org")?,
        12345,
