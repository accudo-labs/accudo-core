...
use accudo_language_e2e_tests::account::Account;
use accudo_sdk::{
    ...
    pub async fn run_workload(
        ...
        for (i, txn) in to_profile_txns.into_iter().enumerate() {
            ...
            if let Some(mut log) = log {
                ...
                aggregate_gas_log = Some(if let Some(aggregate_gas_log) = aggregate_gas_log {
...
impl<'a> ReliableTransactionSubmitter for HarnessReliableTransactionSubmitter<'a> {
    async fn get_account_balance(&self, account_address: AccountAddress) -> anyhow::Result<u64> {
        Ok(self
            ...
            .lock()
            .unwrap()
            .read_accudo_balance(&account_address))
    }
...
mod tests {
    ...
    use accudo_cached_packages::{accudo_stdlib, accudo_token_sdk_builder};
    use accudo_crypto::{bls12381, PrivateKey, Uniform};
    use accudo_sdk::move_types::{identifier::Identifier, language_storage::ModuleId};
    use accudo_transaction_generator_lib::{
        entry_point_trait::EntryPointTrait, publishing::publish_util::PackageHandler,
        workflow_delegator::WorkflowKind,
    };
    use accudo_transaction_workloads_lib::{EntryPoints, LoopType, TokenWorkflowKind};
    use accudo_types::{
    ...
    fn test_gas() {
        ...
        runner.run(
            "Transfer",
            account_1,
            accudo_stdlib::accudo_coin_transfer(account_2_address, 1000),
        );
        ...
        runner.run(
            "RotateConsensusKey",
            account_2,
            accudo_stdlib::stake_rotate_consensus_key(
                pool_address,
                consensus_pubkey,
                proof_of_possession,
            ),
        ...
        runner.run(
            "JoinValidator100",
            account_2,
            accudo_stdlib::stake_join_validator_set(pool_address),
        );
        runner.run(
            "AddStake",
            account_1,
            accudo_stdlib::staking_contract_add_stake(account_2_address, 1000),
        );
        runner.run(
            "UnlockStake",
            account_1,
            accudo_stdlib::staking_contract_unlock_stake(account_2_address, 1000),
        );
        ...
        runner.run(
            "WithdrawStake",
            account_1,
            accudo_stdlib::staking_contract_distribute(account_1_address, account_2_address),
        );
        runner.run(
            "LeaveValidatorSet100",
            account_2,
            accudo_stdlib::stake_leave_validator_set(pool_address),
        );
        ...
        runner.run(
            "CreateCollection",
            account_1,
            accudo_token_sdk_builder::token_create_collection_script(
        ...
        runner.run(
            ...
            account_1,
            accudo_token_sdk_builder::token_create_token_script(
                collection_name.clone(),
                token_name.clone(),
        ...
        runner.run(
            "MintTokenV1",
            account_1,
            accudo_token_sdk_builder::token_mint_script(
                account_1_address,
                collection_name.clone(),
                token_name.clone(),
                1,
        ...
        runner.run(
            "MutateTokenV1",
            account_1,
            accudo_token_sdk_builder::token_mutate_token_properties(
                account_1_address,
                account_1_address,
                collection_name.clone(),
                token_name.clone(),
        ...
        let publisher = &runner.harness.accudo_framework_account();
        runner.publish(
            "PublishLarge",
            publisher,
