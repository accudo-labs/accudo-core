...
use accudo_transaction_workloads_lib::{EntryPoints, MoveVmMicroBenchmark};
use accudo_types::{account_address::AccountAddress, transaction::TransactionPayload};
...
fn run() {
    ...
    for ep in workloads.iter() {
        ...
        for payload in package.publish_transaction_payload(&accudo_types::chain_id::ChainId::test())
        {
