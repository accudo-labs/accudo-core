script {
    ...
    fun main(minter: &signer, dst_addr: address, amount: u64) {
        ...
        let balance = coin::balance<accudo_coin::AccudoCoin>(minter_addr);
        if (balance < U64_MAX - amount - GAS_BUFFER) {
            accudo_coin::mint(minter, minter_addr, amount + GAS_BUFFER);
        };

        accudo_account::transfer(minter, dst_addr, amount);
    }
}
