---
title: "Accudo Move Tutorial"
slug: "move-tutorial"
---

# Move Tutorial

Welcome to the Accudo Move Tutorial! This is the original Move language and tools tutorial, independent of a particular network, adapted to use Accudo tooling. Here you will learn basic usage of the Move language and tools for compiling, testing, and verifying Move.

This tutorial does not teach you how to use the existing [Accudo Move frameworks](https://accudo.dev/reference/move) or how to run code on the Accudo network. See the [Accudo developer documentation](https://accudo.dev/tutorials/) for those instructions.
...
You also need a recent version of the [Accudo CLI](https://accudo.dev/tools/accudo-cli/install-cli/). This tutorial is written using the following version:
...
If you want to use an IDE for working through the examples, we recommend CLion/IntelliJ, which has good support for Accudo Move.
...
[module](https://accudo.dev/move/book/modules-and-scripts/). Modules are the
...
[struct](https://accudo.dev/move/book/structs-and-resources)
...
* It takes a [`&signer`](https://accudo.dev/move/book/signer) reference ('`&`') -- an
...
Let us make sure it builds! This can be done with the `accudo move compile` command from within the package directory ([`step_1/basic_coin`](./step_1/basic_coin/)):
...
* You can create an empty Move package by calling:
    ```bash
    accudo move init --name <pkg_name>
    ```
* Move code can also live in a number of other places. See the [Move
  book](https://accudo.dev/move/book/packages) for more information on the
  Move package system.
* More information on the `Move.toml` file can also be found in the [Package](https://accudo.dev/move/book/packages#movetoml) section of the Move book.
* Move also supports the idea of [named
  addresses](https://accudo.dev/move/book/address#named-addresses); Named
  ...
  that you can deploy, depending on what address(es) you control. They are used quite frequently, and can be defined in the `Move.toml` file in the `[addresses]` section, like so:
    ...
    ```
* [Structures](https://accudo.dev/move/book/structs-and-resources) in Move
  can be given different
  [abilities](https://accudo.dev/move/book/abilities) that describe what
...
* [Functions](https://accudo.dev/move/book/functions) are default
    private, and can also be `public`,
    [`public(friend)`](https://accudo.dev/move/book/friends).
    A function marked as `entry` can be called as a transaction.
* `move_to` is one of the [five different global storage
  operators](https://accudo.dev/move/book/global-storage-operators).
</details>
...
You can run the tests with the `accudo move test` command:
...
* There are a number of [test-related annotations](https://accudo.dev/move/book/unit-testing#testing-annotations-their-meaning-and-usage) that are worth exploring.
  You'll see some of these used in Step 5.

#### Exercises
* Change the assertion to `11` so that the test fails. Find a flag that you can
  pass to the `accudo move test` command that will dump the global state when
...
* Find a flag that allows you to gather test coverage information, and
  then experiment with using the `accudo move coverage` command to examine
...
> NOTE: The coin and balance interfaces are for illustration of Move concepts only. Accudo uses a different, richer [coin type](https://accudo.dev/reference/move?branch=mainnet&page=accudo-framework/doc/coin.md) contained in the Accudo framework.
...
```
Read more on Move function [visibilities](https://accudo.dev/move/book/functions#visibility).
</details>
<details>
...
if an [abort](https://accudo.dev/move/book/abort-and-assert) is raised no unwinding of state
needs to be performed, as no changes from that transaction will be persisted to the blockchain.

[`error` module]: https://github.com/accudo-labs/accudo-core/blob/main/accudo-move/framework/move-stdlib/sources/error.move
...
to get a mutable reference to the global storage, and `&mut` is used to create a mutable [reference](https://accudo.dev/move/book/references) to a field of a
...
To get started, run the `accudo move test` command in the [`step_5/basic_coin`](./step_5/basic_coin) directory:
...
In Move, we can use
[generics](https://accudo.dev/move/book/generics)
...
Read more about [phantom type](https://accudo.dev/move/book/generics#phantom-type-parameters) parameters in the Accudo Move Book.
</details>

## Step 7:  Use the Move Prover
    
> NOTE: Before running the Move Prover, ensure that the [Move Prover](https://accudo.dev/build/cli/setup-cli/install-move-prover/) and associated tools are installed.
...
[Move Prover](https://accudo.dev/move/prover/move-prover)
