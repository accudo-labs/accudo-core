...
module coin_listing {
    ...
    use accudo_framework::object::{Self, ConstructorRef, Object, ObjectCore};
    ...
    inline fun init<CoinType>(
        ...
        accudo_account::transfer_coins<CoinType>(
            seller,
    ...
    public entry fun purchase<CoinType>(
        ...
        let (price, type) = if (exists<AuctionListing<CoinType>>(listing_addr)) {
            ...
            if (option::is_some(&current_bid)) {
                ...
                accudo_account::deposit_coins(bidder, coins);
            } else {
    ...
    inline fun complete_purchase<CoinType>(
        ...
        if (royalty_charge != 0) {
            ...
            accudo_account::deposit_coins(royalty_addr, royalty);
        };
        ...
        accudo_account::deposit_coins(fee_schedule::fee_address(fee_schedule), commission);

        // Seller gets what is left
        accudo_account::deposit_coins(seller, coins);
...
module listing_tests {
    ...
    use accudo_framework::object::{Self, Object};
    use accudo_framework::timestamp;

    use accudo_token::token as tokenv1;

    use accudo_token_objects::token::Token;
    ...
    fun test_fixed_price(
        ...
        let (marketplace_addr, seller_addr, purchaser_addr) =
            test_utils::setup(accudo_framework, marketplace, seller, purchaser);

        let (token, fee_schedule, listing) = fixed_price_listing(marketplace, seller);

        assert!(coin::balance<AccudoCoin>(marketplace_addr) == 1, 0);
        assert!(coin::balance<AccudoCoin>(seller_addr) == 9999, 0);
        assert!(listing::listed_object(listing) == object::convert(token), 0);
        assert!(listing::fee_schedule(listing) == fee_schedule, 0);
        assert!(coin_listing::price<AccudoCoin>(listing) == option::some(500), 0);
        assert!(!coin_listing::is_auction<AccudoCoin>(listing), 0);

        coin_listing::purchase<AccudoCoin>(purchaser, listing);

        assert!(object::owner(token) == purchaser_addr, 0);
        assert!(coin::balance<AccudoCoin>(marketplace_addr) == 6, 0);
        assert!(coin::balance<AccudoCoin>(seller_addr) == 10494, 0);
        assert!(coin::balance<AccudoCoin>(purchaser_addr) == 9500, 0);
    }

    #[test(accudo_framework = @0x1, marketplace = @0x111, seller = @0x222, purchaser = @0x333)]
    fun test_fixed_price_high_royalty(
        ...
        assert!(coin::balance<AccudoCoin>(marketplace_addr) == 1, 0);
        assert!(coin::balance<AccudoCoin>(seller_addr) == 10499, 0);
        assert!(coin::balance<AccudoCoin>(purchaser_addr) == 9500, 0);
    }
    ...
    fun test_fixed_price_end(
        ...
        assert!(coin::balance<AccudoCoin>(marketplace_addr) == 1, 0);
        coin_listing::end_fixed_price<AccudoCoin>(seller, listing);
        assert!(coin::balance<AccudoCoin>(marketplace_addr) == 1, 0);
        assert!(coin::balance<AccudoCoin>(seller_addr) == 9999, 0);
    ...
    fun test_auction_purchase(
        ...
        assert!(coin_listing::price<AccudoCoin>(listing) == option::some(500), 0);
        assert!(coin_listing::is_auction<AccudoCoin>(listing), 0);
        assert!(coin_listing::starting_bid<AccudoCoin>(listing) == 100, 0);
        assert!(coin_listing::bid_increment<AccudoCoin>(listing) == 50, 0);
        assert!(coin_listing::auction_end_time<AccudoCoin>(listing) == timestamp::now_seconds() + 200, 0);
        assert!(coin_listing::minimum_bid_time_before_end<AccudoCoin>(listing) == 150, 0);
        assert!(coin_listing::current_amount<AccudoCoin>(listing) == option::none(), 0);
        assert!(coin_listing::current_bidder<AccudoCoin>(listing) == option::none(), 0);
    ...
    fun test_auction_bid_then_purchase(
        ...
        coin_listing::bid<AccudoCoin>(seller, listing, 100);
        assert!(coin_listing::current_amount<AccudoCoin>(listing) == option::some(100), 0);
        assert!(coin_listing::current_bidder<AccudoCoin>(listing) == option::some(seller_addr), 0);
        assert!(coin::balance<AccudoCoin>(marketplace_addr) == 3, 0);
        assert!(coin::balance<AccudoCoin>(seller_addr) == 9897, 0);

        // Return the bid and insert a new bid
        coin_listing::bid<AccudoCoin>(purchaser, listing, 150);
        assert!(coin_listing::current_amount<AccudoCoin>(listing) == option::some(150), 0);
        assert!(coin_listing::current_bidder<AccudoCoin>(listing) == option::some(purchaser_addr), 0);
        assert!(coin::balance<AccudoCoin>(marketplace_addr) == 5, 0);
        assert!(coin::balance<AccudoCoin>(seller_addr) == 9997, 0);
        assert!(coin::balance<AccudoCoin>(purchaser_addr) == 9848, 0);

        // Return the bid and replace with a purchase
        coin_listing::purchase<AccudoCoin>(purchaser, listing);
        assert!(object::owner(token) == purchaser_addr, 0);
        assert!(coin::balance<AccudoCoin>(marketplace_addr) == 10, 0);
        assert!(coin::balance<AccudoCoin>(purchaser_addr) == 9498, 0);
    }
    ...
    fun test_auction_bidding(
        ...
        assert!(coin_listing::auction_end_time<AccudoCoin>(listing) == end_time, 0);
        ...
        test_utils::increment_timestamp(150);
        coin_listing::complete_auction<AccudoCoin>(accudo_framework, listing);
        assert!(object::owner(token) == purchaser_addr, 0);
        assert!(coin::balance<AccudoCoin>(marketplace_addr) == 6, 0);
        assert!(coin::balance<AccudoCoin>(seller_addr) == 10146, 0);
    }
    ...
    fun test_not_started_fixed_price(
        ...
        let listing = coin_listing::init_fixed_price_internal<AccudoCoin>(
            seller,
    ...
    fun test_not_started_auction(
        ...
        let token = test_utils::mint_tokenv2(seller);
        let fee_schedule = test_utils::fee_schedule(marketplace);
        let listing = coin_listing::init_auction_internal<AccudoCoin>(
            seller,
            object::convert(token),
            fee_schedule,
            timestamp::now_seconds() + 1,
        ...
        coin_listing::bid<AccudoCoin>(purchaser, listing, 1000);
    }
    ...
    #[test(accudo_framework = @0x1, marketplace = @0x111, seller = @0x222, purchaser = @0x333)]
    #[expected_failure(abort_code = 0x10004, location = accudo_framework::fungible_asset)]
    ...
    fun test_not_enough_coin_auction_bid(
        ...
        let (_token, _fee_schedule, listing) = auction_listing(marketplace, seller);
        coin_listing::bid<AccudoCoin>(purchaser, listing, 100000);
    }
    ...
    fun test_bid_too_low(
        ...
        let (_token, _fee_schedule, listing) = auction_listing(marketplace, seller);
        coin_listing::bid<AccudoCoin>(purchaser, listing, 100);
        coin_listing::bid<AccudoCoin>(purchaser, listing, 125);
    }
    ...
    fun test_auction_view_on_fixed_price(
        ...
        coin_listing::auction_end_time<AccudoCoin>(listing);
    }
    ...
    fun test_bad_fixed_price_end(
        ...
        let (_token, _fee_schedule, listing) = fixed_price_listing(marketplace, seller);
        coin_listing::end_fixed_price<AccudoCoin>(purchaser, listing);
    }
    ...
    inline fun fixed_price_listing_for_tokenv1_with_token(
        ...
        let fee_schedule = test_utils::fee_schedule(marketplace);
        let listing = coin_listing::init_fixed_price_for_tokenv1_internal<AccudoCoin>(
            seller,
            creator_addr,
            collection_name,
            token_name,
    ...
    inline fun auction_listing_for_tokenv1(
        ...
        let (creator_addr, collection_name, token_name, property_version) =
            tokenv1::get_token_id_fields(&token_id);
        let fee_schedule = test_utils::fee_schedule(marketplace);
        let listing = coin_listing::init_auction_for_tokenv1_internal<AccudoCoin>(
            seller,
            creator_addr,
            collection_name,
            token_name,
