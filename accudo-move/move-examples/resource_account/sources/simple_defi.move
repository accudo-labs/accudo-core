...
/// (2) when exchanging coins, we call `exchange_to` to swap `AccudoCoin` to `ChloesCoin`, and `exchange_from` to swap `AccudoCoin` from `ChloesCoin`
module resource_account::simple_defi {
    use std::signer;
    use std::string;

    use accudo_framework::account;
    use accudo_framework::coin::{Self, Coin, MintCapability, BurnCapability};
    ...
    /// Exchange AccudoCoin to ChloesCoin
    public fun exchange_to(a_coin: Coin<AccudoCoin>): Coin<ChloesCoin> acquires ModuleData {
    ...
    /// Exchange ChloesCoin to AccudoCoin
    public fun exchange_from(c_coin: Coin<ChloesCoin>): Coin<AccudoCoin> acquires ModuleData {
        ...
        coin::withdraw<AccudoCoin>(&resource_signer, amount)
    }
    ...
    public entry fun exchange_to_entry(account: &signer, amount: u64) acquires ModuleData {
        let a_coin = coin::withdraw<AccudoCoin>(account, amount);
    ...
    #[test(origin_account = @0xcafe, resource_account = @0xc3bb8488ab1a5815a9d543d7e41b0e0df46a7396f89b22821f07a4362f75ddc5, framework = @accudo_framework)]
    public entry fun test_exchange_to_and_exchange_from(origin_account: signer, resource_account: signer, framework: signer) acquires ModuleData {
        ...
        let (accudo_coin_burn_cap, accudo_coin_mint_cap) = accudo_coin::initialize_for_test(&framework);
        set_up_test(&origin_account, &resource_account);

        // exchange from 5 accudo coins to 5 chloe's coins & assert the results are expected
        let five_a_coins = coin::mint(5, &accudo_coin_mint_cap);
        let c_coins = exchange_to(five_a_coins);
        assert!(coin::value(&c_coins) == 5, 0);
        assert!(coin::balance<AccudoCoin>(signer::address_of(&resource_account)) == 5, 1);
        assert!(coin::balance<ChloesCoin>(signer::address_of(&resource_account)) == 0, 2);

        // exchange from 5 chloe's coins to 5 accudo coins & assert the results are expected
        let a_coins = exchange_from(c_coins);
        assert!(coin::value(&a_coins) == 5, 0);
        assert!(coin::balance<AccudoCoin>(signer::address_of(&resource_account)) == 0, 3);
        ...
        coin::burn(a_coins, &accudo_coin_burn_cap);
        coin::destroy_mint_cap<AccudoCoin>(accudo_coin_mint_cap);
        coin::destroy_burn_cap<AccudoCoin>(accudo_coin_burn_cap);
    }
}
