module raffle::raffle_test {
    ...
    #[test_only]
    use accudo_framework::coin::MintCapability;

    #[test_only]
    use accudo_std::debug;
    ...
    #[test_only]
    use accudo_std::crypto_algebra::enable_cryptography_algebra_natives;
    #[test_only]
    use accudo_framework::randomness;

    #[test_only]
    fun give_coins(mint_cap: &MintCapability<AccudoCoin>, to: &signer) {
        let to_addr = signer::address_of(to);
        if (!account::exists_at(to_addr)) {
            account::create_account_for_test(to_addr);
        };
        coin::register<AccudoCoin>(to);
    ...
    fun test_raffle(
        ...
        let (burn_cap, mint_cap) = accudo_coin::initialize_for_test(&fx);

        // Create fake coins for users participating in raffle & initialize accudo_framework
        ...
        while (i < num_players) {
            ...
            if (player == winner) {
                assert!(coin::balance<AccudoCoin>(player) == raffle::get_ticket_price() * num_players, 1);
            } else {
                assert!(coin::balance<AccudoCoin>(player) == 0, 1);
            };
