...
import { useWallet, InputTransactionData } from "@accudo-labs/wallet-adapter-react";

import { MODULE_ADDRESS } from "./constants";
import { accudoClient } from "./utils/accudoClient";
...
function App() {
  ...
  const fetchList = async () => {
    if (!account) return [];
    try {
      const todoListResource = await accudoClient().getAccountResource({
      ...
      while (counter <= taskCounter) {
        ...
        const task = await accudoClient().getTableItem<Task>({handle:tableHandle, data:tableItem});
        tasks.push(task);
        counter++;
      }
  ...
  const addNewList = async () => {
    ...
    try {
      ...
      await accudoClient().waitForTransaction({transactionHash:response.hash});
      setAccountHasList(true);
    } catch (error: any) {
      setAccountHasList(false);
    }
