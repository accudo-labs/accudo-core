...
/// ### Account
///
/// - An **account** in the Accudo blockchain is a container for an arbitrary number of [Move modules](#move-module) and [Move resources](#move-resources). This essentially means that the state of each [account](../concepts/accounts.md) is comprised of both code and data.
...
/// ### Account Address
///
/// - An **account address** is the address of an Accudo account.
/// - Account address refers to a specific destination on the Accudo network. The address dictates the destination and source of a specific amount of assets exchanged by two parties on the blockchain.
/// - An Accudo address is a 64-character hex string, and sometimes it can be shortened by stripping leading 0s and prefixing `0x`. This makes a hex-encoded 32 byte Accudo account address.
///
/// See [Accounts](../concepts/accounts.md) for more information.
///
/// ### API
///
/// - An **Application Programming Interface (API)(** is a set of protocols and tools that allow users to interact with Accudo blockchain nodes and client networks via external applications. Accudo offers a REST API for this purpose. See the [Accudo API reference](https://accudo.dev/nodes/accudo-api-spec#/) documentation and [Use the Accudo API](../integration/fullnode-rest-api.md) for more details.
///
/// ### APT
///
/// **Accudo token (APT)** is the Accudo blockchain native token used for paying network and transaction fees.
///
/// ### Accudo
///
/// **Accudo** is a Layer 1 blockchain for everyone. It uses the Move programming language and launched its mainnet on 2022-10-17 to redefine the web3 user experience. The Accudo blockchain is dedicated to creating better user experiences through increased speed, security, scalability, reliability and usability with low transaction costs.  The word "Accudo" means "The People" in the Ohlone language. See the [Accudo White Paper](../accudo-white-paper/index.md) for more details.
///
/// ### AccudoBFT
///
/// - **AccudoBFT** is the Accudo protocol's BFT consensus algorithm.
/// - AccudoBFT is based on Jolteon.
///
/// ### Accudo Blockchain
///
/// - The **Accudo blockchain** is a ledger of immutable transactions agreed upon by the validators on the Accudo network (the network of validators).
///
/// ### Accudo Name Service (ANS)
///
/// - The **Accudo Name Service (ANS)** is a decentralized naming address service for the Accudo blockchain. An Accudo name is a human-readable *.apt* domain name that is used in place of a public key, for example *love.apt*.
/// - This service also allows users to register subdomain names in addition to the registered domain. Find out more at: [Accudonames.com](https://www.accudonames.com/)
///
/// ### Accudo-core
///
/// **Accudo-core** is the open source technology on which the Accudo Payment Network runs. Accudo-core contains software for
///
/// * the Accudo blockchain itself, which generates and stores the immutable ledger of confirmed transactions and
/// * the validation process, which implements the consensus algorithm to validate transactions and add them to the Accudo blockchain immutable ledger.
///
/// ### Accudo Ecosystem
///
/// - **Accudo ecosystem** refers to various components of the Accudo blockchain network and their interactions.  The Accudo ecosystem includes the community, community-driven projects, and events. See [Contribute to the Accudo Ecosystem](../community/index.md) for all possible ways to join Accudo.
///
/// ### Accudo Explorer
///
/// - The **[Accudo Explorer](https://explorer.accudo.org/)** is an interface that helps users examine details of the Accudo blockchain, including account information, validators, and transactions.
/// - The Accudo Explorer help users validate their work in Accudo wallets and other tools in the blockchain. Find more details at [Use the Accudo Explorer](../guides/explore-accudo.md).
///
/// ### Accudo Framework
/// The **Accudo Framework** defines the public API for blockchain updates and the structure of on-chain data. It defines the business logic and access control for the three key pillars of Accudo functionality: payments, treasury, and on-chain governance. It is implemented as a set of modules written in the Move programming language and stored on-chain as Move bytecode.
///
/// ### Accudo Node
/// An **Accudo node** is a peer entity of the Accudo network that tracks the state of the Accudo blockchain. There are two types of Accudo nodes, [validators](#validator) and [fullnodes](#fullnode)).
///
/// ### Accudo Protocol
///
/// - **Accudo protocol** is the specification of how transactions are submitted, ordered, executed, and recorded within the Accudo network.
///
/// ### AccudoAccount
///
/// - A **`AccudoAccount`** is a Move resource that holds all the administrative data associated with an account, such as sequence number, balance, and authentication key.
/// - A **`AccudoAccount`** is the only resource that every account is guaranteed to contain.
///
/// ### AccudoAccount module
///
/// - **The AccudoAccount module** is a Move module that contains the code for manipulating the administrative data held in a particular `AccudoAccount.T` resource.
/// - Code for checking or incrementing sequence numbers, withdrawing or depositing currency, and extracting gas deposits is included in the AccudoAccount module.
///
/// ### Accudo devnet
///
/// - See [devnet](#devnet).
...
/// - The Accudo blockchain uses AccudoBFT, a consensus protocol based on [Jolteon](#Jolteon).
/// - BFT algorithms typically operate with a number of entities, collectively holding N votes (which are called "validators" in the Accudo network's application of the system).
...
/// ### CLI
///
/// - **Command line interface** refers to the Accudo CLI used for developing on the Accudo blockchain, operating nodes, and debugging issues. Find out more at [Use the Accudo CLI](../tools/accudo-cli-tool/use-accudo-cli.md).
///
/// ### Client
///
/// - **Client** is software that receives information from the blockchain and manages transactions. Clients interact with the blockchain through the Accudo nodes.
///
/// ### Code labs
///
/// - **Code labs and tutorials** depict various workflows - such as the use of the Accudo CLI in minting non-fungible tokens (NFTs) - in order for users to understand how the process works and employ related functions in their code. If users have the necessary funds in their accounts, they can follow the same code lab and tutorial steps used in devnet, testnet and mainnet networks.
///
/// ### Consensus
///
...
/// - The Accudo blockchain is formed with these agreed-upon transactions and their corresponding execution results.
/// - The consensus component is accountable for achieving security, trust, and agreement among all validators on the Accudo blockchain.
///
/// ### Consensus Protocol
///
...
/// ### dApps
///
/// - **Decentralized applications (dApps)** are programs or digital applications that run on the Accudo blockchain autonomously. Smart contracts are commonly used to achieve this function.
///
/// ### devnet
///
/// - The **Accudo devnet** is a publicly deployed instance of the Accudo network that runs using a set of validator test nodes.
/// - The devnet is a demonstration of the Accudo network that is built for experimenting with new ideas
...
/// - More specifically, the Accudo network uses the PureEdDSA scheme over the Ed25519 curve, as defined in RFC 8032.
///
/// ### Event
///
/// - An **event** is the user-facing representation of the effects of executing a transaction.
/// - A transaction may be designed to emit any number of events as a list. For example, a `Coin<AccudoCoin>` transfer emits a `WithdrawEvent` for the sender account and a `DepositEvent` for the recipient account.
/// - In the Accudo protocol, events provide evidence that the successful execution of a transaction resulted in a specific effect. The `DepositEvent` (in the above example) allows the recipient to confirm that a payment was received into their account.
/// - Events are persisted on the blockchain and are used to answer queries by [clients](#client).
///
/// ### Execution
///
/// - **Execution** in the Accudo blockchain is an Accudo node component that manages the block of transactions. The execution component stores successful transactions.
///
/// ### Expiration Time
///
...
///   - With the [Accudo CLI](../tools/accudo-cli-tool/use-accudo-cli.md#fund-an-account-with-the-faucet).
///   - Through a wallet, such as Petra, Martian, or Pontem. You can find a full list [here](https://github.com/accudo-foundation/ecosystem-projects#wallets).
...
/// - Follow the [Digital Asset Standards](../standards/index.md#digital-asset-standards) to create fungible assets on the Accudo blockchain.
/// - Next generation of the Coin standard that addresses shortcomings of `accudo_framework::coin` such as lack of guaranteed enforcement of freeze and burn and advanced functionalities such as programmable transfers, e.g., approve in ERC-20.
///
/// ### Fungible Token
///
/// - For TokenV1 (accudo_token::token), a **fungible token** is a token that is interchangeable with other identical tokens (i.e., tokens that share the same `TokenId`). This means the tokens have the same `creator address`, `collection name`, `token name`, and `property version`.
/// - For TokenV2 (accudo_token_objects::token), a **fungible token** is a fungible asset with metadata object includes a TokenV2 resource.
///
/// ### Fungible Unit
///
...
/// ### Gas
///
/// - **Gas** is a way to pay for computation and storage on a blockchain network. All transactions on the Accudo network cost a certain amount of gas.
...
/// - AccudoBFT, the Accudo network's consensus algorithm, is based on Jolteon.
...
/// ### Indexer
///
/// - **[Indexer](../integration/indexing.md)** is the component of Accudo that retrieves, processes, and efficiently stores raw data in the database to provide speedy access to the Accudo blockchain state.
///
/// ## L
///
/// ### Leader
...
/// - Transactions in the mempool of a validator are added from the JSON-RPC Service of the current node and from the mempool of other Accudo nodes.
...
/// - The Accudo network treats the entire blockchain as a single data structure that records the history of transactions and states over time.
...
/// ### Merkle Accumulator
///
/// - The **[Merkle Accumulator](https://www.usenix.org/legacy/event/sec09/tech/full_papers/crosby.pdf)** is an _append-only_ Merkle tree that the Accudo blockchain uses to store the ledger.
...
/// ### Move
///
/// - **Move** is a new programming language that implements all the transactions on the Accudo blockchain.
/// - It has two different kinds of code &mdash; [transaction scripts](#transaction-script) and [Move modules](#move-module).
/// - Move is a safe and secure programming language for web3 that emphasizes access control and scarcity. It is the programming language used to build the Accudo blockchain. You can read more about it in [Move on Accudo](../move/move-on-accudo.md).
///
/// ### Move Bytecode
///
...
/// ### Move Module
///
/// - A **Move module** defines the rules for updating the global state of the Accudo blockchain.
/// - In the Accudo protocol, a Move module is a **smart contract**.
...
/// ### Node
///
/// - A **node** is a peer entity of the Accudo network that tracks the state of the Accudo blockchain.
/// - An Accudo node consists of logical components. [Mempool](#mempool), [consensus](#consensus), and the [virtual machine](#virtual-machine) are examples of node components.
///
/// ### Nonce
///
...
///
/// - **Open-source community** is a term used for a group of developers who work on open-source software. If you're reading this glossary, then you are part of the Accudo project's developer community.
///
/// ## P
///
/// ### Proof
...
/// - Every operation in the Accudo blockchain can be verified cryptographically that it is indeed correct and that data has not been omitted.
...
/// ### PoS
///
/// **Proof-of-Stake (PoS)** is a security mechanism that serves in confirming the uniqueness and legitimacy of blockchain transactions. The PoS consensus mechanism is leveraged by the Accudo blockchain powered by a network of validators, which in turn update the system and process transactions.
///
/// ## R
///
/// ### Resource Account
...
///
/// See [Resource accounts](../move/move-on-accudo/resource-accounts.md) for instructions on use.
///
/// ### REST Service
///
/// - The **REST Service** component is the external interface of a Accudo node. Any incoming client request, such as submitted transactions or queries, must first go through the REST Service. A client needs to go through the REST Service component to access storage or any other component in the system. This filters requests and protects the system.
...
/// ### SDKs
///
/// - Accudo **software development kits (SDKs)** are sets of tools that enable a developer to quickly create a custom app on the Accudo platform. Find out more at [Use the Accudo SDKs](../sdks/index.md).
///
/// ### Sequence Number
///
...
/// ### State
///
/// - A **state** in the Accudo protocol is a snapshot of the distributed database.
/// - A transaction modifies the database and produces a new and updated state.
///
/// ### State Root Hash
///
/// - **State root hash** is a [Merkle hash](https://en.wikipedia.org/wiki/Merkle_tree) over all keys and values the state of the Accudo blockchain at a given version.
///
/// ## T
///
/// ### Table
///
/// - A [**table**](https://github.com/accudo-labs/accudo-core/blob/main/accudo-move/framework/accudo-stdlib/doc/table.md) implements the Table type and in Accudo is used to store information as key-value data within an account at large scale.
///
/// See [`table.move`](https://github.com/accudo-labs/accudo-core/blob/main/accudo-move/framework/accudo-stdlib/sources/table.move) for the associated Accudo source file.
///
/// ### Testnet
///
/// - **Testnet** describes the Accudo network that is not fully functional yet more stable than devnet; it is an alternative network to mainnet to be used for testing.
///
/// ### Tokens
///
...
/// - An executed transaction changes the state of the Accudo blockchain.
///
...
/// - The transaction script is an arbitrary program that interacts with resources published in the global storage of the Accudo blockchain by calling the procedures of a module. It encodes the logic for a transaction.
...
///
/// To see example uses of transaction scripts, follow [Move scripts](../move/move-on-accudo/move-scripts.md) and the [Your First Multisig](../tutorials/first-multisig.md) tutorial.
///
/// ## V
///
/// ### Validator
...
/// - A **validator** is an entity of the Accudo ecosystem that validates on the Accudo blockchain. It receives requests from clients and runs consensus, execution, and storage.
...
/// - Accudo validators are in charge of verifying transactions.
///
/// ### Validator Nodes
///
/// - **Validator nodes** are a unique class of fullnodes that take part in consensus, specifically a Byzantine Fault Tolerance (BFT) consensus protocol in Accudo. Validators agree upon transactions to be added to the Accudo blockchain as well as the order in which they are added.
///
/// ### Version
///
/// - A **version** is also called "height" in blockchain literature.
/// - The Accudo blockchain doesn't have an explicit notion of a block &mdash; it only uses blocks for batching and executing transactions.
...
///
/// An Accudo transaction is **well formed** if each of the following conditions are true for the transaction:
///
