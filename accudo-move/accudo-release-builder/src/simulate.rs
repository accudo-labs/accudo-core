...
use accudo_vm::{data_cache::AsMoveResolver, move_vm_ext::SessionId, AccudoVM};
 ...
 *
 **************************************************************************************************/
static MODULE_ID_ACCUDO_GOVERNANCE: Lazy<ModuleId> = Lazy::new(|| {
    ModuleId::new(
        AccountAddress::ONE,
        Identifier::new("accudo_governance").unwrap(),
    )
});
...
/// Patches `accudo_framework::accudo_governance::resolve_multi_step_proposal` so that
/// it returns the requested signer directly, skipping the governance process altogether.
fn patch_accudo_governance(
    state_view: &impl SimulationStateStore,
    forbid_next_execution_hash: bool,
) -> Result<()> {
    use Bytecode::*;

    patch_module(state_view, &MODULE_ID_ACCUDO_GOVERNANCE, |m| {
...
pub async fn simulate_multistep_proposal(
    ...
    let mut client_builder = Client::builder(AccudoBaseUrl::Custom(remote_url));
    ...
    let debugger =
        AccudoDebugger::rest_client(client.clone()).context("failed to create AccudoDebugger")?;
    ...
    std::io::stdout().flush()?;
    let mut rng = accudo_keygen::KeyGen::from_seed([0; 32]);
        ...
        proposal_scripts.iter().zip(compiled_scripts).enumerate()
    {
        ...
        patch_accudo_governance(&state_view, forbid_next_execution_hash)
