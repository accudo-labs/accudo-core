...
pub struct FrameworkReleaseConfig {
    ...
    /// If set to None, we will use the accudo framework under current repo.
...
pub fn generate_upgrade_proposals(
    ...
    const ACCUDO_GIT_PATH: &str = "https://github.com/accudo-labs/accudo-core.git";

    // NOTE: This is skipping 0x7 (accudo-experimental package) which is only meant to be released
    // to devnet (or local testnet) via the genesis process and never released/upgraded in testnet
    // or mainnet.
    let mut package_path_list = [
        ("0x1", "accudo-move/framework/move-stdlib"),
        ("0x1", "accudo-move/framework/accudo-stdlib"),
        ("0x1", "accudo-move/framework/accudo-framework"),
        ("0x3", "accudo-move/framework/accudo-token"),
        ("0x4", "accudo-move/framework/accudo-token-objects"),
    ];
    ...
    let commit_info = if let Some(revision) = &config.git_hash {
        ...
        let repository = Repository::clone(ACCUDO_GIT_PATH, temp_root_path.path())?;
        ...
        accudo_build_info::get_git_hash()
    };
    ...
    for (publish_addr, relative_package_path) in package_path_list.iter() {
        ...
        // 0-move-stdlib.move	2-accudo-framework.move	4-gas-schedule.move	6-features.move
        // 1-accudo-stdlib.move	3-accudo-token.move	5-version.move		7-consensus-config.move
        // The first framework file being generated is 3-accudo-token.move. It's using the next_execution_hash being passed in (so in this case, the hash of 4-gas-schedule.move being passed in mod.rs).
        // The second framework file being generated would be 2-accudo-framework.move, and it's using the hash of 3-accudo-token.move (which would be result.last()).
