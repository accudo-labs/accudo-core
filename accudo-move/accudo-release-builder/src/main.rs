...
pub struct Argument {
    ...
    cmd: Commands,
    #[clap(long)]
    accudo_core_path: Option<PathBuf>,
}

// TODO(vgao1996): unify with `ReplayNetworkSelection` in the `accudo` crate.
#[derive(Clone, Debug)]
...
impl NetworkSelection {
    fn to_url(&self) -> anyhow::Result<Url> {
        ...
        let s = match &self {
            Mainnet => "https://fullnode.mainnet.accudo.org",
            Testnet => "https://fullnode.testnet.accudo.org",
            Devnet => "https://fullnode.devnet.accudo.org",
...
pub enum Commands {
    ...
    PrintConfigs {
        /// Url endpoint for the desired network. e.g: https://fullnode.mainnet.accudo.org/v1.
        #[clap(short, long)]
        endpoint: url::Url,
...
pub enum InputOptions {
    FromDirectory {
        /// Path to the localnet folder. If you are running localnet via cli, it should be `.accudo/testnet`.
        #[clap(short, long)]
        test_dir: PathBuf,
    },
    FromArgs {
...
async fn main() -> anyhow::Result<()> {
    ...
    initialize_accudo_core_path(args.accudo_core_path.clone());
    ...
    match args.cmd {
        Commands::GenerateProposals {
            ...
            accudo_release_builder::ReleaseConfig::load_config(release_config.as_path())
        ...
        Commands::WriteDefault { output_path } => {
            accudo_release_builder::ReleaseConfig::default().save_config(output_path.as_path())
        },
        Commands::ValidateProposals {
            ...
            let root_key_path = accudo_temppath::TempPath::new();
            root_key_path.create_as_file()?;

            let mut network_config = match input_option {
                InputOptions::FromDirectory { test_dir } => {
                    accudo_release_builder::validate::NetworkConfig::new_from_dir(
                        endpoint.clone(),
                        test_dir.as_path(),
                    )?
                },
                InputOptions::FromArgs {
                    ...
                    accudo_release_builder::validate::NetworkConfig {
                        root_key_path,
                        validator_account,
                        validator_key,
            ...
            if mint_to_validator {
                let mut client = Client::builder(AccudoBaseUrl::Custom(endpoint));
            ...
            network_config
                ...
                .await?;
            accudo_release_builder::validate::validate_config_and_generate_release(
                config,
        ...
        Commands::GenerateGasSchedule {
            ...
            let gas_schedule = accudo_gas_schedule_updator::current_gas_schedule(version);
        ...
        Commands::PrintConfigs {
            ...
            use accudo_types::on_chain_config::*;
            ...
            println!(
                ...
                serde_yaml::to_string(
                    &accudo_release_builder::components::feature_flags::Features::from(&features)
                )?
            );
