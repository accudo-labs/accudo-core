...
use crate::{assert_success, AccudoPackageHooks};
use accudo_cached_packages::accudo_stdlib;
use accudo_framework::{natives::code::PackageMetadata, BuildOptions, BuiltPackage};
use accudo_gas_profiling::TransactionGasLog;
use accudo_gas_schedule::{
    AccudoGasParameters, FromOnChainGasSchedule, InitialGasSchedule, ToOnChainGasSchedule,
};
...
use accudo_rest_client::AccudoBaseUrl;
...
impl<O: OutputLogger> MoveHarnessImpl<O> {
    ...
    pub fn new() -> Self {
        register_package_hooks(Box::new(AccudoPackageHooks {}));
        Self {
    ...
    ///
    /// If you hit rate limits, you can create a free Accudo Build account and generate an API key:
    /// - https://build.accudo.org/docs/start#api-quick-start
    fn new_with_remote_state_impl(
        network_url: AccudoBaseUrl,
        txn_id: u64,
        api_key: Option<&str>,
    ) -> Self {
        register_package_hooks(Box::new(AccudoPackageHooks {}));
        ...
        let gas_params = AccudoGasParameters::from_on_chain_gas_schedule(
    ...
    /// Creates a [`MoveHarness`] from a remote network state at the version specified by the
    /// transaction id.
    pub fn new_with_remote_state(network_url: AccudoBaseUrl, txn_id: u64) -> Self {
    ...
    pub fn new_account_with_balance_at(&mut self, addr: AccountAddress, balance: u64) -> Account {
        ...
        // Mint the account 10M Accudo coins (with 8 decimals).
    ...
    /// Gets the account where the Accudo framework is installed (0x1).
    pub fn accudo_framework_account(&mut self) -> Account {
    ...
    pub fn create_publish_built_package(
        ...
        self.create_transaction_payload(
            account,
            accudo_stdlib::code_publish_package_txn(
    ...
    pub fn create_object_code_deployment_built_package(
        ...
        self.create_transaction_payload(
            account,
            accudo_stdlib::object_code_deployment_publish(
                bcs::to_bytes(&metadata).expect("PackageMetadata has BCS"),
                code,
            ),
        )
    ...
    pub fn create_object_code_upgrade_built_package(
        ...
        self.create_transaction_payload(
            account,
            accudo_stdlib::object_code_deployment_upgrade(
                bcs::to_bytes(&metadata).expect("PackageMetadata has BCS"),
                code,
                code_object,
            ),
    ...
    pub fn object_code_freeze_code_object(
        ...
        let txn = self.create_transaction_payload(
            account,
            accudo_stdlib::object_code_deployment_freeze_code_object(code_object),
        );
        self.run(txn)
    }
    ...
    pub fn read_accudo_balance(&self, addr: &AccountAddress) -> u64 {
        self.read_resource::<CoinStoreResource<AccudoCoinType>>(
            addr,
            CoinStoreResource::<AccudoCoinType>::struct_tag(),
        )
        .map(|c| c.coin())
        .unwrap_or(0)
            + self
                .read_resource_from_resource_group::<FungibleStoreResource>(
                    &accudo_types::account_config::fungible_store::primary_apt_store(*addr),
    ...
    pub fn enable_features(&mut self, enabled: Vec<FeatureFlag>, disabled: Vec<FeatureFlag>) {
        let acc = self.accudo_framework_account();
        self.executor
            .enable_features(acc.address(), enabled, disabled);
    }

    fn override_one_gas_param(&mut self, param: &str, param_value: u64) {
        // TODO: The AccudoGasParameters::zeros() schedule doesn't do what we want, so
        ...
        let entries = AccudoGasParameters::initial()
            .to_on_chain_gas_schedule(accudo_gas_schedule::LATEST_GAS_FEATURE_VERSION);
        let entries = entries
            .into_iter()
            .map(|(name, val)| {
                if name == param {
        ...
        let gas_schedule = GasScheduleV2 {
            feature_version: accudo_gas_schedule::LATEST_GAS_FEATURE_VERSION,
            entries,
        };
        ...
        self.executor
            .exec("accudo_governance", "force_end_epoch", vec![], vec![
                core_signer_arg,
            ]);
    }
    ...
    pub fn modify_gas_schedule(&mut self, modify: impl FnOnce(&mut AccudoGasParameters)) {
    ...
    pub fn get_gas_params(&self) -> (u64, AccudoGasParameters) {
        let gas_schedule: GasScheduleV2 = self.get_gas_schedule();
        let feature_version = gas_schedule.feature_version;
        let params = AccudoGasParameters::from_on_chain_gas_schedule(
            &gas_schedule.into_btree_map(),
            feature_version,
        )
        .unwrap();
...
macro_rules! enable_golden {
    ($h:expr) => {
        $h.internal_set_golden(
            std::file!(),
            accudo_language_e2e_tests::current_function_name!(),
        )
    };
}
...
macro_rules! assert_success {
    ($s:expr $(,)?) => {{
        assert_eq!($s, accudo_types::transaction::TransactionStatus::Keep(
            accudo_types::transaction::ExecutionStatus::Success))
    }};
    ($s:expr, $($arg:tt)+) => {{
        assert_eq!(
            $s,
            accudo_types::transaction::TransactionStatus::Keep(
                accudo_types::transaction::ExecutionStatus::Success),
            $($arg)+
        )
    }};
...
macro_rules! assert_out_of_gas {
    ($s:expr $(,)?) => {{
        assert_eq!($s, accudo_types::transaction::TransactionStatus::Keep(
            accudo_types::transaction::ExecutionStatus::OutOfGas))
    }};
    ($s:expr, $($arg:tt)+) => {{
        assert_eq!(
            $s,
...
macro_rules! assert_abort {
    ...
    ($s:expr, $c:ident $(,)?) => {{
        assert!(matches!(
            $s,
            accudo_types::transaction::TransactionStatus::Keep(
                accudo_types::transaction::ExecutionStatus::MoveAbort { code, .. }
            )
            if code == $c,
        ));
    }};
    ($s:expr, $c:pat $(,)?) => {{
        assert!(matches!(
            $s,
            accudo_types::transaction::TransactionStatus::Keep(
                accudo_types::transaction::ExecutionStatus::MoveAbort { code: $c, .. }
            ),
        ));
    }};
...
#[cfg(test)]
mod tests {
    use accudo_types::transaction::{ExecutionStatus, TransactionStatus};
