...
fn exchange_e2e_test() {
    ...
    let result = h.run_transaction_payload(
        &origin_account,
        accudo_stdlib::resource_account_create_resource_account_and_publish_package(
            vec![],
    ...
    assert_coin_balance(
        ...
        ACCUDO_COIN_STRUCT_STRING,
        20,
    );

    // swap from 5 accudo coins to 5 chloe's coins
    ...
    assert_coin_balance(&mut h, &resource_address, ACCUDO_COIN_STRUCT_STRING, 5);
    assert_coin_balance(&mut h, &resource_address, CHLOE_COIN_STRUCT_STRING, 0);

    // swap to 3 accudo coins from 3 chloe's accudo coins
    ...
    assert_coin_balance(&mut h, &resource_address, ACCUDO_COIN_STRUCT_STRING, 2);
    assert_coin_balance(&mut h, &resource_address, CHLOE_COIN_STRUCT_STRING, 0);
}
