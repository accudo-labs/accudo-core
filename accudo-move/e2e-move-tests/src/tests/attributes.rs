...
use accudo_types::{
    ...
    vm::module_metadata::{KnownAttribute, RuntimeModuleMetadataV1, ACCUDO_METADATA_KEY_V1},
};
...
fn test_bad_fun_attribute_in_compiled_module() {
    ...
    let metadata = Metadata {
        key: ACCUDO_METADATA_KEY_V1.to_vec(),
...
fn test_bad_view_attribute_in_compiled_module() {
    ...
    let result = h.run_transaction_payload(
        &account,
        accudo_stdlib::code_publish_package_txn(metadata, code),
    );
