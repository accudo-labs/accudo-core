...
use accudo_language_e2e_tests::account::TransactionBuilder;
use accudo_types::{
...
fn test_script_with_type_parameter() {
    ...
    let package = BuiltPackage::build(
        ...
        accudo_framework::BuildOptions::default(),
    )
    .expect("building package must succeed");
...
fn test_script_with_signer_parameter() {
    ...
    assert_eq!(
        ...
        TransactionStatus::Keep(
            accudo_types::transaction::ExecutionStatus::MiscellaneousError(Some(
                accudo_types::vm_status::StatusCode::INVALID_MAIN_FUNCTION_SIGNATURE
            ))
        )
    );
}
