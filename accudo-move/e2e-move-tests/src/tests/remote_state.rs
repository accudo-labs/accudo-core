...
use accudo_framework::BuildOptions;
use accudo_rest_client::AccudoBaseUrl;
use accudo_types::{account_address::AccountAddress, on_chain_config::FeatureFlag};
use move_core_types::{language_storage::TypeTag, value::MoveValue};
use move_model::metadata::LanguageVersion;
use std::str::FromStr;

const ACCUDO_COIN_STRUCT_STRING: &str = "0x1::accudo_coin::AccudoCoin";
...
fn get_account_apt_balance(h: &mut MoveHarness, addr: AccountAddress) -> u64 {
    let bytes = h
        .execute_view_function(
            ...
            vec![TypeTag::from_str(ACCUDO_COIN_STRUCT_STRING).unwrap()],
            vec![addr.to_vec()],
        )
        .values
        .unwrap()
...
async fn view_existing_account_balance() {
    ...
    let mut h = MoveHarness::new_with_remote_state(AccudoBaseUrl::Testnet, TESTNET_TXN_VERSION);
...
async fn transfer_to_existing_account() {
    ...
    let status = h.run_entry_function(
        ...
        vec![TypeTag::from_str("0x1::accudo_coin::AccudoCoin").unwrap()],
        vec![
...
async fn test_option() {
    let mut h = MoveHarness::new_with_remote_state(AccudoBaseUrl::Testnet, 6691904943);
    h.enable_features(
        vec![
    ...
    let build_options = accudo_framework::BuildOptions {
