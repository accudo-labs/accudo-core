...
use accudo_crypto::{hash::CryptoHash, SigningKey};
use accudo_framework::BuiltPackage;
use accudo_language_e2e_tests::account::{Account, AccountPublicKey, TransactionBuilder};
use accudo_types::{
...
/// Initializes an Accudo VM and sets the keyless configuration via script.
...
fn spend_keyless_account(
    ...
    let payload = accudo_stdlib::accudo_coin_transfer(recipient, 1);
