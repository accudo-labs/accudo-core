...
fn test_basic_fungible_token() {
    ...
    let root = h.accudo_framework_account();

    let mut build_options = accudo_framework::BuildOptions::default();
    build_options
        .named_addresses
...
fn test_prologue_speculation() {
    ...
    let sink_txn = harness.create_transaction_payload(
        &independent_account,
        accudo_account_batch_transfer(vec![AccountAddress::random(); 50], vec![10_000_000_000; 50]),
    );
    ...
    let transfer_1_txn =
        harness.create_transaction_payload(&dst_1, accudo_account_transfer(*dst_2.address(), 1));
    let transfer_2_txn =
        harness.create_transaction_payload(&dst_2, accudo_account_transfer(*dst_3.address(), 1));
    let transfer_3_txn =
        harness.create_transaction_payload(&dst_3, accudo_account_transfer(*dst_1.address(), 1));
