...
use crate::{
    check_accudo_packages_availability, compile_accudo_packages, compile_package,
...
fn add_accudo_packages_to_state_store(
    state_store: &impl SimulationStateStore,
...
impl Execution {
    ...
    pub async fn execute_txns(&self, begin: Version, num_txns_to_execute: u64) -> Result<()> {
        let accudo_commons_path = self.input_path.join(ACCUDO_COMMONS);
        if !check_accudo_packages_availability(accudo_commons_path.clone()) {
            return Err(anyhow::Error::msg("accudo packages are missing"));
        }

        let mut compiled_cache = CompilationCache::default();
        if self.execution_mode.is_v1_or_compare() {
            compile_accudo_packages(
                &accudo_commons_path,
    ...
    fn execute_one_txn(
        ...
        if let Some(txn_idx) = data_manager.get_txn_index(cur_version) {
            ...
            if txn_idx.package_info.is_compilable()
                && !is_accudo_package(&txn_idx.package_info.package_name)
            {
    ...
    pub(crate) fn execute_and_compare(
        ...
        debugger: Option<Arc<dyn AccudoValidatorInterface + Send>>,
    ) {
    ...
    fn execute_code(
        ...
        txn: &Transaction,
        compiled_package_cache: &HashMap<PackageInfo, HashMap<ModuleId, Vec<u8>>>,
        debugger_opt: Option<Arc<dyn AccudoValidatorInterface + Send>>,
        v2_flag: bool,
    ) -> Result<(WriteSet, Vec<ContractEvent>), VMStatus> {
        // Always add Accudo (0x1) packages.
        add_accudo_packages_to_state_store(&state, compiled_package_cache);
