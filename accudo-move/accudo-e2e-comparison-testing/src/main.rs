...
pub enum Cmd {
    ...
    Dump {
        /// Endpoint url to obtain the txn data, e.g. `https://api.mainnet.accudo.org/v1` for mainnet.
        /// To avoid rate limiting, users need to apply for API key from `https://developers.accudo.org/`
    ...
    Online {
        ...
        /// e.g. `https://api.mainnet.accudo.org/v1` for mainnet.
        /// To avoid rate limiting, users need to apply for API key from `https://developers.accudo.org/`
        /// and set the env variable X_API_KEY using the obtained key
        endpoint: String,
        /// Path to the dumped data
...
async fn main() -> Result<()> {
    ...
    match args.cmd {
        Cmd::Dump {
            ...
            if !skip_source_code {
                prepare_accudo_packages(output.join(ACCUDO_COMMONS)).await;
            }
        ...
        Cmd::Execute {
            ...
            prepare_accudo_packages(input.join(ACCUDO_COMMONS)).await;
