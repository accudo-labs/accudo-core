...
use accudo_framework::{
    natives::code::PackageMetadata, unzip_metadata_str, BuiltPackage, ACCUDO_PACKAGES,
};
...
const ACCUDO_PACKAGES_DIR_NAMES: [&str; 5] = [
    "accudo-framework",
    "move-stdlib",
    "accudo-stdlib",
    "accudo-token",
    "accudo-token-objects",
];
...
pub const ACCUDO_COMMONS: &str = "accudo-commons";
...
fn is_accudo_package(package_name: &str) -> bool {
    ACCUDO_PACKAGES.contains(&package_name)
}

fn get_accudo_dir(package_name: &str) -> Option<&str> {
    if is_accudo_package(package_name) {
        for i in 0..ACCUDO_PACKAGES.len() {
            if ACCUDO_PACKAGES[i] == package_name {
                return Some(ACCUDO_PACKAGES_DIR_NAMES[i]);
            }
        }
    }
    None
...
async fn download_accudo_packages(path: &Path) -> anyhow::Result<()> {
    let git_url = "https://github.com/accudo-labs/accudo-core";
    let tmp_dir = TempDir::new()?;
    Command::new("git")
        .args(["clone", git_url, tmp_dir.path().to_str().unwrap()])
        .output()
        .map_err(|_| anyhow::anyhow!("Failed to clone Git repository"))?;
    let source_framework_path = PathBuf::from(tmp_dir.path()).join("accudo-move/framework");
    for package_name in ACCUDO_PACKAGES {
        let source_framework_path =
            source_framework_path.join(get_accudo_dir(package_name).unwrap());
        let target_framework_path = PathBuf::from(path).join(get_accudo_dir(package_name).unwrap());
        Command::new("cp")
            .arg("-r")
...
fn check_accudo_packages_availability(path: PathBuf) -> bool {
    if !path.exists() {
        return false;
    }
    for package in ACCUDO_PACKAGES {
        if !path.join(get_accudo_dir(package).unwrap()).exists() {
            return false;
        }
    }
    true
...
pub async fn prepare_accudo_packages(path: PathBuf) {
    ...
    if success {
        ...
        download_accudo_packages(&path).await.unwrap();
    }
}
...
fn compile_accudo_packages(
    accudo_commons_path: &Path,
    compiled_package_map: &mut HashMap<PackageInfo, HashMap<ModuleId, Vec<u8>>>,
    v2_flag: bool,
) -> anyhow::Result<()> {
    for package in ACCUDO_PACKAGES {
        let root_package_dir = accudo_commons_path.join(get_accudo_dir(package).unwrap());
        let compiler_version = if v2_flag {
            Some(CompilerVersion::latest_stable())
        } else {
            Some(CompilerVersion::V1)
        };
        // For simplicity, all packages including accudo token are stored under 0x1 in the map
...
fn compile_package(
    ...
    let mut build_options = accudo_framework::BuildOptions {
    ...
    for manifest_dep in manifest_deps {
        ...
        for pack_dep in &root_package_metadata.deps {
            ...
            if pack_dep_name == manifest_dep_name {
                if is_accudo_package(&pack_dep_name) {
                    fix_manifest_dep(
                        dep,
                        &format!(
                            "{}/{}",
                            ACCUDO_COMMONS,
                            get_accudo_dir(&pack_dep_name).unwrap()
                        ),
                    );
                    break;
                }
