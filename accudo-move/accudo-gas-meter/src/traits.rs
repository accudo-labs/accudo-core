...
use accudo_gas_algebra::{
    AbstractValueSize, Fee, FeePerGasUnit, Gas, GasExpression, GasScalingFactor, Octa,
};
use accudo_gas_schedule::{gas_feature_versions::RELEASE_V1_30, VMGasParameters};
use accudo_types::{
...
/// Trait representing a gas meter used inside the Accudo VM.
///
/// It extends Move VM's `GasMeter` trait with a few Accudo-specific callbacks, along with
/// some primitives that would allow gas meters to be composable.
pub trait AccudoGasMeter: MoveGasMeter {
...
pub trait CacheValueSizes: AccudoGasMeter {
