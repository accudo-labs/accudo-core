...
use accudo_validator_interface::{
    AccudoValidatorInterface, DBDebuggerInterface, DebuggerStateView, RestDebuggerInterface,
};
use accudo_vm::{
    accudo_vm::AccudoVMBlockExecutor, data_cache::AsMoveResolver, AccudoVM, VMBlockExecutor,
};
...
pub struct AccudoDebugger {
    debugger: Arc<dyn AccudoValidatorInterface + Send>,
}

impl AccudoDebugger {
    pub fn new(debugger: Arc<dyn AccudoValidatorInterface + Send>) -> Self {
        Self { debugger }
    }
...
fn execute_block_no_limit(
    ...
    let executor = AccudoVMBlockExecutor::new();
    executor
