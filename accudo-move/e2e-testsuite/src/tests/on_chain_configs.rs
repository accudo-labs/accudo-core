...
use accudo_language_e2e_tests::{common_transactions::peer_to_peer_txn, executor::FakeExecutor};
use accudo_types::{
    account_config::CORE_CODE_ADDRESS,
    on_chain_config::{AccudoVersion, OnChainConfig},
    transaction::TransactionStatus,
};
use accudo_vm::data_cache::AsMoveResolver;

#[test]
fn initial_accudo_version() {
    let mut executor = FakeExecutor::from_head_genesis();
    let resolver = executor.get_state_view().as_move_resolver();
    let version = accudo_types::on_chain_config::ACCUDO_MAX_KNOWN_VERSION;

    assert_eq!(AccudoVersion::fetch_config(&resolver).unwrap(), version);
    let account = executor.new_account_at(CORE_CODE_ADDRESS);
    let txn_0 = account
        .transaction()
        .payload(accudo_stdlib::version_set_for_next_epoch(version.major + 1))
        .sequence_number(0)
        .sign();
    let txn_1 = account
        .transaction()
        .payload(accudo_stdlib::accudo_governance_force_end_epoch())
        .sequence_number(1)
        .sign();
    executor.new_block();
    ...
    let resolver = executor.get_state_view().as_move_resolver();
    assert_eq!(
        AccudoVersion::fetch_config(&resolver).unwrap(),
        AccudoVersion {
