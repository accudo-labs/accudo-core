module accudo_std::type_info {
    ...
    /// Returns the current chain ID, mirroring what `accudo_framework::chain_id::get()` would return, except in `#[test]`
    /// functions, where this will always return `4u8` as the chain ID, whereas `accudo_framework::chain_id::get()` will
    /// return whichever ID was passed to `accudo_framework::chain_id::initialize_for_test()`.
    public fun chain_id(): u8 {
        if (!features::accudo_stdlib_chain_id_enabled()) {
    ...
    fun test_type_of() {
        ...
        assert!(type_info.account_address() == @accudo_std, 0);
    ...
    fun test_chain_id(fx: signer) {
        ...
        features::change_feature_flags_for_testing(&fx, vector[features::get_accudo_stdlib_chain_id_feature()], vector[]);
    ...
    fun verify_type_of() {
        ...
        spec {
            assert account_address == @accudo_std;
