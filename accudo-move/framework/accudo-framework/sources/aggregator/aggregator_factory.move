...
module accudo_framework::aggregator_factory {
    use std::error;
    ...
    friend accudo_framework::optional_aggregator;
    ...
    public(friend) fun create_aggregator_internal(): Aggregator acquires AggregatorFactory {
        assert!(
            exists<AggregatorFactory>(@accudo_framework),
            error::not_found(EAGGREGATOR_FACTORY_NOT_FOUND)
        );

        let aggregator_factory = borrow_global_mut<AggregatorFactory>(@accudo_framework);
    ...
    public fun create_aggregator(account: &signer, limit: u128): Aggregator acquires AggregatorFactory {
        // deprecated. Currently used only in accudo-move/e2e-move-tests/src/tests/aggregator.data/pack/sources/aggregator_test.move

        // Only Accudo Framework (0x1) account can call this for now.
    ...
    #[test_only]
    public fun initialize_aggregator_factory_for_test(accudo_framework: &signer) {
        initialize_aggregator_factory(accudo_framework);
    }
