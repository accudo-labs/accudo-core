spec accudo_framework::aggregator_factory {
    use accudo_framework::aggregator;
    /// <high-level-req>
    /// No.: 1
    /// Requirement: During the module's initialization, it guarantees that the Accudo framework is the caller and that the
    /// AggregatorFactory resource will move under the Accudo framework account.
    /// Criticality: High
    ...
    /// resource transfers to the Accudo framework account.
    ...
    /// under the Accudo account.
    /// Criticality: High
    /// Implementation: The create_aggregator_internal function asserts that AggregatorFactory exists for the Accudo
    /// account.
    /// Enforcement: Formally verified via [high-level-req-2](CreateAggregatorInternalAbortsIf).
    ///
    /// No.: 3
    /// Requirement: Only the Accudo framework address may create an aggregator instance currently.
    /// Criticality: Low
    /// Implementation: The create_aggregator function ensures that the address calling it is the Accudo framework
    /// address.
    ...
    spec initialize_aggregator_factory(accudo_framework: &signer) {
        use std::signer;
    ...
    spec schema CreateAggregatorInternalAbortsIf {
        aborts_if !exists<AggregatorFactory>(@accudo_framework);
    }
