...
module accudo_framework::storage_gas {
    ...
    public(friend) fun set_config(accudo_framework: &signer, config: StorageGasConfig) acquires StorageGasConfig {
        system_addresses::assert_accudo_framework(accudo_framework);
        *borrow_global_mut<StorageGasConfig>(@accudo_framework) = config;
    }
    ...
    public fun initialize(accudo_framework: &signer) {
        system_addresses::assert_accudo_framework(accudo_framework);
        assert!(
            !exists<StorageGasConfig>(@accudo_framework),
        ...
        move_to(accudo_framework, StorageGasConfig {
            item_config,
            byte_config,
        });

        assert!(
            !exists<StorageGas>(@accudo_framework),
            error::already_exists(ESTORAGE_GAS)
        );
        move_to(accudo_framework, StorageGas {
            per_item_read: 300 * k,
            per_item_create: 5 * m,
    ...
    public(friend) fun on_reconfig() acquires StorageGas, StorageGasConfig {
        ...
        let gas_config = borrow_global<StorageGasConfig>(@accudo_framework);
        let gas = borrow_global_mut<StorageGas>(@accudo_framework);
    ...
    fun test_initialize_and_reconfig(framework: signer) acquires StorageGas, StorageGasConfig {
        ...
        on_reconfig();
        let gas_parameter = borrow_global<StorageGas>(@accudo_framework);
