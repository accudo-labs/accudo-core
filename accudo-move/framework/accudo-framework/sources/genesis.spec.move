spec accudo_framework::genesis {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: All the core resources and modules should be created during genesis and owned by the Accudo framework
    /// account.
    ...
    /// formally verified by a post condition to ensure that all the critical resources are owned by the Accudo framework.
    /// Enforcement: Formally verified via [high-level-req-1](initialize).
    ///
    /// No.: 2
    /// Requirement: Addresses ranging from 0x0 - 0xa should be reserved for the framework and part of accudo governance.
    /// Criticality: Critical
    ...
    ///
    /// No.: 3
    /// Requirement: The Accudo coin should be initialized during genesis and only the Accudo framework account should own
    /// the mint and burn capabilities for the APT token.
    /// Criticality: Critical
    /// Implementation: Both mint and burn capabilities are wrapped inside the stake::AccudoCoinCapabilities and
    /// transaction_fee::AccudoCoinCapabilities resources which are stored under the accudo framework account.
    /// Enforcement: Formally verified via [high-level-req-3](initialize_accudo_coin).
    ///
    /// No.: 4
    ...
    spec initialize {
        ...
        // property 2: Addresses ranging from 0x0 - 0xa should be reserved for the framework and part of accudo governance.
        ...
        // property 1: All the core resources and modules should be created during genesis and owned by the Accudo framework account.
        /// [high-level-req-1]
        ensures exists<accudo_governance::GovernanceResponsbility>(@accudo_framework);
        ensures exists<consensus_config::ConsensusConfig>(@accudo_framework);
        ensures exists<execution_config::ExecutionConfig>(@accudo_framework);
        ensures exists<version::Version>(@accudo_framework);
        ensures exists<stake::ValidatorSet>(@accudo_framework);
        ensures exists<stake::ValidatorPerformance>(@accudo_framework);
        ensures exists<storage_gas::StorageGasConfig>(@accudo_framework);
        ensures exists<storage_gas::StorageGas>(@accudo_framework);
        ensures exists<gas_schedule::GasScheduleV2>(@accudo_framework);
        ensures exists<aggregator_factory::AggregatorFactory>(@accudo_framework);
        ensures exists<coin::SupplyConfig>(@accudo_framework);
        ensures exists<chain_id::ChainId>(@accudo_framework);
        ensures exists<reconfiguration::Configuration>(@accudo_framework);
        ensures exists<block::BlockResource>(@accudo_framework);
        ensures exists<state_storage::StateStorageUsage>(@accudo_framework);
        ensures exists<timestamp::CurrentTimeMicroseconds>(@accudo_framework);
        ensures exists<account::Account>(@accudo_framework);
        ensures exists<version::SetVersionCapability>(@accudo_framework);
        ensures exists<staking_config::StakingConfig>(@accudo_framework);
    }

    spec initialize_accudo_coin {
        // property 3: The Accudo coin should be initialized during genesis and only the Accudo framework account should
        // own the mint and burn capabilities for the APT token.
        /// [high-level-req-3]
        requires !exists<stake::AccudoCoinCapabilities>(@accudo_framework);
        ensures exists<stake::AccudoCoinCapabilities>(@accudo_framework);
        requires exists<transaction_fee::AccudoCoinCapabilities>(@accudo_framework);
        ensures exists<transaction_fee::AccudoCoinCapabilities>(@accudo_framework);
    }
    ...
    spec create_initialize_validators_with_commission {
        ...
        include accudo_coin::ExistsAccudoCoin;
    }
    ...
    spec set_genesis_end {
        ...
        /// [high-level-req-4]
        requires len(global<stake::ValidatorSet>(@accudo_framework).active_validators) >= 1;
        // property 5: The end of genesis should be marked on chain.
        /// [high-level-req-5]
        let addr = std::signer::address_of(accudo_framework);
        aborts_if addr != @accudo_framework;
        aborts_if exists<chain_status::GenesisEndMarker>(@accudo_framework);
        ensures global<chain_status::GenesisEndMarker>(@accudo_framework) == chain_status::GenesisEndMarker {};
    }

    spec schema InitalizeRequires {
        execution_config: vector<u8>;
        requires !exists<account::Account>(@accudo_framework);
        requires chain_status::is_operating();
        requires len(execution_config) > 0;
        requires exists<staking_config::StakingRewardsConfig>(@accudo_framework);
        requires exists<coin::CoinInfo<AccudoCoin>>(@accudo_framework);
        include CompareTimeRequires;
    }

    spec schema CompareTimeRequires {
        let staking_rewards_config = global<staking_config::StakingRewardsConfig>(@accudo_framework);
