...
module accudo_std::big_ordered_map {
    use std::error;
    use std::vector;
    use std::option::{Self as option, Option};
    use std::bcs;
    use accudo_std::ordered_map::{Self, OrderedMap};
    use accudo_std::cmp;
    use accudo_std::storage_slots_allocator::{Self, StorageSlotsAllocator, StoredSlot};
    use accudo_std::math64::{max, min};
    ...
    fun add_or_upsert_impl<K: drop + copy + store, V: store>(self: &mut BigOrderedMap<K, V>, key: K, value: V, allow_overwrite: bool): Option<Child<V>> {
        ...
        if (path_to_leaf.is_empty()) {
            ...
            loop {
                ...
                let last_value = current_node.children.internal_new_end_iter().iter_prev(&current_node.children).iter_remove(&mut current_node.children);
    ...
    fun print_map<K: store, V: store>(self: &BigOrderedMap<K, V>) {
        // uncomment to debug:
        // accudo_std::debug::print(&std::string::utf8(b"print map"));
        // accudo_std::debug::print(self);
    ...
    fun print_map_for_node<K: store + copy + drop, V: store>(self: &BigOrderedMap<K, V>, node_index: u64, level: u64) {
        ...
        accudo_std::debug::print(&level);
        accudo_std::debug::print(&node_index);
        accudo_std::debug::print(node);
    ...
    fun destroy_and_validate<K: drop + copy + store, V: drop + store>(self: BigOrderedMap<K, V>) {
        ...
        while (!it.iter_is_end(&self)) {
            ...
            assert!(self.internal_find(it.iter_borrow_key()).iter_is_end(&self), error::invalid_state(EINTERNAL_INVARIANT_BROKEN));
    ...
    fun validate_subtree<K: drop + copy + store, V: store>(self: &BigOrderedMap<K, V>, node_index: u64, expected_lower_bound_key: Option<K>, expected_max_key: Option<K>) {
        ...
        if (expected_max_key.is_some()) {
            ...
            assert!(&expected_max_key == node.children.internal_new_end_iter().iter_prev(&node.children).iter_borrow_key(&node.children), error::invalid_state(EINTERNAL_INVARIANT_BROKEN));
        };

        if (expected_lower_bound_key.is_some()) {
            let expected_lower_bound_key = expected_lower_bound_key.extract();
            assert!(cmp::compare(&expected_lower_bound_key, node.children.internal_new_begin_iter().iter_borrow_key(&node.children)).is_lt(), error::invalid_state(EINTERNAL_INVARIANT_BROKEN));
        };
    }
    ...
    fun test_lower_bound() {
        ...
        assert!(map.internal_lower_bound(&0).key == 1, 0);
        assert!(map.internal_lower_bound(&4).key == 5, 1);
        assert!(map.internal_lower_bound(&9).key == 10, 2);
        assert!(map.internal_lower_bound(&13).iter_is_end(&map), 3);

        map.remove(&3);
        assert!(map.internal_lower_bound(&3).key == 5, 4);
        map.remove(&5);
    ...
    #[test]
    #[expected_failure(abort_code = 0x10002, location = accudo_std::ordered_map)] /// EKEY_NOT_FOUND
    ...
    fun test_large_data_set_helper(inner_max_degree: u16, leaf_max_degree: u16, reuse_slots: bool) {
        ...
        for (i in 0..len) {
            ...
            // accudo_std::debug::print(&it);

            let it_next = it.iter_next(&map);
            let it_after = map.internal_lower_bound(&(*element + 1));

            // accudo_std::debug::print(&it_next);
            // accudo_std::debug::print(&it_after);
            // accudo_std::debug::print(&std::string::utf8(b"bla"));
