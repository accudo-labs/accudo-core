spec accudo_framework::system_addresses {
    /// <high-level-req>
    /// No.: 1
    ...
    ///
    /// No.: 2
    /// Requirement: Asserting that a provided address corresponds to the Accudo Framework Resources address should always
    /// yield a true result when matched.
    /// Criticality: High
    /// Implementation: The assert_accudo_framework function ensures that the provided signer belongs to the
    /// @accudo_framework account.
    /// Enforcement: Formally verified via [high-level-req-2](AbortsIfNotAccudoFramework).
    ///
    /// No.: 3
    ...
    spec assert_accudo_framework(account: &signer) {
        pragma opaque;
    ...
    spec schema AbortsIfNotAccudoFramework {
        account: signer;
        /// [high-level-req-2]
        aborts_if signer::address_of(account) != @accudo_framework with error::PERMISSION_DENIED;
    }
