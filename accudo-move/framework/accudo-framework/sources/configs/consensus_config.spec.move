spec accudo_framework::consensus_config {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: During genesis, the Accudo framework account should be assigned the consensus config resource.
    /// Criticality: Medium
    /// Implementation: The consensus_config::initialize function calls the assert_accudo_framework function to ensure
    /// that the signer is the accudo_framework and then assigns the ConsensusConfig resource to it.
    /// Enforcement: Formally verified via [high-level-req-1](initialize).
    ///
    /// No.: 2
    /// Requirement: Only accudo framework account is allowed to update the consensus configuration.
    /// Criticality: Medium
    /// Implementation: The consensus_config::set function ensures that the signer is accudo_framework.
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<ConsensusConfig>(@accudo_framework);
    }

    /// Ensure caller is admin.
    /// Aborts if StateStorageUsage already exists.
    spec initialize(accudo_framework: &signer, config: vector<u8>) {
        use std::signer;
        let addr = signer::address_of(accudo_framework);
        /// [high-level-req-1]
        aborts_if !system_addresses::is_accudo_framework_address(addr);
        aborts_if exists<ConsensusConfig>(@accudo_framework);
        /// [high-level-req-3.1]
    ...
    spec set(account: &signer, config: vector<u8>) {
        ...
        ensures global<ConsensusConfig>(@accudo_framework).config == config;
    }
    ...
    spec validator_txn_enabled(): bool {
        pragma opaque;
        aborts_if !exists<ConsensusConfig>(@accudo_framework);
        ensures [abstract] result == spec_validator_txn_enabled_internal(global<ConsensusConfig>(@accudo_framework).config);
    }
