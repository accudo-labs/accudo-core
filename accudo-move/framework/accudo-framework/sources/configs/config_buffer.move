...
module accudo_framework::config_buffer {
    use std::error;
    use std::string::String;
    use accudo_std::any;
    use accudo_std::any::Any;
    use accudo_std::simple_map;
    use accudo_std::simple_map::SimpleMap;
    ...
    friend accudo_framework::jwks;
    friend accudo_framework::jwk_consensus_config;
    friend accudo_framework::keyless_account;
    friend accudo_framework::randomness_api_v0_config;
    friend accudo_framework::randomness_config;
    friend accudo_framework::randomness_config_seqnum;
    ...
    public fun initialize(accudo_framework: &signer) {
        system_addresses::assert_accudo_framework(accudo_framework);
        if (!exists<PendingConfigs>(@accudo_framework)) {
            move_to(accudo_framework, PendingConfigs {
    ...
    public fun does_exist<T: store>(): bool acquires PendingConfigs {
        if (exists<PendingConfigs>(@accudo_framework)) {
            let config = borrow_global<PendingConfigs>(@accudo_framework);
    ...
    public(friend) fun upsert<T: drop + store>(config: T) acquires PendingConfigs {
        let configs = borrow_global_mut<PendingConfigs>(@accudo_framework);
