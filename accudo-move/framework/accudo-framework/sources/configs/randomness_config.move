...
module accudo_framework::randomness_config {
    use std::string;
    use accudo_std::copyable_any;
    use accudo_std::copyable_any::Any;
    use accudo_std::fixed_point64::FixedPoint64;
    ...
    public fun initialize(framework: &signer, config: RandomnessConfig) {
        ...
        if (!exists<RandomnessConfig>(@accudo_framework)) {
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires RandomnessConfig {
        ...
        if (config_buffer::does_exist<RandomnessConfig>()) {
            ...
            if (exists<RandomnessConfig>(@accudo_framework)) {
                *borrow_global_mut<RandomnessConfig>(@accudo_framework) = new_config;
            } else {
    ...
    public fun enabled(): bool acquires RandomnessConfig {
        if (exists<RandomnessConfig>(@accudo_framework)) {
            let config = borrow_global<RandomnessConfig>(@accudo_framework);
    ...
    public fun current(): RandomnessConfig acquires RandomnessConfig {
        if (exists<RandomnessConfig>(@accudo_framework)) {
            *borrow_global<RandomnessConfig>(@accudo_framework)
        } else {
            new_off()
        }
    }
