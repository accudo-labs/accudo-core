...
module accudo_framework::version {
    use std::error;
    use std::signer;
    ...
    public(friend) fun initialize(accudo_framework: &signer, initial_version: u64) {
        system_addresses::assert_accudo_framework(accudo_framework);

        move_to(accudo_framework, Version { major: initial_version });
        // Give accudo framework account capability to call set version. This allows on chain governance to do it through
        // control of the accudo framework account.
        move_to(accudo_framework, SetVersionCapability {});
    }
    ...
    public entry fun set_version(account: &signer, major: u64) acquires Version {
        ...
        let old_major = borrow_global<Version>(@accudo_framework).major;
        assert!(old_major < major, error::invalid_argument(EINVALID_MAJOR_VERSION_NUMBER));

        let config = borrow_global_mut<Version>(@accudo_framework);
        config.major = major;
    ...
    /// Example usage:
    /// - `accudo_framework::version::set_for_next_epoch(&framework_signer, new_version);`
    ...
    public(friend) fun on_new_epoch(framework: &signer) acquires Version {
        ...
        if (config_buffer::does_exist<Version>()) {
            ...
            if (exists<Version>(@accudo_framework)) {
                *borrow_global_mut<Version>(@accudo_framework) = new_value;
            } else {
    ...
    public entry fun test_set_version(accudo_framework: signer) acquires Version {
        initialize(&accudo_framework, 1);
        assert!(borrow_global<Version>(@accudo_framework).major == 1, 0);
        set_version(&accudo_framework, 2);
        assert!(borrow_global<Version>(@accudo_framework).major == 2, 1);
    }

    #[test(accudo_framework = @accudo_framework, core_resources = @core_resources)]
    ...
    #[test(accudo_framework = @accudo_framework, random_account = @0x123)]
