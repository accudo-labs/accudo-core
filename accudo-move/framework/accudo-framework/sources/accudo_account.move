module accudo_framework::accudo_account {
    use accudo_framework::account::{Self, new_event_handle};
    ...
    use accudo_framework::event::{EventHandle, emit_event, emit};
    use accudo_framework::fungible_asset::{Self, Metadata, BurnRef, FungibleAsset};
    ...
    use std::signer;
    use std::vector;
    use accudo_framework::object::Object;

    friend accudo_framework::genesis;
    friend accudo_framework::resource_account;
    ...
    public entry fun transfer(source: &signer, to: address, amount: u64) {
        ...
        if (features::operations_default_to_fa_apt_store_enabled()) {
            ...
            if (!coin::is_account_registered<AccudoCoin>(to)) {
                coin::register<AccudoCoin>(&create_signer(to));
            };
            coin::transfer<AccudoCoin>(source, to, amount)
        }
    }
    ...
    public fun deposit_coins<CoinType>(
        ...
        if (!account::exists_at(to)) {
            ...
            spec {
                // TODO(fa_migration)
                // assert coin::spec_is_account_registered<AccudoCoin>(to);
                // assume accudo_std::type_info::type_of<CoinType>() == accudo_std::type_info::type_of<AccudoCoin>() ==>
    ...
    public fun assert_account_is_registered_for_apt(addr: address) {
        ...
        assert!(
            coin::is_account_registered<AccudoCoin>(addr),
        ...
        if (fungible_asset::store_exists(store_addr)) {
            ...
            object::object_address(
                &primary_fungible_store::create_primary_store(
                    owner, object::address_to_object<Metadata>(@accudo_fungible_asset)
                )
            )
        }
    }
    ...
    inline fun primary_fungible_store_address(account: address): address {
        object::create_user_derived_object_address(account, @accudo_fungible_asset)
    }
    ...
    public fun test_transfer(alice: &signer, core: &signer) {
        ...
        let (burn_cap, mint_cap) = accudo_framework::accudo_coin::initialize_for_test(core);
        create_account(signer::address_of(alice));
        coin::deposit(signer::address_of(alice), coin::mint(10000, &mint_cap));
        transfer(alice, bob, 500);
        assert!(coin::balance<AccudoCoin>(bob) == 500, 0);
        transfer(alice, carol, 500);
        assert!(coin::balance<AccudoCoin>(carol) == 500, 1);
        transfer(alice, carol, 1500);
        assert!(coin::balance<AccudoCoin>(carol) == 2000, 2);
    ...
    public fun test_transfer_to_resource_account(
        ...
        let (burn_cap, mint_cap) = accudo_framework::accudo_coin::initialize_for_test(core);
        assert!(coin::is_account_registered<AccudoCoin>(resource_acc_addr), 0);

        create_account(signer::address_of(alice));
        coin::deposit(signer::address_of(alice), coin::mint(10000, &mint_cap));
        transfer(alice, resource_acc_addr, 500);
        assert!(coin::balance<AccudoCoin>(resource_acc_addr) == 500, 1);
    ...
    public fun test_batch_transfer(
        ...
        assert!(coin::balance<AccudoCoin>(recipient_1_addr) == 100, 0);
        assert!(coin::balance<AccudoCoin>(recipient_2_addr) == 500, 1);
        coin::destroy_burn_cap(burn_cap);
        coin::destroy_mint_cap(mint_cap);
    }
