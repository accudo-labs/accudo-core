...
module accudo_framework::keyless_account {
    ...
    use std::vector;
    use accudo_std::crypto_algebra;
    use accudo_std::ed25519;
    use accudo_framework::chain_status;
    use accudo_framework::system_addresses;

    // The `accudo_framework::reconfiguration_with_dkg` module needs to be able to call `on_new_epoch`.
    ...
    #[resource_group_member(group = accudo_framework::keyless_account::Group)]
    ...
    public(friend) fun on_new_epoch(fx: &signer) acquires Groth16VerificationKey, Configuration {
        ...
        if (config_buffer::does_exist<Groth16VerificationKey>()) {
            ...
            if (exists<Groth16VerificationKey>(@accudo_framework)) {
                *borrow_global_mut<Groth16VerificationKey>(@accudo_framework) = vk;
            } else {
                move_to(fx, vk);
            }
        };
        ...
        if (config_buffer::does_exist<Configuration>()) {
            ...
            if (exists<Configuration>(@accudo_framework)) {
                *borrow_global_mut<Configuration>(@accudo_framework) = config;
            } else {
                move_to(fx, config);
            }
        };
