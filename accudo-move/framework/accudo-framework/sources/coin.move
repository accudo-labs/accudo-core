...
module accudo_framework::coin {
    ...
    use accudo_framework::aggregator::Aggregator;
    use accudo_framework::event::{Self, EventHandle};
    use accudo_framework::guid;
    use accudo_framework::optional_aggregator::{Self, OptionalAggregator};
    use accudo_framework::permissioned_signer;
    use accudo_framework::system_addresses;

    use accudo_framework::fungible_asset::{
        Self,
        FungibleAsset,
        Metadata,
        MintRef,
    ...
    use accudo_framework::object::{Self, Object, object_address};
    use accudo_framework::primary_fungible_store;
    use accudo_std::type_info::{Self, TypeInfo};
    use accudo_framework::create_signer;

    friend accudo_framework::accudo_coin;
    ...
    public fun paired_metadata<CoinType>(): Option<Object<Metadata>> acquires CoinConversionMap {
        if (exists<CoinConversionMap>(@accudo_framework)) {
            let map =
                &borrow_global<CoinConversionMap>(@accudo_framework).coin_to_fungible_asset_map;
    ...
    public entry fun create_coin_conversion_map(accudo_framework: &signer) {
    ...
    /// Create APT pairing by passing `AccudoCoin`.
    ...
    inline fun is_apt<CoinType>(): bool {
        type_info::type_name<CoinType>() == string::utf8(b"0x1::accudo_coin::AccudoCoin")
    }
    ...
    inline fun create_and_return_paired_metadata_if_not_exist<CoinType>(
        ...
        assert!(
            exists<CoinConversionMap>(@accudo_framework),
            error::not_found(ECOIN_CONVERSION_MAP_NOT_FOUND)
        );
        let map = borrow_global_mut<CoinConversionMap>(@accudo_framework);
        let type = type_info::type_of<CoinType>();
            ...
            let metadata_object_cref =
                if (is_apt) {
                    ...
                    object::create_named_object(
                        &create_signer::create_signer(@accudo_fungible_asset),
    ...
    public fun allow_supply_upgrades(
        _accudo_framework: &signer, _allowed: bool
    ) {
    ...
    fun mint_internal<CoinType>(amount: u64): Coin<CoinType> acquires CoinInfo {
        ...
        if (option::is_some(maybe_supply)) {
            ...
            spec {
                ...
                use accudo_framework::aggregator;
    ...
    #[test(source = @0x2, framework = @accudo_framework)]
    ...
    public entry fun test_burn_from_with_capability(
        ...
        assert!(coin_balance<FakeMoney>(source_addr) == 0, 0);

        burn_from<FakeMoney>(source_addr, 30, &burn_cap);
        assert!(balance<FakeMoney>(source_addr) == 20, 0);
        assert!(
            primary_fungible_store::balance(
    ...
    public entry fun burn_frozen(
        ...
        let (transfer_ref, receipt) = get_paired_transfer_ref(&freeze_cap);
        ...
        return_paired_transfer_ref(transfer_ref, receipt);
    ...
    #[test(account = @0x1)]
    #[expected_failure(abort_code = 0x50003, location = accudo_framework::fungible_asset)]
    ...
    #[test(framework = @accudo_framework, other = @0x123)]
    ...
    #[test(other = @0x123)]
    fun test_migration_coin_store_with_non_coin_type(
        other: signer
    ) acquires CoinConversionMap, CoinStore, CoinInfo {
        migrate_to_fungible_store<String>(&other);
    }

    #[test(framework = @accudo_framework)]
    ...
    #[test(framework = @accudo_framework)]
    #[expected_failure(abort_code = 0x20001, location = accudo_framework::aggregator)]
    ...
    fun test_supply(
        ...
        assert!(
            coin_supply<FakeMoney>() == option::some(100),
            0
        );
        assert!(
        ...
        assert!(supply<FakeMoney>() == option::some(150), 0);
        assert!(
            coin_supply<FakeMoney>() == option::some(50),
            0
        );
        assert!(
            fungible_asset::supply(ensure_paired_metadata<FakeMoney>())
    ...
    #[test(account = @accudo_framework, aaron = @0xaa10, bob = @0xb0b)]
    fun test_force_deposit(
        ...
        assert!(
            event::emitted_events<fungible_asset::Deposit>().length() == 0,
            10
        );
        assert!(
            event::emitted_events<fungible_asset::Withdraw>().length() == 0,
            10
        );
        ...
        assert!(
            event::emitted_events<fungible_asset::Deposit>().length() == 1,
            10
        );
        ...
        assert!(
            event::emitted_events<fungible_asset::Withdraw>().length() == 1,
            10
        );
    ...
    #[test(account = @accudo_framework, bob = @0xb0b)]
    ...
    #[test(account = @accudo_framework)]
    #[expected_failure(abort_code = 0x50024, location = accudo_framework::fungible_asset)]
