spec accudo_framework::staking_contract {
    /// <high-level-req>
    /// No.: 1
    ...
    contract_creation_seed: vector<u8>,
        ...
        include WithdrawAbortsIf<AccudoCoin> { account: staker };
    ...
    spec add_stake(staker: &signer, operator: address, amount: u64) {
        ...
        include WithdrawAbortsIf<AccudoCoin> { account: staker };
        let balance = global<coin::CoinStore<AccudoCoin>>(staker_address).coin.value;
        let post post_coin = global<coin::CoinStore<AccudoCoin>>(staker_address).coin.value;
    ...
    spec schema PreconditionsInCreateContract {
        requires exists<stake::ValidatorPerformance>(@accudo_framework);
        requires exists<stake::ValidatorSet>(@accudo_framework);
        requires exists<staking_config::StakingRewardsConfig>(
            @accudo_framework
        ) || !std::features::spec_periodical_reward_rate_decrease_enabled();
        requires exists<accudo_framework::timestamp::CurrentTimeMicroseconds>(@accudo_framework);
    ...
    spec schema CreateStakePoolAbortsIf {
        ...
        let allowed = global<stake::AllowedValidators>(@accudo_framework);
        aborts_if exists<stake::AllowedValidators>(@accudo_framework) && !contains(allowed.accounts, resource_addr);
