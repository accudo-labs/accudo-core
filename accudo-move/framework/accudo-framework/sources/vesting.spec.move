spec accudo_framework::vesting {
    ...
    /// period.
    /// Enforcement: Audited the check for the end of vesting cliff: [https://github.com/accudo-labs/accudo-core/blob/main/accudo-move/framework/accudo-framework/sources/vesting.move#L566](vest) module.
    ///
    /// No.: 5
    ...
    spec create_vesting_contract {
        ...
        pragma verify = false;
        /// [high-level-req-10]
        aborts_if withdrawal_address == @accudo_framework || withdrawal_address == @vm_reserved;
        aborts_if !exists<account::Account>(withdrawal_address);
        aborts_if !exists<coin::CoinStore<AccudoCoin>>(withdrawal_address);
    ...
    spec set_beneficiary(
        ...
        // TODO(fa_migration)
        // aborts_if !coin::spec_is_account_registered<AccudoCoin>(new_beneficiary);
        include VerifyAdminAbortsIf;
    ...
    spec create_vesting_contract_account(
        ...
        aborts_if !exists<coin::CoinStore<AccudoCoin>>(resource_addr) && !accudo_std::type_info::spec_is_struct<AccudoCoin>();
        aborts_if !exists<coin::CoinStore<AccudoCoin>>(resource_addr) && ea && acc.guid_creation_num + 2 > MAX_U64;
        aborts_if !exists<coin::CoinStore<AccudoCoin>>(resource_addr) && ea && acc.guid_creation_num + 2 >= account::MAX_GUID_CREATION_NUM;
        ensures exists<account::Account>(resource_addr) && post_acc.authentication_key == account::ZERO_AUTH_KEY &&
                exists<coin::CoinStore<AccudoCoin>>(resource_addr);
    ...
    spec withdraw_stake(vesting_contract: &VestingContract, contract_address: address): Coin<AccudoCoin> {
        // TODO: Calls `staking_contract::distribute` which is not verified.
        pragma verify = false;
    ...
    spec schema DistributeInternalAbortsIf {
        ...
        aborts_if !exists<stake::ValidatorSet>(@accudo_framework);
        let validator_set = global<stake::ValidatorSet>(@accudo_framework);
