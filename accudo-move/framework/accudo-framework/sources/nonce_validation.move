module accudo_framework::nonce_validation {
    use accudo_std::table::{Self, Table};
    use accudo_std::timestamp;
    use accudo_std::big_ordered_map::{Self, BigOrderedMap};
    use accudo_std::accudo_hash::sip_hash_from_value;
    use accudo_std::error;
    ...
    public(friend) fun initialize(accudo_framework: &signer) {
        initialize_nonce_table(accudo_framework);
    }

    public entry fun initialize_nonce_table(accudo_framework: &signer) {
        system_addresses::assert_accudo_framework(accudo_framework);
        if (!exists<NonceHistory>(@accudo_framework)) {
            let table = table::new();
            let nonce_history = NonceHistory {
                nonce_table: table,
                next_key: 0,
            };
            move_to<NonceHistory>(accudo_framework, nonce_history);
        };
    }
    ...
    public entry fun add_nonce_buckets(count: u64) acquires NonceHistory {
        assert!(exists<NonceHistory>(@accudo_framework), error::invalid_state(E_NONCE_HISTORY_DOES_NOT_EXIST));
        let nonce_history = &mut NonceHistory[@accudo_framework];
        for (i in 0..count) {
    ...
    fun check_if_nonce_exists_in_history(
        ...
        let bucket_index = sip_hash_from_value(&nonce_key) % NUM_BUCKETS;
        let nonce_history = &NonceHistory[@accudo_framework];
        if (nonce_history.nonce_table.contains(bucket_index)) {
