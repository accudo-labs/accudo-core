spec accudo_framework::state_storage {
    /// <high-level-req>
    /// No.: 1
    /// Requirement: Given the blockchain is in an operating state, the resources for tracking state storage usage and gas
    /// parameters must exist for the Accudo framework address.
    /// Criticality: Critical
    /// Implementation: The initialize function ensures only the Accudo framework address can call it.
    ...
    /// storage usage will be moved under the Accudo framework account with default initial values.
    /// Criticality: Medium
    /// Implementation: The resource for tracking state storage usage may only be initialized with specific values and
    /// published under the accudo_framework account.
    ...
    /// usage will be moved under the Accudo framework account with default initial values.
    /// Criticality: Medium
    /// Implementation: The resource for tracking state storage usage may only be initialized with specific values and
    /// published under the accudo_framework account.
    ...
    spec module {
        ...
        /// [high-level-req-1]
        /// [high-level-req-5.3]
        invariant [suspendable] chain_status::is_operating() ==> exists<StateStorageUsage>(@accudo_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<GasParameter>(@accudo_framework);
    }
    ...
    spec initialize(accudo_framework: &signer) {
        ...
        /// [high-level-req-3]
        aborts_if exists<StateStorageUsage>(@accudo_framework);
        ensures exists<StateStorageUsage>(@accudo_framework);
        let post state_usage = global<StateStorageUsage>(@accudo_framework);
        /// [high-level-req-2]
    ...
    spec on_new_block(epoch: u64) {
        ...
        aborts_if false;
        ensures epoch == global<StateStorageUsage>(@accudo_framework).epoch;
    }
