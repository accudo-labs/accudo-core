...
module accudo_framework::block {
    use std::error;
    use std::vector;
    use std::option;
    use accudo_std::table_with_length::{Self, TableWithLength};
    use std::option::Option;
    use accudo_framework::randomness;
    ...
    public(friend) fun initialize(accudo_framework: &signer, epoch_interval_microsecs: u64) {
        ...
        move_to<CommitHistory>(accudo_framework, CommitHistory {
            max_capacity: 2000,
            next_idx: 0,
        ...
        move_to<BlockResource>(
            ...
            BlockResource {
                ...
                new_block_events: account::new_event_handle<NewBlockEvent>(accudo_framework),
                update_epoch_interval_events: account::new_event_handle<UpdateEpochIntervalEvent>(accudo_framework),
            }
        );
    }
    ...
    public fun update_epoch_interval_microsecs(
        ...
        let block_resource = borrow_global_mut<BlockResource>(@accudo_framework);
    ...
    public fun get_epoch_interval_secs(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@accudo_framework).epoch_interval / 1000000
    }
    ...
    fun block_prologue_common(
        ...
        let block_metadata_ref = borrow_global_mut<BlockResource>(@accudo_framework);
    ...
    public fun get_current_block_height(): u64 acquires BlockResource {
        borrow_global<BlockResource>(@accudo_framework).height
    }
    ...
    fun emit_new_block_event(
        ...
        if (exists<CommitHistory>(@accudo_framework)) {
            let commit_history_ref = borrow_global_mut<CommitHistory>(@accudo_framework);
    ...
    public entry fun test_update_epoch_interval(accudo_framework: signer) acquires BlockResource {
        account::create_account_for_test(@accudo_framework);
        initialize(&accudo_framework, 1);
        assert!(borrow_global<BlockResource>(@accudo_framework).epoch_interval == 1, 0);
        update_epoch_interval_microsecs(&accudo_framework, 2);
        assert!(borrow_global<BlockResource>(@accudo_framework).epoch_interval == 2, 1);
    }

    #[test(accudo_framework = @accudo_framework, account = @0x123)]
