...
module accudo_framework::timestamp {
    ...
    public(friend) fun set_time_has_started(accudo_framework: &signer) {
        system_addresses::assert_accudo_framework(accudo_framework);
        let timer = CurrentTimeMicroseconds { microseconds: 0 };
        move_to(accudo_framework, timer);
    }
    ...
    public fun update_global_time(
        ...
        // Can only be invoked by AccudoVM signer.
        system_addresses::assert_vm(account);

        let global_timer = borrow_global_mut<CurrentTimeMicroseconds>(@accudo_framework);
    ...
    public fun set_time_has_started_for_testing(account: &signer) {
        if (!exists<CurrentTimeMicroseconds>(@accudo_framework)) {
    ...
    public fun now_microseconds(): u64 acquires CurrentTimeMicroseconds {
        borrow_global<CurrentTimeMicroseconds>(@accudo_framework).microseconds
    }
