spec accudo_framework::coin {
    ...
    /// Can only be updated by `@accudo_framework`.
    spec allow_supply_upgrades(_accudo_framework: &signer, _allowed: bool) {
        aborts_if true;
    }
    ...
    spec fun spec_paired_metadata<CoinType>(): Option<Object<Metadata>> {
        if (exists<CoinConversionMap>(@accudo_framework)) {
            let map =
                global<CoinConversionMap>(@accudo_framework).coin_to_fungible_asset_map;
    ...
    /// The creator of `CoinType` must be `@accudo_framework`.
    ...
    // `account` must be `@accudo_framework`.
    spec initialize_with_parallelizable_supply<CoinType>(
        ...
        aborts_if monitor_supply
            && !exists<aggregator_factory::AggregatorFactory>(@accudo_framework);
