spec accudo_framework::transaction_validation {
    /// <high-level-req>
    /// No.: 1
    ...
    /// Ensure caller is `accudo_framework`.
    ...
    spec schema PrologueCommonAbortsIf {
        use accudo_framework::timestamp::{CurrentTimeMicroseconds};
        use accudo_framework::chain_id::{ChainId};
        sender: &signer;
        gas_payer: &signer;
        ...
        aborts_if !exists<CurrentTimeMicroseconds>(@accudo_framework);
        aborts_if !(timestamp::now_seconds() < txn_expiration_time);

        aborts_if !exists<ChainId>(@accudo_framework);
    ...
    /// `AccudoCoinCapabilities` and `CoinInfo` should exists.
    ...
    /// Abort according to the conditions.
    /// `AccudoCoinCapabilities` and `CoinInfo` should exist.
    /// Skip transaction_fee::burn_fee verification.
    ...
    spec schema EpilogueGasPayerAbortsIf {
        ...
        use accudo_framework::coin::{CoinStore, CoinInfo};
        use accudo_framework::optional_aggregator;
        use accudo_framework::transaction_fee::{AccudoCoinCapabilities, AccudoCoinMintCapability};
        ...
        // TODO(fa_migration)
        // let pre_balance = global<coin::CoinStore<AccudoCoin>>(gas_payer).coin.value;
        // let post balance = global<coin::CoinStore<AccudoCoin>>(gas_payer).coin.value;
        let pre_account = global<account::Account>(addr);
        let post account = global<account::Account>(addr);

        aborts_if !exists<CoinStore<AccudoCoin>>(gas_payer);
        ...
        let apt_addr = type_info::type_of<AccudoCoin>().account_address;
        let maybe_apt_supply = global<CoinInfo<AccudoCoin>>(apt_addr).supply;
        ...
        let post post_maybe_apt_supply = global<CoinInfo<AccudoCoin>>(apt_addr).supply;
        ...
        aborts_if amount_to_burn > 0 && !exists<AccudoCoinCapabilities>(@accudo_framework);
        aborts_if amount_to_burn > 0 && !exists<CoinInfo<AccudoCoin>>(apt_addr);
        ...
        let total_supply = coin::supply<AccudoCoin>;
        let post post_total_supply = coin::supply<AccudoCoin>;

        aborts_if amount_to_mint > 0 && !exists<CoinStore<AccudoCoin>>(addr);
        aborts_if amount_to_mint > 0 && !exists<AccudoCoinMintCapability>(@accudo_framework);
        ...
        aborts_if (amount_to_mint != 0) && !exists<coin::CoinInfo<AccudoCoin>>(accudo_addr);
        include coin::CoinAddAbortsIf<AccudoCoin> { amount: amount_to_mint };
    }
}
