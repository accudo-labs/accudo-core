...
module accudo_framework::accudo_coin {
    use std::error;
    use std::signer;
    use std::string;
    use std::vector;
    ...
    use accudo_framework::coin::{Self, BurnCapability, MintCapability};
    ...
    public(friend) fun initialize(accudo_framework: &signer): (BurnCapability<AccudoCoin>, MintCapability<AccudoCoin>) {
        system_addresses::assert_accudo_framework(accudo_framework);

        let (burn_cap, freeze_cap, mint_cap) = coin::initialize_with_parallelizable_supply<AccudoCoin>(
            accudo_framework,
            string::utf8(b"Accudo Coin"),
            string::utf8(b"APT"),
            8, // decimals
            true, // monitor_supply
        );

        // Accudo framework needs mint cap to mint coins to initial validators. This will be revoked once the validators
        // have been initialized.
        move_to(accudo_framework, MintCapStore { mint_cap });
    ...
    public(friend) fun destroy_mint_cap(accudo_framework: &signer) acquires MintCapStore {
        system_addresses::assert_accudo_framework(accudo_framework);
        let MintCapStore { mint_cap } = move_from<MintCapStore>(@accudo_framework);
    ...
    public(friend) fun configure_accounts_for_test(
        ...
        // Mint the core resource account AccudoCoin for gas so it can execute system transactions.
        let coins = coin::mint<AccudoCoin>(
            18446744073709551615,
            &mint_cap,
        );
        coin::deposit<AccudoCoin>(signer::address_of(core_resources), coins);
    ...
    public entry fun mint(
        ...
        let coins_minted = coin::mint<AccudoCoin>(amount, mint_cap);
        coin::deposit<AccudoCoin>(dst_addr, coins_minted);
    }
    ...
    #[test_only]
    use accudo_framework::fungible_asset::FungibleAsset;
    ...
    public fun mint_apt_fa_for_test(amount: u64): FungibleAsset acquires MintCapStore {
        ...
        coin::coin_to_fungible_asset(
            coin::mint(
                amount,
                &borrow_global<MintCapStore>(@accudo_framework).mint_cap
            )
        )
    }
    ...
    public fun ensure_initialized_with_apt_fa_metadata_for_test() {
        let accudo_framework = account::create_signer_for_test(@accudo_framework);
        if (!exists<MintCapStore>(@accudo_framework)) {
            ...
            let (burn_cap, mint_cap) = initialize(&accudo_framework);
    ...
    #[test_only]
    public fun initialize_for_test(accudo_framework: &signer): (BurnCapability<AccudoCoin>, MintCapability<AccudoCoin>) {
        aggregator_factory::initialize_aggregator_factory_for_test(accudo_framework);
        let (burn_cap, mint_cap) = initialize(accudo_framework);
        coin::create_coin_conversion_map(accudo_framework);
        coin::create_pairing<AccudoCoin>(accudo_framework);
    ...
    public fun initialize_for_test_without_aggregator_factory(
        accudo_framework: &signer
    ): (BurnCapability<AccudoCoin>, MintCapability<AccudoCoin>) {
        let (burn_cap, mint_cap) = initialize(accudo_framework);
        coin::create_coin_conversion_map(accudo_framework);
        coin::create_pairing<AccudoCoin>(accudo_framework);
        (burn_cap, mint_cap)
