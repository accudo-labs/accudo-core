spec accudo_framework::storage_gas {
    // -----------------
    // Struct invariants
    // -----------------
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGasConfig>(@accudo_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<StorageGas>(@accudo_framework);
    }
    ...
    spec set_config(accudo_framework: &signer, config: StorageGasConfig) {
    ...
    spec initialize(accudo_framework: &signer) {
        ...
        aborts_if exists<StorageGasConfig>(@accudo_framework);
        aborts_if exists<StorageGas>(@accudo_framework);

        /// [high-level-req-1]
        ensures exists<StorageGasConfig>(@accudo_framework);
        ensures exists<StorageGas>(@accudo_framework);
    }
    ...
    spec on_reconfig {
        ...
        aborts_if !exists<StorageGasConfig>(@accudo_framework);
        aborts_if !exists<StorageGas>(@accudo_framework);
        aborts_if !exists<state_storage::StateStorageUsage>(@accudo_framework);
    }
