...
module accudo_framework::dkg {
    use std::error;
    use std::option;
    use std::option::Option;
    use accudo_framework::event::emit;
    use accudo_framework::randomness_config::RandomnessConfig;
    ...
    public(friend) fun start(
        ...
        let dkg_state = borrow_global_mut<DKGState>(@accudo_framework);
    ...
    public fun incomplete_session(): Option<DKGSessionState> acquires DKGState {
        if (exists<DKGState>(@accudo_framework)) {
            borrow_global<DKGState>(@accudo_framework).in_progress
        } else {
            option::none()
        }
    }
