module accudo_framework::account_abstraction {
    use std::bcs;
    use std::hash;
    ...
    use accudo_framework::features;
    use accudo_framework::function_info::{Self, FunctionInfo};
    use accudo_framework::object;
    use accudo_framework::auth_data::AbstractionAuthData;
    use accudo_framework::system_addresses;
    use accudo_framework::permissioned_signer::is_permissioned_signer;
    #[test_only]
    use accudo_framework::account::create_account_for_test;
    #[test_only]
    use accudo_framework::auth_data;

    friend accudo_framework::transaction_validation;
    #[test_only]
    friend accudo_framework::account_abstraction_tests;
    ...
    public entry fun register_derivable_authentication_function(
        ...
        DerivableDispatchableAuthenticator[@accudo_framework].auth_functions.add(
            function_info::new_function_info_from_address(module_address, module_name, function_name),
            DerivableRegisterValue::Empty,
        );
    }

    public entry fun initialize(accudo_framework: &signer) {
        system_addresses::assert_accudo_framework(accudo_framework);
        move_to(
            accudo_framework,
    ...
    inline fun resource_addr(source: address): address {
        object::create_user_derived_object_address(source, @accudo_fungible_asset)
    }
    ...
    inline fun dispatchable_derivable_authenticator_internal(): &BigOrderedMap<FunctionInfo, DerivableRegisterValue> {
        assert!(exists<DerivableDispatchableAuthenticator>(@accudo_framework), error::not_found(EDERIVABLE_AA_NOT_INITIALIZED));
        &DerivableDispatchableAuthenticator[@accudo_framework].auth_functions
   }
    ...
    fun authenticate(
        ...
        // Returned signer MUST represent the same account address. Otherwise, it may break the invariant of Accudo blockchain!
        assert!(
