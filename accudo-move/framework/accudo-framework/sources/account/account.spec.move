spec accudo_framework::account {
    ...
    /// to the OriginatingAddress under the accudo_framework account.
    ...
    /// Only the address `@accudo_framework` can call.
    /// OriginatingAddress does not exist under `@accudo_framework` before the call.
    spec initialize(accudo_framework: &signer) {
        ...
        aborts_if exists<OriginatingAddress>(accudo_addr);
        ensures exists<OriginatingAddress>(accudo_addr);
    }
    ...
    spec create_account(new_address: address): signer {
        ...
        aborts_if new_address == @vm_reserved || new_address == @accudo_framework || new_address == @accudo_token;
    ...
    spec rotate_authentication_key(
        ...
        let address_map = global<OriginatingAddress>(@accudo_framework).address_map;
        let new_auth_key = from_bcs::deserialize<address>(new_auth_key_vector);

        aborts_if !exists<OriginatingAddress>(@accudo_framework);
        aborts_if !from_bcs::deserializable<address>(account_resource.authentication_key);
    ...
    spec offer_rotation_capability(
        ...
        let proof_challenge = RotationCapabilityOfferProofChallengeV2 {
            chain_id: global<chain_id::ChainId>(@accudo_framework).id,
            sequence_number: account_resource.sequence_number,
            source_address,
            recipient_address,
        };
        aborts_if !exists<chain_id::ChainId>(@accudo_framework);
    ...
    spec update_auth_key_and_originating_address_table(
        ...
        modifies global<OriginatingAddress>(@accudo_framework);
        include UpdateAuthKeyAndOriginatingAddressTableAbortsIf;
    }
    spec schema UpdateAuthKeyAndOriginatingAddressTableAbortsIf {
        ...
        ensures table::spec_contains(global<OriginatingAddress>(@accudo_framework).address_map, from_bcs::deserialize<address>(new_auth_key_vector));
    }
