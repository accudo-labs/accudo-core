...
module accudo_framework::multisig_account {
    use accudo_framework::account::{Self, SignerCapability, new_event_handle, create_resource_address};
    ...
    use accudo_framework::timestamp::now_seconds;
    ...
    /// account.
    const DOMAIN_SEPARATOR: vector<u8> = b"accudo_framework::multisig_account";
    ...
    fun create_multisig_account(owner: &signer): (signer, SignerCapability) {
        ...
        // flow.
        if (!coin::is_account_registered<AccudoCoin>(address_of(&multisig_signer))) {
            coin::register<AccudoCoin>(&multisig_signer);
        };
    ...
    #[test_only]
    use accudo_framework::coin::{destroy_mint_cap, destroy_burn_cap};
    ...
    fun setup() {
        ...
        let (burn, mint) = accudo_coin::initialize_for_test(framework_signer);
