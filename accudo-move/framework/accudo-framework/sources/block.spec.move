spec accudo_framework::block {
    ...
    /// the module's context, and moved under the Accudo framework account, (2) the block height is set to zero during
    /// initialization, and (3) the epoch interval is greater than zero.
    /// Enforcement: Formally Verified via [high-level-req-1](Initialize).
    ///
    /// No.: 2
    /// Requirement: Only the Accudo framework address may execute the following functionalities: (1) initialize
    /// BlockResource, and (2) update the epoch interval.
    /// Criticality: Critical
    /// Implementation: The initialize and  update_epoch_interval_microsecs functions ensure that only accudo_framework
    /// can call them.
    ...
    spec module {
        ...
        invariant [suspendable] chain_status::is_operating() ==> exists<BlockResource>(@accudo_framework);
        // After genesis, `CommitHistory` exist.
        invariant [suspendable] chain_status::is_operating() ==> exists<CommitHistory>(@accudo_framework);
    }
    ...
    spec emit_genesis_block_event {
        ...
        requires event::counter(global<BlockResource>(@accudo_framework).new_block_events) == 0;
    ...
    spec initialize(accudo_framework: &signer, epoch_interval_microsecs: u64) {
        use std::signer;
        /// [high-level-req-1]
        include Initialize;
    ...
    spec get_epoch_interval_secs(): u64 {
        aborts_if !exists<BlockResource>(@accudo_framework);
    }
    ...
    spec schema EmitWritesetBlockEvent {
        ...
        aborts_if !exists<BlockResource>(@accudo_framework);
        aborts_if !exists<reconfiguration::Configuration>(@accudo_framework);
