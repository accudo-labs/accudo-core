...
module accudo_framework::transaction_fee {
    use accudo_framework::coin::{Self, AggregatableCoin, BurnCapability, MintCapability};
    use accudo_framework::accudo_account;
    use accudo_framework::accudo_coin::AccudoCoin;
    use accudo_framework::fungible_asset::BurnRef;
    ...
    struct AccudoCoinCapabilities has key {
        burn_cap: BurnCapability<AccudoCoin>,
    }

    /// Stores burn capability to burn the gas fees.
    struct AccudoFABurnCapabilities has key {
        burn_ref: BurnRef,
    }

    /// Stores mint capability to mint the refunds.
    struct AccudoCoinMintCapability has key {
    ...
    public(friend) fun burn_fee(account: address, fee: u64) acquires AccudoFABurnCapabilities, AccudoCoinCapabilities {
        if (exists<AccudoFABurnCapabilities>(@accudo_framework)) {
            let burn_ref = &borrow_global<AccudoFABurnCapabilities>(@accudo_framework).burn_ref;
            accudo_account::burn_from_fungible_store_for_gas(burn_ref, account, fee);
        } else {
            let burn_cap = &borrow_global<AccudoCoinCapabilities>(@accudo_framework).burn_cap;
            if (features::operations_default_to_fa_apt_store_enabled()) {
                let (burn_ref, burn_receipt) = coin::get_paired_burn_ref(burn_cap);
                accudo_account::burn_from_fungible_store_for_gas(&burn_ref, account, fee);
                coin::return_paired_burn_ref(burn_ref, burn_receipt);
            } else {
                coin::burn_from_for_gas<AccudoCoin>(
                    account,
                    fee,
                    burn_cap,
                );
    ...
    public(friend) fun mint_and_refund(account: address, refund: u64) acquires AccudoCoinMintCapability {
        let mint_cap = &borrow_global<AccudoCoinMintCapability>(@accudo_framework).mint_cap;
        let refund_coin = coin::mint(refund, mint_cap);
        coin::deposit_for_gas_fee(account, refund_coin);
    }

    /// Only called during genesis.
    public(friend) fun store_accudo_coin_burn_cap(accudo_framework: &signer, burn_cap: BurnCapability<AccudoCoin>) {
        system_addresses::assert_accudo_framework(accudo_framework);

        if (features::operations_default_to_fa_apt_store_enabled()) {
            let burn_ref = coin::convert_and_take_paired_burn_ref(burn_cap);
            move_to(accudo_framework, AccudoFABurnCapabilities { burn_ref });
        } else {
            move_to(accudo_framework, AccudoCoinCapabilities { burn_cap })
        }
    }

    public entry fun convert_to_accudo_fa_burn_ref(accudo_framework: &signer) acquires AccudoCoinCapabilities {
        assert!(features::operations_default_to_fa_apt_store_enabled(), EFA_GAS_CHARGING_NOT_ENABLED);
        system_addresses::assert_accudo_framework(accudo_framework);
        let AccudoCoinCapabilities {
            burn_cap,
        } = move_from<AccudoCoinCapabilities>(signer::address_of(accudo_framework));
        let burn_ref = coin::convert_and_take_paired_burn_ref(burn_cap);
        move_to(accudo_framework, AccudoFABurnCapabilities { burn_ref });
    }
    ...
    public(friend) fun store_accudo_coin_mint_cap(accudo_framework: &signer, mint_cap: MintCapability<AccudoCoin>) {
        system_addresses::assert_accudo_framework(accudo_framework);
        move_to(accudo_framework, AccudoCoinMintCapability { mint_cap })
    }
    ...
    struct CollectedFeesPerBlock has key {
        amount: AggregatableCoin<AccudoCoin>,
    ...
    #[deprecated]
    /// DEPRECATED
    public fun initialize_fee_collection_and_distribution(_accudo_framework: &signer, _burn_percentage: u8) {
