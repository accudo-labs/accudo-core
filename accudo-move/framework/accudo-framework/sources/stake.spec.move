spec accudo_framework::stake {
    ...
    /// Criticality: Low
    /// Implementation: The total staked value (AccudoCoin) of a stake pool is grouped by: active, inactive,
    ...
    spec module {
        ...
        invariant [suspendable] exists<ValidatorSet>(@accudo_framework) ==> validator_set_is_valid();
        // After genesis, `AccudoCoinCapabilities`, `ValidatorPerformance` and `ValidatorSet` exist.
        invariant [suspendable] chain_status::is_operating() ==> exists<AccudoCoinCapabilities>(@accudo_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<ValidatorPerformance>(@accudo_framework);
        invariant [suspendable] chain_status::is_operating() ==> exists<ValidatorSet>(@accudo_framework);
    ...
    spec schema ValidatorNotChangeDuringReconfig {
        ensures (reconfiguration_state::spec_is_in_progress() && old(exists<ValidatorSet>(@accudo_framework))) ==>
            old(global<ValidatorSet>(@accudo_framework)) == global<ValidatorSet>(@accudo_framework);
    }
    ...
    spec fun validator_set_is_valid(): bool {
        let validator_set = global<ValidatorSet>(@accudo_framework);
    ...
    spec initialize_validator(
        ...
        let allowed = global<AllowedValidators>(@accudo_framework);
        aborts_if exists<ValidatorConfig>(addr);
        aborts_if exists<AllowedValidators>(@accudo_framework) && !vector::spec_contains(allowed.accounts, addr);
    ...
    spec initialize(accudo_framework: &signer) {
        ...
        let accudo_addr = signer::address_of(accudo_framework);
        aborts_if !system_addresses::is_accudo_framework_address(accudo_addr);
        aborts_if exists<ValidatorSet>(accudo_addr);
        aborts_if exists<ValidatorPerformance>(accudo_addr);
        ensures exists<ValidatorSet>(accudo_addr);
        ensures global<ValidatorSet>(accudo_addr).consensus_scheme == 0;
        ensures exists<ValidatorPerformance>(accudo_addr);
    }
    ...
    )
    {
        ...
        aborts_if !exists<StakingConfig>(@accudo_framework);
        aborts_if !exists<ValidatorSet>(@accudo_framework);
        aborts_if reconfiguration_state::spec_is_in_progress();

        let stake_pool = global<StakePool>(pool_address);
        let validator_set = global<ValidatorSet>(@accudo_framework);
        let post p_validator_set = global<ValidatorSet>(@accudo_framework);
        aborts_if signer::address_of(operator) != stake_pool.operator_address;
        aborts_if option::is_some(spec_find_validator(validator_set.active_validators, pool_address)) ||
                    option::is_some(spec_find_validator(validator_set.pending_inactive, pool_address)) ||
                        option::is_some(spec_find_validator(validator_set.pending_active, pool_address));
    ...
    )
    {
        ...
        let validator_set = global<ValidatorSet>(@accudo_framework);
        let bool_find_validator = !option::is_some(spec_find_validator(validator_set.active_validators, pool_address)) &&
                    !option::is_some(spec_find_validator(validator_set.pending_inactive, pool_address)) &&
                        !option::is_some(spec_find_validator(validator_set.pending_active, pool_address));
        aborts_if bool_find_validator && !exists<timestamp::CurrentTimeMicroseconds>(@accudo_framework);
        ...
        aborts_if !exists<coin::CoinStore<AccudoCoin>>(addr);
        include coin::DepositAbortsIf<AccudoCoin>{account_addr: addr};

        let coin_store = global<coin::CoinStore<AccudoCoin>>(addr);
        let post p_coin_store = global<coin::CoinStore<AccudoCoin>>(addr);
        ensures bool_find_validator && timestamp::now_seconds() > stake_pool.locked_until_secs
                    && exists<account::Account>(addr) && exists<coin::CoinStore<AccudoCoin>>(addr) ==>
                        coin_store.coin.value + new_withdraw_amount_1 == p_coin_store.coin.value;
        ensures !(bool_find_validator && exists<timestamp::CurrentTimeMicroseconds>(@accudo_framework))
                    && exists<account::Account>(addr) && exists<coin::CoinStore<AccudoCoin>>(addr) ==>
    ...
    spec leave_validator_set(
        ...
        let validator_set = global<ValidatorSet>(@accudo_framework);
        let validator_find_bool = option::is_some(spec_find_validator(validator_set.pending_active, pool_address));
        let active_validators = validator_set.active_validators;
        let pending_active = validator_set.pending_active;

        let post post_validator_set = global<ValidatorSet>(@accudo_framework);
        ...
        ensures !validator_find_bool ==> !option::is_some(spec_find_validator(p_validator_set.pending_active, pool_address));
    }
    ...
    spec unlock_with_cap(amount: u64, owner_cap: &OwnerCapability) {
        ...
        let min_amount = accudo_std::math64::min(amount,pre_stake_pool.active.value);
    ...
    spec increase_lockup_with_cap(owner_cap: &OwnerCapability) {
        let config = global<staking_config::StakingConfig>(@accudo_framework);
        let pool_address = owner_cap.pool_address;
        let pre_stake_pool = global<StakePool>(pool_address);
        let post stake_pool = global<StakePool>(pool_address);
    ...
    spec reactivate_stake_with_cap(owner_cap: &OwnerCapability, amount: u64) {
        ...
        let post stake_pool = global<StakePool>(pool_address);
        modifies global<StakePool>(pool_address);
        let min_amount = accudo_std::math64::min(amount, pre_stake_pool.pending_inactive.value);
    ...
    spec on_new_epoch {
        ...
        include accudo_framework::accudo_coin::ExistsAccudoCoin;
    ...
    spec update_performance_statistics {
        ...
        let validator_perf = global<ValidatorPerformance>(@accudo_framework);
        let post post_validator_perf = global<ValidatorPerformance>(@accudo_framework);
    ...
    spec schema DistributeRewardsAbortsIf {
        ...
        let addr = type_info::type_of<AccudoCoin>().account_address;
        aborts_if (rewards_amount > 0) && !exists<coin::CoinInfo<AccudoCoin>>(addr);
        modifies global<coin::CoinInfo<AccudoCoin>>(addr);
        include (rewards_amount > 0) ==> coin::CoinAddAbortsIf<AccudoCoin> { amount: amount };
    }
    ...
    spec schema GetReconfigStartTimeRequirement {
        requires exists<timestamp::CurrentTimeMicroseconds>(@accudo_framework);
        include reconfiguration_state::StartTimeSecsRequirement;
    }

    spec fun spec_get_reconfig_start_time_secs(): u64 {
        if (exists<reconfiguration_state::State>(@accudo_framework)) {
    ...
    spec update_voting_power_increase(increase_amount: u64) {
        ...
        let accudo = @accudo_framework;
        let pre_validator_set = global<ValidatorSet>(accudo);
        let post validator_set = global<ValidatorSet>(accudo);
        let staking_config = global<staking_config::StakingConfig>(accudo);
    ...
    spec configure_allowed_validators(accudo_framework: &signer, accounts: vector<address>) {
        let accudo_framework_address = signer::address_of(accudo_framework);
        aborts_if !system_addresses::is_accudo_framework_address(accudo_framework_address);
        let post allowed = global<AllowedValidators>(accudo_framework_address);
    ...
    spec fun spec_validator_index_upper_bound(): u64 {
        len(global<ValidatorPerformance>(@accudo_framework).validators)
    }
    ...
    spec schema ResourceRequirement {
        requires exists<AccudoCoinCapabilities>(@accudo_framework);
        requires exists<ValidatorPerformance>(@accudo_framework);
        requires exists<ValidatorSet>(@accudo_framework);
        requires exists<StakingConfig>(@accudo_framework);
        requires exists<StakingRewardsConfig>(@accudo_framework) || !features::spec_periodical_reward_rate_decrease_enabled();
        requires exists<timestamp::CurrentTimeMicroseconds>(@accudo_framework);
    }
    ...
    spec fun spec_get_reward_rate_1(config: StakingConfig): num {
        if (features::spec_periodical_reward_rate_decrease_enabled()) {
            let epoch_rewards_rate = global<staking_config::StakingRewardsConfig>(@accudo_framework).rewards_rate;
            if (epoch_rewards_rate.value == 0) {
                0
            } else {
                let denominator_0 = accudo_std::fixed_point64::spec_divide_u128(staking_config::MAX_REWARDS_RATE, epoch_rewards_rate);
                let denominator = if (denominator_0 > MAX_U64) {
                    MAX_U64
                } else {
                    denominator_0
                };
                let nominator = accudo_std::fixed_point64::spec_multiply_u128(denominator, epoch_rewards_rate);
                nominator
            }
        } else {
                config.rewards_rate
