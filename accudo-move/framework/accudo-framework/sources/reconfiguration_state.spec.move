spec accudo_framework::reconfiguration_state {

    spec module {
        use accudo_framework::chain_status;
        invariant [suspendable] chain_status::is_operating() ==> exists<State>(@accudo_framework);
    }

    spec initialize(fx: &signer) {
        use std::signer;
        use accudo_std::from_bcs;
        aborts_if signer::address_of(fx) != @accudo_framework;
        let post post_state = global<State>(@accudo_framework);
        ensures exists<State>(@accudo_framework);
        ensures !exists<State>(@accudo_framework) ==> from_bcs::deserializable<StateInactive>(post_state.variant.data);
    }
    ...
    spec fun spec_is_in_progress(): bool {
        if (!exists<State>(@accudo_framework)) {
            false
        } else {
            copyable_any::type_name(global<State>(@accudo_framework).variant).bytes == b"0x1::reconfiguration_state::StateActive"
        }
    }
    ...
    spec on_reconfig_start {
        ...
        let pre_state = global<State>(@accudo_framework);
        let post post_state = global<State>(@accudo_framework);
        ensures (exists<State>(@accudo_framework) && copyable_any::type_name(pre_state.variant).bytes
    ...
    spec fun spec_start_time_secs(): u64 {
        use accudo_std::from_bcs;
        let state = global<State>(@accudo_framework);
        from_bcs::deserialize<StateActive>(state.variant.data).start_time_secs
    }

    spec schema StartTimeSecsRequirement {
        requires exists<State>(@accudo_framework);
        requires copyable_any::type_name(global<State>(@accudo_framework).variant).bytes
            == b"0x1::reconfiguration_state::StateActive";
        include UnpackRequiresStateActive {
            x:  global<State>(@accudo_framework).variant
        };
    }
    ...
    spec schema StartTimeSecsAbortsIf {
        aborts_if !exists<State>(@accudo_framework);
        include  copyable_any::type_name(global<State>(@accudo_framework).variant).bytes
            == b"0x1::reconfiguration_state::StateActive" ==>
        copyable_any::UnpackAbortsIf<StateActive> {
            self: global<State>(@accudo_framework).variant
        };
        aborts_if copyable_any::type_name(global<State>(@accudo_framework).variant).bytes
            != b"0x1::reconfiguration_state::StateActive";
    }
