...
module accudo_framework::stake {
    use std::error;
    use std::features;
    use std::option::{Self, Option};
    use std::signer;
    use std::vector;
    use accudo_std::bls12381;
    use accudo_std::math64::min;
    use accudo_std::big_ordered_map::{Self, BigOrderedMap};
    use accudo_std::table::Table;
    use accudo_framework::aggregator_v2::{Self, Aggregator};
    use accudo_framework::accudo_coin::AccudoCoin;
    use accudo_framework::account;
    use accudo_framework::coin::{Self, Coin, MintCapability};
    use accudo_framework::event::{Self, EventHandle};
    use accudo_framework::timestamp;
    use accudo_framework::system_addresses;
    use accudo_framework::staking_config::{Self, StakingConfig, StakingRewardsConfig};
    use accudo_framework::chain_status;
    use accudo_framework::permissioned_signer;

    friend accudo_framework::block;
    friend accudo_framework::genesis;
    friend accudo_framework::reconfiguration;
    friend accudo_framework::reconfiguration_with_dkg;
    friend accudo_framework::transaction_fee;
    ...
    struct StakePool has key {
        // active stake
        active: Coin<AccudoCoin>,
        // inactive stake, can be withdrawn
        inactive: Coin<AccudoCoin>,
        // pending activation for next epoch
        pending_active: Coin<AccudoCoin>,
        // pending deactivation for next epoch
        pending_inactive: Coin<AccudoCoin>,
    ...
    struct AccudoCoinCapabilities has key {
        mint_cap: MintCapability<AccudoCoin>,
    }
    ...
    struct ValidatorFees has key {
        fees_table: Table<address, Coin<AccudoCoin>>,
    }
    ...
    public fun get_validator_state(pool_address: address): u64 acquires ValidatorSet {
        let validator_set = borrow_global<ValidatorSet>(@accudo_framework);
    ...
    public fun get_current_epoch_proposal_counts(validator_index: u64): (u64, u64) acquires ValidatorPerformance {
        let validator_performances = &borrow_global<ValidatorPerformance>(@accudo_framework).validators;
    ...
    public fun get_pending_transaction_fee(): vector<u64> acquires PendingTransactionFee {
        ...
        let fee_table = &borrow_global<PendingTransactionFee>(@accudo_framework).pending_fee_by_validator;
    ...
    public(friend) fun initialize(accudo_framework: &signer) {
        system_addresses::assert_accudo_framework(accudo_framework);

        move_to(accudo_framework, ValidatorSet {
            consensus_scheme: 0,
        ...
        move_to(accudo_framework, ValidatorPerformance {
    ...
    public(friend) fun store_accudo_coin_mint_cap(accudo_framework: &signer, mint_cap: MintCapability<AccudoCoin>) {
        system_addresses::assert_accudo_framework(accudo_framework);
        move_to(accudo_framework, AccudoCoinCapabilities { mint_cap })
    }

    /// Allow on chain governance to remove validators from the validator set.
    public fun remove_validators(
        accudo_framework: &signer,
        validators: &vector<address>,
    ) acquires ValidatorSet {
        assert_reconfig_not_in_progress();
        system_addresses::assert_accudo_framework(accudo_framework);
        let validator_set = borrow_global_mut<ValidatorSet>(@accudo_framework);
    ...
    public fun initialize_pending_transaction_fee(framework: &signer) {
        ...
        if (!exists<PendingTransactionFee>(@accudo_framework)) {
    ...
    public fun set_transaction_fee_config(framework: &signer, config: TransactionFeeConfig) acquires TransactionFeeConfig {
        ...
        if (exists<TransactionFeeConfig>(@accudo_framework)) {
            *borrow_global_mut<TransactionFeeConfig>(@accudo_framework) = config;
        } else {
    ...
    public(friend) fun record_fee(
        ...
        let pending_fee = borrow_global_mut<PendingTransactionFee>(@accudo_framework);
        let i = 0;
    ...
    fun initialize_owner(owner: &signer) acquires AllowedValidators {
        ...
        move_to(owner, StakePool {
            active: coin::zero<AccudoCoin>(),
            pending_active: coin::zero<AccudoCoin>(),
            pending_inactive: coin::zero<AccudoCoin>(),
            inactive: coin::zero<AccudoCoin>(),
            locked_until_secs: 0,
    ...
    public entry fun add_stake(owner: &signer, amount: u64) acquires OwnerCapability, StakePool, ValidatorSet {
        ...
        assert_owner_cap_exists(owner_address);
        let ownership_cap = borrow_global<OwnerCapability>(owner_address);
        add_stake_with_cap(ownership_cap, coin::withdraw<AccudoCoin>(owner, amount));
    }

    /// Add `coins` into `pool_address`. this requires the corresponding `owner_cap` to be passed in.
    public fun add_stake_with_cap(owner_cap: &OwnerCapability, coins: Coin<AccudoCoin>) acquires StakePool, ValidatorSet {
        assert_reconfig_not_in_progress();
        let pool_address = owner_cap.pool_address;
        assert_stake_pool_exists(pool_address);
        ...
        if (is_current_epoch_validator(pool_address)) {
            coin::merge<AccudoCoin>(&mut stake_pool.pending_active, coins);
        } else {
            coin::merge<AccudoCoin>(&mut stake_pool.active, coins);
        };
    ...
    public fun unlock_with_cap(amount: u64, owner_cap: &OwnerCapability) acquires StakePool {
        ...
        coin::merge<AccudoCoin>(&mut stake_pool.pending_inactive, unlocked_stake);
    ...
    public entry fun withdraw(
        ...
        coin::deposit<AccudoCoin>(owner_address, coins);
    }
    ...
    public fun withdraw_with_cap(
        ...
        withdraw_amount: u64
    ): Coin<AccudoCoin> acquires StakePool, ValidatorSet {
        assert_reconfig_not_in_progress();
        let pool_address = owner_cap.pool_address;
        assert_stake_pool_exists(pool_address);
        let stake_pool = borrow_global_mut<StakePool>(pool_address);
        ...
        if (withdraw_amount == 0) return coin::zero<AccudoCoin>();
    ...
    public(friend) fun update_performance_statistics(
        ...
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@accudo_framework);
    ...
    public(friend) fun on_new_epoch(
    ) acquires AccudoCoinCapabilities, PendingTransactionFee, StakePool, TransactionFeeConfig, ValidatorConfig, ValidatorPerformance, ValidatorSet {
        let validator_set = borrow_global_mut<ValidatorSet>(@accudo_framework);
        let config = staking_config::get();
        let validator_perf = borrow_global_mut<ValidatorPerformance>(@accudo_framework);
        ...
        if (exists<PendingTransactionFee>(@accudo_framework)) {
            let pending_fee_by_validator = &mut borrow_global_mut<PendingTransactionFee>(@accudo_framework).pending_fee_by_validator;
    ...
    public fun next_validator_consensus_infos(): vector<ValidatorConsensusInfo> acquires ValidatorSet, ValidatorPerformance, StakePool, ValidatorConfig {
        // Init.
        let cur_validator_set = borrow_global<ValidatorSet>(@accudo_framework);
        let staking_config = staking_config::get();
        let validator_perf = borrow_global<ValidatorPerformance>(@accudo_framework);
    ...
    fun update_stake_pool(
    ...
    ) acquires AccudoCoinCapabilities, PendingTransactionFee, StakePool, TransactionFeeConfig, ValidatorConfig {
        let stake_pool = borrow_global_mut<StakePool>(pool_address);
        let validator_config = borrow_global<ValidatorConfig>(pool_address);
        ...
        let fee_active = 0;
        let fee_limit = if (exists<TransactionFeeConfig>(@accudo_framework)) {
            let TransactionFeeConfig::V0 { max_fee_octa_allowed_per_epoch_per_pool } = borrow_global<TransactionFeeConfig>(@accudo_framework);
        ...
        if (std::features::is_distribute_transaction_fee_enabled()) {
            let mint_cap = &borrow_global<AccudoCoinCapabilities>(@accudo_framework).mint_cap;
            if (fee_active > 0) {
    ...
    fun distribute_rewards(
        stake: &mut Coin<AccudoCoin>,
        num_successful_proposals: u64,
        num_total_proposals: u64,
        rewards_rate: u64,
        rewards_rate_denominator: u64,
    ): u64 acquires AccudoCoinCapabilities {
    ...
    public fun configure_allowed_validators(
        ...
        let accudo_framework_address = signer::address_of(accudo_framework);
        system_addresses::assert_accudo_framework(accudo_framework);
        if (!exists<AllowedValidators>(accudo_framework_address)) {
            move_to(accudo_framework, AllowedValidators { accounts });
        } else {
            let allowed = borrow_global_mut<AllowedValidators>(accudo_framework_address);
            allowed.accounts = accounts;
        }
    }

    fun is_allowed(account: address): bool acquires AllowedValidators {
        if (!exists<AllowedValidators>(@accudo_framework)) {
            true
        } else {
            let allowed = borrow_global<AllowedValidators>(@accudo_framework);
    ...
    #[test_only]
    use accudo_framework::accudo_coin;
    use accudo_std::bls12381::proof_of_possession_from_bytes;
    use accudo_framework::reconfiguration_state;
    use accudo_framework::validator_consensus_info;
    use accudo_framework::validator_consensus_info::ValidatorConsensusInfo;
    #[test_only]
    use accudo_std::fixed_point64;
    ...
    #[test_only]
    public fun initialize_for_test(accudo_framework: &signer) {
        reconfiguration_state::initialize(accudo_framework);
        initialize_for_test_custom(accudo_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 1000000);
        // In the test environment, the periodical_reward_rate_decrease feature is initially turned off.
        features::change_feature_flags_for_testing(accudo_framework, vector[], vector[features::get_periodical_reward_rate_decrease_feature()]);
    }
    ...
    public fun initialize_for_test_custom(
        ...
        timestamp::set_time_has_started_for_testing(accudo_framework);
        reconfiguration_state::initialize(accudo_framework);
        if (!exists<ValidatorSet>(@accudo_framework)) {
            initialize(accudo_framework);
        };
        ...
        if (!exists<AccudoCoinCapabilities>(@accudo_framework)) {
            let (burn_cap, mint_cap) = accudo_coin::initialize_for_test(accudo_framework);
            store_accudo_coin_mint_cap(accudo_framework, mint_cap);
            coin::destroy_burn_cap<AccudoCoin>(burn_cap);
        };
    ...
    // This function assumes the stake module already the capability to mint accudo coins.
    #[test_only]
    public fun mint_coins(amount: u64): Coin<AccudoCoin> acquires AccudoCoinCapabilities {
        let mint_cap = &borrow_global<AccudoCoinCapabilities>(@accudo_framework).mint_cap;
        coin::mint(amount, mint_cap)
    }

    #[test_only]
    public fun mint(account: &signer, amount: u64) acquires AccudoCoinCapabilities {
        coin::register<AccudoCoin>(account);
        coin::deposit(signer::address_of(account), mint_coins(amount));
    }

    #[test_only]
    public fun mint_and_add_stake(
        account: &signer, amount: u64) acquires AccudoCoinCapabilities, OwnerCapability, StakePool, ValidatorSet {
        mint(account, amount);
    ...
    public fun initialize_test_validator(
        ...
        should_end_epoch: bool,
    ) acquires AllowedValidators, AccudoCoinCapabilities, OwnerCapability, PendingTransactionFee, StakePool, TransactionFeeConfig, ValidatorConfig, ValidatorPerformance, ValidatorSet {
    ...
    #[test(accudo_framework = @accudo_framework, validator = @0x123)]
    #[expected_failure(abort_code = 0x10007, location = Self)]
    public entry fun test_inactive_validator_can_add_stake_if_exceeding_max_allowed(
        accudo_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, AccudoCoinCapabilities, OwnerCapability, PendingTransactionFee, StakePool, TransactionFeeConfig, ValidatorConfig, ValidatorPerformance, ValidatorSet {
        initialize_for_test(accudo_framework);
    ...
    #[test(accudo_framework = @0x1, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x10007, location = Self)]
    public entry fun test_pending_active_validator_cannot_add_stake_if_exceeding_max_allowed(
        ...
        initialize_for_test_custom(accudo_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 100000);
    ...
    #[test(accudo_framework = @accudo_framework, validator_1 = @0x123, validator_2 = @0x234)]
    #[expected_failure(abort_code = 0x10007, location = Self)]
    ...
    public entry fun test_end_to_end(
        ...
        assert!(coin::balance<AccudoCoin>(validator_address) == 800, 2);
        ...
        assert!(coin::balance<AccudoCoin>(validator_address) == 850, 6);
        assert_validator_state(validator_address, 102, 51, 0, 0, 0);
        withdraw(validator, 51);
        assert!(coin::balance<AccudoCoin>(validator_address) == 901, 7);
    ...
    public entry fun test_cannot_reduce_lockup(
        ...
        staking_config::update_recurring_lockup_duration_secs(accudo_framework, 1);
    ...
    public entry fun test_inactive_validator_cannot_join_if_exceed_increase_limit(
        ...
        initialize_for_test_custom(accudo_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 50);
        let (_sk_1, pk_1, pop_1) = generate_identity();
        let (_sk_2, pk_2, pop_2) = generate_identity();
    ...
    public entry fun test_pending_active_validator_can_add_more_stake(
        ...
        initialize_for_test_custom(accudo_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 10000);
    ...
    public entry fun test_pending_active_validator_cannot_add_more_stake_than_limit(
        ...
        initialize_for_test_custom(accudo_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 10, 100);
        // Need 1 validator to be in the active validator set so joining limit works.
        let (_sk_1, pk_1, pop_1) = generate_identity();
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100, true, true);
    ...
    public entry fun test_pending_active_validator_leaves_validator_set(
        ...
        assert!(borrow_global<ValidatorSet>(@accudo_framework).total_joining_power == 100, 0);
        ...
        assert!(borrow_global<ValidatorSet>(@accudo_framework).total_joining_power == 0, 1);
    }
    ...
    public entry fun test_active_validator_withdraw_should_cap_by_inactive_stake(
        ...
        assert!(coin::balance<AccudoCoin>(validator_address) == 1001, 2);
    ...
    #[test(accudo_framework = @accudo_framework, validator = @0x123, validator_2 = @0x234)]
    ...
    #[test(accudo_framework = @0x1, validator_1 = @0x123, validator_2 = @0x234, validator_3 = @0x345)]
    public entry fun test_multiple_validators_join_and_leave(
        ...
        initialize_for_test_custom(accudo_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 100);
        let (_sk_1, pk_1, pop_1) = generate_identity();
        ...
        assert!(
            vector::borrow(
                &borrow_global<ValidatorSet>(@accudo_framework).pending_inactive,
                0
        ...
        assert!(
            vector::borrow(
                &borrow_global<ValidatorSet>(@accudo_framework).pending_active,
                0
        ...
        assert!(
            vector::borrow(
                &borrow_global<ValidatorSet>(@accudo_framework).active_validators,
                0
    ...
    public entry fun test_delegated_staking_with_owner_cap(
        ...
        // Cleanups.
        coin::register<AccudoCoin>(validator);
    ...
    public entry fun test_validator_cannot_join_post_genesis(
        ...
        initialize_for_test_custom(accudo_framework, 100, 10000, LOCKUP_CYCLE_SECONDS, false, 1, 100, 100);
    ...
    #[test(
        accudo_framework = @accudo_framework,
        validator_1 = @accudo_framework,
        validator_2 = @0x2,
        validator_3 = @0x3,
        validator_4 = @0x4,
        validator_5 = @0x5
    ...
    fun test_validator_consensus_infos_from_validator_set(
        ...
        end_epoch();
        let vci_vec_0 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@accudo_framework));
        ...
        assert!(vector[@0x5, @accudo_framework, @0x3] == vci_addrs, 1);
        ...
        let vci_vec_1 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@accudo_framework));
        assert!(vci_vec_0 == vci_vec_1, 11);
        join_validator_set(validator_2, v2_addr);
        let vci_vec_2 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@accudo_framework));
        assert!(vci_vec_0 == vci_vec_2, 12);
        leave_validator_set(validator_1, v1_addr);
        let vci_vec_3 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@accudo_framework));
        assert!(vci_vec_0 == vci_vec_3, 13);
        join_validator_set(validator_4, v4_addr);
        let vci_vec_4 = validator_consensus_infos_from_validator_set(borrow_global<ValidatorSet>(@accudo_framework));
    ...
    public entry fun test_validator_rewards_rate_decrease_over_time(
        ...
        features::change_feature_flags_for_testing(accudo_framework, vector[features::get_periodical_reward_rate_decrease_feature()], vector[]);
    ...
    public entry fun test_invalid_config(
        accudo_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, AccudoCoinCapabilities, OwnerCapability, StakePool, ValidatorConfig, ValidatorSet {
        initialize_for_test_custom(accudo_framework, 50, 10000, LOCKUP_CYCLE_SECONDS, true, 1, 100, 100);
    ...
    public entry fun test_removing_validator_from_active_set(
        ...
        initialize_test_validator(&pk_1, &pop_1, validator_1, 100, true, false);
        initialize_test_validator(&pk_2, &pop_2, validator_2, 100, true, true);
        assert!(vector::length(&borrow_global<ValidatorSet>(@accudo_framework).active_validators) == 2, 0);
        ...
        remove_validators(accudo_framework, &vector[validator_to_remove]);
        assert!(vector::length(&borrow_global<ValidatorSet>(@accudo_framework).active_validators) == 1, 0);
        assert!(get_validator_state(validator_to_remove) == VALIDATOR_STATUS_PENDING_INACTIVE, 1);
    }

    #[test(vm = @0x0, accudo_framework = @0x1, validator_0 = @0x123, validator_1 = @0x234)]
    public entry fun test_transaction_fee(
        ...
        initialize_for_test(accudo_framework);
        initialize_pending_transaction_fee(accudo_framework);
        features::change_feature_flags_for_testing(accudo_framework, vector[features::get_distribute_transaction_fee_feature()], vector[]);
    ...
    public entry fun test_transaction_fee_limit(
...

        {
            ...
            set_transaction_fee_config(accudo_framework, config);
            end_epoch();

            assert!(event::was_event_emitted(&DistributeTransactionFee { pool_address: address_0, fee_amount: 11 }), 0);
            assert!(event::was_event_emitted(&DistributeTransactionFee { pool_address: address_1, fee_amount: 20 }), 0);
        };

        // Make sure we can set the config multiple times.
        let config = TransactionFeeConfig::V0 {
            max_fee_octa_allowed_per_epoch_per_pool: 200,
        };
        set_transaction_fee_config(accudo_framework, config);
    }
    ...
    #[test(accudo_framework = @0x1, validator = @0x123)]
    public entry fun test_allowed_validators(
        accudo_framework: &signer,
        validator: &signer,
    ) acquires AllowedValidators, OwnerCapability, StakePool, ValidatorSet {
        let addr = signer::address_of(validator);
        let (burn, mint) = accudo_coin::initialize_for_test(accudo_framework);
        configure_allowed_validators(accudo_framework, vector[addr]);
    ...
    public entry fun test_not_allowed_validators(
        ...
        configure_allowed_validators(accudo_framework, vector[]);
        let (burn, mint) = accudo_coin::initialize_for_test(accudo_framework);
