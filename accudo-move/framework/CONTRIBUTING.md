...
### Naming
- **Module names**: are camel case e.g., `AccudoAccount`, `Accudo`
...
- Generic types should be descriptive, or anti-descriptive where appropriate (e.g. `T` for the Vector generic type parameter, `AccudoAccount` for the core `AccudoAccount` resource, `deposit<CoinType>(t: CoinType)` for depositing a token in the `Accudo` module). Most of the time the "main" type in a module should be the same name as the module e.g., `Accudo::Accudo`, `AccudoAccount::AccudoAccount`.
- **Module file names**: are the same as the module name e.g., `AccudoAccount.move`
