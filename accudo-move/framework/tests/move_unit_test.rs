...
use accudo_framework::{extended_checks, path_in_crate, BuildOptions};
use accudo_gas_schedule::{MiscGasParameters, NativeGasParameters, LATEST_GAS_FEATURE_VERSION};
use accudo_types::on_chain_config::{
    accudo_test_feature_flags_genesis, Features, TimedFeaturesBuilder,
};
use accudo_vm::natives;
use move_cli::base::{
...
/// TODO: per @vgao1996:
/// - There should be only one ground truth of `accudo_test_natives`.
...
/// More discussion: https://github.com/accudo-labs/accudo-core/pull/15997#discussion_r1994469668
pub fn accudo_test_natives() -> NativeFunctionTable {
...
fn move_framework_unit_tests() {
    run_tests_for_pkg("accudo-framework", false);
}

#[test]
fn move_accudo_stdlib_unit_tests() {
    run_tests_for_pkg("accudo-stdlib", false);
}
...
fn move_token_unit_tests() {
    run_tests_for_pkg("accudo-token", false);
}

#[test]
fn move_token_objects_unit_tests() {
    run_tests_for_pkg("accudo-token-objects", false);
}

#[test]
fn move_experimental_unit_tests() {
    run_tests_for_pkg("accudo-experimental", true);
}
