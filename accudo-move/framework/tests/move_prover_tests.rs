...
use accudo_framework::{extended_checks, prover::ProverOptions};
...
pub fn run_prover_for_pkg(
    ...
    if no_tools {
        ...
        See https://github.com/accudo-labs/accudo-core/blob/main/accudo-move/framework/FRAMEWORK-PROVER-GUIDE.md \
        for instructions, or \
...
fn move_framework_prover_tests() {
    run_prover_for_pkg("accudo-framework", 1, None);
}

#[test]
fn move_token_prover_tests() {
    run_prover_for_pkg("accudo-token", 1, None);
}

#[test]
fn move_accudo_stdlib_prover_tests() {
    run_prover_for_pkg("accudo-stdlib", 1, None);
}
