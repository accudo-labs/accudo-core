...
module accudo_experimental::confidential_proof {
    use std::error;
    use std::option;
    use std::option::Option;
    use std::vector;
    use accudo_std::ristretto255::{Self, CompressedRistretto, Scalar};
    use accudo_std::ristretto255_bulletproofs::{Self as bulletproofs, RangeProof};

    use accudo_experimental::confidential_balance;
    use accudo_experimental::ristretto255_twisted_elgamal as twisted_elgamal;

    friend accudo_experimental::confidential_asset;
    ...
    const FIAT_SHAMIR_WITHDRAWAL_SIGMA_DST: vector<u8> = b"AccudoConfidentialAsset/WithdrawalProofFiatShamir";
    const FIAT_SHAMIR_TRANSFER_SIGMA_DST: vector<u8> = b"AccudoConfidentialAsset/TransferProofFiatShamir";
    const FIAT_SHAMIR_ROTATION_SIGMA_DST: vector<u8> = b"AccudoConfidentialAsset/RotationProofFiatShamir";
    const FIAT_SHAMIR_NORMALIZATION_SIGMA_DST: vector<u8> = b"AccudoConfidentialAsset/NormalizationProofFiatShamir";

    const BULLETPROOFS_DST: vector<u8> = b"AccudoConfidentialAsset/BulletproofRangeProof";
    ...
    public fun prove_transfer(
        ...
        (
            TransferProof {
                sigma_proof: TransferSigmaProof {
                    xs: proof_xs,
                    alphas: TransferSigmaProofAlphas { a1s, a2, a3s, a4s, a5, a6s }
                },
                zkrp_new_balance,
                zkrp_transfer_amount
            },
    ...
    fun serialize_transfer_sigma_proof(proof: &TransferSigmaProof): vector<u8> {
        ...
        proof.alphas.a6s.for_each_ref(|alpha| {
            bytes.append(ristretto255::scalar_to_bytes(alpha));
        });
