...
///
/// This module allows users to "register" a veiled account for any pre-existing `accudo_framework::Coin` type `T` via
...
/// Accudo framework. This is because coin amounts must be encrypted with an *efficient*, additively-homomorphic encryption
...
/// ```
///
/// Recall that: A coin has a *decimal precision* $d$ (e.g., for `AccudoCoin`, $d = 8$; see `initialize` in
/// `accudo_coin.move`). This precision $d$ is used when displaying a `u64` amount, by dividing the amount by $10^d$.
...
module accudo_experimental::veiled_coin {
    use std::error;
    use std::option::Option;
    use std::signer;
    use std::vector;

    use accudo_std::ristretto255;
    use accudo_std::ristretto255_bulletproofs as bulletproofs;
    use accudo_std::ristretto255_bulletproofs::RangeProof;
    use accudo_std::ristretto255_elgamal as elgamal;
    use accudo_std::ristretto255_pedersen as pedersen;
    #[test_only]
    ...
    use accudo_experimental::helpers;
    use accudo_experimental::sigma_protos;
    ...
    const VEILED_COIN_BULLETPROOFS_DST: vector<u8> = b"AccudoVeiledCoin/BulletproofRangeProof";
    ...
    /// represent normal `accudo_framework::coin::Coin` values.
    ...
    fun get_resource_account_signer(): signer acquires VeiledCoinMinter {
        account::create_signer_with_capability(
            &borrow_global<VeiledCoinMinter>(@accudo_experimental).signer_cap
        )
    }
