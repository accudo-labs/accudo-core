...
module accudo_experimental::veiled_coin_tests {
    #[test_only]
    use std::features;
    #[test_only]
    use std::signer;
    ...
    #[test_only]
    use accudo_experimental::veiled_coin;
    #[test_only]
    use accudo_experimental::helpers::generate_elgamal_keypair;
    #[test_only]
    ...
    fun set_up_for_veiled_coin_test(
        ...
        assert!(signer::address_of(&accudo_fx) != signer::address_of(sender), 1);
        assert!(signer::address_of(&accudo_fx) != signer::address_of(recipient), 2);
        ...
        account::create_account_for_test(signer::address_of(&accudo_fx)); // needed in `coin::create_fake_money`
        ...
        // Create `amount` of `FakeCoin` coins at the Accudo 0x1 address (must do) and register a `FakeCoin` coin
        // store for the `sender`.
        coin::create_fake_money(
            &accudo_fx,
            veiled_coin::cast_u32_to_u64_amount(sender_amount + recipient_amount)
        );
        println(b"Created fake money inside @accudo_framework");
    ...
    #[
        test(
            veiled_coin = @accudo_experimental,
            accudo_fx = @accudo_framework,
            sender = @0xc0ffee,
            recipient = @0x1337
        )
    ]
    fun veil_test(
        ...
        print(&@accudo_experimental);
        println(b"@accudo_framework:");
        print(&@accudo_framework);
    ...
    #[test(
        veiled_coin = @accudo_experimental, accudo_fx = @accudo_framework, sender = @0x1337
    )]
    fun unveil_test(
        veiled_coin: signer, accudo_fx: signer, sender: signer
    ) {
        ...
        set_up_for_veiled_coin_test(&veiled_coin, accudo_fx, &sender, &sender, 500, 0);
