...
module accudo_token_objects::collection {
    use std::error;
    use std::features;
    use std::option::{Self, Option};
    use std::signer;
    use std::string::{Self, String};
    use accudo_framework::aggregator_v2::{Self, Aggregator, AggregatorSnapshot};
    use accudo_framework::event;
    use accudo_framework::object::{Self, ConstructorRef, ExtendRef, Object};

    use accudo_token_objects::royalty::{Self, Royalty};

    friend accudo_token_objects::token;
    ...
    fun test_create_mint_burn_for_unlimited(creator: &signer) acquires FixedSupply, UnlimitedSupply, ConcurrentSupply {
        ...
        let cid = aggregator_v2::read_snapshot(&increment_supply(&collection, creator_address).destroy_some());
    ...
    #[expected_failure(abort_code = 0x50003, location = accudo_framework::object)]
    ...
    entry fun test_create_and_transfer_as_owner(creator: &signer, trader: &signer, accudo_framework: &signer) {
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x1000A, location = accudo_token_objects::collection)]
    ...
    #[test(creator = @0x123)]
    #[expected_failure(abort_code = 0x20009, location = accudo_token_objects::collection)]
