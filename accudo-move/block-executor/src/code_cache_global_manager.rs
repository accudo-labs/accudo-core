...
use accudo_gas_schedule::gas_feature_versions::RELEASE_V1_34;
use accudo_types::{
    block_executor::{
...
macro_rules! alert_or_println {
    ($($arg:tt)*) => {
        if cfg!(any(test, feature = "testing")) {
            ...
            use accudo_vm_logging::{alert, prelude::CRITICAL_ERRORS};
...
pub struct ModuleCacheManager<K, D, V, E> {
    ...
    environment: Option<AccudoEnvironment>,
    ...
    E: WithSize,
{
    ...
    fn check_ready(
        &mut self,
        storage_environment: AccudoEnvironment,
        config: &BlockExecutorModuleCacheLocalConfig,
        transaction_slice_metadata: TransactionSliceMetadata,
    ) -> Result<(), VMStatus> {
...
/// Module cache manager used by Accudo block executor. Ensures that only one thread has exclusive
/// access to it at a time.
pub struct AccudoModuleCacheManager {
    inner: Mutex<ModuleCacheManager<ModuleId, CompiledModule, Module, AccudoModuleExtension>>,
}

impl AccudoModuleCacheManager {
    ...
    /// is ready for execution and updates states. If fails, [AccudoModuleCacheManagerGuard::None]
    ...
    fn try_lock_inner(
        ...
        Ok(match self.inner.try_lock() {
            ...
            None => {
                ...
                AccudoModuleCacheManagerGuard::None {
    ...
    /// Tries to lock the manager using [AccudoModuleCacheManager::try_lock_inner]. Additionally, if
    /// the module cache is empty, can prefetch Accudo framework into it.
    pub fn try_lock(
        &self,
        state_view: &impl StateView,
        config: &BlockExecutorModuleCacheLocalConfig,
        ...
        if guard.module_cache().num_modules() == 0 && config.prefetch_framework_code {
            prefetch_accudo_framework(state_view, &mut guard).map_err(|err| {
                alert_or_println!("Failed to load Accudo framework to module cache: {:?}", err);
...
/// A guard that can be acquired from [AccudoModuleCacheManager]. Variants represent successful and
/// no-successful lock acquisition.
pub enum AccudoModuleCacheManagerGuard<'a> {
    /// Holds the guard to the [AccudoModuleCacheManager], and has exclusive access to it.
    Guard {
        guard: MutexGuard<
            'a,
            ModuleCacheManager<ModuleId, CompiledModule, Module, AccudoModuleExtension>,
        >,
    },
    /// Either there is no [AccudoModuleCacheManager], or acquiring the lock for it failed.
    None {
        environment: AccudoEnvironment,
        module_cache: GlobalModuleCache<ModuleId, CompiledModule, Module, AccudoModuleExtension>,
    },
}

impl AccudoModuleCacheManagerGuard<'_> {
    /// Returns the references to the environment. If environment is not set, panics.
    pub fn environment(&self) -> &AccudoEnvironment {
        use AccudoModuleCacheManagerGuard::*;
        match self {
            Guard { guard } => guard
                .environment
                .as_ref()
    ...
    pub fn module_cache(
        &self,
    ) -> &GlobalModuleCache<ModuleId, CompiledModule, Module, AccudoModuleExtension> {
        use AccudoModuleCacheManagerGuard::*;
        match self {
    ...
    pub fn module_cache_mut(
        &mut self,
    ) -> &mut GlobalModuleCache<ModuleId, CompiledModule, Module, AccudoModuleExtension> {
        use AccudoModuleCacheManagerGuard::*;
        match self {
    ...
    /// A guard in [AccudoModuleCacheManagerGuard::None] state with empty module cache and default
    /// environment. Use for testing only.
    #[cfg(test)]
    pub(crate) fn none() -> Self {
        use accudo_types::state_store::MockStateView;
        Self::none_for_state_view(&MockStateView::empty())
    }

    /// A guard in [AccudoModuleCacheManagerGuard::None] state with empty module cache and the
    ...
    pub(crate) fn none_for_state_view(state_view: &impl StateView) -> Self {
        AccudoModuleCacheManagerGuard::None {
            environment: AccudoEnvironment::new(state_view),
            module_cache: GlobalModuleCache::empty(),
        }
    }
}

/// If Accudo framework exists, loads "transaction_validation.move" and all its transitive
/// dependencies from storage into provided module cache. If loading fails for any reason, a panic
/// error is returned.
fn prefetch_accudo_framework(
    state_view: &impl StateView,
    guard: &mut AccudoModuleCacheManagerGuard,
) -> Result<(), PanicError> {
    let code_storage = state_view.as_accudo_code_storage(guard.environment());
    ...
    cfg_if! {
        if #[cfg(fuzzing)] {
            let maybe_loaded = code_storage
                ...
                .map_err(|err| {
                    PanicError::CodeInvariantError(format!("Unable to fetch Accudo framework: {:?}", err))
                })?;
        } else {
            let maybe_loaded = code_storage
...
mod test {
    ...
    #[test]
    fn test_prefetch_existing_accudo_framework() {
        let state_view = InMemoryStateStore::from_head_genesis();

        let mut guard = AccudoModuleCacheManagerGuard::none_for_state_view(&state_view);
        assert_eq!(guard.module_cache().num_modules(), 0);

        let result = prefetch_accudo_framework(&state_view, &mut guard);
        assert!(result.is_ok());
        assert!(guard.module_cache().num_modules() > 0);
    }

    #[test]
    fn test_prefetch_non_existing_accudo_framework() {
    ...
    fn test_check_ready() {
        ...
        assert_ok!(manager.check_ready(AccudoEnvironment::new(&state_view), &config, metadata_1));
        ...
        assert_ok!(manager.check_ready(AccudoEnvironment::new(&state_view), &config, metadata_2));
        ...
        assert_ok!(manager.check_ready(AccudoEnvironment::new(&state_view), &config, metadata_3));
        ...
        assert_ok!(manager.check_ready(AccudoEnvironment::new(&state_view), &config, metadata_4));
        ...
        assert_ok!(manager.check_ready(AccudoEnvironment::new(&state_view), &config, metadata_5));
        ...
        assert_ok!(manager.check_ready(AccudoEnvironment::new(&state_view), &config, metadata_6));
    ...
    fn test_try_lock_inner_single_thread() {
        let manager = AccudoModuleCacheManager::new();
        ...
        assert!(matches!(guard, AccudoModuleCacheManagerGuard::Guard { .. }));
    }

    #[test]
    fn test_try_lock_inner_multiple_threads() {
        let manager = Arc::new(AccudoModuleCacheManager::new());
        ...
        for _ in 0..num_threads {
            let handle = std::thread::spawn({
                ...
                move || {
                    ...
                    if matches!(guard, AccudoModuleCacheManagerGuard::Guard { .. }) {
                        1
                    } else {
                        0
                    }
