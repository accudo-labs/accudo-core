...
impl<K: Ord + Clone + Debug + Eq + PartialEq + Hash, E: Clone> MockOutputBuilder<K, E> {
    ...
    pub(crate) fn add_module_reads<S: AccudoCodeStorage>(
        &mut self,
        view: &S,
        module_ids: &[ModuleId],
    ...
    E: Send + Sync + Debug + Clone + TransactionEvent + 'static,
{
    ...
    fn committed_output(&self) -> &OnceCell<accudo_types::transaction::TransactionOutput> {
    ...
    E: Send + Sync + Debug + Clone + TransactionEvent + 'static,
{
    ...
    fn init(_environment: &AccudoEnvironment, _state_view: &impl TStateView<Key = K>) -> Self {
        Self::new()
    }
