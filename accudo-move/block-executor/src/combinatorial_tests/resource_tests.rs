...
///
/// # Arguments
/// * `guard` - The AccudoModuleCacheManagerGuard to populate with empty modules
...
pub(crate) fn populate_guard_with_modules(
    guard: &mut AccudoModuleCacheManagerGuard<'_>,
    module_ids: &[ModuleId],
) {
    for module_id in module_ids {
        ...
        let module_code = Arc::new(ModuleCode::from_arced_verified(
            Arc::new(module),
            Arc::new(AccudoModuleExtension::new(StateValue::new_legacy(
...
{
    let mut guard = AccudoModuleCacheManagerGuard::none();
