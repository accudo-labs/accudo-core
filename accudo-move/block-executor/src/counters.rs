...
use accudo_mvhashmap::BlockStateStats;
use accudo_types::fee_statement::FeeStatement;
...
pub static BLOCK_EXECUTOR_INNER_EXECUTE_BLOCK: Lazy<Histogram> = Lazy::new(|| {
    register_histogram!(
        // metric name
        "accudo_executor_block_executor_inner_execute_block_seconds",
        // metric description
...
pub static MODULE_PUBLISHING_FALLBACK_COUNT: Lazy<IntCounter> = Lazy::new(|| {
    register_int_counter!(
        "accudo_execution_module_publishing_fallback_count",
...
pub static SPECULATIVE_ABORT_COUNT: Lazy<IntCounter> = Lazy::new(|| {
    register_int_counter!(
        "accudo_execution_speculative_abort_count",
...
pub static EXCEED_PER_BLOCK_GAS_LIMIT_COUNT: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        "accudo_execution_gas_limit_count",
        "Count of times the BlockSTM is early halted due to exceeding the per-block gas limit",
        &["mode"]
    )
    .unwrap()
...
pub static EXCEED_PER_BLOCK_OUTPUT_LIMIT_COUNT: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        "accudo_execution_output_limit_count",
        "Count of times the BlockSTM is early halted due to exceeding the per-block output size limit",
        &["mode"]
    )
    .unwrap()
...
pub static PARALLEL_EXECUTION_SECONDS: Lazy<Histogram> = Lazy::new(|| {
    register_histogram!(
        // metric name
        "accudo_parallel_execution_seconds",
        // metric description
...
pub static RAYON_EXECUTION_SECONDS: Lazy<Histogram> = Lazy::new(|| {
    register_histogram!(
        // metric name
        "accudo_rayon_execution_seconds",
        // metric description
...
pub static PARALLEL_FINALIZE_SECONDS: Lazy<Histogram> = Lazy::new(|| {
    register_histogram!(
        // metric name
        "accudo_finalize_parallel_execution_seconds",
        // metric description
...
pub static VM_INIT_SECONDS: Lazy<Histogram> = Lazy::new(|| {
    register_histogram!(
        // metric name
        "accudo_execution_vm_init_seconds",
        // metric description
...
pub static TASK_VALIDATE_SECONDS: Lazy<Histogram> = Lazy::new(|| {
    register_histogram!(
        // metric name
        "accudo_execution_task_validate_seconds",
        // metric description
...
pub static WORK_WITH_TASK_SECONDS: Lazy<Histogram> = Lazy::new(|| {
    register_histogram!(
        // metric name
        "accudo_execution_work_with_task_seconds",
        // metric description
...
pub static TASK_EXECUTE_SECONDS: Lazy<Histogram> = Lazy::new(|| {
    register_histogram!(
        // metric name
        "accudo_execution_task_execute_seconds",
        // metric description
...
pub static DEPENDENCY_WAIT_SECONDS: Lazy<Histogram> = Lazy::new(|| {
    register_histogram!(
        "accudo_execution_dependency_wait",
...
pub static EFFECTIVE_BLOCK_GAS: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        "accudo_execution_effective_block_gas",
...
pub static APPROX_BLOCK_OUTPUT_SIZE: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        "accudo_execution_approx_block_output_size",
...
pub static BLOCK_COMMITTED_TXNS: Lazy<HistogramVec> = Lazy::new(|| {
    register_histogram_vec!(
        "accudo_execution_block_committed_txns",
...
pub static BLOCK_VIEW_DISTINCT_KEYS: Lazy<HistogramVec> = Lazy::new(|| {
    register_avg_counter_vec(
        "accudo_execution_block_view_distinct_keys",
        "Size (number of keys) ",
        &["mode", "object_type"],
    )
});
...
pub static BLOCK_VIEW_BASE_VALUES_MEMORY_USAGE: Lazy<HistogramVec> = Lazy::new(|| {
    register_avg_counter_vec(
        "accudo_execution_block_view_base_values_memory_usage",
...
pub static HOT_STATE_OP_ACCUMULATOR_COUNTER: Lazy<IntCounterVec> = Lazy::new(|| {
    register_int_counter_vec!(
        // metric name
        "accudo_hot_state_op_accumulator_counter",
        // metric description
