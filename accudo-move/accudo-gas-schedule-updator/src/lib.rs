...
fn generate_script(gas_schedule: &GasScheduleV2) -> Result<String> {
    ...
    emitln!(writer, "use accudo_framework::gas_schedule;");
    emitln!(writer);
...
fn accudo_framework_path() -> PathBuf {
    Path::join(
        Path::new(env!("CARGO_MANIFEST_DIR")),
        "../framework/accudo-framework",
    )
}
...
pub fn current_gas_schedule(feature_version: u64) -> GasScheduleV2 {
    GasScheduleV2 {
        feature_version,
        entries: AccudoGasParameters::initial().to_on_chain_gas_schedule(feature_version),
    }
}
