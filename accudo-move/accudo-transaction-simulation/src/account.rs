...
//! Test infrastructure for modeling Accudo accounts.
//! Ported from the old `e2e-tests` crate.

use accudo_crypto::ed25519::*;
 ...
 *
 **************************************************************************************************/
/// Details about a Accudo account.
///
/// Tests will typically create a set of `Account` instances to run transactions on. This type
/// encodes the logic to operate on and verify operations on any Accudo account.
///
...
impl Account {
    ...
    /// not automatically get added to the Accudo store. To add an account to the store, use
    ...
    pub fn with_keypair(privkey: Ed25519PrivateKey, pubkey: Ed25519PublicKey) -> Self {
        let addr = accudo_types::account_address::from_public_key(&pubkey);
        Account {
            addr,
            privkey,
            pubkey: AccountPublicKey::Ed25519(pubkey),
    ...
    /// Creates a new account representing the accudo root account in memory.
    ///
    /// The address will be [`accudo_test_root_address`][account_config::accudo_test_root_address], and
    /// the account will use [`GENESIS_KEYPAIR`][static@GENESIS_KEYPAIR] as its keypair.
    pub fn new_accudo_root() -> Self {
        Self::new_genesis_account(account_config::accudo_test_root_address())
    }
...
impl CoinStore {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        let coin_store = CoinStoreResource::<AccudoCoinType>::new(
            self.coin,
            self.frozen,
...
impl AccountData {
    ...
    pub fn to_writeset(&self) -> WriteSet {
        ...
        if let Some(coin_store) = &self.coin_store {
            write_set.push((
                StateKey::resource_typed::<CoinStoreResource<AccudoCoinType>>(self.address())
                    .unwrap(),
