...
use crate::{
    ...
    natives::accudo_natives_with_builder,
    prod_configs::{
        accudo_default_ty_builder, accudo_prod_ty_builder, accudo_prod_vm_config,
        get_timed_feature_override,
    },
};
use accudo_gas_algebra::DynamicExpression;
use accudo_gas_schedule::{AccudoGasParameters, MiscGasParameters, NativeGasParameters};
use accudo_native_interface::SafeNativeBuilder;
use accudo_types::{
    chain_id::ChainId,
    on_chain_config::{
...
use accudo_vm_types::storage::StorageGasParameters;
...
pub struct AccudoEnvironment(Arc<Environment>);

impl AccudoEnvironment {
    ...
    /// on-chain.
    #[inline]
    pub fn gas_params(&self) -> &Result<AccudoGasParameters, String> {
        &self.0.gas_params
    }
...
impl Clone for AccudoEnvironment {
    fn clone(&self) -> Self {
        Self(self.0.clone())
    }
}

impl PartialEq for AccudoEnvironment {
    fn eq(&self, other: &Self) -> bool {
        self.0.hash == other.0.hash
    }
}

impl Eq for AccudoEnvironment {}

impl WithRuntimeEnvironment for AccudoEnvironment {
...
struct Environment {
    ...
    /// on-chain.
    gas_params: Result<AccudoGasParameters, String>,
...
impl Environment {
    fn new(
        ...
        let (native_gas_params, misc_gas_params, ty_builder) = match &gas_params {
            Ok(gas_params) => {
                let ty_builder = accudo_prod_ty_builder(gas_feature_version, gas_params);
                (
            ...
            Err(_) => {
                let ty_builder = accudo_default_ty_builder();
                (
        ...
        let natives = accudo_natives_with_builder(&mut builder, inject_create_signer_for_gov_sim);
        let vm_config =
            accudo_prod_vm_config(gas_feature_version, &features, &timed_features, ty_builder);
        let verifier_bytes =
...
pub mod tests {
    ...
    fn test_environment_eq() {
        let state_view = MockStateView::empty();
        let environment_1 = AccudoEnvironment::new(&state_view);
        let environment_2 = AccudoEnvironment::new(&state_view);
    ...
    fn test_environment_ne() {
        ...
        for i in 0..state_views.len() {
            for j in 0..state_views.len() {
                if i != j {
                    let environment_1 = AccudoEnvironment::new(&state_views[i]);
                    let environment_2 = AccudoEnvironment::new(&state_views[j]);
                    assert!(environment_1 != environment_2);
                }
            }
        }
    ...
    fn test_environment_with_injected_create_signer_for_gov_sim() {
        ...
        let not_injected_envs = [
            AccudoEnvironment::new(&state_view),
            AccudoEnvironment::new_with_gas_hook(&state_view, Arc::new(|_| {})),
        ...
        // Injected.
        let env = AccudoEnvironment::new_with_injected_create_signer_for_gov_sim(&state_view);
        #[allow(deprecated)]
