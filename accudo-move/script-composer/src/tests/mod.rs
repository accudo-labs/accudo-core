...
fn simple_builder() {
    ...
    load_module(&mut builder, &h, "0x1::accudo_account");
    builder
        .add_batched_call(
            "0x1::accudo_account".to_string(),
            "transfer".to_string(),
            vec![],
            vec![
    ...
    assert_eq!(h.read_accudo_balance(bob.address()), 1_000_000_000_000_010);
...
fn chained_deposit() {
    ...
    load_module(&mut builder, &h, "0x1::accudo_coin");
    load_module(&mut builder, &h, "0x1::primary_fungible_store");
    let mut returns_1 = builder
        .add_batched_call(
            "0x1::coin".to_string(),
            "withdraw".to_string(),
            vec!["0x1::accudo_coin::AccudoCoin".to_string()],
            vec![
                CallArgument::new_signer(0),
                CallArgument::new_bytes(MoveValue::U64(10).simple_serialize().unwrap()),
            ],
