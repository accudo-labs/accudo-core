...
use accudo_gas_schedule::{InitialGasSchedule, TransactionGasParameters};
...
use accudo_types::{
    account_config::{
        accudo_test_root_address, primary_apt_store, AccountResource, FungibleStoreResource,
        ObjectGroupResource,
    },
...
/// The Accudo transaction test adapter.
///
...
///   - It executes transactions through AccudoVM, instead of MoveVM directly
struct AccudoTestAdapter<'a> {
...
/// Accudo-specific arguments for the publish command.
#[derive(Parser, Debug)]
struct AccudoPublishArgs {
...
/// Accudo-specifc arguments for the run command.
#[derive(Parser, Debug)]
struct AccudoRunArgs {
    #[clap(long = "private-key", value_parser = RawPrivateKey::parse)]
    private_key: Option<RawPrivateKey>,
...
/// Accudo-specifc arguments for the init command.
#[derive(Parser, Debug)]
struct AccudoInitArgs {
...
impl accudo_storage_interface::DbReader for FakeDbReader {
...
/// Default private key mappings for special Accudo accounts.
fn accudo_framework_private_key_mapping() -> Vec<(String, Ed25519PrivateKey)> {
...
fn compile_framework_sources(
    ...
    let named_address_mapping_strings: Vec<String> = accudo_framework::named_addresses()
        .iter()
    ...
    let options = move_compiler_v2::Options {
        ...
        known_attributes: accudo_framework::extended_checks::get_all_attribute_names().clone(),
        language_version,
...
// accudo-experimental can be using latest language features, while others cannot.
// So we need to compile it twice, once without accudo-experimental, and once with.

static PRECOMPILED_ACCUDO_FRAMEWORK_V2: Lazy<PrecompiledFilesModules> = Lazy::new(|| {
    compile_framework_sources(
        accudo_cached_packages::head_release_bundle()
            .files()
            .unwrap()
            .iter()
            .filter(|f| !f.contains("accudo-move/framework/accudo-experimental/sources"))
            .cloned()
            .collect::<Vec<_>>(),
        None,
    )
...
static PRECOMPILED_ACCUDO_FRAMEWORK_V2_WITH_EXPERIMENTAL: Lazy<PrecompiledFilesModules> =
    Lazy::new(|| {
        let named_address_mapping_strings: Vec<String> = accudo_framework::named_addresses()
            .iter()
            .map(|(string, num_addr)| format!("{}={}", string, num_addr))
        ...
        // accudo-experimental can be using latest language features, while others cannot.
        // So we need to compile it twice, once without accudo-experimental, and once with.
        // (in the second pass, we need to provide both, in case accudo-experimental depends on other modules)

        let all_sources = accudo_cached_packages::head_release_bundle()
            .files()
            .unwrap();
...
impl AccudoTestAdapter<'_> {
    ...
    fn fetch_account_resource(&self, signer_addr: &AccountAddress) -> AccountResource {
        self.storage
            ...
            .unwrap_or(AccountResource::new(
                0,
                signer_addr.to_vec(),
                accudo_types::event::EventHandle::new(
                    accudo_types::event::EventKey::new(0, *signer_addr),
                    0,
                ),
                accudo_types::event::EventHandle::new(
                    accudo_types::event::EventKey::new(1, *signer_addr),
                    0,
                ),
            ))
    }

    /// Obtain the AccudoCoin amount under address `signer_addr`
    ...
    fn run_transaction(&mut self, txn: Transaction) -> Result<TransactionOutput> {
        ...
        let mut outputs = AccudoVMBlockExecutor::new()
    ...
    fn create_and_fund_account(&mut self, account_addr: AccountAddress, amount: u64) {
        let parameters = self
            .fetch_transaction_parameters(&accudo_test_root_address(), None, None, None, None)
            .unwrap();

        let txn = RawTransaction::new(
            accudo_test_root_address(),
            parameters.sequence_number,
            accudo_cached_packages::accudo_stdlib::accudo_account_create_account(account_addr),
        ...
        let txn = RawTransaction::new(
            ...
            accudo_cached_packages::accudo_stdlib::accudo_coin_mint(account_addr, amount),
            parameters.max_gas_amount,
            parameters.gas_unit_price,
            parameters.expiration_timestamp_secs,
            ChainId::test(),
        ...
        self.run_transaction(Transaction::UserTransaction(txn))
            .expect("Failed to mint accudo coin. This should not happen.");
    }
}

impl<'a> MoveTestAdapter<'a> for AccudoTestAdapter<'a> {
    type ExtraInitArgs = AccudoInitArgs;
    type ExtraPublishArgs = AccudoPublishArgs;
    type ExtraRunArgs = AccudoRunArgs;
    type ExtraValueArgs = ();
    type Subcommand = AccudoSubCommand;
    ...
    fn known_attributes(&self) -> &BTreeSet<String> {
        accudo_framework::extended_checks::get_all_attribute_names()
    }
    ...
    fn init(
        ...
        let mut named_address_mapping = accudo_framework::named_addresses().clone();
        ...
        for (name, private_key) in accudo_framework_private_key_mapping() {
        ...
        let warnings_opt = match (syntax, opt_model) {
            ...
            (_, Some(model)) => {
                let _runtime_metadata =
                    accudo_framework::extended_checks::run_extended_checks(&model);
    ...
    fn view_data(
        ...
        match self.storage.get_state_value_bytes(&state_key).unwrap() {
            ...
            Some(data) => {
                let annotated =
                    AccudoValueAnnotator::new(&self.storage).view_resource(&struct_tag, &data)?;
                Ok(format!("{}", annotated))
            },
        }
    }
    ...
    fn handle_subcommand(&mut self, input: TaskInput<Self::Subcommand>) -> Result<Option<String>> {
        match input.command {
            AccudoSubCommand::BlockCommand(block_cmd) => {
            ...
            AccudoSubCommand::ViewTableCommand(view_table_cmd) => {
    ...
    fn deserialize(&self, bytes: &[u8], layout: &MoveTypeLayout) -> Option<Value> {
        let environment = AccudoEnvironment::new(&self.storage);
        let code_storage = self.storage.as_accudo_code_storage(&environment);
...
fn precompiled_v2_framework() -> &'static PrecompiledFilesModules {
    &PRECOMPILED_ACCUDO_FRAMEWORK_V2
}

fn precompiled_v2_framework_with_experimental() -> &'static PrecompiledFilesModules {
    &PRECOMPILED_ACCUDO_FRAMEWORK_V2_WITH_EXPERIMENTAL
}
...
pub fn run_accudo_test_with_config(
    path: &Path,
    ...
    set_paranoid_type_checks(true);
    run_test_impl::<AccudoTestAdapter>(config, path, v2_lib, &suffix)
}
