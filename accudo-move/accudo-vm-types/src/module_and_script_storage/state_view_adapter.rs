...
/// directly by clients - only via [AsAccudoCodeStorage] trait. Can be used to resolve both modules
/// and cached scripts.
#[derive(Delegate)]
#[delegate(
...
pub struct AccudoCodeStorageAdapter<'ctx, S, E> {
    storage: UnsyncCodeStorage<UnsyncModuleStorage<'ctx, StateViewAdapter<'ctx, S, E>>>,
}

impl<'ctx, S, E> NoOpLayoutCache for AccudoCodeStorageAdapter<'ctx, S, E> {}

#[delegate_to_methods]
#[delegate(ScriptCache, target_ref = "as_script_cache")]
impl<S, E> AccudoCodeStorageAdapter<'_, S, E>
where
    S: StateView,
    E: WithRuntimeEnvironment,
{
    ...
    pub fn into_verified_module_code_iter(
        ...
        impl Iterator<
            Item = (
                ModuleId,
                Arc<ModuleCode<CompiledModule, Module, AccudoModuleExtension>>,
            ),
        >,
        PanicError,
    > {
            ...
            .map(|(key, verified_code)| {
                ...
                let extension = state_view
                    ...
                    .map_or_else(
                        ...
                        |state_value| Ok(AccudoModuleExtension::new(state_value)),
                    )?;
...
impl<S: StateView, E: WithRuntimeEnvironment> AccudoModuleStorage
    for AccudoCodeStorageAdapter<'_, S, E>
{
...
/// block executor, e.g., for single transaction simulation, in Accudo debugger, etc.
pub trait AsAccudoCodeStorage<'ctx, S, E> {
    fn as_accudo_code_storage(
        &'ctx self,
        environment: &'ctx E,
    ) -> AccudoCodeStorageAdapter<'ctx, S, E>;
}

impl<'ctx, S, E> AsAccudoCodeStorage<'ctx, S, E> for S
where
    S: StateView,
    E: WithRuntimeEnvironment,
{
    fn as_accudo_code_storage(
        ...
        AccudoCodeStorageAdapter { storage }
    }
}
