...
use accudo_framework::{accudo_coin_transfer, EntryFunctionCall};
use accudo_types::AccountAddress;
...
fn demo_p2p_entry_function() {
    ...
    let payload = accudo_coin_transfer(payee.clone(), amount);
    let function_call = EntryFunctionCall::decode(&payload);
    match function_call {
        Some(EntryFunctionCall::AccudoCoinTransfer { amount: a, to: p }) => {
            assert_eq!(a, amount);
