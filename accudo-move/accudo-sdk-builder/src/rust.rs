...
/// If `local_types` is true, we generate a file suitable for the Accudo codebase itself
...
struct RustEmitter<T> {
    ...
    /// Whether we are targeting the Accudo repository itself (as opposed to generated Accudo types).
    local_types: bool,
}
    ...
    fn get_external_definitions(local_types: bool) -> serde_generate::ExternalDefinitions {
        let definitions = if local_types {
            vec![
                ...
                ("accudo_types::transaction", vec![
                    "TransactionPayload",
                    "EntryFunction",
                ]),
                ("accudo_types::account_address", vec!["AccountAddress"]),
            ]
        } else {
            vec![("accudo_types", vec![
                "AccountAddress",
                "TypeTag",
                "Script",
                "EntryFunction",
...
pub fn decode(payload: &TransactionPayload) -> Option<EntryFunctionCall> {{
    ...
    fn output_transaction_script_name_method(
        ...
        writeln!(
            self.out,
            r#"
/// Return the name of an Accudo `Script` from a structured object `ScriptCall`.
...
pub struct Installer {
    install_dir: PathBuf,
    accudo_types_version: String,
}

impl Installer {
    pub fn new(install_dir: PathBuf, accudo_types_version: String) -> Self {
        Installer {
            install_dir,
            accudo_types_version,
        }
    }
}
...
serde_bytes = "0.11.6"
accudo-types = {{ path = "../accudo-types", version = "{}" }}
"#,
            name, version, self.accudo_types_version,
        )?;
