...
//!
//! '''bash
//! cargo run -p accudo-sdk-builder -- --help
//! '''
...
struct Options {
    ...
    /// Also install the accudo types described by the given YAML file, along with the BCS runtime.
    #[clap(long)]
    with_accudo_types: Option<PathBuf>,
    ...
    /// and `accudo_types` is assumed to be in "go_module/path/accudo_types".
    #[clap(long)]
    ...
    /// Optional version number for the `accudo_types` module (useful in Rust).
    /// If `--with-accudo-types` is passed, this will be the version of the generated `accudo_types` module.
    #[clap(long, default_value = "0.1.0")]
    accudo_version_number: String,

    /// Optional package name (Python) or module path (Go) of the `accudo_types` dependency.
    #[clap(long)]
...
fn main() {
    ...
    let abis = accudo_sdk_builder::read_abis(&options.abi_directories)
    ...
    let install_dir = match options.target_source_dir {
        None => {
            ...
            match options.language {
                Language::Rust => {
                    accudo_sdk_builder::rust::output(&mut out, &abis, /* local types */ true)
                        .unwrap()
                },
                Language::Go => {
                    accudo_sdk_builder::golang::output(
                        &mut out,
    ...
    // Accudo types
    if let Some(registry_file) = options.with_accudo_types {
        ...
        if let Language::Rust = options.language {
            accudo_sdk_builder::rust::replace_keywords(&mut registry)
        }

        let (package_name, _package_path) = match options.language {
            Language::Rust => (
                if options.accudo_version_number == "0.1.0" {
                    "accudo-types".to_string()
                } else {
                    format!("accudo-types:{}", options.accudo_version_number)
                },
                vec!["accudo-types"],
            ),
            Language::Go => ("accudotypes".to_string(), vec!["accudotypes"]),
        };
