...
pub fn output(
    ...
    accudo_module_path: Option<String>,
    package_name: String,
    abis: &[EntryABI],
) -> Result<()> {
        ...
        .filter(|abi| {
            if let EntryABI::EntryFunction(sf) = abi {
                sf.module_name().name().as_str() != "code"
                    ...
                    && sf.module_name().name().as_str() != "accudo_account"
...
struct GoEmitter<T> {
    ...
    serde_module_path: Option<String>,
    /// Go module path for Accudo types.
    ...
    T: Write,
{
    fn output_script_call_enum_with_imports(&mut self, abis: &[EntryABI]) -> Result<()> {
        let accudo_types_package = match &self.accudo_module_path {
            Some(path) => format!("{}/accudotypes", path),
            None => "accudotypes".into(),
        };
        let mut external_definitions =
            crate::common::get_external_definitions(&accudo_types_package);
        // We need BCS for argument encoding and decoding
        external_definitions.insert(
            "github.com/accudo-labs/serde-reflection/serde-generate/runtime/golang/bcs".to_string(),
            Vec::new(),
        );
    ...
    fn output_encode_method(&mut self, abis: &[EntryABI]) -> Result<()> {
        ...
        if !transaction_script_abis.is_empty() {
            writeln!(
                self.out,
                r#"
// Build an Accudo `Script` from a structured object `ScriptCall`.
func EncodeScript(call ScriptCall) accudotypes.Script {{"#
            )?;
            self.out.indent();
        ...
        if !entry_fun_abis.is_empty() {
            writeln!(
                self.out,
                r#"
// Build an Accudo `TransactionPayload` from a structured object `EntryFunctionCall`.
func EncodeEntryFunction(call EntryFunctionCall) accudotypes.TransactionPayload {{"#
            )?;
            self.out.indent();
            writeln!(self.out, "switch call := call.(type) {{")?;
    ...
    fn output_transaction_script_decode_method(&mut self) -> Result<()> {
        writeln!(
            self.out,
            r#"
// Try to recognize an Accudo `Script` and convert it into a structured object `ScriptCall`.
func DecodeScript(script *accudotypes.Script) (ScriptCall, error) {{
    ...
    fn output_entry_function_decode_method(&mut self) -> Result<()> {
        writeln!(
            self.out,
            r#"
// Try to recognize an Accudo `TransactionPayload` and convert it into a structured object `EntryFunctionCall`.
func DecodeEntryFunctionPayload(script accudotypes.TransactionPayload) (EntryFunctionCall, error) {{
    switch script := script.(type) {{
        case *accudotypes.TransactionPayload__EntryFunction:
    ...
    fn output_transaction_script_encoder_function(
        ...
        writeln!(
            self.out,
            "\n{}\nfunc Encode{}({}) accudotypes.Script {{",
        ...
        writeln!(
            self.out,
            r#"return accudotypes.Script {{
	Code: append([]byte(nil), {}_code...),
	TyArgs: []accudotypes.TypeTag{{{}}},
	Args: []accudotypes.TransactionArgument{{{}}},
}}"#,
            abi.name(),
    ...
    fn output_entry_function_encoder_function(&mut self, abi: &EntryFunctionABI) -> Result<()> {
        writeln!(
            self.out,
            "\n{}\nfunc Encode{}{}({}) accudotypes.TransactionPayload {{",
            Self::quote_doc(abi.doc()),
            abi.module_name().name().to_string().to_upper_camel_case(),
            abi.name().to_upper_camel_case(),
            [
        ...
        writeln!(
            self.out,
            r#"return &accudotypes.TransactionPayload__EntryFunction {{
            accudotypes.EntryFunction {{
                Module: {},
                Function: {},
                TyArgs: []accudotypes.TypeTag{{{}}},
                Args: [][]byte{{{}}},
    ...
    fn output_transaction_script_decoder_function(
        ...
        writeln!(
            self.out,
            "\nfunc decode_{}(script *accudotypes.Script) (ScriptCall, error) {{",
            abi.name(),
        )?;
        self.out.indent();
        writeln!(
...
"#,
    ...
    fn output_entry_function_decoder_function(&mut self, abi: &EntryFunctionABI) -> Result<()> {
        writeln!(
            self.out,
            "\nfunc decode_{}_{}(script accudotypes.TransactionPayload) (EntryFunctionCall, error) {{",
...
"#,
    ...
    fn output_transaction_script_decoder_map(
        ...
        writeln!(
            self.out,
            r#"
var script_decoder_map = map[string]func(*accudotypes.Script) (ScriptCall, error) {{"#
        )?;
        self.out.indent();
        for abi in abis {
    ...
    fn output_entry_function_decoder_map(&mut self, abis: &[EntryFunctionABI]) -> Result<()> {
        writeln!(
            self.out,
            r#"
var entry_function_decoder_map = map[string]func(accudotypes.TransactionPayload) (EntryFunctionCall, error) {{"#
        )?;
        self.out.indent();
        for abi in abis {
            writeln!(
...
"#,
    ...
    fn output_decoding_helper(&mut self, type_tag: &TypeTag) -> Result<()> {
        ...
        writeln!(
            self.out,
            r#"
func decode_{0}_argument(arg accudotypes.TransactionArgument) (value {1}, err error) {{
	if arg, ok := arg.(*accudotypes.TransactionArgument__{2}); ok {{
		{3}
	}} else {{
...
"#,
    ...
    fn quote_module_id(module_id: &ModuleId) -> String {
        format!(
            "accudotypes.ModuleId {{ Address: {}, Name: {} }}",
    ...
    fn quote_type_parameters(ty_args: &[TypeArgumentABI]) -> Vec<String> {
        ty_args
            .iter()
            .map(|ty_arg| format!("{} accudotypes.TypeTag", ty_arg.name()))
            .collect()
    }
    ...
    fn quote_type(type_tag: &TypeTag) -> String {
        ...
        match type_tag {
            ...
            U256 => unimplemented!(),
            Address => "accudotypes.AccountAddress".into(),
            Vector(type_tag) => {
    ...
    fn quote_transaction_argument_for_script(type_tag: &TypeTag, name: &str) -> String {
        use TypeTag::*;
        match type_tag {
            Bool => format!("(*accudotypes.TransactionArgument__Bool)(&{})", name),
            U8 => format!("(*accudotypes.TransactionArgument__U8)(&{})", name),
            U16 => format!("(*accudotypes.TransactionArgument__U16)(&{})", name),
            U32 => format!("(*accudotypes.TransactionArgument__U32)(&{})", name),
            U64 => format!("(*accudotypes.TransactionArgument__U64)(&{})", name),
            U128 => format!("(*accudotypes.TransactionArgument__U128)(&{})", name),
            U256 => format!("(*accudotypes.TransactionArgument__U256)(&{})", name),
            Address => format!("&accudotypes.TransactionArgument__Address{{{}}}", name),
            Vector(type_tag) => match type_tag.as_ref() {
                U8 => format!("(*accudotypes.TransactionArgument__U8Vector)(&{})", name),
                _ => common::type_not_allowed(type_tag),
            },
            // TODO(#17645): signed integers
            Struct(_) | Signer | Function(..) | I8 | I16 | I32 | I64 | I128 | I256 => {
