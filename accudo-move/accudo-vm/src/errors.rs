...
use crate::transaction_validation::ACCUDO_TRANSACTION_VALIDATION;
...
pub fn convert_prologue_error(
    ...
    Err(match status {
        ...
        VMStatus::MoveAbort(location, code)
            if !ACCUDO_TRANSACTION_VALIDATION.is_account_module_abort(&location) =>
        {
            let new_major_status = match error_split(code) {
                ...
                (category, reason) => {
                    let err_msg = format!("[accudo_vm] Unexpected prologue Move abort: {:?}::{:?} (Category: {:?} Reason: {:?})",
        ...
        status @ VMStatus::ExecutionFailure { .. } | status @ VMStatus::Error { .. } => {
            speculative_error!(
                log_context,
                format!("[accudo_vm] Unexpected prologue error: {:?}", status),
            );
            VMStatus::Error {
                status_code: StatusCode::UNEXPECTED_ERROR_FROM_KNOWN_MOVE_FUNCTION,
...
pub fn convert_epilogue_error(
    ...
    Err(match status {
            ...
            if !ACCUDO_TRANSACTION_VALIDATION.is_account_module_abort(&location) =>
        {
            ...
            let err_msg = format!("[accudo_vm] Unexpected success epilogue Move abort: {:?}::{:?} (Category: {:?} Reason: {:?})",
			location, code, category, reason);
            speculative_error!(log_context, err_msg.clone());
            VMStatus::error(
                StatusCode::UNEXPECTED_ERROR_FROM_KNOWN_MOVE_FUNCTION,
...
pub fn expect_only_successful_execution(
    ...
    Err(match status {
        ...
        status => {
            ...
            let err_msg = format!(
                "[accudo_vm] Unexpected error from known Move function, '{}'. Error: {:?}",
