...
use crate::{
    block_executor::AccudoVMBlockExecutorWrapper,
    counters::*,
    ...
    move_vm_ext::{
        ...
        AccudoMoveResolver, AsExecutorView, AsResourceGroupView, MoveVmExt, SessionExt, SessionId,
...
use accudo_framework::natives::code::PublishRequest;
use accudo_gas_algebra::{Gas, GasQuantity, NumBytes, Octa};
use accudo_gas_meter::{AccudoGasMeter, GasAlgebra};
use accudo_gas_schedule::{
    gas_feature_versions,
    gas_feature_versions::{RELEASE_V1_10, RELEASE_V1_27, RELEASE_V1_38},
    AccudoGasParameters, VMGasParameters,
};
use accudo_logger::{enabled, prelude::*, Level};
use accudo_metrics_core::IntCounterVecHelper;
#[cfg(any(test, feature = "testing"))]
use accudo_types::state_store::StateViewId;
use accudo_types::{
...
use accudo_vm_types::{
    ...
    module_and_script_storage::{
        code_storage::AccudoCodeStorage, module_storage::AccudoModuleStorage, AsAccudoCodeStorage,
    },
...
pub(crate) fn get_system_transaction_output(
    session: SessionExt<impl AccudoMoveResolver>,
...
impl AccudoVM {
    ...
    pub fn new(env: &AccudoEnvironment, state_view: &impl StateView) -> Self {
        let resolver = state_view.as_move_resolver();
        let module_storage = state_view.as_accudo_code_storage(env);
    ...
    pub fn new_session<'r, R: AccudoMoveResolver>(
        &self,
        resolver: &'r R,
    ...
    pub(crate) fn gas_params(
        &self,
        log_context: &AdapterLogSchema,
    ) -> Result<&AccudoGasParameters, VMStatus> {
    ...
    #[inline(always)]
    pub fn environment(&self) -> AccudoEnvironment {
    ...
    #[cfg(any(test, feature = "testing"))]
    pub fn gas_params_for_test(&self) -> Result<&AccudoGasParameters, VMStatus> {
    ...
    pub(crate) fn failed_transaction_cleanup(
        ...
        gas_meter: &mut impl AccudoGasMeter,
        txn_data: &TransactionMetadata,
        ...
        if self.gas_feature_version() >= 12 {
            ...
            if let Err(err) = gas_meter.algebra().check_consistency() {
                println!(
                    "[accudo-vm][gas-meter][failure-epilogue] {}",
                    err.message()
    ...
    fn finish_aborted_transaction(
        ...
        let (previous_session_change_set, fee_statement) = if should_create_account_resource {
            ...
            let fee_statement =
                AccudoVM::fee_statement_from_gas_meter(txn_data, gas_meter, ZERO_STORAGE_REFUND);
    ...
    fn success_transaction_cleanup(
        ...
        if self.gas_feature_version() >= 12 {
            ...
            if let Err(err) = gas_meter.algebra().check_consistency() {
                println!(
                    "[accudo-vm][gas-meter][success-epilogue] {}",
                    err.message()
                        .unwrap_or("No message found -- this should not happen.")
                );
        ...
        let fee_statement = AccudoVM::fee_statement_from_gas_meter(
            txn_data,
            gas_meter,
    ...
    fn validate_and_execute_script<'a>(
        &self,
        session: &mut SessionExt<impl AccudoMoveResolver>,
        serialized_signers: &SerializedSigners,
        code_storage: &impl AccudoCodeStorage,
        // Note: cannot use AccudoGasMeter because it is not implemented for
        //       UnmeteredGasMeter.
    ...
    fn execute_script_or_entry_function<'a, 'r>(
        &self,
        resolver: &'r impl AccudoMoveResolver,
        code_storage: &impl AccudoCodeStorage,
        ...
        fail_point!("accudo_vm::execute_script_or_entry_function", |_| {
            Err(VMStatus::Error {
    ...
    fn validate_signed_transaction(
        ...
        let keyless_authenticators = accudo_types::keyless::get_authenticators(transaction)
    ...
    fn execute_user_transaction_impl(
        ...
        let _timer = VM_TIMER.timer_with_label("AccudoVM::execute_user_transaction_impl");
        ...
        let should_create_account_resource_timer =
            VM_TIMER.timer_with_label("AccudoVM::create_account_resource_lazily");
        ...
        let payload_timer =
            VM_TIMER.timer_with_label("AccudoVM::execute_user_transaction_impl [payload]");
    ...
    pub fn execute_user_transaction_with_custom_gas_meter<'a, C, G, F>(
        &self,
        resolver: &'a impl AccudoMoveResolver,
        code_storage: &'a C,
        txn: &SignedTransaction,
        log_context: &AdapterLogSchema,
        make_gas_meter: F,
    ...
    where
        C: AccudoCodeStorage + BlockSynchronizationKillSwitch,
        G: AccudoGasMeter,
    ...
    pub fn execute_user_transaction_with_modified_gas_meter<'a, G, F>(
        &self,
        resolver: &'a impl AccudoMoveResolver,
        code_storage: &'a (impl AccudoCodeStorage + BlockSynchronizationKillSwitch),
        txn: &SignedTransaction,
        log_context: &AdapterLogSchema,
        modify_gas_meter: F,
        auxiliary_info: &AuxiliaryInfo,
    ...
    pub fn execute_user_transaction(
        &self,
        resolver: &impl AccudoMoveResolver,
        code_storage: &(impl AccudoCodeStorage + BlockSynchronizationKillSwitch),
        txn: &SignedTransaction,
        log_context: &AdapterLogSchema,
        auxiliary_info: &AuxiliaryInfo,
    ) -> (VMStatus, VMOutput) {
        ...
        match write_set_payload {
            WriteSetPayload::Direct(change_set) => {
                ...
                if !Self::should_restart_execution(change_set.events()) {
                    ...
                    error!(
                        "[accudo_vm] direct write set finished without requiring should_restart_execution");
                }
    ...
    fn validate_waypoint_change_set(
        ...
        if has_new_block_event && has_new_epoch_event {
            ...
            error!(
                *log_context,
                "[accudo_vm] waypoint txn needs to emit new epoch and block"
            );
    ...
    pub fn execute_view_function(
        ...
        let env = AccudoEnvironment::new(state_view);
        let vm = AccudoVM::new(&env, state_view);
    ...
    fn gas_used(max_gas_amount: Gas, gas_meter: &impl AccudoGasMeter) -> u64 {
        max_gas_amount
            .checked_sub(gas_meter.balance())
    ...
    pub fn execute_single_transaction(
        ...
        Ok(match txn.expect_valid() {
            Transaction::BlockMetadata(block_metadata) => {
                fail_point!("accudo_vm::execution::block_metadata");
            ...
            Transaction::BlockMetadataExt(block_metadata_ext) => {
                fail_point!("accudo_vm::execution::block_metadata_ext");
            ...
            Transaction::UserTransaction(txn) => {
                fail_point!("accudo_vm::execution::user_transaction");
                    ...
                    match vm_status.status_code() {
                        ...
                        | StatusCode::TYPE_RESOLUTION_FAILURE => {
                            speculative_error!(
                                log_context,
                                format!(
                                    "[accudo_vm] Transaction breaking invariant violation: {:?}\ntxn: {:?}",
                                    vm_status,
                        ...
                        if vm_status.sub_status()
                            == Some(unknown_invariant_violation::EPARANOID_FAILURE) =>
                            {
                                error!(
                                *log_context,
                                "[accudo_vm] Transaction breaking paranoid mode: {:?}\ntxn: {:?}",
                                vm_status,
                                bcs::to_bytes::<SignedTransaction>(txn),
                            );
                            },
                        ...
                        if vm_status.sub_status()
                            == Some(unknown_invariant_violation::EREFERENCE_SAFETY_FAILURE) =>
                            {
                                error!(
                                *log_context,
                                "[accudo_vm] Transaction breaking paranoid reference safety check. txn: {:?}, status: {:?}",
                                bcs::to_bytes::<SignedTransaction>(txn),
                                vm_status,
                            );
                            },
...
pub struct AccudoVMBlockExecutor {
    ...
    module_cache_manager: AccudoModuleCacheManager,
}

impl AccudoVMBlockExecutor {
    ...
    pub fn execute_block_with_config(
        ...
        let result = AccudoVMBlockExecutorWrapper::execute_block::<
            _,
...
impl VMBlockExecutor for AccudoVMBlockExecutor {
    fn new() -> Self {
        Self {
            module_cache_manager: AccudoModuleCacheManager::new(),
        }
    }
    ...
    fn execute_block(
        ...
        let config = BlockExecutorConfig {
            local: BlockExecutorLocalConfig {
                blockstm_v2: AccudoVM::get_blockstm_v2_enabled(),
                concurrency_level: AccudoVM::get_concurrency_level(),
                allow_fallback: true,
                discard_failed_blocks: AccudoVM::get_discard_failed_blocks(),
    ...
    fn execute_block_sharded<S: StateView + Sync + Send + 'static, C: ExecutorClient<S>>(
        ...
        let ret = sharded_block_executor.execute_block(
            state_view,
            transactions,
            AccudoVM::get_concurrency_level(),
            onchain_config,
        );
        if ret.is_ok() {
            // Record the histogram count for transactions per block.
...
impl VMValidator for AccudoVM {
    ...
    fn validate_transaction(
        ...
        {
            if let accudo_types::transaction::authenticator::TransactionAuthenticator::SingleSender{ .. } = transaction.authenticator_ref() {
...
// Ensure encapsulation of AccudoVM APIs by using a wrapper.
pub struct AccudoSimulationVM;

impl AccudoSimulationVM {
    ...
    pub fn create_vm_and_simulate_signed_transaction(
        ...
        let env = AccudoEnvironment::new(state_view);
        let mut vm = AccudoVM::new(&env, state_view);
        ...
        let code_storage = state_view.as_accudo_code_storage(&env);
...
#[cfg(test)]
mod tests {
    use crate::{move_vm_ext::MoveVmExt, AccudoVM};
    use accudo_types::{
        account_address::AccountAddress,
    ...
    fn vm_thread_safe() {
        ...
        assert_send::<AccudoVM>();
        assert_sync::<AccudoVM>();
    ...
    fn should_restart_execution_on_new_epoch() {
        ...
        assert!(AccudoVM::should_restart_execution(&[(
            new_epoch_event,
            None
        )]));
        assert!(AccudoVM::should_restart_execution(&[(
