...
use crate::move_vm_ext::{
    resource_state_key, AccudoMoveResolver, AsExecutorView, AsResourceGroupView,
...
use accudo_vm_environment::gas::get_gas_feature_version;
use accudo_vm_types::{
    resolver::{
...
/// Adapter to convert a `ExecutorView` into a `AccudoMoveResolver`.
///
...
impl<E: ExecutorView> AccudoMoveResolver for StorageAdapter<'_, E> {}
...
/// Converts `StateView` into `AccudoMoveResolver`.
...
pub(crate) mod tests {
    use super::*;
    use accudo_vm_types::resource_group_adapter::GroupSizeKind;
