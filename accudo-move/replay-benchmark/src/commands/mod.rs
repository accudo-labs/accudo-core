...
) -> anyhow::Result<AccudoDebugger> {
    let builder = Client::builder(AccudoBaseUrl::Custom(Url::parse(&rest_endpoint)?));
    let client = if let Some(api_key) = api_key {
        builder.api_key(&api_key)?.build()
    } else {
        builder.build()
    };
    AccudoDebugger::rest_client(client)
}
...
pub struct RestAPI {
    #[clap(
        long,
        help = "Fullnode's REST API query endpoint, e.g., https://api.mainnet.accudo.org/v1 \
                for mainnet"
    )]
    rest_endpoint: String,
