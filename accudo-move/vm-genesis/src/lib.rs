...
use accudo_framework::{ReleaseBundle, ReleasePackage};
use accudo_gas_schedule::{
    AccudoGasParameters, InitialGasSchedule, ToOnChainGasSchedule, LATEST_GAS_FEATURE_VERSION,
};
use accudo_types::{
    account_config::{
        self, accudo_test_root_address, events::NewEpochEvent, CORE_CODE_ADDRESS,
...
use accudo_vm::{
    ...
    move_vm_ext::{
        convert_modules_into_write_ops, AccudoMoveResolver, GenesisMoveVm, GenesisRuntimeBuilder,
        SessionExt,
    },
};
use accudo_vm_types::{
    change_set::VMChangeSet,
    module_and_script_storage::{module_storage::AccudoModuleStorage, AsAccudoCodeStorage},
...
const GOVERNANCE_MODULE_NAME: &str = "accudo_governance";
...
const ACCUDO_COINS_BASE_WITH_DECIMALS: u64 = u64::pow(10, 8);
...
pub fn default_gas_schedule() -> GasScheduleV2 {
    GasScheduleV2 {
        ...
        entries: AccudoGasParameters::initial().to_on_chain_gas_schedule(LATEST_GAS_FEATURE_VERSION),
    }
}

pub fn encode_accudo_mainnet_genesis_transaction(
    ...
    let module_storage = state_view.as_accudo_code_storage(&genesis_runtime_environment);
    ...
    initialize_accudo_coin(&mut session, &module_storage, &mut traversal_context);
...
pub fn encode_genesis_transaction(
    accudo_root_key: Ed25519PublicKey,
...
fn initialize(
    ...
    exec_function(
        ...
        serialize_values(&vec![
            ...
            MoveValue::U64(ACCUDO_MAX_KNOWN_VERSION.major),
...
fn initialize_core_resources_and_accudo_coin(
    session: &mut SessionExt<impl AccudoMoveResolver>,
    module_storage: &impl AccudoModuleStorage,
    traversal_context: &mut TraversalContext,
    core_resources_key: &Ed25519PublicKey,
...
fn allow_core_resources_to_set_version(
    ...
    exec_function(
        ...
        "initialize_for_test",
        vec![],
        serialize_values(&vec![MoveValue::Signer(accudo_test_root_address())]),
    );
}
...
fn code_to_writes_for_publishing(
    ...
    let module_storage = genesis_state_view.as_accudo_code_storage(genesis_runtime_environment);
...
pub fn generate_genesis_change_set_for_testing_with_count(
    ...
    let framework = match genesis_options {
        GenesisOptions::Head => accudo_cached_packages::head_release_bundle(),
        GenesisOptions::Testnet => accudo_framework::testnet_release_bundle(),
        GenesisOptions::Mainnet => {
            // We don't yet have mainnet, so returning testnet here
            accudo_framework::testnet_release_bundle()
        },
    };
...
pub fn generate_genesis_change_set_for_mainnet(genesis_options: GenesisOptions) -> ChangeSet {
    let framework = match genesis_options {
        ...
        GenesisOptions::Testnet => accudo_framework::testnet_release_bundle(),
        // We don't yet have mainnet, so returning testnet here
        GenesisOptions::Mainnet => accudo_framework::testnet_release_bundle(),
    };
...
pub fn test_genesis_change_set_and_validators(
    ...
    generate_test_genesis(accudo_cached_packages::head_release_bundle(), count)
}

#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Validator {
    /// The Accudo account address of the validator or the admin in the case of a commissioned or
    /// vesting managed validator.
    pub owner_address: AccountAddress,
    /// The Accudo account address of the validator's operator (same as `address` if the validator is
    /// its own operator).
...
pub fn generate_test_genesis(
    ...
    let genesis = encode_genesis_change_set(
        ...
        &GenesisConfiguration {
            ...
            // 1M ACCUDO coins (with 8 decimals).
...
fn mainnet_genesis_config() -> GenesisConfiguration {
    ...
    GenesisConfiguration {
        ...
        is_test: false,
        min_stake: 1_000_000 * ACCUDO_COINS_BASE_WITH_DECIMALS, // 1M APT
        // 400M APT
        min_voting_threshold: (400_000_000 * ACCUDO_COINS_BASE_WITH_DECIMALS as u128),
        max_stake: 50_000_000 * ACCUDO_COINS_BASE_WITH_DECIMALS, // 50M APT.
        recurring_lockup_duration_secs: 30 * 24 * 3600,         // 1 month
        required_proposer_stake: 1_000_000 * ACCUDO_COINS_BASE_WITH_DECIMALS, // 1M APT
        rewards_apy_percentage: 10,
...
pub fn test_mainnet_end_to_end() {
    ...
    let balance = 10_000_000 * ACCUDO_COINS_BASE_WITH_DECIMALS;
    let non_validator_balance = 10 * ACCUDO_COINS_BASE_WITH_DECIMALS;
    ...
    let transaction = encode_accudo_mainnet_genesis_transaction(
        &accounts,
        &employees,
        &validators,
        accudo_cached_packages::head_release_bundle(),
