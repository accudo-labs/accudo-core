...
pub use accudo_netcore::transport::tcp::{resolve_and_connect, TCPBufferCfg, TcpSocket};
use accudo_netcore::transport::{proxy_protocol, tcp, ConnectionOrigin, Transport};
...
/// tcp::Transport with Accudo-specific configuration applied.
pub const ACCUDO_TCP_TRANSPORT: tcp::TcpTransport = tcp::TcpTransport {
    // Use default options.
    ttl: None,
    // Use TCP_NODELAY for Accudo tcp connections.
    nodelay: Some(true),
...
async fn upgrade_inbound<T: TSocket>(
    ...
    // try to negotiate common accudonet version and supported application protocols
...
/// The common AccudoNet Transport.
///
...
// ideas in this PR thread: https://github.com/accudo-labs/accudo-core/pull/3478#issuecomment-617385633
pub struct AccudoNetTransport<TTransport> {
...
impl<TTransport> AccudoNetTransport<TTransport>
where
...
// If using `AccudoNetTransport` as a `Transport` trait, then all upgrade futures
...
impl<TTransport: Transport> Transport for AccudoNetTransport<TTransport>
where
