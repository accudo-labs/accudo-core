...
use crate::{
    ...
    transport::{self, AccudoNetTransport, Connection, ACCUDO_TCP_TRANSPORT},
    ProtocolId,
};
...
use accudo_netcore::transport::{
    tcp::{TCPBufferCfg, TcpSocket, TcpTransport},
    Transport,
};
use accudo_time_service::TimeService;
use accudo_types::{chain_id::ChainId, network_address::NetworkAddress, PeerId};
...
struct PeerManagerContext {
    ...
    pm_reqs_tx: accudo_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>,
    pm_reqs_rx: accudo_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
    connection_reqs_tx: accudo_channel::Sender<PeerId, ConnectionRequest>,
    connection_reqs_rx: accudo_channel::Receiver<PeerId, ConnectionRequest>,

    peers_and_metadata: Arc<PeersAndMetadata>,
    upstream_handlers:
        HashMap<ProtocolId, accudo_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>>,
...
impl PeerManagerContext {
    ...
    fn new(
        ...
        upstream_handlers: HashMap<
            ProtocolId,
            accudo_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>,
        >,
        connection_event_handlers: Vec<conn_notifs_channel::Sender>,
    ...
    fn add_upstream_handler(
        ...
        channel: accudo_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>,
    ) -> &mut Self {
...
type MemoryPeerManager =
    PeerManager<AccudoNetTransport<MemoryTransport>, NoiseStream<accudo_memsocket::MemorySocket>>;
type TcpPeerManager = PeerManager<AccudoNetTransport<TcpTransport>, NoiseStream<TcpSocket>>;
...
impl PeerManagerBuilder {
    ...
    pub fn create(
        ...
        let (pm_reqs_tx, pm_reqs_rx) = accudo_channel::new(
            QueueStyle::FIFO,
            channel_size,
        ...
        let (connection_reqs_tx, connection_reqs_rx) =
            accudo_channel::new(QueueStyle::FIFO, channel_size, None);
    ...
    pub fn connection_reqs_tx(&self) -> accudo_channel::Sender<PeerId, ConnectionRequest> {
    ...
    pub fn build(&mut self, executor: &Handle) -> &mut Self {
        ...
        accudo_tcp_transport.set_tcp_buffers(&tcp_cfg);
    ...
    ) -> accudo_channel::Receiver<(PeerId, ProtocolId), ReceivedMessage> {
        // Register the direct send and rpc protocols
        self.transport_context()
            .add_protocols(&config.direct_send_protocols_and_preferences);
        self.transport_context()
