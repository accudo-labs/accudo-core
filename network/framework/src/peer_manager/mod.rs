...
use accudo_config::network_id::{NetworkContext, PeerNetworkId};
use accudo_logger::prelude::*;
use accudo_netcore::transport::{ConnectionOrigin, Transport};
...
use accudo_config::config::PeerRole;
    ...
    TSocket: AsyncRead + AsyncWrite,
{
    ...
    active_peers: HashMap<
        ...
        (
            ConnectionMetadata,
            accudo_channel::Sender<ProtocolId, PeerRequest>,
        ),
    >,
    /// Shared metadata storage about trusted peers and metadata
    peers_and_metadata: Arc<PeersAndMetadata>,
    /// Channel to receive requests from other actors.
    requests_rx: accudo_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
    ...
    upstream_handlers:
        Arc<HashMap<ProtocolId, accudo_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>>>,
    ...
    transport_reqs_tx: accudo_channels::Sender<TransportRequest>,
    /// Sender for connection events.
    transport_notifs_tx: accudo_channels::Sender<TransportNotification<TSocket>>,
    ...
    transport_notifs_rx: accudo_channels::Receiver<TransportNotification<TSocket>>,
    ...
    TSocket: TransportTSocket,
{
    ...
    pub fn new(
        ...
        let (transport_notifs_tx, transport_notifs_rx) = accudo_channels::new(
            channel_size,
            &counters::PENDING_CONNECTION_HANDLER_NOTIFICATIONS,
        );
        let (transport_reqs_tx, transport_reqs_rx) =
            accudo_channels::new(channel_size, &counters::PENDING_PEER_MANAGER_DIAL_REQUESTS);
    ...
    fn handle_connection_event(&mut self, event: TransportNotification<TSocket>) {
        ...
        match event {
            ...
            TransportNotification::Disconnected(lost_conn_metadata, reason) => {
                // See: https://github.com/accudo-labs/accudo-core/issues/3128#issuecomment-605351504 for
    ...
    fn add_peer(&mut self, connection: Connection<TSocket>) -> Result<(), Error> {
        ...
        let (peer_reqs_tx, peer_reqs_rx) = accudo_channel::new(
            QueueStyle::FIFO,
            self.channel_size,
