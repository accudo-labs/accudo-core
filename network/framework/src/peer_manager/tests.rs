...
fn build_test_peer_manager(
    ...
    PeerManager<
        ...
        MemorySocket,
    >,
    accudo_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>,
    accudo_channel::Sender<PeerId, ConnectionRequest>,
    conn_notifs_channel::Receiver,
) {
    let (peer_manager_request_tx, peer_manager_request_rx) =
        accudo_channel::new(QueueStyle::FIFO, 1, None);
    let (connection_reqs_tx, connection_reqs_rx) = accudo_channel::new(QueueStyle::FIFO, 1, None);
    let (hello_tx, _hello_rx) = accudo_channel::new(QueueStyle::FIFO, 1, None);
