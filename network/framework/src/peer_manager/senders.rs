...
use accudo_channels::{self, accudo_channel};
...
pub struct PeerManagerRequestSender {
    inner: accudo_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>,
}
...
pub struct ConnectionRequestSender {
    inner: accudo_channel::Sender<PeerId, ConnectionRequest>,
}

impl PeerManagerRequestSender {
    /// Construct a new PeerManagerRequestSender with a raw channel::Sender
    pub fn new(inner: accudo_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>) -> Self {
        Self { inner }
    }
...
impl ConnectionRequestSender {
    /// Construct a new ConnectionRequestSender with a raw accudo_channel::Sender
    pub fn new(inner: accudo_channel::Sender<PeerId, ConnectionRequest>) -> Self {
        Self { inner }
    }
