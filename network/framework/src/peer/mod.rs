...
pub struct Peer<TSocket> {
    ...
    connection_notifs_tx: accudo_channels::Sender<TransportNotification<TSocket>>,
    /// Channel to receive requests from PeerManager to send messages and rpcs.
    peer_reqs_rx: accudo_channel::Receiver<ProtocolId, PeerRequest>,
    ...
    TSocket: AsyncRead + AsyncWrite + Send + 'static,
{
    ...
    fn start_writer_task(
        ...
        accudo_channel::Sender<(), NetworkMessage>,
        oneshot::Sender<()>,
    ) {
        let remote_peer_id = connection_metadata.remote_peer_id;
        let (write_reqs_tx, mut write_reqs_rx): (accudo_channel::Sender<(), NetworkMessage>, _) =
            accudo_channel::new(
                QueueStyle::KLAST,
                1024,
        ...
        let (mut msg_tx, msg_rx) = accudo_channels::new(1024, &counters::PENDING_MULTIPLEX_MESSAGE);
        let (stream_msg_tx, stream_msg_rx) =
            accudo_channels::new(1024, &counters::PENDING_MULTIPLEX_STREAM);
                ...
                match self.upstream_handlers.get(&direct.protocol_id) {
                    ...
                    Some(handler) => {
                        ...
                        match handler.push(key, ReceivedMessage::new(message, sender)) {
                            Err(_err) => {
                                // NOTE: accudo_channel never returns other than Ok(()), but we might switch to tokio::sync::mpsc and then this would work
    ...
    fn handle_inbound_message(
        ...
        write_reqs_tx: &mut accudo_channel::Sender<(), NetworkMessage>,
    ) -> Result<(), PeerManagerError> {
        trace!(
            NetworkSchema::new(&self.network_context)
                .connection_metadata(&self.connection_metadata),
    ...
    async fn do_shutdown(
        mut self,
        write_req_tx: accudo_channel::Sender<(), NetworkMessage>,
