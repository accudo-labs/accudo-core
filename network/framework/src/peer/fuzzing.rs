...
use accudo_proptest_helpers::ValueGenerator;
...
pub fn fuzz(data: &[u8]) {
    ...
    let (connection_notifs_tx, connection_notifs_rx) = accudo_channels::new_test(8);
    let channel_size = 8;

    let (peer_reqs_tx, peer_reqs_rx) = accudo_channel::new(QueueStyle::FIFO, channel_size, None);
