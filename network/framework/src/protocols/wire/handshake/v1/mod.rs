...
//! These should serialize as per the [AccudoNet Handshake v1 Specification].
//!
...
impl ProtocolId {
    ...
    pub fn from_bytes<T: DeserializeOwned>(&self, bytes: &[u8]) -> anyhow::Result<T> {
        ...
        let result = match self.encoding() {
            ...
            Encoding::CompressedBcs(limit) => {
                let compression_client = self.get_compression_client();
                let raw_bytes = accudo_compression::decompress(
                    &bytes.to_vec(),
                    compression_client,
                    MAX_APPLICATION_MESSAGE_SIZE,
                )
...
/// AccudoNet peers in order to negotiate the set of common supported protocols for
/// use on a new AccudoNet connection.
#[derive(Clone, Debug, Default, Deserialize, Eq, PartialEq, Serialize)]
#[cfg_attr(any(test, feature = "fuzzing"), derive(Arbitrary))]
pub struct ProtocolIdSet(accudo_bitvec::BitVec);
...
/// Enum representing different versions of the Accudo network protocol. These
...
pub enum HandshakeError {
    #[error("accudo-handshake: the received message has a different chain id: {0}, expected: {1}")]
