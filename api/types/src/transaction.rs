    ...
    pub fn from_transaction_onchain_data(
        ...
        if txn.version > latest_ledger_version {
            match txn.transaction {
                accudo_types::transaction::Transaction::UserTransaction(txn) => {
...
pub struct TransactionOnChainData {
    ...
    pub transaction: accudo_types::transaction::Transaction,
    /// Information about the transaction
    pub info: accudo_types::transaction::TransactionInfo,
    /// Events emitted by the transaction
    pub events: Vec<ContractEvent>,
    /// The accumulator root hash at this version
    pub accumulator_root_hash: accudo_crypto::HashValue,
    /// Final state of resources changed by the transaction
    pub changes: accudo_types::write_set::WriteSet,
}
...
impl
    From<(
        u64,
        accudo_types::transaction::Transaction,
        accudo_types::transaction::TransactionInfo,
        Vec<ContractEvent>,
        accudo_crypto::HashValue,
        accudo_types::write_set::WriteSet,
    )> for TransactionOnChainData
{
    fn from(
...
/// Enum of the different types of transactions in Accudo
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Union)]
...
impl UserTransactionRequest {
    pub fn replay_protector(&self) -> accudo_types::transaction::ReplayProtector {
        if let Some(nonce) = self.replay_protection_nonce {
            accudo_types::transaction::ReplayProtector::Nonce(nonce.0)
        } else {
            accudo_types::transaction::ReplayProtector::SequenceNumber(self.sequence_number.0)
        }
    }
}
...
pub struct BlockMetadataTransaction {
    ...
    /// If some, it means the internal txn type is `accudo_types::transaction::Transaction::BlockMetadataExt`.
    /// Otherwise, it is `accudo_types::transaction::Transaction::BlockMetadata`.
    ///
...
impl
    From<(
        accudo_types::validator_txn::ValidatorTransaction,
        TransactionInfo,
        Vec<Event>,
        u64,
    )> for ValidatorTransaction
{
    fn from(
        ...
        match txn {
            accudo_types::validator_txn::ValidatorTransaction::DKGResult(dkg_transcript) => {
            ...
            accudo_types::validator_txn::ValidatorTransaction::ObservedJWKUpdate(
...
/// A more API-friendly representation of the on-chain `accudo_types::jwks::QuorumCertifiedUpdate`.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
...
/// A more API-friendly representation of the on-chain `accudo_types::aggregate_signature::AggregateSignature`.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
...
/// A more API-friendly representation of the on-chain `accudo_types::jwks::ProviderJWKs`.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
...
impl TryFrom<&MultiEd25519Signature> for TransactionAuthenticator {
    ...
    fn try_from(value: &MultiEd25519Signature) -> Result<Self, Self::Error> {
        ...
        Ok(TransactionAuthenticator::multi_ed25519(
            ...
            accudo_crypto::multi_ed25519::MultiEd25519Signature::new_with_signatures_and_bitmap(
                ed25519_signatures,
...
impl From<accudo_types::transaction::PersistedAuxiliaryInfo> for PersistedAuxiliaryInfo {
    fn from(info: accudo_types::transaction::PersistedAuxiliaryInfo) -> Self {
        match info {
            accudo_types::transaction::PersistedAuxiliaryInfo::None => Self {
                transaction_index: None,
            },
            accudo_types::transaction::PersistedAuxiliaryInfo::V1 { transaction_index } => Self {
