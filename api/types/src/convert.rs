...
pub struct MoveConverter<'a, S> {
    inner: AccudoValueAnnotator<'a, S>,
    db: Arc<dyn DbReader>,
...
impl<'a, S: StateView> MoveConverter<'a, S> {
    pub fn new(
        ...
        Self {
            inner: AccudoValueAnnotator::new(inner),
            db,
            indexer_reader,
        }
    }
    ...
    pub fn try_into_onchain_transaction(
        ...
        Ok(match data.transaction {
            ...
            accudo_types::transaction::Transaction::ValidatorTransaction(txn) => {
    ...
    pub fn into_transaction_info(
        &self,
        version: u64,
        info: &accudo_types::transaction::TransactionInfo,
        accumulator_root_hash: HashValue,
        write_set: accudo_types::write_set::WriteSet,
    ...
    pub fn try_into_transaction_payload(
        &self,
        payload: accudo_types::transaction::TransactionPayload,
    ) -> Result<TransactionPayload> {
        use accudo_types::transaction::{EntryFunction, Script, TransactionPayload::*};
        ...
        let ret = match payload {
            ...
            Multisig(multisig) => {
                let transaction_payload = if let Some(payload) = multisig.transaction_payload {
                    match payload {
                        accudo_types::transaction::MultisigTransactionPayload::EntryFunction(
                            entry_function,
                        ) => {
            ...
            Payload(accudo_types::transaction::TransactionPayloadInner::V1 {
                executable,
                extra_config,
            }) => match extra_config {
                accudo_types::transaction::TransactionExtraConfig::V1 {
                    multisig_address,
                    replay_protection_nonce: _,
                } => {
                    if let Some(multisig_address) = multisig_address {
                        match executable {
                            accudo_types::transaction::TransactionExecutable::EntryFunction(
                                entry_function,
                            ) => TransactionPayload::MultisigPayload(MultisigPayload {
                            ...
                            accudo_types::transaction::TransactionExecutable::Script(_) => {
                                bail!(
                                    "Script executable is not supported for multisig transactions"
                                )
                            },
                            accudo_types::transaction::TransactionExecutable::Empty => {
                                TransactionPayload::MultisigPayload(MultisigPayload {
                                    multisig_address: multisig_address.into(),
                        ...
                        match executable {
                            ...
                            accudo_types::transaction::TransactionExecutable::Script(script) => {
    ...
    pub fn try_into_write_set_payload(
        &self,
        payload: accudo_types::transaction::WriteSetPayload,
    ) -> Result<WriteSetPayload> {
        use accudo_types::transaction::WriteSetPayload::*;
        let ret = match payload {
    ...
    pub fn try_into_raw_transaction_poem(
        ...
        Ok(RawTransaction::new(
            ...
            self.try_into_accudo_core_transaction_payload(
                payload,
    ...
    pub fn try_into_accudo_core_transaction_payload(
        &self,
        payload: TransactionPayload,
        nonce: Option<u64>,
    ) -> Result<accudo_types::transaction::TransactionPayload> {
            ...
            TransactionPayload::MultisigPayload(multisig) => {
                if let Some(nonce) = nonce {
                    ...
                    let transaction_payload: Option<
                        accudo_types::transaction::MultisigTransactionPayload,
