...
use accudo_types::vm_status::StatusCode;
use poem_openapi::{Enum, Object};
use serde::{Deserialize, Serialize};
use std::fmt::Formatter;

/// This is the generic struct we use for all API errors, it contains a string
/// message and an Accudo API specific error code.
#[derive(Debug, Clone, Serialize, Deserialize, Object)]
pub struct AccudoError {
    /// A message describing the error
    pub message: String,
    pub error_code: AccudoErrorCode,
    /// A code providing VM error details when submitting transactions to the VM
    pub vm_error_code: Option<u64>,
}

impl std::fmt::Display for AccudoError {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "Error({:?}): {:#}", self.error_code, self.message)
    }
}

impl std::error::Error for AccudoError {}
...
fn test_serialize_deserialize() {
    let with_code = AccudoError::new_with_vm_status(
        "Invalid transaction",
        AccudoErrorCode::VmError,
        accudo_types::vm_status::StatusCode::UNKNOWN_MODULE,
    );
    let _: AccudoError = bcs::from_bytes(&bcs::to_bytes(&with_code).unwrap()).unwrap();
    let _: AccudoError = serde_json::from_str(&serde_json::to_string(&with_code).unwrap()).unwrap();

    let without_code =
        AccudoError::new_with_error_code("some message", AccudoErrorCode::MempoolIsFull);
    let _: AccudoError = bcs::from_bytes(&bcs::to_bytes(&without_code).unwrap()).unwrap();
    let _: AccudoError =
