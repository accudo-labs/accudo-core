...
pub struct HashValue(pub accudo_crypto::hash::HashValue);

impl From<accudo_crypto::hash::HashValue> for HashValue {
    fn from(val: accudo_crypto::hash::HashValue) -> Self {
        Self(val)
    }
}

impl From<HashValue> for accudo_crypto::hash::HashValue {
...
impl FromStr for HashValue {
    ...
    fn from_str(s: &str) -> anyhow::Result<Self, anyhow::Error> {
        if let Some(hex) = s.strip_prefix("0x") {
            Ok(hex.parse::<accudo_crypto::hash::HashValue>()?.into())
        } else {
            Ok(s.parse::<accudo_crypto::hash::HashValue>()?.into())
        }
    }
}
...
impl HashValue {
    pub fn zero() -> Self {
        Self(accudo_crypto::hash::HashValue::zero())
    }
}
