...
This module provides a REST API for client applications to query the Accudo blockchain.
...
- https://fullnode.devnet.accudo.org/v1/spec#/ (and testnet / mainnet, based on the API rollout schedule)

All commands here are relative to the root of `accudo-core`.

1. Make your changes to the API code, i.e. the code in `api/src/`.
2. Regenerate the API spec `.yaml` and `.json` files by running these commands from the root of `accudo-core`:
```
cargo run -p accudo-openapi-spec-generator -- -f yaml -o api/doc/spec.yaml
cargo run -p accudo-openapi-spec-generator -- -f json -o api/doc/spec.json
```
