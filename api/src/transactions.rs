...
use accudo_crypto::{hash::CryptoHash, signing_message};
...
pub enum SubmitTransactionPost {
    ...
    // TODO: https://github.com/accudo-labs/accudo-core/issues/2275
    #[oai(content_type = "application/x.accudo.signed_transaction+bcs")]
    Bcs(Bcs),
}
...
impl TransactionsApi {
    ...
    ///   1. Hash message bytes: "RawTransaction" bytes + BCS bytes of [Transaction](https://accudo-labs.github.io/accudo-core/accudo_types/transaction/enum.Transaction.html).
    ...
    /// Make sure to use the `application/x.accudo.signed_transaction+bcs` Content-Type.
    ...
    async fn encode_submission(
        ...
        data.0
            ...
            .map_err(|err| {
                BasicError::bad_request_with_code_no_info(err, AccudoErrorCode::InvalidInput)
            })?;
...
impl TransactionsApi {
    ...
    async fn create_internal(&self, txn: SignedTransaction) -> Result<(), AccudoError> {
        ...
        match mempool_status.code {
            ...
            MempoolStatusCode::MempoolIsFull | MempoolStatusCode::TooManyTransactions => {
                Err(AccudoError::new_with_error_code(
                    &mempool_status.message,
                    AccudoErrorCode::MempoolIsFull,
                ))
            },
            MempoolStatusCode::VmError => {
                if let Some(status) = vm_status_opt {
                    Err(AccudoError::new_with_vm_status(
                        format!(
            ...
            MempoolStatusCode::InvalidSeqNumber => Err(AccudoError::new_with_error_code(
                mempool_status.message,
                AccudoErrorCode::SequenceNumberTooOld,
            )),
            MempoolStatusCode::InvalidUpdate => Err(AccudoError::new_with_error_code(
                mempool_status.message,
                AccudoErrorCode::InvalidTransactionUpdate,
            )),
            MempoolStatusCode::UnknownStatus => Err(AccudoError::new_with_error_code(
                format!("Transaction was rejected with status {}", mempool_status,),
                AccudoErrorCode::InternalError,
            )),
            MempoolStatusCode::RejectedByFilter => Err(AccudoError::new_with_error_code(
                mempool_status.message,
                AccudoErrorCode::RejectedByFilter,
            )),
        }
    }
    ...
    async fn create(
        ...
        match self.create_internal(txn.clone()).await {
            ...
            Err(error) => match error.error_code {
                AccudoErrorCode::InternalError => Err(
                    SubmitTransactionError::internal_from_accudo_error(error, ledger_info),
                ),
                AccudoErrorCode::VmError
                | AccudoErrorCode::SequenceNumberTooOld
                | AccudoErrorCode::InvalidTransactionUpdate => Err(
                    SubmitTransactionError::bad_request_from_accudo_error(error, ledger_info),
                ),
                AccudoErrorCode::MempoolIsFull => Err(
                    SubmitTransactionError::insufficient_storage_from_accudo_error(
                        error,
                        ledger_info,
                    ),
                ),
                _ => Err(SubmitTransactionError::internal_from_accudo_error(
                    error,
                    ledger_info,
                )),
            },
    ...
    // TODO: This function leverages a lot of types from accudo_types, use the
    ...
    pub fn simulate(
        ...
        let (vm_status, output) =
            AccudoSimulationVM::create_vm_and_simulate_signed_transaction(&txn, &state_view);
        ...
        let txn = accudo_types::transaction::Transaction::UserTransaction(txn);
        let zero_hash = accudo_crypto::HashValue::zero();
        let info = accudo_types::transaction::TransactionInfo::new(
            txn.hash(),
            zero_hash,
            zero_hash,
            None,
    ...
    pub fn get_signing_message(
        ...
        if accept_type == &AcceptType::Bcs {
            return Err(BasicError::bad_request_with_code_no_info(
                ...
                AccudoErrorCode::BcsNotSupported,
            ));
        }
        ...
        let raw_txn: RawTransaction = state_view
            ...
            .map_err(|err| {
                BasicError::bad_request_with_code(err, AccudoErrorCode::InvalidInput, &ledger_info)
            })?;
