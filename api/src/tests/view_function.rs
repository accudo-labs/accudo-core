...
use accudo_config::config::{NodeConfig, ViewFilter, ViewFunctionId};
...
fn build_coin_balance_request(address: &AccountAddress) -> Value {
    json!({
        ...
        "type_arguments": vec!["0x1::accudo_coin::AccudoCoin"],
    })
}
    ...
    let resp = context
        ...
        .post(
            ...
            json!({
                ...
                "type_arguments": ["0x1::accudo_coin::NewCoin"], // Does not exist
            }),
        )
        .await;
    context.check_golden_output_no_prune(resp);
...
async fn test_view_does_not_exist() {
    ...
    let resp = context
        ...
        .post(
            "/view",
            json!({
                "function":"0x1::accudo_account::fake_function",
                "arguments": vec![owner.address().to_string()],
                "type_arguments": [],
            }),
        )
...
async fn test_simple_view_invalid(
    ...
    let resp = context
        ...
        .post(
            "/view",
            json!({
                "function":"0x1::accudo_account::assert_account_exists",
                "arguments": vec![owner.address().to_string()],
                "type_arguments": [],
            }),
        )
...
async fn test_view_tuple(use_txn_payload_v2_format: bool, use_orderless_transactions: bool) {
    ...
    let payload = accudo_stdlib::publish_module_source(
        "test_module",
        r#"
