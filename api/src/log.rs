...
static REQUEST_SOURCE_CLIENT_REGEX: Lazy<Regex> =
    Lazy::new(|| Regex::new(r"accudo-[a-zA-Z\-]+/[0-9A-Za-z\.\-]+").unwrap());
...
pub async fn middleware_log<E: Endpoint>(next: E, request: Request) -> Result<Response> {
    ...
    REQUEST_SOURCE_CLIENT
        .with_label_values(&[
            determine_request_source_client(&log.accudo_client),
            operation_id,
            log.status.to_string().as_str(),
        ])
        .inc();
...
// Each of our clients includes a header value called X_ACCUDO_CLIENT that identifies
// that client. This string follows a particular format: <identifier>/<version>,
// where <identifier> always starts with `accudo-`. This function ensure this string
...
fn determine_request_source_client(accudo_client: &Option<String>) -> &str {
    // If the header is not set we can't determine the request source.
    let accudo_client = match accudo_client {
        Some(accudo_client) => accudo_client,
    ...
    match REQUEST_SOURCE_CLIENT_REGEX.find_iter(accudo_client).last() {
