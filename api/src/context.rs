...
use accudo_logger::{error, info, Schema};
use accudo_mempool::{MempoolClientRequest, MempoolClientSender, SubmissionStatus};
use accudo_storage_interface::{
    ...
    AccudoDbError, DbReader, Order, MAX_REQUEST_LIMIT,
};
use accudo_types::{
...
impl Context {
    ...
    pub fn latest_state_view_poem<E: InternalError>(
        ...
        self.db
            ...
            .map_err(|e| E::internal_with_code(e, AccudoErrorCode::InternalError, ledger_info))
    }
    ...
    pub fn state_view<E: StdApiError>(
        ...
        let state_view = self
            ...
            .map_err(|err| {
                E::internal_with_code(err, AccudoErrorCode::InternalError, &latest_ledger_info)
            })?;
    ...
    pub fn get_block_timestamp<E: InternalError>(
        ...
        self.db
            ...
            .map_err(|err| E::internal_with_code(err, AccudoErrorCode::InternalError, ledger_info))
    }
    ...
    pub fn render_transactions_sequential<E: InternalError>(
        ...
        mut timestamp: u64,
    ) -> Result<Vec<accudo_api_types::Transaction>, E> {
        if data.is_empty() {
            return Ok(vec![]);
        }
        ...
        let converter = state_view.as_converter(self.db.clone(), self.indexer_reader.clone());
        let txns: Vec<accudo_api_types::Transaction> = data
            .into_iter()
            .map(|t| {
            ...
            .map_err(|err| {
                E::internal_with_code(err, AccudoErrorCode::InternalError, ledger_info)
            })?;
    ...
    ) -> Result<Vec<accudo_api_types::TransactionSummary>, E> {
        if data.is_empty() {
            return Ok(vec![]);
        }

        let txn_summaries: Vec<accudo_api_types::TransactionSummary> = data
            .into_iter()
            .map(|t| {
                Ok(TransactionSummary {
                    ...
                    replay_protector: match t.replay_protector() {
                        accudo_types::transaction::ReplayProtector::Nonce(nonce) => {
                            ReplayProtector::Nonce(nonce.into())
                        },
                        accudo_types::transaction::ReplayProtector::SequenceNumber(seq_num) => {
    ...
    pub fn get_account_ordered_transactions<E: NotFoundError + InternalError>(
        ...
        let txns_res = if !db_sharding_enabled(&self.node_config) {
            ...
            self.indexer_reader
                ...
                .map_err(|e| AccudoDbError::Other(e.to_string()))
        };
        let txns = txns_res
    ...
    pub fn get_gas_schedule<E: InternalError>(
        &self,
        ledger_info: &LedgerInfo,
    ) -> Result<(u64, AccudoGasParameters), E> {
        ...
        // Otherwise refresh the cache
        {
            ...
            let state_view = self
                ...
                .map_err(|e| {
                    E::internal_with_code(e, AccudoErrorCode::InternalError, ledger_info)
                })?;
            ...
            let gas_schedule_params = {
                let may_be_params =
                    GasScheduleV2::fetch_config(&state_view).and_then(|gas_schedule| {
                        ...
                        AccudoGasParameters::from_on_chain_gas_schedule(
                            &gas_schedule,
                            feature_version,
                        )
                        .ok()
                ...
                match may_be_params {
                    ...
                    None => GasSchedule::fetch_config(&state_view)
                        .and_then(|gas_schedule| {
                            let gas_schedule = gas_schedule.into_btree_map();
                            AccudoGasParameters::from_on_chain_gas_schedule(&gas_schedule, 0).ok()
                        })
                        .ok_or_else(|| {
                            E::internal_with_code(
...
pub struct GasScheduleCache {
    ...
    gas_schedule_params: Option<AccudoGasParameters>,
}
    ...
    E: InternalError + Send + 'static,
{
    tokio::task::spawn_blocking(func)
        .await
        .map_err(|err| E::internal_with_code_no_info(err, AccudoErrorCode::InternalError))?
}
