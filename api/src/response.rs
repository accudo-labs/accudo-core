...
//! The Accudo API response / error handling philosophy.
//!
...
pub enum AccudoResponseContent<T: ToJSON + Send + Sync> {
...
/// an error response to manipulate the AccudoError inside it.
pub trait AccudoErrorResponse {
    fn inner_mut(&mut self) -> &mut AccudoError;
}
...
/// implementor also implements AccudoErrorResponse, which saves functions from
...
macro_rules! generate_error_traits {
    ($($trait_name:ident),*) => {
        paste::paste! {
        $(
        pub trait [<$trait_name Error>]: AccudoErrorResponse {
            // With ledger info and an error code
            #[allow(unused)]
            fn [<$trait_name:snake _with_code>]<Err: std::fmt::Display>(
                err: Err,
                error_code: accudo_api_types::AccudoErrorCode,
                ledger_info: &accudo_api_types::LedgerInfo,
            ...
            fn [<$trait_name:snake _with_optional_vm_status_and_ledger_info>]<Err: std::fmt::Display>(
                err: Err,
                error_code: accudo_api_types::AccudoErrorCode,
                vm_status: Option<accudo_types::vm_status::StatusCode>,
                ledger_info: Option<&accudo_api_types::LedgerInfo>
            ) -> Self where Self: Sized;
            ...
            fn [<$trait_name:snake _with_vm_status>]<Err: std::fmt::Display>(
                err: Err,
                error_code: accudo_api_types::AccudoErrorCode,
                vm_status: accudo_types::vm_status::StatusCode,
                ledger_info: &accudo_api_types::LedgerInfo
            ) -> Self where Self: Sized;

            #[allow(unused)]
            fn [<$trait_name:snake _from_accudo_error>](
                accudo_error: accudo_api_types::AccudoError,
                ledger_info: &accudo_api_types::LedgerInfo
            ) -> Self where Self: Sized;
        }
        )*
...
macro_rules! generate_error_response {
    ($enum_name:ident, $(($status:literal, $name:ident)),*) => {
        ...
        pub enum $enum_name {
            $(
            #[oai(status = $status)]
            $name(poem_openapi::payload::Json<Box<accudo_api_types::AccudoError>>,
                // We use just regular u64 here instead of U64 since all header
                // values are implicitly strings anyway.
                /// Chain ID of the current chain
                #[oai(header = "X-Accudo-Chain-Id")] Option<u8>,
                /// Current ledger version of the chain
                #[oai(header = "X-Accudo-Ledger-Version")] Option<u64>,
                /// Oldest non-pruned ledger version of the chain
                #[oai(header = "X-Accudo-Ledger-Oldest-Version")] Option<u64>,
                /// Current timestamp of the chain
                #[oai(header = "X-Accudo-Ledger-TimestampUsec")] Option<u64>,
                /// Current epoch of the chain
                #[oai(header = "X-Accudo-Epoch")] Option<u64>,
                /// Current block height of the chain
                #[oai(header = "X-Accudo-Block-Height")] Option<u64>,
                /// Oldest non-pruned block height of the chain
                #[oai(header = "X-Accudo-Oldest-Block-Height")] Option<u64>,
                /// The cost of the call in terms of gas
                #[oai(header = "X-Accudo-Gas-Used")] Option<u64>,
            ),
            )*
        }
        ...
        impl $crate::response::[<$name Error>] for $enum_name {
            fn [<$name:snake _with_code>]<Err: std::fmt::Display>(
                ...
                let error = accudo_api_types::AccudoError::new_with_error_code(err, error_code);
            ...
            fn [<$name:snake _with_optional_vm_status_and_ledger_info>]<Err: std::fmt::Display>(
                ...
                let error = if let Some(vm_status) = vm_status {
                    accudo_api_types::AccudoError::new_with_vm_status(err, error_code, vm_status)
                } else {
                    accudo_api_types::AccudoError::new_with_error_code(err, error_code)
                };
                let payload = poem_openapi::payload::Json(Box::new(error));
                Self::from($enum_name::$name(
                    payload,
            ...
            fn [<$name:snake _with_vm_status>]<Err: std::fmt::Display>(
                ...
                let error = accudo_api_types::AccudoError::new_with_vm_status(err, error_code, vm_status);
                let payload = poem_openapi::payload::Json(Box::new(error));
                Self::from($enum_name::$name(
                    payload,
                    Some(ledger_info.chain_id),
            ...
            fn [<$name:snake _from_accudo_error>](
                accudo_error: accudo_api_types::AccudoError,
                ledger_info: &accudo_api_types::LedgerInfo
            ) -> Self where Self: Sized {
                let payload = poem_openapi::payload::Json(Box::new(accudo_error));
                Self::from($enum_name::$name(
                    payload,
                    Some(ledger_info.chain_id),
                    Some(ledger_info.ledger_version.into()),
        ...
        // Generate a function that helps get the AccudoError within.
        impl $crate::response::AccudoErrorResponse for $enum_name {
            fn inner_mut(&mut self) -> &mut accudo_api_types::AccudoError {
                match self {
                    $(
    ...
    ($enum_name:ident, $(($status:literal, $name:ident)),*) => {
        ...
        pub enum $enum_name<T: poem_openapi::types::ToJSON + Send + Sync> {
            ...
            $name(
                // We use just regular u64 here instead of U64 since all header
                // values are implicitly strings anyway.
                $crate::response::AccudoResponseContent<T>,
                /// Chain ID of the current chain
                #[oai(header = "X-Accudo-Chain-Id")] u8,
                /// Current ledger version of the chain
                #[oai(header = "X-Accudo-Ledger-Version")] u64,
                /// Oldest non-pruned ledger version of the chain
                #[oai(header = "X-Accudo-Ledger-Oldest-Version")] u64,
                /// Current timestamp of the chain
                #[oai(header = "X-Accudo-Ledger-TimestampUsec")] u64,
                /// Current epoch of the chain
                #[oai(header = "X-Accudo-Epoch")] u64,
                /// Current block height of the chain
                #[oai(header = "X-Accudo-Block-Height")] u64,
                /// Oldest non-pruned block height of the chain
                #[oai(header = "X-Accudo-Oldest-Block-Height")] u64,
                /// The cost of the call in terms of gas
                #[oai(header = "X-Accudo-Gas-Used")] Option<u64>,
                ...
                #[oai(header = "X-Accudo-Cursor")] Option<String>,
            ),
            )*
        }
        ...
        // Generate a From impl that builds a response from AccudoResponseContent.
        ...
        impl <T: poem_openapi::types::ToJSON + Send + Sync> From<($crate::response::AccudoResponseContent<T>, &accudo_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (
                    $crate::response::AccudoResponseContent<T>,
                    &accudo_api_types::LedgerInfo,
                    [<$enum_name Status>]
                ),
            ) -> Self {
                match status {
        ...
        impl<T: poem_openapi::types::ToJSON + Send + Sync> From<(poem_openapi::payload::Json<T>, &accudo_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (poem_openapi::payload::Json<T>, &accudo_api_types::LedgerInfo, [<$enum_name Status>]),
            ) -> Self {
                let content = $crate::response::AccudoResponseContent::Json(value);
                Self::from((content, ledger_info, status))
            }
        }

        // Generate a From impl that builds a response from a Bcs<Vec<u8>> and friends.
        impl<T: poem_openapi::types::ToJSON + Send + Sync> From<($crate::bcs_payload::Bcs, &accudo_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (
                ...
                let content = $crate::response::AccudoResponseContent::Bcs(value);
                Self::from((content, ledger_info, status))
            }
        }
        ...
        impl<T: poem_openapi::types::ToJSON + Send + Sync + serde::Serialize> $enum_name<T> {
                        ...
                        $crate::bcs_payload::Bcs(
                            bcs::to_bytes(&value)
                                .map_err(|e| E::internal_with_code(
                                    e,
                                    accudo_api_types::AccudoErrorCode::InternalError,
                                    ledger_info
                                ))?
                        ),
                        ledger_info,
           ...
           pub fn try_from_json<E: $crate::response::InternalError>(
            ...
            pub fn with_cursor(mut self, new_cursor: Option<accudo_types::state_store::state_key::StateKey>) -> Self {
                match self {
                    $(
                    [<$enum_name>]::$name(_, _, _, _, _, _, _, _, _, ref mut cursor) => {
                        *cursor = new_cursor.map(|c| accudo_api_types::StateKeyWrapper::from(c).to_string());
                    }
                    )*
                }
                self
...
pub fn version_not_found<E: NotFoundError>(ledger_version: u64, ledger_info: &LedgerInfo) -> E {
    build_not_found(
        ...
        AccudoErrorCode::VersionNotFound,
        ledger_info,
    )
}
...
pub fn transaction_not_found_by_version<E: NotFoundError>(
    ...
    build_not_found(
        "Transaction",
        format!("Ledger version({})", ledger_version),
        AccudoErrorCode::TransactionNotFound,
        ledger_info,
    )
}
...
pub fn account_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        AccudoErrorCode::AccountNotFound,
        ledger_info,
    )
}
...
pub fn table_item_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        AccudoErrorCode::TableItemNotFound,
        ledger_info,
    )
}
