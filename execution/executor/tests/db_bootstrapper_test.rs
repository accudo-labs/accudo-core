...
use accudo_crypto::{ed25519::Ed25519PrivateKey, HashValue, PrivateKey, Uniform};
use accudo_db::AccudoDB;
use accudo_executor::{
    block_executor::BlockExecutor,
    db_bootstrapper::{generate_waypoint, maybe_bootstrap},
};
use accudo_executor_test_helpers::{
...
use accudo_types::{
    ...
    account_config::{
        accudo_test_root_address, new_block_event_key, primary_apt_store, FungibleStoreResource,
...
#[test]
fn test_empty_db() {
    let genesis = accudo_vm_genesis::test_genesis_change_set_and_validators(Some(1));
    let genesis_txn = Transaction::GenesisTransaction(WriteSetPayload::Direct(genesis.0));
    let tmp_dir = TempPath::new();
    let db_rw = DbReaderWriter::new(AccudoDB::new_for_test(&tmp_dir));
    ...
    // Bootstrap empty DB.
    let waypoint =
        generate_waypoint::<AccudoVMBlockExecutor>(&db_rw, &genesis_txn).expect("Should not fail.");
    maybe_bootstrap::<AccudoVMBlockExecutor>(&db_rw, &genesis_txn, waypoint).unwrap();
    ...
    assert!(
        maybe_bootstrap::<AccudoVMBlockExecutor>(&db_rw, &genesis_txn, waypoint)
            .unwrap()
            .is_none()
    );
}
...
fn execute_and_commit(txns: Vec<Transaction>, db: &DbReaderWriter, signer: &ValidatorSigner) {
    ...
    let executor = BlockExecutor::<AccudoVMBlockExecutor>::new(db.clone());
    let output = executor
        .execute_block(
...
fn get_accudo_coin_mint_transaction(
    accudo_root_key: &Ed25519PrivateKey,
    accudo_root_seq_num: u64,
    account: &AccountAddress,
    amount: u64,
) -> Transaction {
    get_test_signed_transaction(
        accudo_test_root_address(),
        /* sequence_number = */ accudo_root_seq_num,
        accudo_root_key.clone(),
        accudo_root_key.public_key(),
        Some(accudo_stdlib::accudo_coin_mint(*account, amount)),
    )
}
...
fn get_account_transaction(
    ...
    get_test_signed_transaction(
        ...
        accudo_root_key.clone(),
        accudo_root_key.public_key(),
        Some(accudo_stdlib::accudo_account_create_account(*account)),
    )
}

fn get_accudo_coin_transfer_transaction(
    ...
    get_test_signed_transaction(
        ...
        Some(accudo_stdlib::accudo_coin_transfer(recipient, amount)),
    )
}
...
fn test_new_genesis() {
    let genesis = accudo_vm_genesis::test_genesis_change_set_and_validators(Some(1));
    let genesis_key = &accudo_vm_genesis::GENESIS_KEYPAIR.0;
    let genesis_txn = Transaction::GenesisTransaction(WriteSetPayload::Direct(genesis.0));
    // Create bootstrapped DB.
    let tmp_dir = TempPath::new();
    let db = DbReaderWriter::new(AccudoDB::new_for_test(&tmp_dir));
    let waypoint = bootstrap_genesis::<AccudoVMBlockExecutor>(&db, &genesis_txn).unwrap();
    ...
    let txn3 = get_accudo_coin_mint_transaction(genesis_key, 2, &account1, 200_000_000);
    let txn4 = get_accudo_coin_mint_transaction(genesis_key, 3, &account2, 200_000_000);
    ...
    let genesis_txn = Transaction::GenesisTransaction(WriteSetPayload::Direct(ChangeSet::new(
        WriteSetMut::new(vec![
            ...
            (
                ...
                WriteOp::legacy_modification(
                    accudo_transaction_simulation::FungibleStore::new(
                        account1,
                        AccountAddress::TEN,
                        100_000_000,
                        false,
    ...
    let waypoint = generate_waypoint::<AccudoVMBlockExecutor>(&db, &genesis_txn).unwrap();
    assert!(
        maybe_bootstrap::<AccudoVMBlockExecutor>(&db, &genesis_txn, waypoint)
            .unwrap()
            .is_some()
    );
