...
use accudo_db_indexer::db_indexer::DBIndexer;
...
use accudo_indexer_grpc_table_info::internal_indexer_db_service::InternalIndexerDBService;
use accudo_sdk::{
...
#[cfg(test)]
pub fn create_test_db() -> (Arc<AccudoDB>, LocalAccount) {
    // create test db
    let path = accudo_temppath::TempPath::new();
    let (genesis, validators) = accudo_vm_genesis::test_genesis_change_set_and_validators(Some(1));
    let genesis_txn = Transaction::GenesisTransaction(WriteSetPayload::Direct(genesis));
    let core_resources_account: LocalAccount = LocalAccount::new(
        accudo_test_root_address(),
        AccountKey::from_private_key(accudo_vm_genesis::GENESIS_KEYPAIR.0.clone()),
        0,
    );
    let (accudo_db, _db, executor, _waypoint) =
    ...
    let signer = accudo_types::validator_signer::ValidatorSigner::new(
    ...
    let reconfig1 = core_resources_account.sign_with_transaction_builder(
        txn_factory.payload(accudo_stdlib::accudo_governance_force_end_epoch_test_only()),
    );
    ...
    (accudo_db, core_resources_account)
}
...
fn test_db_indexer_data() {
    ...
    // create test db
    let (accudo_db, core_account) = create_test_db();
    let total_version = accudo_db.expect_synced_version();
    assert_eq!(total_version, 11);
    let temp_path = TempPath::new();
    let mut node_config = accudo_config::config::NodeConfig::default();
    ...
    let db_indexer = DBIndexer::new(internal_indexer_db.clone(), accudo_db.clone());
    ...
    let expected_code = vec![
        ...
        ident_str!("accudo_coin"),
        ident_str!("accudo_hash"),
    ...
    let expected_resources = vec![
        ...
        (false, "0x1::stake::AccudoCoinCapabilities"),
        ...
        (false, "0x1::accudo_governance::VotingRecords"),
        (false, "0x1::state_storage::StateStorageUsage"),
        (false, "0x1::accudo_governance::VotingRecordsV2"),
        ...
        (false, "0x1::accudo_governance::GovernanceConfig"),
        (false, "0x1::accudo_governance::GovernanceEvents"),
        ...
        (false, "0x1::transaction_fee::AccudoCoinMintCapability"),
        (false, "0x1::transaction_fee::AccudoFABurnCapabilities"),
        (false, "0x1::jwk_consensus_config::JWKConsensusConfig"),
        (false, "0x1::accudo_governance::ApprovedExecutionHashes"),
        (false, "0x1::accudo_governance::GovernanceResponsbility"),
        ...
        (false, "0x1::coin::CoinInfo<0x1::accudo_coin::AccudoCoin>"),
        (
            false,
