...
use accudo_crypto::{hash::CryptoHash, PrivateKey};
...
use accudo_storage_interface::state_store::state_view::db_state_view::DbStateViewAtVersion;
use accudo_types::{
    account_config::{accudo_test_root_address, AccountResource, CORE_CODE_ADDRESS},
    block_metadata::BlockMetadata,
    on_chain_config::{AccudoVersion, OnChainConfig, ValidatorSet},
...
fn test_genesis() {
    ...
    let genesis = accudo_vm_genesis::test_genesis_transaction();
    ...
    let (accudo_framework_account_resource, state_proof) = db
        .reader
    ...
    state_proof
        .verify(
            ...
            accudo_framework_account_resource.as_ref(),
        )
        .unwrap();
}
...
fn test_reconfiguration() {
    ...
    // txn3 = set the accudo version for next epoch
    let txn3 = get_test_signed_transaction(
        accudo_test_root_address(),
        /* sequence_number = */ 1,
        genesis_key.clone(),
        genesis_key.public_key(),
        Some(accudo_stdlib::version_set_for_next_epoch(42)),
    );
    ...
    let txn4 = get_test_signed_transaction(
        ...
        genesis_key.clone(),
        genesis_key.public_key(),
        Some(accudo_stdlib::accudo_governance_force_end_epoch_test_only()),
    );
    ...
    assert_eq!(
        AccudoVersion::fetch_config(&db_state_view).unwrap().major,
        42
    );
}
