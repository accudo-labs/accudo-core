...
use accudo_executor::db_bootstrapper::{generate_waypoint, maybe_bootstrap};
...
pub(crate) fn bootstrap_with_genesis(
    ...
    let waypoint =
        generate_waypoint::<AccudoVMBlockExecutor>(&db_rw, get_genesis_txn(&config).unwrap())
            .unwrap();
    maybe_bootstrap::<AccudoVMBlockExecutor>(&db_rw, get_genesis_txn(&config).unwrap(), waypoint)
        .unwrap();
}
