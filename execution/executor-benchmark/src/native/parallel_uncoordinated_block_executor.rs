...
impl CommonNativeRawTransactionExecutor for NativeRawTransactionExecutor {
    ...
    fn reduce_coin_apt_supply(
        ...
        let coin_info = DbAccessUtil::get_value::<CoinInfoResource<AccudoCoinType>>(
...
enum CachedResource {
    ...
    AptCoinStore(CoinStoreResource<AccudoCoinType>),
    AptCoinInfo(CoinInfoResource<AccudoCoinType>),
...
impl NativeValueCacheRawTransactionExecutor {
    ...
    fn update_coin_balance(
        ...
        let coin_store_key = self.db_util.new_state_key_accudo_coin(&account);
        let mut exists = true;
