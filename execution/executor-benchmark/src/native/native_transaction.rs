...
impl NativeTransaction {
    pub fn parse(txn: &SignatureVerifiedTransaction) -> Self {
        match &txn.expect_valid() {
            accudo_types::transaction::Transaction::UserTransaction(user_txn) => {
                // Use executable_ref() to handle both EntryFunction and Payload variants uniformly
                match user_txn.payload().executable_ref() {
                    Ok(accudo_types::transaction::TransactionExecutableRef::EntryFunction(f))
                        if !user_txn.payload().is_multisig() =>
                    {
                        match (
                            *f.module().address(),
                            ...
                            (AccountAddress::ONE, "accudo_account", "fungible_transfer_only") => {
                                Self::FaTransfer {
                            ...
                            (AccountAddress::ONE, "accudo_account", "transfer") => Self::Transfer {
                                sender: user_txn.sender(),
                                sequence_number: user_txn.sequence_number(),
                                recipient: bcs::from_bytes(&f.args()[0]).unwrap(),
                                amount: bcs::from_bytes(&f.args()[1]).unwrap(),
                            ...
                            (AccountAddress::ONE, "accudo_account", "create_account") => {
                                Self::Transfer {
                                    sender: user_txn.sender(),
                                    sequence_number: user_txn.sequence_number(),
                                    recipient: bcs::from_bytes(&f.args()[0]).unwrap(),
                            ...
                            (AccountAddress::ONE, "accudo_account", "batch_transfer") => {
                                Self::BatchTransfer {
                                    sender: user_txn.sender(),
                                    sequence_number: user_txn.sequence_number(),
            ...
            accudo_types::transaction::Transaction::BlockEpilogue(_) => Self::BlockEpilogue,
            accudo_types::transaction::Transaction::BlockMetadata(_) => Self::BlockMetadata,
