...
use accudo_executor_service::remote_executor_client;
use accudo_experimental_ptx_executor::PtxBlockExecutor;
#[cfg(target_os = "linux")]
use accudo_experimental_runtimes::thread_manager::{ThreadConfigStrategy, ThreadManagerBuilder};
use accudo_metrics_core::{register_int_gauge, IntGauge};
use accudo_profiler::{ProfilerConfig, ProfilerHandler};
    ...
    E: VMBlockExecutor + 'static,
{
    match opt.cmd {
        Command::CreateDb {
            ...
            accudo_executor_benchmark::db_generator::create_db_with_accounts::<E>(
                num_accounts,
                init_account_balance,
                opt.block_size,
                data_dir,
        ...
        Command::RunExecutor {
            ...
            // accudo_types::on_chain_config::hack_enable_default_features_for_genesis(enable_feature);
            // accudo_types::on_chain_config::hack_disable_default_features_for_genesis(
            //     disable_feature,
            // );
            ...
            accudo_executor_benchmark::run_benchmark::<E>(
                opt.block_size,
                blocks,
                workload,
        ...
        Command::AddAccounts {
            ...
            accudo_executor_benchmark::add_accounts::<E>(
                num_new_accounts,
                init_account_balance,
                opt.block_size,
                data_dir,
...
fn main() {
    ...
    accudo_node_resource_metrics::register_node_metrics_collector(None);
    ...
    AccudoVM::set_num_shards_once(execution_shards);
    AccudoVM::set_concurrency_level_once(execution_threads_per_shard);
    NativeConfig::set_concurrency_level_once(execution_threads_per_shard);
    AccudoVM::set_processed_transactions_detailed_counters();
    AccudoVM::set_blockstm_v2_enabled_once(opt.use_blockstm_v2);
    ...
    match opt.block_executor_type {
        BlockExecutorTypeOpt::AccudoVMWithBlockSTM => {
            run::<AccudoVMBlockExecutor>(opt);
        },
        BlockExecutorTypeOpt::NativeVMWithBlockSTM => {
            run::<NativeVMBlockExecutor>(opt);
        },
        BlockExecutorTypeOpt::AccudoVMParallelUncoordinated => {
            run::<AccudoVMParallelUncoordinatedBlockExecutor>(opt);
        },
    ...
    if memory_profiling {
        let _mem_end = memory_profiler.end_profiling("./target/release/accudo-executor-benchmark");
    }
}
