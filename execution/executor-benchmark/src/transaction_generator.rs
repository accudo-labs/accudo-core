...
use accudo_types::{
    ...
    account_config::{accudo_test_root_address, AccountResource},
...
fn get_genesis_validator_address(db: &DbReaderWriter) -> AccountAddress {
    ...
    // Try to get the ValidatorSet resource from @accudo_framework
...
impl TransactionGenerator {
    ...
    pub fn read_root_account(genesis_key: Ed25519PrivateKey, db: &DbReaderWriter) -> LocalAccount {
        LocalAccount::new(
            ...
            genesis_key,
            get_sequence_number(accudo_test_root_address(), db.reader.clone()),
        )
    }
    ...
    pub fn create_seed_accounts(
            ...
            .chunks(block_size)
        {
            ...
            let transactions: Vec<_> = chunk
                .iter()
                .map(|new_account| {
                    let payload = accudo_stdlib::accudo_account_transfer(
