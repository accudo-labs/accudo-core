...
use accudo_config::config::{NodeConfig, PrunerConfig, NO_OP_STORAGE_PRUNER_CONFIG};
use accudo_db::AccudoDB;
use accudo_executor::block_executor::BlockExecutor;
use accudo_jellyfish_merkle::metrics::{
    ACCUDO_JELLYFISH_INTERNAL_ENCODED_BYTES, ACCUDO_JELLYFISH_LEAF_ENCODED_BYTES,
};
...
fn create_checkpoint(
    ...
    AccudoDB::create_checkpoint(source_dir, checkpoint_dir, enable_storage_sharding)
    ...
    V: VMBlockExecutor + 'static,
{
    ...
    let (mut config, genesis_key) =
        accudo_genesis::test_utils::test_config_with_custom_features(init_features);
    ...
    if let BenchmarkWorkload::TransactionMix(mix) = &workload {
        for (transaction_type, _) in mix {
            if matches!(transaction_type, CoinTransfer { non_conflicting, .. } if *non_conflicting)
            {
                ...
                // `accudo_executor_benchmark::transaction_generator::TransactionGenerator` needs to hold
    ...
    let initialized_workload = match workload {
        BenchmarkWorkload::TransactionMix(transaction_mix) => {
            ...
            let (transaction_generator_creator, phase) = init_workload::<AccudoVMBlockExecutor>(
                transaction_mix,
                root_account.clone(),
    ...
    assert_eq!(0, accudo_logger::ERROR_LOG_COUNT.get());
    ...
    V: VMBlockExecutor + 'static,
{
    ...
    println!(
        ...
        ACCUDO_JELLYFISH_INTERNAL_ENCODED_BYTES.get()
    );
    println!(
        "Total written leaf nodes value size: {} bytes",
        ACCUDO_JELLYFISH_LEAF_ENCODED_BYTES.get()
    );
}
...
pub fn run_single_with_default_params(
    ...
    AccudoVM::set_num_shards_once(1);
    AccudoVM::set_concurrency_level_once(concurrency_level);
    AccudoVM::set_blockstm_v2_enabled_once(use_blockstm_v2);
    AccudoVM::set_processed_transactions_detailed_counters();
    ...
    create_db_with_accounts::<AccudoVMBlockExecutor>(
    ...
    run_benchmark::<AccudoVMBlockExecutor>(
...
mod tests {
    use crate::{
        ...
        native::{
            accudo_vm_uncoordinated::AccudoVMParallelUncoordinatedBlockExecutor,
    ...
    use accudo_config::config::NO_OP_STORAGE_PRUNER_CONFIG;
    use accudo_crypto::HashValue;
    use accudo_executor::block_executor::BlockExecutor;
    use accudo_executor_types::BlockExecutorTrait;
    use accudo_sdk::{transaction_builder::accudo_stdlib, types::LocalAccount};
    use accudo_temppath::TempPath;
    use accudo_transaction_generator_lib::WorkflowProgress;
    use accudo_transaction_workloads_lib::args::TransactionTypeArg;
    use accudo_types::{
        access_path::Path,
    ...
    fn test_compare_vm_and_vm_uncoordinated() {
        test_compare_prod_and_another_all_types::<AccudoVMParallelUncoordinatedBlockExecutor>(true);
    }
    ...
    fn test_compare_prod_and_another_all_types<E: VMBlockExecutor>(values_match: bool) {
        ...
        test_compare_prod_and_another::<E>(values_match, fa_features.clone(), |address| {
            accudo_stdlib::accudo_account_fungible_transfer_only(address, 1000)
        });

        test_compare_prod_and_another::<E>(values_match, fa_features.clone(), |address| {
            accudo_stdlib::accudo_account_transfer(address, 1000)
        });

        test_compare_prod_and_another::<E>(
            values_match,
            fa_features,
            accudo_stdlib::accudo_account_create_account,
        );
    }
    ...
    fn test_compare_prod_and_another<E: VMBlockExecutor>(
        ...
        let (mut config, genesis_key) =
            accudo_genesis::test_utils::test_config_with_custom_features(features);
        ...
        let (txn, vm_result) = {
            ...
            let vm_executor = BlockExecutor::<AccudoVMBlockExecutor>::new(vm_db.clone());
        ...
        E: VMBlockExecutor + 'static,
    {
        ...
        crate::db_generator::create_db_with_accounts::<AccudoVMBlockExecutor>(
            100, /* num_accounts */
    ...
    fn test_benchmark_default() {
        test_generic_benchmark::<AccudoVMBlockExecutor>(None, true);
    }

    #[test]
    fn test_publish_transaction() {
        AccudoVM::set_num_shards_once(1);
        AccudoVM::set_concurrency_level_once(4);
        AccudoVM::set_processed_transactions_detailed_counters();
        test_generic_benchmark::<AccudoVMBlockExecutor>(
    ...
    fn test_benchmark_transaction() {
        AccudoVM::set_num_shards_once(4);
        AccudoVM::set_concurrency_level_once(4);
        AccudoVM::set_processed_transactions_detailed_counters();
