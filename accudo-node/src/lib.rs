...
use accudo_config::config::{merge_node_config, NodeConfig, PersistableConfig};
use accudo_framework::ReleaseBundle;
use accudo_genesis::builder::GenesisConfiguration;
use accudo_logger::{prelude::*, telemetry_log_writer::TelemetryLog, Level, LoggerFilterUpdater};
use accudo_state_sync_driver::driver_factory::StateSyncRuntimes;
use accudo_types::{
...
/// Runs an Accudo validator or fullnode
#[derive(Clone, Debug, Parser)]
#[clap(name = "Accudo Node", author, version)]
pub struct AccudoNodeArgs {
    ...
    /// Paths to the Accudo framework release package to be used for genesis.
    #[clap(long, requires("test"))]
...
impl AccudoNodeArgs {
    /// Runs an Accudo node based on the given command line arguments and config flags
    pub fn run(self) {
        #[cfg(target_os = "linux")]
...
pub fn start_and_report_ports(
    ...
    // Setup panic handler
    accudo_crash_handler::setup_panic_handler();

    // Create global rayon thread pool
    utils::create_global_rayon_pool(create_global_rayon_pool);

    // Initialize the global accudo-node-identity
    accudo_node_identity::init(config.get_peer_id())?;
...
pub fn start_test_environment_node(
    ...
    let accudo_root_key_path = test_dir.join("mint.key");
    ...
    println!("\tAccudo root key path: {:?}", accudo_root_key_path);
    ...
    println!(
        "\tAccudonet fullnode network endpoint: {}",
    ...
    println!("\nAccudo is running, press ctrl-c to exit\n");
    ...
    R: rand::RngCore + rand::CryptoRng,
{
    ...
    let test_dir =
        test_dir.unwrap_or_else(|| accudo_temppath::TempPath::new().as_ref().to_path_buf());
    ...
    R: rand::RngCore + rand::CryptoRng,
{
    ...
    let builder = accudo_genesis::builder::Builder::new(test_dir, framework.clone())?
        ...
        .with_init_genesis_config(Some(Arc::new(|genesis_config| {
            ...
            match env::var("ENABLE_KEYLESS_DEFAULT") {
                Ok(val) if val.as_str() == "1" => {
                    let response = ureq::get("https://api.devnet.accudo.org/v1/accounts/0x1/resource/0x1::keyless_account::Groth16VerificationKey").call();
    ...
    let mut key_file = fs::File::create(accudo_root_key_path)?;
    ...
    accudo_config::config::sanitize_node_config(validators[0].config.override_config_mut())?;
...
pub fn setup_environment_and_start_node(
    ...
    admin_service.set_accudo_db(db_rw.clone().into());

    // Set the Accudo VM configurations
    utils::set_accudo_vm_configurations(&node_config);

    // Obtain the chain_id from the DB
    let chain_id = utils::fetch_chain_id(&db_rw)?;

    // Set the chain_id in global AccudoNodeIdentity
    accudo_node_identity::set_chain_id(chain_id)?;
    ...
    let (accudo_data_client, state_sync_runtimes, mempool_listener, consensus_notifier) =
...
fn verify_tool() {
    ...
    AccudoNodeArgs::command().debug_assert()
}
