...
use accudo_config::config::{NodeConfig, StateSyncConfig};
use accudo_consensus_notifications::ConsensusNotifier;
use accudo_data_client::{client::AccudoDataClient, poller};
...
use accudo_event_notifications::{
...
use accudo_mempool_notifications::MempoolNotificationListener;
use accudo_network::application::{
    interface::{NetworkClient, NetworkClientInterface, NetworkServiceEvents},
    storage::PeersAndMetadata,
};
use accudo_state_sync_driver::{
    driver_factory::{DriverFactory, StateSyncRuntimes},
    metadata_storage::PersistentMetadataStorage,
};
use accudo_storage_interface::{DbReader, DbReaderWriter};
use accudo_storage_service_client::StorageServiceClient;
use accudo_storage_service_notifications::StorageServiceNotificationListener;
use accudo_storage_service_server::{
...
pub fn start_state_sync_and_get_notification_handles(
    ...
    let (accudo_data_client, accudo_data_client_runtime) =
        setup_accudo_data_client(node_config, network_client, db_rw.reader.clone())?;

    // Start the data streaming service
    let state_sync_config = node_config.state_sync;
    let (streaming_service_client, streaming_service_runtime) =
        setup_data_streaming_service(state_sync_config, accudo_data_client.clone())?;
    ...
    let (mempool_notifier, mempool_listener) =
        accudo_mempool_notifications::new_mempool_notifier_listener_pair(
            state_sync_config
                .state_sync_driver
                .max_pending_mempool_notifications,
        );
    let (consensus_notifier, consensus_listener) =
        accudo_consensus_notifications::new_consensus_notifier_listener_pair(
            state_sync_config
                .state_sync_driver
...
fn setup_data_streaming_service(
    ...
    accudo_data_client: AccudoDataClient,
    ...
    let data_streaming_service = DataStreamingService::new(
        state_sync_config.accudo_data_client,
...
/// Sets up the accudo data client runtime
fn setup_accudo_data_client(
    node_config: &NodeConfig,
    network_client: NetworkClient<StorageServiceMessage>,
    storage: Arc<dyn DbReader>,
) -> anyhow::Result<(AccudoDataClient, Runtime)> {
    ...
    let (accudo_data_client, data_summary_poller) = AccudoDataClient::new(
        ...
        Some(accudo_data_client_runtime.handle().clone()),
    );
    accudo_data_client_runtime.spawn(poller::start_poller(data_summary_poller));

    Ok((accudo_data_client, accudo_data_client_runtime))
}
...
fn setup_state_sync_storage_service(
    ...
    let storage_service_runtime = accudo_runtimes::spawn_named_runtime("stor-server".into(), None);
