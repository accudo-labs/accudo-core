...
use accudo_executor::db_bootstrapper::maybe_bootstrap;
...
pub(crate) fn maybe_apply_genesis(
    ...
    if let Some(genesis) = get_genesis_txn(node_config) {
        let ledger_info_opt =
            maybe_bootstrap::<AccudoVMBlockExecutor>(db_rw, genesis, genesis_waypoint)
...
pub(crate) fn bootstrap_db(
    ...
    let (accudo_db_reader, db_rw, backup_service) = match FastSyncStorageWrapper::initialize_dbs(
        node_config,
...
/// In consensus-only mode, return a in-memory based [FakeAccudoDB] and
/// do not run the backup service.
#[cfg(feature = "consensus-only-perf-test")]
pub(crate) fn bootstrap_db(
    node_config: &NodeConfig,
) -> Result<(Arc<dyn DbReader>, DbReaderWriter, Option<Runtime>)> {
    use accudo_db::db::fake_accudodb::FakeAccudoDB;

    let accudo_db = AccudoDB::open(
    ...
    let (accudo_db, db_rw) = DbReaderWriter::wrap(FakeAccudoDB::new(accudo_db));
    maybe_apply_genesis(&db_rw, node_config)?;
    Ok((accudo_db, db_rw, None))
}
...
fn create_rocksdb_checkpoint_and_change_working_dir(
    ...
    AccudoDB::create_checkpoint(
        &source_dir,
        &checkpoint_dir,
        node_config.storage.rocksdb_configs.enable_storage_sharding,
    )
    .expect("AccudoDB checkpoint creation failed.");

    // Create a consensus db checkpoint
    accudo_consensus::create_checkpoint(&source_dir, &checkpoint_dir)
        .expect("ConsensusDB checkpoint creation failed.");

    // Create a state sync db checkpoint
    let state_sync_db =
        accudo_state_sync_driver::metadata_storage::PersistentMetadataStorage::new(&source_dir);
    state_sync_db
...
pub fn initialize_database_and_checkpoints(
    ...
    let (_accudo_db, db_rw, backup_service, indexer_db_opt, update_receiver) =
