...
accudo-node was compiled with feature flags that shouldn't be enabled.
...
- Recompile `accudo-node` SEPARATELY
...
pub fn fetch_chain_id(db: &DbReaderWriter) -> anyhow::Result<ChainId> {
    let db_state_view = db
        ...
        .map_err(|err| anyhow!("[accudo-node] failed to create db state view {}", err))?;
    Ok(ChainIdResource::fetch_config(&db_state_view)
        .expect("[accudo-node] missing chain ID resource")
        .chain_id())
}

/// Sets the Accudo VM configuration based on the node configurations
pub fn set_accudo_vm_configurations(node_config: &NodeConfig) {
    ...
    AccudoVM::set_concurrency_level_once(effective_concurrency_level as usize);
    AccudoVM::set_discard_failed_blocks(node_config.execution.discard_failed_blocks);
    AccudoVM::set_num_proof_reading_threads_once(
        node_config.execution.num_proof_reading_threads as usize,
    );
    AccudoVM::set_blockstm_v2_enabled_once(node_config.execution.blockstm_v2_enabled);
