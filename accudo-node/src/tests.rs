...
use accudo_config::config::{NodeConfig, WaypointConfig};
use accudo_event_notifications::EventSubscriptionService;
use accudo_infallible::RwLock;
use accudo_storage_interface::{DbReader, DbReaderWriter, DbWriter};
use accudo_temppath::TempPath;
use accudo_types::{chain_id::ChainId, waypoint::Waypoint};
...
#[test]
fn test_accudo_vm_does_not_have_test_natives() {
    accudo_vm::natives::assert_no_test_natives(crate::utils::ERROR_MSG_BAD_FEATURE_FLAGS)
}
...
fn test_create_single_node_test_config() {
    ...
    let test_dir = accudo_temppath::TempPath::new().as_ref().to_path_buf();
    fs::DirBuilder::new()
        .recursive(true)
        .create(&test_dir)
...
#[test]
fn test_verifier_cache_enabled_for_accudo_node() {
    use std::process::{Command, Stdio};
    // Run the shell command `cargo tree -p accudo-node -e features`
    let output = Command::new("cargo")
        ...
        .stdout(Stdio::piped())
        .output()
        .expect("Failed to execute `cargo tree -p accudo-node -e features`");
    let output = String::from_utf8_lossy(&output.stdout);

    let feature = "disable_verifier_cache";
    assert!(
        !output.contains(feature),
        "Feature `{}` should not be enabled for accudo-node",
        feature
    );
}
