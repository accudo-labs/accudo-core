...
impl<T: DeserializeOwned + PrivateKey + ValidCryptoMaterial + Serialize> ConfigKey<T> {
    ...
    pub fn public_key(&self) -> T::PublicKeyMaterial {
        accudo_crypto::PrivateKey::public_key(&self.key)
    }
...
#[cfg(test)]
impl<T: PrivateKey + Serialize + accudo_crypto::Uniform> Default for ConfigKey<T> {
    fn default() -> Self {
        Self {
            key: accudo_crypto::Uniform::generate_for_testing(),
        }
    }
}
